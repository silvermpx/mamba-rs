//! Discovery only: GROUP_M=12 transpose/raw/fused-finalize pipeline screened
//! against the committed GROUP_M=8 fused winner for exact F32 TN d768-in.
//! No dispatcher admission or production changes.

#[path = "support/triad_f32_tn_copyplan_fused_finalize_source.rs"]
mod fused_source;
#[path = "support/triad_f32_tn_copyplan_fused_finalize_groupm12_source.rs"]
mod group_m12_source;
...
const CANDIDATE_GROUP_M: usize = 12;
const RETAINED_GROUP_M: usize = 8;
...
fn k0_route(retained_oracle: bool) -> K0Route {
    if retained_oracle {
        K0Route::PublicAuto
    } else {
        K0Route::ManualZero
...
fn tn_d768_in_maps_to_exact_three_node_fused_finalize_pipeline() {
    assert_eq!((CANDIDATE_GROUP_M, RETAINED_GROUP_M), (12, 8));
    assert_eq!(
    ...
    let group_m12 = group_m12_source::compose_source().unwrap();
    assert_eq!(
        group_m12
            .matches(group_m12_source::GROUP_M12_SYMBOL)
            .count(),
        1
    );
    assert!(group_m12.contains("#define SM89_EXACT_N64_CP_GROUP_M 12"));
    let group_m12_raw = group_m12_source::compose_raw_source().unwrap();
    assert_eq!(
        group_m12_raw
            .matches(group_m12_source::GROUP_M12_RAW_SYMBOL)
            .count(),
        1
    );
}
...
#[test]
fn retained_k0_oracle_route_is_independent_from_candidate() {
    assert_eq!(k0_route(false), K0Route::ManualZero);
    assert_eq!(k0_route(true), K0Route::PublicAuto);
}
...
mod splitm_cuda_suite {
    ...
    const GROUP_M12_RAW: &str = group_m12_source::GROUP_M12_RAW_SYMBOL;
    const GROUP_M12_FUSED: &str = group_m12_source::GROUP_M12_SYMBOL;
    ...
    impl CandidateFixed {
        fn symbol(self) -> &'static str {
            match self {
                ...
                Self::GroupM12Raw => GROUP_M12_RAW,
            }
        }
    }
    ...
    struct Runtime {
        ...
        _group_m12_raw_module: Arc<CudaModule>,
        _group_m12_fused_module: Arc<CudaModule>,
        raw_fixed: CudaFunction,
        fused: CudaFunction,
        group_m12_raw: CudaFunction,
        group_m12_fused: CudaFunction,
        raw_source_sha: String,
        fused_source_sha: String,
        group_m12_raw_source_sha: String,
        group_m12_fused_source_sha: String,
    }
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        let group_m12_raw_source = group_m12_source::compose_raw_source()?;
        let group_m12_raw_source_sha =
            format!("{:x}", Sha256::digest(group_m12_raw_source.as_bytes()));
        let group_m12_raw_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            group_m12_raw_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
                include_paths: cuda_include_paths(),
        ...
        .map_err(|error| format!("compile GROUP_M12 raw CopyPlan: {error:?}"))?;
        let group_m12_raw_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(group_m12_raw_ptx.to_src()))
            .map_err(|error| format!("load GROUP_M12 raw CopyPlan module: {error:?}"))?;
        let group_m12_raw = group_m12_raw_module
            .load_function(GROUP_M12_RAW)
            .map_err(|error| format!("load {GROUP_M12_RAW}: {error:?}"))?;
        let group_m12_fused_source = group_m12_source::compose_source()?;
        let group_m12_fused_source_sha =
            format!("{:x}", Sha256::digest(group_m12_fused_source.as_bytes()));
        let group_m12_fused_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            group_m12_fused_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
                include_paths: cuda_include_paths(),
        ...
        .map_err(|error| format!("compile GROUP_M12 fused CopyPlan: {error:?}"))?;
        let group_m12_fused_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(group_m12_fused_ptx.to_src()))
            .map_err(|error| format!("load GROUP_M12 fused CopyPlan module: {error:?}"))?;
        let group_m12_fused = group_m12_fused_module
            .load_function(GROUP_M12_FUSED)
            .map_err(|error| format!("load {GROUP_M12_FUSED}: {error:?}"))?;
        Ok(Runtime {
            _device: device,
            ctx,
            _raw_module: module,
            _fused_module: fused_module,
            _group_m12_raw_module: group_m12_raw_module,
            _group_m12_fused_module: group_m12_fused_module,
            raw_fixed,
            fused,
            group_m12_raw,
            group_m12_fused,
            raw_source_sha,
            fused_source_sha,
            group_m12_raw_source_sha,
            group_m12_fused_source_sha,
        })
    }
    ...
    fn fixed_function(
        ...
        match candidate {
            ...
            CandidateFixed::GroupM12Raw => Ok(&runtime.group_m12_raw),
        }
    }
    ...
    fn launch(
        ...
        if fixture.cell.batch == 0 {
            if arm == Arm::Fast {
                return Err("Fast comparator does not admit K=0".into());
            }
            return match k0_route(arm == Arm::Retained) {
                K0Route::ManualZero => launch_zero(runtime, fixture),
                K0Route::PublicAuto => gpu_gemm_bi_backward_dw_grad(
                    &runtime.ctx,
                    &GradSlice::from_raw(
                        fixture.output.ptr(),
        ...
        match arm {
            Arm::Candidate => launch_fused_pipeline(
                runtime,
                fixture,
                CandidateFixed::GroupM12Raw,
                &runtime.group_m12_fused,
                GROUP_M12_FUSED,
            ),
            Arm::Reference => {
    ...
    fn compare_raw_partials(
        ...
        for index in raw_plane_indices(fixture.cell).map_err(&harness)? {
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnFusedGroupM12RawPartialV1",
                    "corpus":corpus,
        ...
        match production_exceptional {
            ...
            Err(RawPartialError::Mismatch(production_error)) => {
                println!(
                    "{}",
                    json!({
                        "schema":"MambaBiF32TnFusedGroupM12GroupM8RawStoreFallbackV1",
    ...
    fn prove_group_m12_raw_accumulator(runtime: &Runtime) -> Result<(), String> {
        for (corpus, exceptional) in FUSED_RAW_TARGET_CORPORA {
            let mut fixture = Fixture::new(&runtime.ctx, D768_IN, 1.0, exceptional)?;
            compare_raw_partials(runtime, &mut fixture, CandidateFixed::GroupM12Raw, corpus)
                .map_err(|error| error.to_string())?;
        }
        Ok(())
    }
    ...
    unsafe fn require_fused_graph(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedGroupM12GraphV1",
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
    ...
    unsafe fn require_fast_graph(graph: &CudaGraph) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiF32TnFusedGroupM12GraphV1","arm":"Fast","node_count":count,"abi":"opaque","timing":"whole_graph"})
        );
        Ok(())
    }
    ...
    fn check_exact_case(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedGroupM12BitsV1",
                "case":label,
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "alpha_bits":fixture.alpha.to_bits(),
                "candidate":GROUP_M12_FUSED,
    ...
    fn resources(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedGroupM12ResourceV1",
                "symbol":symbol,
                "threads":threads,
                "registers":registers,
                "local_bytes":local,
        ...
        if registers == 0
            || local != 0
            || shared as usize != static_shared_expected
            || occupancy == 0
        {
    ...
    #[test]
    #[ignore = "Ada CUDA13.2 discovery: exact F32 TN d768-in GROUP_M12 vs GROUP_M8"]
    fn ada_f32_tn_copyplan_fused_finalize_groupm12_d768_in_once7() -> Result<(), String> {
        if cfg!(debug_assertions) {
            return Err("performance requires release".into());
        }
        let quiet = common::gpu_quiet::QuietGpu::for_cuda_ordinal(0)?;
        quiet.require_pre_context("f32-tn-fused-groupm12/pre")?;
        let runtime = new_runtime()?;

        // Prove the exact committed GROUP_M8 three-node comparator before any
        ...
        prove_group_m12_raw_accumulator(&runtime)?;
        let fixed = fixed_function(&runtime, candidate)?;
        resources(fixed, candidate.symbol(), 128, 32_768)?;
        resources(&runtime.fused, FUSED, 128, 32_768)?;
        resources(&runtime.group_m12_raw, GROUP_M12_RAW, 128, 32_768)?;
        resources(&runtime.group_m12_fused, GROUP_M12_FUSED, 128, 32_768)?;
        resources(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedGroupM12ArtifactsV1",
                ...
                "group_m12_raw_source_sha":runtime.group_m12_raw_source_sha,
                "group_m12_fused_source_sha":runtime.group_m12_fused_source_sha,
                "candidate_nodes":[TRANSPOSE,GROUP_M12_RAW,GROUP_M12_FUSED],
        ...
        compare_raw_partials(
            ...
            &mut tail,
            CandidateFixed::GroupM12Raw,
            "tail_chunk_group_m12_accumulator",
        )
        .map_err(|error| error.to_string())?;
        ...
        compare_raw_partials(
            ...
            &mut exceptional,
            CandidateFixed::GroupM12Raw,
            "exceptional_tail_group_m12_accumulator",
        )
        .map_err(|error| error.to_string())?;
        check_exact_case(
            &runtime,
        ...
        while comparator_index < comparators.len() {
            ...
            quiet.require_cohort("f32-tn-fused-groupm12/timed")?;
            ...
            for graph_path in [false, true] {
                for candidate_endpoints in [true, false] {
                    ...
                    println!(
                        "{}",
                        json!({
                            "schema":"MambaBiF32TnFusedGroupM12ScreenV1",
                            "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                            "comparator":if comparator == Arm::Retained {"GROUP_M8"} else {"Fast"},
                            ...
                            "candidate_timed_nodes":[TRANSPOSE,GROUP_M12_RAW,GROUP_M12_FUSED],
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnFusedGroupM12DecisionV1",
                    "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                    "comparator":if comparator == Arm::Retained {"GROUP_M8"} else {"Fast"},
                    "strata":strata,
                    "strict_win":strict_win,
                    "decision":match (comparator,strict_win) {
                        (Arm::Retained,true)=>"group_m12_beats_group_m8",
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedGroupM12FinalDecisionV1",
                "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                "candidate":GROUP_M12_FUSED,
                "retain_against_group_m8_fused":retained_win,
        ...
        quiet.verify_post_cohort("f32-tn-fused-groupm12/post")?;
        Ok(())
    }
}
