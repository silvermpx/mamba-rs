...
const STRICT_THRESHOLD: f64 = 0.99;
...
fn candidate_over_comparator_ratio(
    ...
    let slots = order.candidate_slots();
...
#[test]
fn candidate_is_an_isolated_m64n96_s2_derivative_of_the_retained_joint_tn_body() {
    let source = candidate_source::compose_candidate_source(JOINT_PRIMITIVES, JOINT_SOURCE)
        .expect("compose M64N96/S2 candidate");
    assert_eq!(
        source.matches(candidate_source::GEMM_SYMBOL).count(),
        1,
        "candidate export must be unique"
    );
    let candidate_section = source
        .split_once("namespace sm89_tf32_test_tn_m64n96_s2 {")
        .expect("candidate namespace")
        .1;
    assert!(candidate_section.contains("return k * 96 + chunk * 4 + (column & 3);"));
    assert!(source.contains("float acc[2][3][4];"));
    assert!(source.contains("float* b_stages = a_stages + 2 * 64 * 32;"));
    assert!(source.contains("int warp_n = (warp & 3) * 24;"));
    assert!(source.contains("const int k8_offsets[4] = {0, 8, 16, 24};"));
    assert!(source.contains("fragments.a[m_atom][0] = raw0;"));
    assert!(source.contains("*destination = __fmaf_rn(params.alpha, accumulator, *destination);"));
    assert_eq!(candidate_source::BLOCK_THREADS, 256);
    assert_eq!(candidate_source::DYNAMIC_SHARED_BYTES, 40_960);
    assert_eq!(candidate_source::REGISTER_CAP, 128);
    assert_eq!(candidate_source::REQUIRED_ACTIVE_BLOCKS, 2);
    assert_eq!(candidate_source::K8_ISSUE_OFFSETS, [0, 8, 16, 24]);
    assert_eq!(
        candidate_source::TRANSPOSE_SYMBOL,
        "gemm_bi_tn_sm89_tf32_pre_rna_transpose_32x32_v1"
    );
}

#[test]
fn timing_protocol_requires_once3_then_retained_once7_before_fast() {
    assert!(all_strata_below(&[[0.98, 0.989]; 4], STRICT_THRESHOLD));
    assert!(!all_strata_below(&[[0.98, 0.99]; 4], STRICT_THRESHOLD));
    assert!(!all_strata_below(&[[0.98, 0.989]; 3], STRICT_THRESHOLD));
    ...
    assert!(!may_run_fast(false, true));
    assert!(!may_run_fast(true, false));
    assert_eq!(BracketOrder::Abba.name(), "ABBA");
    assert_eq!(BracketOrder::Baab.name(), "BAAB");
    assert_eq!(
...
#[test]
fn pre_rna_transpose_oracle_rounds_words_and_zero_fills_padding() {
    let input = [
        0x3f80_1000,
        0xbf80_0fff,
        0x4000_1001,
    ...
    assert_eq!(candidate_source::padded_stride(2), Ok(4));
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_TF32_TN_PRE_RNA_M64N96_S2_PRISM_DISCOVERY";
    const CANDIDATE_SHARED: usize = candidate_source::DYNAMIC_SHARED_BYTES as usize;
    const RETAINED_SHARED: usize = 49_152;
    const OPS: usize = 1;
    ...
    impl Shape {
        fn grid(self, arm: Arm) -> Result<(u32, u32, u32), String> {
            let tile_columns = match arm {
                Arm::Candidate => 96,
                Arm::Retained => 64,
                Arm::Fast => return Err("cuBLAS Fast has no fixed CUDA grid".into()),
            };
            let blocks = self
                .k
                .div_ceil(64)
                .checked_mul(self.n.div_ceil(tile_columns))
                .ok_or("TN grid overflows usize")?;
            Ok((
                u32::try_from(blocks).map_err(|_| "TN grid exceeds u32")?,
                1,
                1,
            ))
        }
        ...
        fn config(self, arm: Arm) -> Result<LaunchConfig, String> {
            let shared_mem_bytes = match arm {
                Arm::Candidate => CANDIDATE_SHARED as u32,
                Arm::Retained => RETAINED_SHARED as u32,
                Arm::Fast => return Err("cuBLAS Fast has no direct launch config".into()),
            };
            Ok(LaunchConfig {
        ...
        fn transpose_stride(self) -> Result<usize, String> {
            candidate_source::padded_stride(self.m)
        }
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "pre_rna_m64n96_bk32_s2",
                Self::Retained => "joint_pre_rna_m64n64_bk32_s3",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if cfg!(debug_assertions) {
            return Err("TF32 TN pre-RNA M64N96/S2 timing requires --release".into());
        }
        ...
        {
            return Err(format!(
                "wrong TF32 TN M64N96/S2 discovery compiler: {compiler:?}"
            ));
        }
        let retained_body = format!("{JOINT_PRIMITIVES}\n{JOINT_SOURCE}");
        let candidate_body =
            candidate_source::compose_candidate_source(JOINT_PRIMITIVES, JOINT_SOURCE)?;
        ...
        resource_gate(
            ...
            CANDIDATE_SHARED,
            candidate_source::REQUIRED_ACTIVE_BLOCKS,
        )?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnPreRnaM64N96S2PrismIdentityV1",
                "retained_symbol":candidate_source::RETAINED_GEMM_SYMBOL,
                "candidate_symbol":candidate_source::GEMM_SYMBOL,
                "retained_transform_symbol":candidate_source::TRANSPOSE_SYMBOL,
                ...
                "conversion":"pre_rna_A_ordered_k8_tf32_v1",
                "change":"joint_TN_M64N64_S3_to_M64N96_S2_geometry_only",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnPreRnaM64N96S2PrismResourceV1","symbol":symbol,
                "threads":256,"registers":registers,"local_bytes":local,
                "static_shared_bytes":static_shared,"dynamic_shared_bytes":dynamic_shared_bytes,
        ...
        if registers <= 0
            ...
            || static_shared != 0
            || max_threads < 256
            || max_dynamic < dynamic_shared_bytes as i32
    ...
    fn transpose_resource_gate(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnPreRnaM64N96S2PrismTransformResourceV1",
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32TnPreRnaM64N96S2PrismGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 2 {
            ...
            let expected_transpose = candidate_source::TRANSPOSE_SYMBOL;
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32TnPreRnaM64N96S2PrismGraphV1","arm":arm.name(),"nodes":2,"gemm_symbol":expected_gemm,"transpose_symbol":expected_transpose,"gemm_grid":case.shape.grid(arm)?,"transpose_grid":case.shape.transpose_config()?.grid_dim,"timing":"whole_two_node_graph"})
            );
        }
        Ok(())
    }
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32TnPreRnaM64N96S2PrismBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        if output_after(runtime, &mut fixture, case, Arm::Candidate, None, 1)? != exact_bits {
            return Err(format!(
                "{} candidate differs from retained joint M64N64/S3",
                case.label
            ));
        }
        let candidate_graph = capture(runtime, &mut fixture, case, Arm::Candidate)?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnPreRnaM64N96S2PrismTargetBitsV1",
    ...
    fn prepare_fast(runtime: &Runtime, case: Case, prepared: &mut Prepared) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnPreRnaM64N96S2PrismFastBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnPreRnaM64N96S2PrismScreenV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid(Arm::Candidate)?,
                "candidate":"pre_rna_m64n96_bk32_s2","comparator":comparator.name(),"path":path.name(),
    ...
    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 TN pre-RNA M64N96/S2 canonical Prism once3 -> once7 -> Fast"]
    fn ada_tf32_tn_pre_rna_m64n96_s2_prism_protocol() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-tn-pre-rna-m64n96-s2-prism/pre", 1_800)?;
        } else {
            quiet.require_pre_context("tf32-tn-pre-rna-m64n96-s2-prism/pre")?;
        }
        let runtime = new_runtime()?;
        assert_eq!(PRISM.shape.grid(Arm::Candidate)?, (126, 1, 1));
        assert_eq!(PRISM.shape.grid(Arm::Retained)?, (186, 1, 1));
        assert_eq!(PRISM.shape.transpose_config()?.grid_dim, (12, 145, 1));
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-tn-pre-rna-m64n96-s2-prism/retained", 256)?;
        } else {
            quiet.require_cohort("tf32-tn-pre-rna-m64n96-s2-prism/retained")?;
        }
        let scout_strata = screen_all(&runtime, case, &mut prepared, Arm::Retained, 3)?;
        let scout_win = all_strata_below(&scout_strata, STRICT_THRESHOLD);
        let retained_strata = if scout_win {
            screen_all(&runtime, case, &mut prepared, Arm::Retained, 7)?
        } else {
            Vec::new()
        };
        let retained_win = scout_win && all_strata_below(&retained_strata, STRICT_THRESHOLD);
        let fast_strata = if retained_win {
            prepare_fast(&runtime, case, &mut prepared)?;
            if idle_resident_mode {
                quiet.require_idle_resident("tf32-tn-pre-rna-m64n96-s2-prism/fast", 256)?;
            } else {
                quiet.require_cohort("tf32-tn-pre-rna-m64n96-s2-prism/fast")?;
            }
            screen_all(&runtime, case, &mut prepared, Arm::Fast, 7)?
        } else {
            Vec::new()
        };
        let fast_win = may_run_fast(scout_win, retained_win)
            && all_strata_below(&fast_strata, STRICT_THRESHOLD);
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnPreRnaM64N96S2PrismDecisionV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid(Arm::Candidate)?,
                "scout_windows":3,"scout_strata":scout_strata,"once7_windows":7,
                "retained_strata":retained_strata,"fast_strata":fast_strata,"threshold":STRICT_THRESHOLD,
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-tn-pre-rna-m64n96-s2-prism/post", 256)?;
        } else {
            quiet.verify_post_cohort("tf32-tn-pre-rna-m64n96-s2-prism/post")?;
        }
        Ok(())
    }
}
