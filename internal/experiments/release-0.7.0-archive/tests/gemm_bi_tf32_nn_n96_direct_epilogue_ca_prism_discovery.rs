//! Test-only Ada TF32 NN N96 direct-epilogue ca-cache discovery.
//! Production kernels and dispatch remain untouched.

#[path = "support/triad_tf32_nn_n96_direct_epilogue_ca_source.rs"]
...
#[test]
fn source_adapter_changes_only_cache_operator_and_export() {
    ...
    assert_eq!(candidate.matches("gbf_tf32_copy_ca(").count(), 2);
    assert_eq!(candidate.matches("gbf_tf32_copy_cg(").count(), 0);
    assert_eq!(
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_NN_N96_DIRECT_EPILOGUE_CA_DISCOVERY";
    const SHARED: usize = 86_016;
    const OPS: usize = 20;
    const SCOUT_WINDOWS: usize = 3;
    const RETAIN_WINDOWS: usize = 7;
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "direct_epilogue_ca_n96",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96DirectEpilogueCaIdentityV1",
                ...
                "change":"use_L1_and_L2_cp_async_ca_instead_of_L2_only_cg",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96DirectEpilogueCaResourceV1","symbol":symbol,
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32NnN96DirectEpilogueCaGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 1 {
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32NnN96DirectEpilogueCaGraphV1","arm":arm.name(),"symbol":symbol,"grid":expected_grid,"block":[256,1,1],"shared":SHARED,"timing":"whole_graph"})
            );
        }
        Ok(())
    }
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32NnN96DirectEpilogueCaBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96DirectEpilogueCaTargetBitsV1",
    ...
    fn prepare_fast(runtime: &Runtime, case: Case, prepared: &mut Prepared) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96DirectEpilogueCaFastBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96DirectEpilogueCaScreenV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid()?,
                "candidate":"direct_epilogue_ca_n96","comparator":comparator.name(),"path":path.name(),
    ...
    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 NN N96 direct-epilogue ca-cache protocol"]
    fn ada_tf32_nn_n96_direct_epilogue_ca_prism_protocol() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-n96-direct-epilogue-ca/pre", 1_800)?;
        } else {
            quiet.require_pre_context("tf32-nn-n96-direct-epilogue-ca/pre")?;
        }
        ...
        for case in [PRISM] {
            ...
            let mut scout_strata = Vec::with_capacity(4);
            let mut retained_strata = Vec::with_capacity(4);
            let mut fast_strata = Vec::with_capacity(4);
            if idle_resident_mode {
                quiet
                    .require_idle_resident("tf32-nn-n96-direct-epilogue-ca/retained", 256)?;
            } else {
                quiet.require_cohort("tf32-nn-n96-direct-epilogue-ca/retained")?;
            }
            ...
            if retained_win {
                ...
                if idle_resident_mode {
                    quiet
                        .require_idle_resident("tf32-nn-n96-direct-epilogue-ca/fast", 256)?;
                } else {
                    quiet.require_cohort("tf32-nn-n96-direct-epilogue-ca/fast")?;
                }
                for path in [Path::Eager, Path::Graph] {
                    for order in [
                        retained_source::BracketOrder::Abba,
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiTriadTf32NnN96DirectEpilogueCaDecisionV1","cell":case.label,
                    "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid()?,
                    "scout_strata":scout_strata,"retained_strata":retained_strata,"fast_strata":fast_strata,
                    "strata_order":["eager/ABBA","eager/BAAB","graph/ABBA","graph/BAAB"],
                    "scout_win":scout_win,"retained_win":retained_win,"fast_win":fast_win,
                    "decision":if !scout_win {
                        "stop_once3"
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-n96-direct-epilogue-ca/post", 256)?;
        } else {
            quiet.verify_post_cohort("tf32-nn-n96-direct-epilogue-ca/post")?;
        }
        Ok(())
    }
}
