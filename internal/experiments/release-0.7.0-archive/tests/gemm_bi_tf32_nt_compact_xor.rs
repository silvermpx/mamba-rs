...
mod triad_tf32_nt_a_ldmatrix_n96_s3_source;
#[path = "support/triad_tf32_nt_compact_a_ldmatrix_source.rs"]
mod triad_tf32_nt_compact_a_ldmatrix_source;
#[path = "support/triad_tf32_nt_compact_ab_ldmatrix_source.rs"]
mod triad_tf32_nt_compact_ab_ldmatrix_source;
...
mod triad_tn_transpose_n96_source;
...
enum CandidateVariant {
    ...
    NtALdmatrixN96S3D768In,
    NtALdmatrixN96S3Prism,
    NtScalarAN96S3D768In,
    NtScalarAN96S3Prism,
...
impl CandidateVariant {
    const fn name(self) -> &'static str {
        match self {
            ...
            Self::NtALdmatrixN96S3D768In => "nt_a_ldmatrix_n96_s3_d768_in",
            Self::NtALdmatrixN96S3Prism => "nt_a_ldmatrix_n96_s3_prism",
            Self::NtScalarAN96S3D768In => "nt_scalar_a_n96_s3_d768_in",
            Self::NtScalarAN96S3Prism => "nt_scalar_a_n96_s3_prism",
    ...
    const fn symbol(self) -> &'static str {
        match self {
            ...
            Self::NtALdmatrixN96S3D768In | Self::NtALdmatrixN96S3Prism => {
                triad_tf32_nt_a_ldmatrix_n96_s3_source::SYMBOL
            }
            Self::NtScalarAN96S3D768In | Self::NtScalarAN96S3Prism => {
                triad_tf32_nt_a_ldmatrix_n96_s3_source::RETAINED_SYMBOL
            }
    ...
    const fn shared_bytes(self) -> u32 {
        match self {
            ...
            Self::NtALdmatrixN96S3D768In
            | Self::NtALdmatrixN96S3Prism
            | Self::NtScalarAN96S3D768In
            | Self::NtScalarAN96S3Prism => 86_016,
    ...
    const fn required_occupancy(self) -> u32 {
        match self {
            ...
            Self::TnSmallRegpipeD128In | Self::TnSmallRegpipeD128Out => 3,
            _ => 1,
        }
    }
    ...
    const fn target_dims(self) -> (usize, usize, usize) {
        match self {
            ...
            | Self::NtALdmatrixN96S3D768In
            | Self::NtScalarAN96S3D768In => (2_048, 768, 3_072),
            Self::CompactEightWarpS2D768Out
            | Self::CompactALdmatrixD768Out
            | Self::PaddedDenseD768Out
            ...
            | Self::NtALdmatrixN96S3Prism
            | Self::NtScalarAN96S3Prism => (4_621, 384, 1_928),
            Self::TnPrepackAPrism => (4_621, 384, 1_928),
            Self::TnTransposeN96Prism => (4_621, 384, 1_928),
            Self::TnSmallRegpipeD128In => (1_024, 128, 512),
            Self::TnSmallRegpipeD128Out => (1_024, 256, 128),
            _ => (2_048, 768, 3_072),
        }
    }
    ...
    fn source(self) -> Result<String, String> {
        match self {
            ...
            Self::CompactAbLdmatrixD768In => {
                ...
                let a_ldmatrix =
                    triad_tf32_nt_compact_a_ldmatrix_source::candidate_source(&parent)?;
                triad_tf32_nt_compact_ab_ldmatrix_source::candidate_source(&a_ldmatrix)
            }
            Self::CompactSharedRnaD768In => {
                let parent = compact_eight_warp_s2_candidate_source()?;
            ...
            Self::NtALdmatrixN96S3D768In | Self::NtALdmatrixN96S3Prism => {
                Ok(triad_tf32_nt_a_ldmatrix_n96_s3_source::candidate_source())
            }
            Self::NtScalarAN96S3D768In | Self::NtScalarAN96S3Prism => {
                Ok(triad_tf32_nt_a_ldmatrix_n96_s3_source::retained_source())
            }
            Self::CompactEightWarpS2D768Out | Self::CompactEightWarpS2Prism => {
    ...
    const fn expects_sm89_finalist_auto(self) -> bool {
        self.is_nt_d768_in_shortscreen()
            || matches!(
                self,
                Self::NtALdmatrixN96S3D768In
                    | Self::NtALdmatrixN96S3Prism
                    | Self::NtScalarAN96S3D768In
                    | Self::NtScalarAN96S3Prism
            )
    }
    ...
    const fn compares_retained_a_ldmatrix(self) -> bool {
        matches!(
            self,
            Self::CompactAbLdmatrixD768In | Self::CompactSharedRnaD768In
        )
    }
    ...
    const fn retained_variant(self) -> Option<Self> {
        match self {
            Self::NtALdmatrixN96S3D768In => Some(Self::NtScalarAN96S3D768In),
            Self::NtALdmatrixN96S3Prism => Some(Self::NtScalarAN96S3Prism),
            _ => None,
        }
    }

    const fn has_a_transform(self) -> bool {
        self.is_prepack_a() || self.is_transpose_n96()
    }

    const fn a_transform_symbol(self) -> Option<&'static str> {
        if self.is_prepack_a() {
            Some(triad_tn_prepack_a_source::PACK_SYMBOL)
        } else if self.is_transpose_n96() {
            Some(triad_tn_transpose_n96_source::TRANSPOSE_SYMBOL)
        } else {
            None
        }
    }
    ...
    const fn is_small_tn_regpipe(self) -> bool {
        matches!(
            self,
            Self::TnSmallRegpipeD128In | Self::TnSmallRegpipeD128Out
        )
    ...
    const fn grid_dim(self) -> (u32, u32, u32) {
        ...
        let (tile_m, tile_n) = self.candidate_tile();
        (
            rows.div_ceil(tile_m) as u32 * columns.div_ceil(tile_n) as u32,
            1,
            1,
        )
    }
    ...
    const fn tn_auto_identity(self) -> (&'static str, (u32, u32, u32), (u32, u32, u32), u32) {
        ...
        match self {
            ...
            Self::TnTransposeN96Prism => (
                TN_AUTO_WIDE_SYMBOL,
                ((dims.1.div_ceil(128) * dims.2.div_ceil(64)) as u32, 1, 1),
                (256, 1, 1),
                TN_AUTO_WIDE_SHARED_BYTES,
            ),
            Self::TnSmallRegpipeD128In | Self::TnSmallRegpipeD128Out => {
                (TN_AUTO_SMALL_SYMBOL, self.grid_dim(), (128, 1, 1), 32_768)
            }
            _ if self.is_tn() => (
                TN_AUTO_WIDE_SYMBOL,
                self.grid_dim(),
...
#[test]
fn tn_transpose_n96_binds_three_cells_to_candidate_and_distinct_auto_routes() {
    for (variant, dims, grid, auto) in [
        (
            CandidateVariant::TnTransposeN96D768In,
            (2_048, 768, 3_072),
            (192, 1, 1),
            (TN_AUTO_NARROW_SYMBOL, (576, 1, 1), (128, 1, 1), 36_864),
        ),
        (
            CandidateVariant::TnTransposeN96D768Out,
            (2_048, 1_536, 768),
            (96, 1, 1),
            (TN_AUTO_NARROW_SYMBOL, (288, 1, 1), (128, 1, 1), 36_864),
        ),
        (
            CandidateVariant::TnTransposeN96Prism,
            (4_621, 384, 1_928),
            (63, 1, 1),
            (TN_AUTO_WIDE_SYMBOL, (93, 1, 1), (256, 1, 1), 79_872),
        ),
        ...
        assert!(variant.is_tn());
        assert!(variant.is_transpose_n96());
        ...
        assert_eq!(variant.required_occupancy(), 1);
        assert_eq!(variant.symbol(), triad_tn_transpose_n96_source::GEMM_SYMBOL);
        assert_eq!(
            variant.a_transform_symbol(),
            Some(triad_tn_transpose_n96_source::TRANSPOSE_SYMBOL)
        );
        assert_eq!(variant.tn_auto_identity(), auto);
    }
}

#[test]
fn tn_small_regpipe_binds_two_d128_cells_and_incumbent_physical_identity() {
    for (variant, dims, grid) in [
        (
            CandidateVariant::TnSmallRegpipeD128In,
            (1_024, 128, 512),
            (128, 1, 1),
        ),
        (
            CandidateVariant::TnSmallRegpipeD128Out,
            (1_024, 256, 128),
            (64, 1, 1),
        ),
    ] {
        assert!(variant.is_tn());
        assert!(variant.is_small_tn_regpipe());
        assert!(variant.compares_fast_tn());
        assert_eq!(variant.target_dims(), dims);
        assert_eq!(variant.candidate_tile(), (16, 32));
        assert_eq!(variant.grid_dim(), grid);
        assert_eq!(variant.threads(), 128);
        assert_eq!(variant.shared_bytes(), 32_768);
        assert_eq!(variant.required_occupancy(), 3);
        assert_eq!(
            variant.tn_screen_schema(),
            "MambaBiTf32TnSmallRegpipeDiscoveryScreenV1"
        );
        assert_eq!(
            variant.tn_fast_screen_schema(),
            "MambaBiTf32TnSmallRegpipeFastScreenV1"
        );
        assert_eq!(
            variant.tn_decision_schema(),
        ...
        assert_eq!(variant.symbol(), triad_tf32_tn_small_regpipe_source::SYMBOL);
        assert_eq!(
            variant.tn_auto_identity(),
            (TN_AUTO_SMALL_SYMBOL, grid, (128, 1, 1), 32_768)
        );
        let source = variant.source().unwrap();
        assert_eq!(source.matches(variant.symbol()).count(), 2);
        assert!(!source.contains(TN_AUTO_SMALL_SYMBOL));
    }
}
...
#[test]
fn compact_ab_ldmatrix_adapter_layers_only_the_b_x2_load_on_retained_a() {
    let compact = compact_eight_warp_s2_candidate_source().unwrap();
    let retained_a = triad_tf32_nt_compact_a_ldmatrix_source::candidate_source(&compact).unwrap();
    let candidate =
        triad_tf32_nt_compact_ab_ldmatrix_source::candidate_source(&retained_a).unwrap();
    assert_eq!(
        candidate
            .matches(triad_tf32_nt_compact_ab_ldmatrix_source::SYMBOL)
            .count(),
        2
    );
    assert!(!candidate.contains(triad_tf32_nt_compact_a_ldmatrix_source::SYMBOL));
    assert!(candidate.contains("ldmatrix.sync.aligned.m8n8.x4.shared.b16"));
    assert!(candidate.contains("ldmatrix.sync.aligned.m8n8.x2.shared.b16"));
    assert_eq!(
        candidate.matches("gemm_bi_tf32_mma_m16n8k8(").count(),
        retained_a.matches("gemm_bi_tf32_mma_m16n8k8(").count()
    );
    assert_eq!(
        triad_tf32_nt_compact_ab_ldmatrix_source::restore_parent(&candidate).unwrap(),
        retained_a
    );
}
...
#[test]
fn nt_a_ldmatrix_n96_s3_pairs_bind_both_large_nt_cells_and_their_scalar_retained_body() {
    for (candidate, retained, dims, grid) in [
        (
            CandidateVariant::NtALdmatrixN96S3D768In,
            CandidateVariant::NtScalarAN96S3D768In,
            triad_tf32_nt_a_ldmatrix_n96_s3_source::TARGET_D768_IN,
            (128, 1, 1),
        ),
        (
            CandidateVariant::NtALdmatrixN96S3Prism,
            CandidateVariant::NtScalarAN96S3Prism,
            triad_tf32_nt_a_ldmatrix_n96_s3_source::TARGET_PRISM,
            (148, 1, 1),
        ),
    ] {
        assert_eq!(candidate.target_dims(), dims);
        assert_eq!(candidate.grid_dim(), grid);
        assert_eq!(candidate.candidate_tile(), (128, 96));
        assert_eq!(
            candidate.threads(),
            triad_tf32_nt_a_ldmatrix_n96_s3_source::BLOCK_THREADS
        );
        assert_eq!(
            candidate.shared_bytes(),
            triad_tf32_nt_a_ldmatrix_n96_s3_source::DYNAMIC_SHARED_BYTES
        );
        assert_eq!(candidate.required_occupancy(), 1);
        assert_eq!(candidate.retained_variant(), Some(retained));
        assert!(candidate.expects_sm89_finalist_auto());
        assert!(retained.expects_sm89_finalist_auto());
        assert_eq!(
            candidate.symbol(),
            triad_tf32_nt_a_ldmatrix_n96_s3_source::SYMBOL
        );
        ...
        assert!(
            candidate
                .source()
                .unwrap()
                .contains("ldmatrix.sync.aligned.m8n8.x4.shared.b16")
        );
        let candidate_source = candidate.source().unwrap();
        assert!(candidate_source.contains("const int k_offsets[4] = {0, 8, 16, 24};"));
        assert!(candidate_source.contains("gemm_bi_tf32_rna(__uint_as_float(raw0))"));
        assert!(candidate_source.contains("gemm_bi_tf32_mma_m16n8k8("));
        assert!(candidate_source.contains("params.alpha == 1.0f"));
        assert!(candidate_source.contains(
            "float* output, const float* a, const float* b, const float* bias,\n    Sm80Tf32KernelParams params"
        ));
        assert!(
            !retained
                .source()
                .unwrap()
                .rsplit_once("struct __align__(16) NtN96S3Storage")
                .unwrap()
                .1
                .contains("ldmatrix.sync.aligned.m8n8.x4.shared.b16")
        );
...
fn compact_ab_ldmatrix_binds_only_d768_in_with_retained_compact_resources() {
    ...
    assert_eq!(
        variant.symbol(),
        triad_tf32_nt_compact_ab_ldmatrix_source::SYMBOL
    );
    assert_eq!(variant.target_dims(), (2_048, 768, 3_072));
    assert_eq!(variant.grid_dim(), (192, 1, 1));
    assert_eq!(variant.threads(), 256);
...
mod cuda_suite {
    ...
    struct Candidate {
        ...
        pack_count: u64,
        transpose_params: Option<TransposeParams>,
        params: Params,
        ctx: GpuCtx,
    }
    ...
    fn compile_candidate(
        ...
        let pack_function = if let Some(transform_symbol) = variant.a_transform_symbol() {
            Some(module.load_function(transform_symbol).map_err(|error| {
                format!("load {} A-transform symbol: {error:?}", variant.name())
            })?)
        } else {
            None
        };
    ...
    fn launch_transpose_a(
        ...
        let output = output.pointer(ctx)?;
        let mut builder = ctx.stream.launch_builder(function);
        builder.arg(&input).arg(&output).arg(&params);
        unsafe { builder.launch(config) }
            .map(|_| ())
            .map_err(|error| format!("launch TN raw-bit A transpose: {error:?}"))
    }

    #[allow(clippy::too_many_arguments)]
    fn launch_a_transform(
        ctx: &GpuCtx,
        function: &CudaFunction,
        input: &GuardedBuffer,
        output: &GuardedBuffer,
        ...
        if variant.is_transpose_n96() {
            launch_transpose_a(
                ...
                output,
                config,
                transpose_params.ok_or("missing TN transpose parameters")?,
            )
        } else {
            launch_pack_a(ctx, function, input, output, config, pack_count)
        }
    }
    ...
    impl Candidate {
        ...
        fn new_reusing(
            ...
            if size_of::<TransposeParams>() != 12 || align_of::<TransposeParams>() != 4 {
                return Err(format!(
                    "TN transpose parameter ABI changed: size={} align={}",
                    size_of::<TransposeParams>(),
                    align_of::<TransposeParams>()
                ));
            }
            let ctx = GpuCtx::new(device)?;
            let output = GuardedBuffer::new(&ctx, words.0.clone())?;
            let a = GuardedBuffer::new(&ctx, words.1.clone())?;
            let transpose_stride = variant
                .is_transpose_n96()
                .then(|| triad_tn_transpose_n96_source::padded_stride(dims.0))
                .transpose()?;
            let transformed_a_len = if let Some(stride) = transpose_stride {
                dims.1
                    .checked_mul(stride)
                    .ok_or("TN transposed A extent overflows usize")?
            } else {
                words.1.len()
            };
            let packed_a = variant
                .has_a_transform()
                .then(|| GuardedBuffer::new(&ctx, vec![0xdead_beef; transformed_a_len]))
                .transpose()?;
            ...
            let (tile_m, tile_n) = variant.candidate_tile();
            let config = LaunchConfig {
                grid_dim: (
                    (rows as u32).div_ceil(tile_m as u32)
                        * (columns as u32).div_ceil(tile_n as u32),
                    1,
                    1,
                ),
                block_dim: (variant.threads(), 1, 1),
                shared_mem_bytes: variant.shared_bytes(),
            };
            let (lda, ldb, ldc) = if let Some(stride) = transpose_stride {
                (stride, dims.2, dims.2)
            } else if variant.is_tn() {
                (dims.1, dims.2, dims.2)
            } else {
                (dims.2, dims.2, dims.1)
            };
            let (param_m, param_k, param_n) = if variant.uses_fixed_n96_source() {
            ...
            let params = Params {
                ...
                m: i32::try_from(param_m).map_err(|_| "M exceeds i32")?,
                k: i32::try_from(param_k).map_err(|_| "K exceeds i32")?,
                n: i32::try_from(param_n).map_err(|_| "N exceeds i32")?,
            ...
            let transpose_params = transpose_stride
                .map(|output_stride| -> Result<TransposeParams, String> {
                    Ok(TransposeParams {
                        rows: i32::try_from(dims.0).map_err(|_| "transpose rows exceed i32")?,
                        columns: i32::try_from(dims.1)
                            .map_err(|_| "transpose columns exceed i32")?,
                        output_stride: i32::try_from(output_stride)
                            .map_err(|_| "transpose stride exceeds i32")?,
                    })
                })
                .transpose()?;
            let pack_config = if let Some(transpose) = transpose_params {
                Some(LaunchConfig {
                    grid_dim: (
                        (transpose.columns.max(1) as u32).div_ceil(32),
                        (transpose.output_stride.max(1) as u32).div_ceil(32),
                        1,
                    ),
                    block_dim: (32, 8, 1),
                    shared_mem_bytes: 0,
                })
            } else if variant.is_prepack_a() {
                Some(LaunchConfig {
        ...
        fn validate_transposed_a(&self) -> Result<(), String> {
            if !self.variant.is_transpose_n96() {
                return Ok(());
            }
            let params = self
                .transpose_params
                .ok_or("missing TN transpose parameters during preflight")?;
            let rows = usize::try_from(params.rows).map_err(|_| "negative transpose rows")?;
            let columns =
                usize::try_from(params.columns).map_err(|_| "negative transpose columns")?;
            let stride = usize::try_from(params.output_stride)
                .map_err(|_| "negative transpose output stride")?;
            let actual = self
                .packed_a
                .as_ref()
                .ok_or("missing TN transposed A scratch during preflight")?
                .snapshot(&self.ctx, false)?;
            triad_tn_transpose_n96_source::validate_transposed_words(
                &self.a.active,
                rows,
                columns,
                stride,
    ...
    fn validate_candidate_graph(
        ...
        for node in nodes {
            ...
            if name == variant.symbol() {
            ...
            } else if Some(name.as_ref()) == variant.a_transform_symbol() {
                ...
                let third_size = if variant.is_transpose_n96() { 12 } else { 8 };
                for (index, expected) in [(0, 8), (8, 8), (16, third_size)].into_iter().enumerate()
                {
                    let mut offset = 0;
                    let mut size = 0;
                    cuda_ok(
                    ...
                    if (offset, size) != expected {
                        return Err(format!(
                            "{} A-transform ABI parameter {index} changed: {:?}",
                            variant.name(),
                            (offset, size)
                        ));
                    }
        ...
        if saw_pack != expected_pack.is_some() {
            return Err(format!(
                "{} graph A-transform membership changed",
                variant.name()
            ));
        }
    ...
    fn validate_resources(candidate: &Candidate, source_sha: &str) -> Result<(), String> {
        ...
        if let Some(pack) = &candidate.pack_function {
            ...
            let expected_static = if variant.is_transpose_n96() { 4_224 } else { 0 };
            if variant.is_transpose_n96() {
                println!(
                    "{{\"schema\":\"MambaBiTf32TnTransposeN96TransformResourceV1\",\"variant\":\"{}\",\"symbol\":\"{}\",\"local_bytes\":{pack_local},\"static_shared_bytes\":{pack_static},\"expected_static_shared_bytes\":{expected_static},\"max_threads\":{pack_threads}}}",
                    variant.name(),
                    variant.a_transform_symbol().unwrap(),
                );
            } else {
                println!(
            ...
            if pack_local != 0 || pack_static != expected_static || pack_threads < 256 {
                return Err(format!(
                    "{} A-transform resource gate failed: local={pack_local} static={pack_static} expected_static={expected_static} max_threads={pack_threads}",
                    variant.name()
                ));
            }
        }
    ...
    fn validate_reference(
        ...
        let (strides, op, symbol, module, shared, block, grid) = if variant.is_tn() {
            ...
            let finalist = variant.expects_sm89_finalist_auto()
                && actual_auto
    ...
    fn check_bits(
        ...
        for path in [Path::Eager, Path::Graph] {
            for repeat in 0..2 {
                ...
                candidate.validate_transposed_a()?;
    ...
    fn screen_nt_fast(
        ...
        let schema = match candidate.variant {
            CandidateVariant::CompactSharedRnaD768In => "MambaBiTf32NtSharedStageRnaFastScreenV1",
            CandidateVariant::CompactAbLdmatrixD768In => {
                "MambaBiTf32NtCompactAbLdmatrixFastScreenV1"
            }
            _ => "MambaBiTf32NtCompactALdmatrixFastScreenV1",
        };
        println!(
    ...
    fn screen_nt_retained(
        ...
        let (m, k, n) = candidate.variant.target_dims();
        let schema = match candidate.variant {
            CandidateVariant::CompactSharedRnaD768In => {
                "MambaBiTf32NtSharedStageRnaRetainedScreenV1"
            }
            CandidateVariant::CompactAbLdmatrixD768In => {
                "MambaBiTf32NtCompactAbLdmatrixRetainedScreenV1"
            }
            CandidateVariant::NtALdmatrixN96S3D768In | CandidateVariant::NtALdmatrixN96S3Prism => {
                "MambaBiTf32NtALdmatrixN96S3RetainedScreenV1"
            }
            _ => return Err("retained-A screen requires a derived A-ldmatrix candidate".into()),
        };
        println!(
    ...
    fn screen_tn(
        ...
        if candidate.variant.is_transpose_n96() {
            println!(
                "{{\"schema\":\"{schema}\",\"op\":\"TN\",\"variant\":\"{}\",\"symbol\":\"{}\",\"transform_symbol\":\"{}\",\"dynamic_shared_bytes\":{},\"shape\":[{m},{k},{n}],\"physical_n96_shape\":[{k},{m},{n}],\"alpha\":1.0,\"beta\":1.0,\"bias\":false,\"path\":\"{}\",\"order\":\"{}\",\"windows\":{WINDOWS},\"warmups_per_arm\":{TN_WARMUPS},\"logical_gemms_per_observation\":1,\"candidate_timed_nodes\":[\"transpose_a_raw_bits\",\"gemm\"],\"reseed_scope\":\"C+A+B+transposed_A_scratch\",\"reseed_position\":\"before_start_event\",\"post_download_before_next_reset\":true,\"observation_arms\":[\"{}\",\"{}\",\"{}\",\"{}\"],\"observations_us\":{observations},\"ratio_direction\":\"candidate_pipeline_over_actual_auto\",\"ratio_p50\":{p50:.9},\"ratio_p95\":{p95:.9},\"auto_samples_us\":{},\"candidate_samples_us\":{},\"ratios\":{}}}",
                candidate.variant.name(),
                candidate.variant.symbol(),
                candidate.variant.a_transform_symbol().unwrap(),
        ...
        } else if candidate.variant.is_prepack_a() {
            println!(
    ...
    fn screen_tn_fast(
        ...
        if candidate.variant.is_transpose_n96() {
            println!(
                "{{\"schema\":\"MambaBiTf32TnTransposeN96FastScreenV1\",\"op\":\"TN\",\"variant\":\"{}\",\"candidate_symbol\":\"{}\",\"transform_symbol\":\"{}\",\"fast_symbol\":\"{}\",\"fast_compute\":\"CUBLAS_COMPUTE_32F_FAST_TF32\",\"shape\":[{m},{k},{n}],\"path\":\"{}\",\"order\":\"{}\",\"windows\":{WINDOWS},\"warmups_per_arm\":{TN_WARMUPS},\"logical_gemms_per_observation\":1,\"candidate_timed_nodes\":[\"transpose_a_raw_bits\",\"gemm\"],\"reseed_scope\":\"C+A+B+transposed_A_scratch\",\"reseed_position\":\"before_start_event\",\"post_download_before_next_reset\":true,\"observation_arms\":[\"{}\",\"{}\",\"{}\",\"{}\"],\"observations_us\":{observations},\"ratio_direction\":\"candidate_pipeline_over_fast\",\"ratio_p50\":{p50:.9},\"ratio_p95\":{p95:.9},\"fast_samples_us\":{},\"candidate_samples_us\":{},\"ratios\":{}}}",
                candidate.variant.name(),
                candidate.variant.symbol(),
                candidate.variant.a_transform_symbol().unwrap(),
                fast.symbol,
    ...
    fn run_tn_in_context(variant: CandidateVariant, existing_own_context: bool) {
        ...
        if variant.is_transpose_n96() {
            ...
            println!(
                "{{\"schema\":\"{schema}\",\"op\":\"TN\",\"variant\":\"{}\",\"symbol\":\"{}\",\"actual_auto_symbol\":\"{actual_auto_symbol}\",\"fast_symbol\":\"{}\",\"candidate_grid\":[{},{},{}],\"actual_auto_grid\":[{},{},{}],\"candidate_block\":[{},1,1],\"actual_auto_block\":[{},{},{}],\"candidate_dynamic_shared_bytes\":{},\"actual_auto_dynamic_shared_bytes\":{actual_auto_shared_bytes},\"shape\":[{m},{k},{n}],\"alpha\":1.0,\"beta\":1.0,\"bias\":false,\"candidate_timed_nodes\":[\"gemm\"],\"source_sha256\":\"{source_sha}\",\"pre\":{pre:?},\"timed_pre\":{timed_pre:?},\"post\":{post:?},\"strata_order\":[\"eager/ABBA\",\"eager/BAAB\",\"graph/ABBA\",\"graph/BAAB\"],\"strata_fields\":[\"ratio_p50\",\"ratio_p95\"],\"actual_auto_strata\":{},\"fast_tf32_strata\":{},\"retain_against_actual_auto\":{retain_auto},\"retain_against_fast\":{retain_fast},\"retain\":{retain},\"decision\":\"{}\",\"promotion\":false}}",
                variant.name(),
                variant.symbol(),
    ...
    fn all_nt_n96_strata_below(strata: &[(f64, f64)]) -> bool {
        strata.len() == 4
            && strata
                .iter()
    ...
    fn screen_nt_n96_pair(
        ...
        for path in [Path::Eager, Path::Graph] {
            for order in [Order::Abba, Order::Baab] {
                strata.push(
                    screen_nt_retained(candidate, retained, path, order, expected, windows)
                        .unwrap(),
                );
            }
        }
    ...
    fn run_nt_n96_pair(variant: CandidateVariant) {
        let retained_variant = variant
            .retained_variant()
            .expect("N96 paired protocol requires an A-scalar retained variant");
        assert!(
            !cfg!(debug_assertions),
            "{} NT N96 discovery requires --release",
            variant.name()
        );
        let quiet = QuietGpu::for_cuda_ordinal(0).unwrap();
        ...
        let (mut candidate, candidate_source_sha) =
            Candidate::new(&device, variant, target, &target_words, 1.0).unwrap();
        validate_resources(&candidate, &candidate_source_sha).unwrap();
        let (mut retained, retained_source_sha) =
            Candidate::new(&device, retained_variant, target, &target_words, 1.0).unwrap();
        validate_resources(&retained, &retained_source_sha).unwrap();
        let exact_bits = check_bits(
            &mut actual_auto,
            &auto_ctx,
            &mut candidate,
            &target_words,
        ...
        assert_eq!(
            exact_bits,
            retained_bits,
            "{} candidate and retained body differ",
            variant.name()
        );

        let timed_pre = quiet
            .require_cohort(&format!("{cohort}retained-once3"))
            .unwrap();
        let scout_strata = screen_nt_n96_pair(&mut candidate, &mut retained, &exact_bits, 3);
        let scout_win = all_nt_n96_strata_below(&scout_strata);
        let retained_strata = if scout_win {
            screen_nt_n96_pair(&mut candidate, &mut retained, &exact_bits, 7)
        } else {
            Vec::new()
        };
        let retained_win = scout_win && all_nt_n96_strata_below(&retained_strata);
        let mut auto_strata = Vec::new();
        if retained_win {
            quiet
                .require_cohort(&format!("{cohort}public-auto-once7"))
                .unwrap();
            for path in [Path::Eager, Path::Graph] {
                for order in [Order::Abba, Order::Baab] {
                    auto_strata.push(
        ...
        let auto_win = retained_win && all_nt_n96_strata_below(&auto_strata);
        let (fast_strata, fast_source) = if auto_win {
            let mut fast = FastNt::new(&device, target, &target_words).unwrap();
            let fast_bits = check_fast_nt_bits(&mut fast).unwrap();
            let fast_source = fast.symbol.clone();
            let strata = screen_nt_n96_fast(&mut candidate, &mut fast, &exact_bits, &fast_bits, 7);
            (strata, Some(fast_source))
        } else {
            (Vec::new(), None)
        };
        let fast_win = retained_win && all_nt_n96_strata_below(&fast_strata);
        let post = quiet.verify_post_cohort(&format!("{cohort}post")).unwrap();
        let (m, k, n) = target;
        let scout = scout_strata
            .iter()
            .map(|(p50, p95)| [*p50, *p95])
            .collect::<Vec<_>>();
        let retained = retained_strata
            .iter()
            .map(|(p50, p95)| [*p50, *p95])
            .collect::<Vec<_>>();
        let auto = auto_strata
            .iter()
            .map(|(p50, p95)| [*p50, *p95])
            .collect::<Vec<_>>();
        let fast = fast_strata
            .iter()
            .map(|(p50, p95)| [*p50, *p95])
            .collect::<Vec<_>>();
        println!(
            "{{\"schema\":\"MambaBiTf32NtALdmatrixN96S3DecisionV1\",\"variant\":\"{}\",\"retained_variant\":\"{}\",\"shape\":[{m},{k},{n}],\"candidate_grid\":[{},{},{}],\"candidate_symbol\":\"{}\",\"retained_symbol\":\"{}\",\"candidate_source_sha256\":\"{candidate_source_sha}\",\"retained_source_sha256\":\"{retained_source_sha}\",\"pre\":{pre:?},\"timed_pre\":{timed_pre:?},\"post\":{post:?},\"candidate_retained_exact_bits\":true,\"eager_repeats\":2,\"graph_repeats\":2,\"guards_and_input_immutability\":true,\"resource_and_occupancy_gate\":true,\"scout_windows\":3,\"scout_strata\":{},\"once7_windows\":7,\"retained_strata\":{},\"public_auto_strata\":{},\"fast_strata\":{},\"fast_symbol\":{},\"public_auto_deferred_until_retained_win\":true,\"fast_deferred_until_public_auto_win\":true,\"threshold\":0.99,\"scout_win\":{scout_win},\"retained_win\":{retained_win},\"public_auto_win\":{auto_win},\"fast_win\":{fast_win},\"decision\":\"{}\",\"promotion\":false}}",
            variant.name(),
            retained_variant.name(),
            variant.grid_dim().0,
            variant.grid_dim().1,
            variant.grid_dim().2,
            variant.symbol(),
            retained_variant.symbol(),
            json_pairs(&scout),
            json_pairs(&retained),
            json_pairs(&auto),
            json_pairs(&fast),
    ...
    fn run(variant: CandidateVariant) {
        ...
        if variant.retained_variant().is_some() {
            run_nt_n96_pair(variant);
            return;
        }
        assert!(
            !cfg!(debug_assertions),
        ...
        let mut retained = variant.compares_retained_a_ldmatrix().then(|| {
            ...
            for path in [Path::Eager, Path::Graph] {
                for order in [Order::Abba, Order::Baab] {
                    retained_strata.push(
                        screen_nt_retained(&mut candidate, retained, path, order, &golden, WINDOWS)
                            .unwrap(),
                    );
                }
            }
        ...
        if variant.compares_retained_a_ldmatrix() {
            ...
            let schema = match variant {
                CandidateVariant::CompactSharedRnaD768In => "MambaBiTf32NtSharedStageRnaDecisionV1",
                CandidateVariant::CompactAbLdmatrixD768In => {
                    "MambaBiTf32NtCompactAbLdmatrixDecisionV1"
                }
                _ => unreachable!("retained-A decision requires a derived candidate"),
            };
            println!(
                "{{\"schema\":\"{schema}\",\"variant\":\"{}\",\"symbol\":\"{}\",\"actual_auto_symbol\":\"{SM89_FINALIST_SYMBOL}\",\"actual_auto_role\":\"identity_and_exact_bits_only\",\"retained_variant\":\"{}\",\"retained_symbol\":\"{}\",\"fast_symbol\":\"{}\",\"fast_compute\":\"CUBLAS_COMPUTE_32F_FAST_TF32\",\"candidate_grid\":[192,1,1],\"candidate_block\":[256,1,1],\"dynamic_shared_bytes\":49152,\"required_occupancy\":2,\"shape\":[{m},{k},{n}],\"source_sha256\":\"{source_sha}\",\"pre\":{pre:?},\"timed_pre\":{timed_pre:?},\"post\":{post:?},\"strata_fields\":[\"ratio_p50\",\"ratio_p95\"],\"candidate_over_retained_a_ldmatrix\":{},\"candidate_over_fast\":{},\"retain\":{retain},\"decision\":\"{}\",\"promotion\":false}}",
                variant.name(),
                variant.symbol(),
    ...
    #[test]
    #[ignore = "requires exclusive Ada CC8.9 CUDA13.2; NT N96 S3 A-ldmatrix paired retained screen"]
    fn ada_tf32_nt_a_ldmatrix_n96_s3_d768_in_discovery_once3_then_once7() {
        run(CandidateVariant::NtALdmatrixN96S3D768In);
    }

    #[test]
    #[ignore = "requires exclusive Ada CC8.9 CUDA13.2; NT N96 S3 A-ldmatrix paired retained screen"]
    fn ada_tf32_nt_a_ldmatrix_n96_s3_prism_discovery_once3_then_once7() {
        run(CandidateVariant::NtALdmatrixN96S3Prism);
    }

    #[test]
    #[ignore = "requires exclusive Ada CC8.9 CUDA13.2; compact A+B ldmatrix NT d768-in discovery"]
    fn ada_tf32_nt_compact_ab_ldmatrix_d768_in_discovery_once7() {
        run(CandidateVariant::CompactAbLdmatrixD768In);
    }
    ...
    #[test]
    #[ignore = "requires exclusive Ada CC8.9 CUDA13.2; whole-pipeline TN raw-transpose RNA-N96 d768-in discovery"]
    fn ada_tf32_tn_d768_in_transpose_rna_n96_discovery_once7() {
        run(CandidateVariant::TnTransposeN96D768In);
    }

    #[test]
    #[ignore = "requires exclusive Ada CC8.9 CUDA13.2; whole-pipeline TN raw-transpose RNA-N96 d768-out discovery"]
    fn ada_tf32_tn_d768_out_transpose_rna_n96_discovery_once7() {
        run(CandidateVariant::TnTransposeN96D768Out);
    }
    ...
    #[test]
    #[ignore = "requires exclusive Ada CC8.9 CUDA13.2; TF32 TN small register-pipeline d128-in discovery"]
    fn ada_tf32_tn_d128_in_small_regpipe_discovery_once7() {
        run(CandidateVariant::TnSmallRegpipeD128In);
    }

    #[test]
    #[ignore = "requires exclusive Ada CC8.9 CUDA13.2; TF32 TN small register-pipeline d128-out discovery"]
    fn ada_tf32_tn_d128_out_small_regpipe_discovery_once7() {
        run(CandidateVariant::TnSmallRegpipeD128Out);
    }
}
