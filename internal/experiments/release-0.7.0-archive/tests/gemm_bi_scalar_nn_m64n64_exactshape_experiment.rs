...
const LB3_SYMBOL: &str = "gemm_bi_nn_m64n64_bk16_s2_fixed_semantics_lb3_exp_v1";
const LB4_SYMBOL: &str = "gemm_bi_nn_m64n64_bk16_s2_fixed_semantics_lb4_exp_v1";
const EXACTSHAPE_SYMBOL: &str = "gemm_bi_nn_m64n64_bk16_s2_exactshape_exp_v1";
const CUDA_SOURCE_FILE: &str = "gemm_bi_scalar_nn_m64n64_exactshape_experiment.cu";
const SCREEN_WINDOWS: usize = 21;
const OFFICIAL_WINDOWS: usize = 101;
const MIN_SPEEDUP_P05: f64 = 1.002;
const MIN_SPEEDUP_P50: f64 = 1.002;

const QUALIFIED_CELLS: [(usize, usize, usize); 7] = [
    (2_048, 3_072, 768),
    (4_096, 3_072, 1_536),
    (512, 3_072, 768),
    (4_096, 512, 768),
...
fn exact_windows(value: Option<&OsStr>, expected: usize) -> Result<usize, String> {
    if !matches!(expected, SCREEN_WINDOWS | OFFICIAL_WINDOWS) {
        return Err(format!("unsupported qualification window count {expected}"));
    }
    ...
    if actual != expected {
        return Err(format!(
            "qualification requires exactly {expected} windows, received {actual}"
        ));
    }
    Ok(actual)
}
...
fn validate_qualified_dims(dims: (usize, usize, usize)) -> Result<(), String> {
    if !QUALIFIED_CELLS.contains(&dims) {
        return Err(format!(
            "M64N64 fixed-semantics experiment rejected unqualified dimensions {dims:?}"
        ));
    }
    Ok(())
}
...
fn ptxas_resource_contract(
    ...
    let max_registers = match (target, arm) {
        ("sm_80", ResourceArm::Production) => 123,
        ("sm_80", ResourceArm::Lb3 | ResourceArm::Lb4 | ResourceArm::ExactShape) => 123,
        ("sm_120", ResourceArm::Production) => 101,
        ("sm_120", ResourceArm::Lb3 | ResourceArm::Lb4 | ResourceArm::ExactShape) => 101,
        _ => return Err(format!("unsupported PTXAS resource target {target}")),
    };
    Ok(PtxasResourceContract { max_registers })
}
...
fn validate_ptxas_register_observation(
    ...
    if registers.len() != 1 || registers[0] == 0 || registers[0] > contract.max_registers {
        return Err(format!(
            "register census is {registers:?}, required range=1..={}",
            contract.max_registers
        ));
    }
    Ok(())
}
...
fn sm120_driver_resource_contract(arm: ResourceArm) -> DriverResourceContract {
    match arm {
        ...
        ResourceArm::Lb3 | ResourceArm::Lb4 | ResourceArm::ExactShape => DriverResourceContract {
            max_registers: 103,
            min_occupancy: 4,
        },
    }
...
fn validate_driver_resource_observation(
    ...
    if registers <= 0
        || registers > contract.max_registers
        || occupancy < contract.min_occupancy
        || local != 0
        || static_shared != 0
    {
        return Err(format!(
            "registers={registers} max_registers={} occupancy={occupancy} min_occupancy={} local={local} static_shared={static_shared}",
            contract.max_registers, contract.min_occupancy
        ));
    }
    Ok(())
}
...
const EXCEPTIONAL_CASES: [ExceptionalCase; 8] = [
    ExceptionalCase {
        ...
        frozen_output_bits: 0x0000_0001,
    },
    ExceptionalCase {
        reduction: 15,
        a_bits: 0x007f_ffff,
        b_bits: 1.0_f32.to_bits(),
        frozen_output_bits: 0x007f_ffff,
    },
    ExceptionalCase {
        reduction: 16,
        a_bits: 1.0_f32.to_bits(),
        b_bits: f32::INFINITY.to_bits(),
        frozen_output_bits: 0x7f80_0000,
    },
    ExceptionalCase {
        reduction: 31,
        a_bits: (-1.0_f32).to_bits(),
        b_bits: f32::INFINITY.to_bits(),
        frozen_output_bits: 0xff80_0000,
    },
    ExceptionalCase {
        reduction: 32,
        a_bits: 0x7fc2_3456,
        b_bits: 1.0_f32.to_bits(),
        frozen_output_bits: 0x7fff_ffff,
    },
    ExceptionalCase {
        reduction: 63,
        a_bits: 0x7fa1_2345,
    ...
    ExceptionalCase {
        ...
        b_bits: (-1.0_f32).to_bits(),
        frozen_output_bits: 0x0000_0000,
    },
    ExceptionalCase {
        reduction: 767,
        a_bits: (-0.0_f32).to_bits(),
...
#[test]
fn fixed_semantics_source_exports_only_the_three_tournament_arms() {
    let source = read_candidate_source().expect("candidate CUDA source must exist");
    for symbol in [LB3_SYMBOL, LB4_SYMBOL, EXACTSHAPE_SYMBOL] {
    ...
    for required in [
        ...
        "__launch_bounds__(QUALIFIED_NN_M64N64_THREADS, 3)",
        "__launch_bounds__(QUALIFIED_NN_M64N64_THREADS, 4)",
        "sizeof(QualifiedNnM64N64Params) == 24",
...
#[test]
fn qualified_shape_gate_accepts_seven_cells_and_rejects_every_neighbor() {
    assert_eq!(QUALIFIED_CELLS.len(), 7);
    assert!(QUALIFIED_CELLS.contains(&DIMS));
    for dims in QUALIFIED_CELLS {
        assert!(validate_qualified_dims(dims).is_ok());
        for neighbor in [
            ...
            assert!(
                validate_qualified_dims(neighbor).is_err(),
                "accepted {neighbor:?}"
            );
        }
    }
...
fn resource_contracts_are_arch_specific_strict_and_fail_closed() {
    ...
    for arm in [ResourceArm::Lb3, ResourceArm::Lb4, ResourceArm::ExactShape] {
        assert_eq!(
            ptxas_resource_contract("sm_80", arm).unwrap().max_registers,
            123
        );
        assert_eq!(
            ptxas_resource_contract("sm_120", arm)
                .unwrap()
                .max_registers,
            101
        );
    }
    assert!(ptxas_resource_contract("sm_89", ResourceArm::Lb3).is_err());
    ...
    assert_eq!(
        sm120_driver_resource_contract(ResourceArm::Lb4),
        DriverResourceContract {
            max_registers: 103,
            min_occupancy: 4,
        }
    ...
    let ptxas = ptxas_resource_contract("sm_120", ResourceArm::Lb3).unwrap();
    assert!(validate_ptxas_register_observation(ptxas, &[101]).is_ok());
    for invalid in [vec![], vec![0], vec![102], vec![101, 101]] {
        assert!(validate_ptxas_register_observation(ptxas, &invalid).is_err());
    }

    let driver = sm120_driver_resource_contract(ResourceArm::Lb3);
    assert!(validate_driver_resource_observation(driver, 103, 4, 0, 0).is_ok());
    for invalid in [
        (104, 4, 0, 0),
        (103, 3, 0, 0),
        (103, 4, 1, 0),
...
#[test]
fn exceptional_literals_are_frozen_from_existing_cuda_13_2_evidence() {
    assert_eq!(
        EXCEPTIONAL_CASES.map(|case| case.frozen_output_bits),
        [
            0x0000_0001,
            0x007f_ffff,
            0x7f80_0000,
...
fn windows_percentiles_and_promotion_thresholds_are_fail_closed() {
    assert_eq!(exact_windows(None, SCREEN_WINDOWS).unwrap(), 21);
    assert_eq!(exact_windows(None, OFFICIAL_WINDOWS).unwrap(), 101);
    assert_eq!(
    ...
    assert!(exact_windows(Some(OsStr::new("21")), OFFICIAL_WINDOWS).is_err());
    assert!(exact_windows(None, 100).is_err());
    assert!(percentile(&[], 0.5).is_err());
    for invalid in [f64::NAN, f64::INFINITY, 0.0, -1.0] {
        assert!(percentile(&[1.0, invalid], 0.5).is_err());
    }
    assert!(validate_speedup(21, &[1.0019; 21]).is_err());
    assert!(validate_speedup(21, &[1.002; 21]).is_ok());
    assert!(validate_speedup(101, &vec![1.002; 101]).is_ok());
}
...
mod cuda_experiment {
    ...
    use super::{
        DIMS, EXCEPTIONAL_CASES, LB3_SYMBOL, OFFICIAL_WINDOWS, PRODUCTION_SYMBOL, ResourceArm,
        SCREEN_WINDOWS, exact_windows, percentile, ptxas_resource_contract, read_candidate_source,
        sm120_driver_resource_contract, validate_driver_resource_observation,
        validate_ptxas_register_observation, validate_qualified_dims, validate_speedup,
    };
    ...
    impl Arm {
        ...
        fn resource_arm(self) -> ResourceArm {
            match self {
                ...
                Self::Candidate => ResourceArm::Lb3,
            }
        }
    }
    ...
    const _: [(); 24] = [(); std::mem::size_of::<CandidateParams>()];
    const _: [(); 4] = [(); std::mem::align_of::<CandidateParams>()];
    ...
    fn compile_ptx(target: &'static str) -> Result<String, String> {
        ...
        cudarc::nvrtc::compile_ptx_with_opts(compose_source()?, options)
            ...
            .map_err(|error| format!("compile M64N64 fixed-semantics experiment: {error:?}"))
    }
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if identity.compute_capability != (12, 0) || identity.multiprocessor_count != 170 {
            return Err(format!(
                "M64N64 fixed-semantics experiment requires CC12.0/170SM, found CC{}.{} with {} SMs",
        ...
        let module = device
            ...
            .map_err(|error| format!("load M64N64 experiment module: {error:?}"))?;
        Ok(Runtime {
            production_ctx,
            stream,
            module,
    ...
    fn launch(
        ...
        {
            return Err("M64N64 launch operands or contiguous strides drifted".into());
        }
        ...
        {
            return Err("M64N64 launch requires non-null 16-byte-aligned C/A/B".into());
        }
        match arm {
            Arm::Production => {
            ...
            Arm::Candidate => {
                ...
                builder.arg(&a);
                builder.arg(&b);
                builder.arg(&fixture.candidate_params);
                unsafe { builder.launch(arm.config()) }
                    .map(|_| ())
                    .map_err(|error| format!("launch {}: {error:?}", arm.symbol()))
            }
    ...
    fn graph_identity(graph: &CudaGraph, arm: Arm) -> Result<[u8; 32], String> {
        ...
        digest.update(b"scalar-nn-m64n64-fixed-graph-args.v1");
        let sizes: &[usize] = match arm {
            Arm::Production => &[8, 8, 8, 8, 32],
            Arm::Candidate => &[8, 8, 8, 24],
        };
    ...
    fn validate_witnesses(fixture: &Fixture, production: &[u32]) -> Result<(), String> {
        if fixture.kind == 1 {
            ...
            if production[0] != case.frozen_output_bits {
                return Err(format!(
                    "production exceptional witness {} drifted: expected 0x{:08x}, received 0x{:08x}",
                    fixture.kind - 2,
                    case.frozen_output_bits,
                    production[0]
                ));
            }
        }
    ...
    fn validate_driver_resources(kernels: &Kernels) -> Result<(), String> {
        ...
        for arm in [Arm::Production, Arm::Candidate] {
            ...
            eprintln!(
                "scalar_nn_m64n64_fixed resource symbol={} threads={} dynamic_shared={} static_shared={} registers={} local={} occupancy={}",
                arm.symbol(),
                config.block_dim.0,
    ...
    fn validate_ptx(ptx: &str) -> Result<(), String> {
        for arm in [Arm::Production, Arm::Candidate] {
            ...
            let expected_parameters = match arm {
                Arm::Production => 5,
                Arm::Candidate => 4,
            };
            if parameters.matches(".param").count() != expected_parameters
    ...
    fn validate_ptxas_and_sass(target: &str, ptx: &str) -> Result<(), String> {
        ...
        let stem = format!("mamba-nn-m64n64-fixed-{target}-{nonce}");
        ...
        let result = (|| {
            ...
            for arm in [Arm::Production, Arm::Candidate] {
                ...
                eprintln!(
                    "scalar_nn_m64n64_fixed ptxas target={target} symbol={} registers={}",
                    arm.symbol(),
                    registers[0]
                );
    ...
    fn paired(
        ...
        let label = format!("scalar-nn-m64n64-fixed/{path:?}/{order:?}/{windows}");
        ...
        let body = (|| {
            ...
            eprintln!(
                "scalar_nn_m64n64_fixed path={path:?} order={order:?} windows={windows} production_us_p50={:.6} candidate_us_p50={:.6} speedup_p05={:.9} speedup_p50={:.9} speedup_p95={:.9}",
    ...
    fn run_timing(windows: usize) -> Result<(), String> {
        ...
        let label = format!("scalar-nn-m64n64-fixed-{windows}");
    ...
    fn candidate_screen_21_windows_abba_baab() -> Result<(), String> {
        ...
        let windows = exact_windows(
            std::env::var_os("MAMBA_RS_NN_M64N64_FIXED_WINDOWS").as_deref(),
            SCREEN_WINDOWS,
        )?;
        run_timing(windows)
    }
