//! Test-only Ada TF32 NT d768-in stage-sliced A-ldmatrix discovery.

#[path = "support/triad_tf32_nt_compact_a_ldmatrix_sliced_d768_in_source.rs"]
...
const ENV: &str = "MAMBA_TRIAD_TF32_NT_A_LDMATRIX_SLICED_D768_IN_DISCOVERY";

#[test]
fn d768_in_target_grid_contract_is_192() {
    assert_eq!(candidate_source::TARGET, (2_048, 768, 3_072));
    assert_eq!(candidate_source::TARGET_GRID, 192);
}

#[test]
fn d768_in_launch_env_is_shape_specific() {
    assert_eq!(
        ENV,
        "MAMBA_TRIAD_TF32_NT_A_LDMATRIX_SLICED_D768_IN_DISCOVERY"
    );
}
...
mod cuda_suite {
    ...
    use super::{ENV, TimingPhase, may_run_fast, windows_for};
    const OPS: usize = 20;
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixSlicedCompileV1",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixSlicedIdentityV1",
    ...
    fn resource_gate(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixSlicedResourceV1",
    ...
    fn graph_identity(graph: &CudaGraph, shape: Shape, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32NtALdmatrixSlicedGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 1 {
    ...
    fn check_exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixSlicedBitsV1",
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixSlicedScreenV1",
    ...
    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; TF32 NT sliced d768-in once3/once7"]
    fn ada_tf32_nt_a_ldmatrix_sliced_d768_in_scout3_then_once7() -> Result<(), String> {
        ...
        quiet.require_pre_context("tf32-nt-a-ldmatrix-sliced/pre")?;
        let runtime = new_runtime()?;
        assert_eq!(
            (TARGET.m, TARGET.k_out, TARGET.reduction),
            candidate_source::TARGET
        );
        assert_eq!(TARGET.grid()?, (candidate_source::TARGET_GRID, 1, 1));
        ...
        quiet.require_cohort("tf32-nt-a-ldmatrix-sliced/scout")?;
        ...
        if !scout_pass {
            println!(
                "{}",
                json!({
                    "schema":"MambaBiTriadTf32NtALdmatrixSlicedDecisionV1",
                    "cell":TARGET.label,"retained_scout":scout,
                    "decision":"stop_scout_retained_loss","promotion":false,
                })
            );
            quiet.verify_post_cohort("tf32-nt-a-ldmatrix-sliced/scout-post")?;
            return Ok(());
        }
        quiet.require_cohort("tf32-nt-a-ldmatrix-sliced/qualification")?;
        ...
        if !may_run_fast(&retained) {
            ...
            quiet.verify_post_cohort("tf32-nt-a-ldmatrix-sliced/retained-post")?;
            return Ok(());
        }
        ...
        quiet.verify_post_cohort("tf32-nt-a-ldmatrix-sliced/post")?;
        Ok(())
    }
}
