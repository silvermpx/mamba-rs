...
pub const SYMBOL_PREFIX: &str = "gemm_bi_tn_test_m64n96_bk64_s2_regpipe_vec2_";
...
pub fn candidate_source(production: &str) -> Result<String, String> {
    ...
    if source.contains("GEMM_BI_TC64_LDB") || source.contains("GEMM_BI_HALF_TN_INDEX(") {
        return Err("M64N96 transform retained a square shared-layout anchor".into());
    }
    Ok(source)
}
...
pub fn restore_retained_source(candidate: &str, production: &str) -> Result<String, String> {
    ...
    if candidate != expected {
        return Err("M64N96 candidate differs from the fail-closed transform".into());
    }
    retained_source(production)
}

pub const fn target_grid() -> u32 {
    TARGET.1.div_ceil(64) as u32 * TARGET.2.div_ceil(96) as u32
}

pub const fn target_staged_half_elements_m64n96() -> usize {
    target_grid() as usize * (64 + 96) * 64
}
...
pub const fn resident_warps() -> u32 {
    BLOCK_THREADS / 32 * REQUIRED_OCCUPANCY
}
...
fn replace_exact(
    ...
    if actual != expected {
        return Err(format!(
            "M64N96 {label} anchor expected {expected}, observed {actual}: {before:?}"
        ));
    }
...
mod tests {
    ...
    #[test]
    fn six_warps_own_every_m64n96_output_once() {
        let mut owners = vec![0u8; 64 * 96];
        for warp in 0..6 {
            let warp_m = (warp / 3) * 32;
    ...
    #[test]
    fn compact_x_and_padded_y_layouts_have_unique_copy_ownership() {
        for (width, stride, offset) in [
            (64usize, 64usize, x_offset as fn(usize, usize) -> usize),
            (96usize, 128usize, y_offset as fn(usize, usize) -> usize),
        ] {
        ...
        assert_eq!(2 * 64 * (64 + 128) * 2, STATIC_SHARED_BYTES);
    }
    ...
    fn transform_changes_only_cta_geometry_layout_and_export() {
        ...
        assert!(candidate.contains("#define GEMM_BI_TC64_BM 64"));
        assert!(candidate.contains("#define GEMM_BI_TC64_BN 96"));
        assert!(candidate.contains("#define GEMM_BI_TC64_THREADS 192"));
        assert!(candidate.contains("int warpM = (warp / 3) * 32;"));
        assert!(candidate.contains("int warpN = (warp % 3) * 32;"));
    ...
    #[test]
    fn target_reduces_staged_traffic_and_avoids_m64n128_tail_imbalance() {
        assert_eq!(target_grid(), TARGET_GRID);
        assert_eq!(target_staged_half_elements_m64n96(), 1_966_080);
        assert_eq!(target_staged_half_elements_m64n64(), 2_359_296);
        assert_eq!(
            target_staged_half_elements_m64n96() * 6,
            target_staged_half_elements_m64n64() * 5
        );
        assert_eq!(resident_warps(), 12);
        assert_eq!(
            RETAINED_BLOCK_THREADS / 32 * RETAINED_REQUIRED_OCCUPANCY,
            12
        );
        assert_eq!(TARGET_GRID.div_ceil(142) * 3, 6);
        assert_eq!(288u32.div_ceil(142) * 2, 6);
        assert_eq!(144u32.div_ceil(142) * 4, 8);
    }
