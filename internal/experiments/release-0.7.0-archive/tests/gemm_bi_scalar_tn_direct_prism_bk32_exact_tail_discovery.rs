...
#[path = "support/triad_f32_tn_direct_prism_bk32_exact_tail_source.rs"]
mod direct_source;
...
#[test]
fn tn_prism_bk32_maps_six_exact_chunks_to_two_node_pipelines() {
    assert_eq!(
    ...
    let retained = retained_source::compose_source();
    assert_eq!(retained.matches(retained_source::SYMBOL).count(), 1);
    assert_ne!(direct_source::SYMBOL, retained_source::SYMBOL);
}

#[test]
fn bk32_exact_tail_does_not_append_ffmas_to_any_chunk_chain() {
    let chunks = PRISM.chunks().unwrap();
    let exact = chunks.iter().map(|chunk| chunk.len).collect::<Vec<_>>();
    let padded_bk32 = chunks
        .iter()
        .map(|chunk| chunk.len.div_ceil(32) * 32)
        .collect::<Vec<_>>();
    assert_eq!(exact, [784, 784, 784, 784, 784, 701]);
    assert_eq!(padded_bk32, [800, 800, 800, 800, 800, 704]);
    assert_eq!(
        padded_bk32.iter().sum::<usize>() - exact.iter().sum::<usize>(),
        83
    );
    let source = direct_source::compose_source();
    assert!(source.contains("for (int kk = 0; kk < valid_k; ++kk)"));
}
...
fn cuda_harness_stops_in_order_after_exact_resources_once3_and_once7() {
    let harness = include_str!("gemm_bi_scalar_tn_direct_prism_bk32_exact_tail_discovery.rs");
    ...
    assert!(harness.contains("const DIRECT_STATIC_SHARED: usize = 32_768"));
    assert!(harness.contains("const DIRECT_MIN_OCCUPANCY: u32 = 3"));
    assert!(harness.contains("launch_retained_direct_partials(runtime, fixture)?"));
    assert!(harness.contains("require_retained_direct_graph(&comparator_graph"));
    assert!(
...
mod splitm_cuda_suite {
    ...
    const RETAINED_DIRECT: &str = retained_source::SYMBOL;
    ...
    const DIRECT_STATIC_SHARED: usize = 32_768;
    const DIRECT_MIN_OCCUPANCY: u32 = 3;
    ...
    struct Runtime {
        ...
        _retained_direct_module: Arc<CudaModule>,
        retained_direct: CudaFunction,
        retained_direct_source_sha: String,
    }
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        let retained_direct_source = retained_source::compose_source();
        let retained_direct_source_sha =
            format!("{:x}", Sha256::digest(retained_direct_source.as_bytes()));
        let retained_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            retained_direct_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
                include_paths: cuda_include_paths(),
        ...
        .map_err(|error| format!("compile retained direct TN BK16: {error:?}"))?;
        let retained_direct_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(retained_ptx.to_src()))
            .map_err(|error| format!("load retained direct TN BK16 module: {error:?}"))?;
        let retained_direct = retained_direct_module
            .load_function(RETAINED_DIRECT)
            .map_err(|error| format!("load {RETAINED_DIRECT}: {error:?}"))?;
        Ok(Runtime {
            _device: device,
            ctx,
            ...
            direct,
            direct_source_sha,
            _retained_direct_module: retained_direct_module,
            retained_direct,
            retained_direct_source_sha,
        })
    }
    ...
    fn launch_direct_kernel(
        ...
        unsafe { builder.launch(config(fixture.cell.direct_grid(), (128, 1, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch raw {symbol}: {error:?}"))
    }

    fn launch_direct_partials(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        launch_direct_kernel(runtime, fixture, &runtime.direct, DIRECT)
    }

    fn launch_retained_direct_partials(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        launch_direct_kernel(runtime, fixture, &runtime.retained_direct, RETAINED_DIRECT)
    }
    ...
    fn launch(
        ...
        match arm {
            ...
            Arm::Retained => {
                launch_retained_direct_partials(runtime, fixture)?;
                launch_reducer(runtime, fixture, fixture.candidate_partial.ptr())
            }
            Arm::Auto => gpu_gemm_bi_backward_dw_grad(
                &runtime.ctx,
    ...
    unsafe fn require_direct_graph(
        graph: &CudaGraph,
        fixture: &Fixture,
        candidate: CandidateFixed,
        symbol: &str,
        ...
        let partial = nodes
            ...
            .ok_or_else(|| format!("{arm_label} omitted {symbol}"))?;
        let reducer = nodes
            .iter()
            .find(|node| node.symbol == REDUCER)
            .ok_or_else(|| format!("direct candidate omitted {REDUCER}"))?;
        require_launch(partial, symbol, fixture.cell.direct_grid(), (128, 1, 1), 0)?;
        require_launch(
            reducer,
            REDUCER,
            fixture.cell.reducer_grid(),
        ...
        if unsafe { graph_arg::<u64>(partial, 0) }? != fixture.candidate_partial.ptr()
            ...
            || unsafe { graph_arg::<i32>(partial, 6) }? != M_CHUNK as i32
        {
            return Err(format!("{arm_label} partial ABI/arguments changed"));
        }
        if unsafe { graph_arg::<u64>(reducer, 0) }? != fixture.output.ptr()
            || unsafe { graph_arg::<u64>(reducer, 1) }? != fixture.candidate_partial.ptr()
            || unsafe { graph_arg::<f32>(reducer, 2) }?.to_bits() != fixture.alpha.to_bits()
        ...
        unsafe { require_chain(graph, &[partial.node, reducer.node], arm_label) }?;
        println!(
            "{}",
            json!({
    ...
    unsafe fn require_candidate_graph(
        ...
        unsafe { require_direct_graph(graph, fixture, candidate, DIRECT, "Candidate") }
    }
    ...
    unsafe fn require_retained_direct_graph(
        ...
        unsafe { require_direct_graph(graph, fixture, candidate, RETAINED_DIRECT, "RetainedBK16") }
    }
    ...
    fn screen_comparator(
        ...
        quiet.require_cohort("f32-tn-direct-prism-bk32-exact-tail/timed")?;
    ...
    #[test]
    #[ignore = "Ada CUDA13.2 discovery: exact F32 TN canonical Prism direct BK32 exact-tail"]
    fn ada_f32_tn_direct_prism_bk32_exact_tail_discovery() -> Result<(), String> {
        ...
        quiet.require_pre_context("f32-tn-direct-prism-bk32-exact-tail/pre")?;
        ...
        resources(&runtime.retained_direct, RETAINED_DIRECT, 128, 16_384)?;
        resources(
        ...
        println!(
            ...
            json!({
                ...
                "retained_direct_source_sha":runtime.retained_direct_source_sha,
                "candidate_nodes":[DIRECT,REDUCER],
                "retained_nodes":[RETAINED_DIRECT,REDUCER],
                "actual_auto_nodes":[PARTIAL,REDUCER],
                "candidate_grid":[186,1,6],
                "retained_grid":[186,1,6],
        ...
        if !retained_once3_win {
            quiet.verify_post_cohort("f32-tn-direct-prism-bk32-exact-tail/once3-stop")?;
            return Ok(());
        }
        ...
        if retained_once7_win {
            ...
            println!(
                ...
                json!({
                    ...
                    "shape":[PRISM.batch,PRISM.k_out,PRISM.n_out],
                    "candidate":DIRECT,
                    "retain_against_committed_bk16":true,
                    "fast_win":fast_win,
        ...
        quiet.verify_post_cohort("f32-tn-direct-prism-bk32-exact-tail/post")?;
        Ok(())
    }
}
