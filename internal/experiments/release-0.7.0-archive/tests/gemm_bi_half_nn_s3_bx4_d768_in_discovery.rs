...
fn bx4_lane_quadrants_reproduce_two_literal_x2_atoms_exhaustively() {
    for pair in 0..2 {
        for lane in 0..32 {
            ...
            let actual = candidate_source::bx4_coordinate(pair, lane);
            let expected_atom = pair * 2 + usize::from(q >= 2);
            let expected_half = q & 1;
            let expected = (
                row_in_matrix + expected_half * 8,
                expected_atom * 8,
                (expected_atom & 1) * 2 + expected_half,
            );
            assert_eq!(actual, expected, "pair={pair} lane={lane}");
        }
    }
}

#[test]
fn bx4_literal_addresses_are_aligned_for_every_issue_warp_and_lane() {
    for issue in 0..4 {
        for warp_n in [0usize, 32, 64, 96] {
            for pair in 0..2 {
                for lane in 0..32 {
                    let (row, column, _) = candidate_source::bx4_coordinate(pair, lane);
                    let reduction = issue * 16 + row;
                    let column = warp_n + column;
                    let physical = reduction * 128 + (column ^ ((reduction & 7) * 8));
...
#[test]
fn source_transform_is_reversible_and_changes_only_b_loads_and_f16_export() {
    ...
    assert_eq!(candidate.matches("load_fragments_bx4(").count(), 8);
    assert!(candidate.contains(candidate_source::SYMBOL));
    assert!(!candidate.contains(&format!("void {}(", candidate_source::RETAINED_SYMBOL)));
    for frozen in [
        ...
        "sm89_fixed_half_swizzle::scalar_epilogue(",
    ] {
        assert_eq!(
            candidate.matches(frozen).count(),
            retained.matches(frozen).count()
        );
    }
}

#[test]
fn d768_in_geometry_resources_and_strict_policy_are_frozen() {
    assert_eq!(candidate_source::TARGET, (2_048, 768, 3_072));
    assert_eq!(
        2_048usize.div_ceil(128) * 768usize.div_ceil(128),
        candidate_source::TARGET_GRID as usize
    );
    assert_eq!(candidate_source::TARGET_GRID, 96);
    ...
    assert_eq!(candidate_source::MAX_REGISTERS, 192);
...
mod cuda_suite {
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        if !candidate_ptx
            .to_src()
            .contains("gemm_bi_nn_test_fixed_sm89_tc128_s3_bx4_f16")
            || !retained_ptx
                .to_src()
                .contains("gemm_bi_nn_fixed_sm89_tc128_s3_v1_f16")
        {
    ...
    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_F16_NN_S3_BX4_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_F16_NN_S3_BX4_DISCOVERY=1".into());
        }
        let device = GpuDevice::new(0)?;
        if device.compute_capability != (8, 9) || device.multiprocessor_count() != 142 {
            return Err(format!(
                "Fixed S3 B-x4 discovery requires 142-SM Ada, got {:?}/{}",
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function, shared, occupancy) in [
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfNnS3Bx4ResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":{shared},\"active_ctas\":{active},\"required_active_ctas\":{occupancy}}}",
                arm.name()
            );
        }
        Ok(())
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNnS3Bx4BitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha\":{},\"exceptional\":{},\"candidate_vs_fixed_s3_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.m,
            shape.k_out,
            shape.reduction,
    ...
    fn timed_pair(
        ...
        for window in 0..windows {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiHalfNnS3Bx4TimingV1\",\"phase\":\"{phase}\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw
    ...
    fn ada_f16_nn_fixed_s3_bx4_d768_in_scout_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("half-nn-s3-bx4/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("half-nn-s3-bx4/cohort")?;
        ...
        let scout_pass = candidate_source::all_strata_below(&scout_strata, 0.99);
        println!(
            "{{\"schema\":\"MambaBiHalfNnS3Bx4ScoutDecisionV1\",\"shape\":[2048,768,3072],\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{scout_pass},\"decision\":\"{}\"}}",
            scout_strata,
            if scout_pass {
                "advance_to_once7"
            } else {
        ...
        if !scout_pass {
            drop(runtime);
            quiet.verify_post_cohort("half-nn-s3-bx4/post")?;
            return Ok(());
        }
        ...
        let retained_pass = candidate_source::all_strata_below(&retained_strata, 0.99);
        println!(
            "{{\"schema\":\"MambaBiHalfNnS3Bx4RetainedDecisionV1\",\"shape\":[2048,768,3072],\"candidate\":\"{}\",\"comparator\":\"{}\",\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_strata,
            if retained_pass {
        ...
        let fast_pass = candidate_source::all_strata_below(&fast_strata, 0.99);
        println!(
            "{{\"schema\":\"MambaBiHalfNnS3Bx4FinalDecisionV1\",\"shape\":[2048,768,3072],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"retained_strata\":{:?},\"fast_strata\":{:?},\"strict_fast_p50_p95_lt_0_99\":{fast_pass},\"decision\":\"{}\",\"promotion\":false}}",
