//! Discovery only: one direct-TN M64N64/BK16/S2 raw-partial launch reads
//! row-major X and writes the retained two exact SplitM planes, followed by the
//! unchanged production FP64 reducer. No dispatcher admission or production changes.

#[path = "support/triad_f32_tn_direct_d768_in_bk16_source.rs"]
mod direct_source;
...
#[test]
fn tn_d768_in_bk16_maps_two_exact_chunks_to_two_node_candidate() {
    assert_eq!(
    ...
    assert_eq!(D768_IN.direct_grid(), (576, 1, 2));
    ...
    assert_eq!(tail.direct_grid(), (6, 1, 2));
    ...
    assert!(direct.contains("PRISM_DIRECT_TN_BK 16"));
    assert!(direct.contains("const int fc = (int)blockIdx.z"));
    assert_eq!((DIRECT_MAX_REGISTERS, DIRECT_STATIC_SHARED), (128, 16_384));
    assert_eq!(DIRECT_MIN_OCCUPANCY, 4);
}
...
#[test]
fn cuda_harness_routes_bk16_reducer_and_screens_fast_only_after_retained_win() {
    let harness = include_str!("gemm_bi_scalar_tn_direct_d768_in_bk16_retained_dual_discovery.rs");
    assert!(harness.contains("launch_direct_partials(runtime, fixture)?;"));
    assert!(harness.contains("launch_reducer(runtime, fixture, fixture.candidate_partial.ptr())"));
    assert!(harness.contains("Arm::Retained => launch_retained_dual_fused(runtime, fixture)"));
    assert!(harness.contains("launch_direct_partials(runtime, fixture).map_err(&harness)?"));
    assert!(
        harness.contains("require_chain(graph, &[direct.node, reducer.node], \"direct BK16\")")
    );
    assert!(
        harness.contains(
    ...
    assert!(harness.contains("check_exact_case(&runtime, &mut auto_proof, candidate, Arm::Auto"));
...
mod splitm_cuda_suite {
    ...
    fn validate_ptxas_and_sass(symbol: &str, ptx: &str) -> Result<(), String> {
        let stem = format!("mamba-direct-d768-in-bk16-{}", std::process::id());
        ...
        let result = (|| {
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnDirectD768InBk16SassV1",
                    "symbol":symbol,
                    "target":"sm_89",
                    "ffma":true,
                    "ldgsts":true,
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        let direct_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            direct_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
        ...
        .map_err(|error| format!("compile direct d768-in BK16 raw: {error:?}"))?;
        validate_ptxas_and_sass(DIRECT, &direct_ptx.to_src())?;
        let direct_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(direct_ptx.to_src()))
            .map_err(|error| format!("load direct d768-in BK16 module: {error:?}"))?;
    ...
    fn compare_direct_raw_partials(
        runtime: &Runtime,
        fixture: &mut Fixture,
        corpus: &str,
    ) -> Result<(), RawPartialError> {
        ...
        for index in raw_plane_indices(fixture.cell).map_err(&harness)? {
                ...
                .position(|(a, b)| a != b)
            {
                let word = begin + relative;
                return Err(RawPartialError::Mismatch(format!(
                    "{corpus} {DIRECT} raw partial fc={index} m=[{},{}), word={relative}: {:08x} != {:08x}",
                    chunks[index].begin,
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnDirectD768InBk16RawPartialV1",
                    "corpus":corpus,
                    "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                    "candidate":DIRECT,
                    "fc":index,
    ...
    fn prove_direct_raw_accumulators(runtime: &Runtime) -> Result<(), String> {
        for (corpus, exceptional) in FUSED_RAW_TARGET_CORPORA {
            let mut fixture = Fixture::new(&runtime.ctx, D768_IN, 1.0, exceptional)?;
            compare_direct_raw_partials(runtime, &mut fixture, corpus)
    ...
    unsafe fn require_candidate_graph(graph: &CudaGraph, fixture: &Fixture) -> Result<(), String> {
        ...
        if nodes.len() != 2 {
            return Err(format!(
                "direct BK16 candidate has {} nodes, expected two",
                nodes.len()
            ));
        }
        let direct = nodes
            ...
            .ok_or_else(|| format!("direct BK16 candidate omitted {DIRECT}"))?;
        let reducer = nodes
            .iter()
            .find(|node| node.symbol == REDUCER)
            .ok_or_else(|| format!("direct BK16 candidate omitted {REDUCER}"))?;
        require_launch(direct, DIRECT, fixture.cell.direct_grid(), (128, 1, 1), 0)?;
        require_launch(
            reducer,
            REDUCER,
            fixture.cell.reducer_grid(),
        ...
        if unsafe { graph_arg::<u64>(direct, 0) }? != fixture.candidate_partial.ptr()
            || unsafe { graph_arg::<u64>(direct, 1) }? != fixture.x.ptr()
            || unsafe { graph_arg::<u64>(direct, 2) }? != fixture.dy.ptr()
            || unsafe { graph_arg::<i32>(direct, 3) }? != fixture.cell.batch as i32
            || unsafe { graph_arg::<i32>(direct, 4) }? != fixture.cell.k_out as i32
            || unsafe { graph_arg::<i32>(direct, 5) }? != fixture.cell.n_out as i32
            || unsafe { graph_arg::<i32>(direct, 6) }? != M_CHUNK as i32
        {
            return Err("direct BK16 raw ABI/arguments changed".into());
        }
        if unsafe { graph_arg::<u64>(reducer, 0) }? != fixture.output.ptr()
            || unsafe { graph_arg::<u64>(reducer, 1) }? != fixture.candidate_partial.ptr()
            || unsafe { graph_arg::<f32>(reducer, 2) }?.to_bits() != fixture.alpha.to_bits()
            ...
            || unsafe { graph_arg::<i32>(reducer, 5) }? != CHUNKS as i32
        {
            return Err("direct BK16 reducer ABI/arguments changed".into());
        }
        unsafe { require_chain(graph, &[direct.node, reducer.node], "direct BK16") }?;
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDirectD768InBk16GraphV1",
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "nodes":[DIRECT,REDUCER],
                "edges":[[DIRECT,REDUCER]],
                "order":["direct_bk16_raw","production_fp64_reduce"]
            })
        );
        Ok(())
    }
    ...
    fn check_exact_case(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDirectD768InBk16BitsV1",
                "case":label,
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
    ...
    fn direct_candidate_resources(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        let registers = function
            .num_regs()
            .map_err(|error| format!("{symbol} regs: {error:?}"))?;
        let local = function
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDirectD768InBk16HardResourceV1",
                "symbol":symbol,
                "threads":128,
                "registers":registers,
                "registers_max":DIRECT_MAX_REGISTERS,
                "local_bytes":local,
                "static_shared_bytes":shared,
    ...
    #[test]
    #[ignore = "Ada CUDA13.2 discovery: exact F32 TN d768-in direct BK16 vs retained dual"]
    fn ada_f32_tn_direct_d768_in_bk16_retained_dual_discovery() -> Result<(), String> {
        ...
        for (function, symbol) in [(&runtime.direct, DIRECT)] {
            if let Err(reason) = direct_candidate_resources(function, symbol) {
                println!(
                    "{}",
                    json!({
                        "schema":"MambaBiF32TnDirectD768InBk16ResourceStopV1",
                        "candidate":[DIRECT,REDUCER],
                        "reason":reason,
        ...
        // Fail closed if the actual dispatcher no longer emits the committed
        // exact two-chunk AUTO plan for this cell.
        ...
        prove_direct_raw_accumulators(&runtime)?;
        let fixed = fixed_function(&runtime, candidate)?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDirectD768InBk16ArtifactsV1",
        ...
        compare_direct_raw_partials(&runtime, &mut tail, "tail_chunk_accumulators")
            .map_err(|error| error.to_string())?;
        ...
        // The direct raw+reducer pair is timed as one logical GEMM. Resets,
        ...
        while comparator_index < comparators.len() {
            ...
            for graph_path in [false, true] {
                for candidate_endpoints in [true, false] {
                    ...
                    println!(
                        "{}",
                        json!({
                            "schema":"MambaBiF32TnDirectD768InBk16ScreenV1",
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnDirectD768InBk16DecisionV1",
                    "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                    "comparator":if comparator == Arm::Retained {"N64_G8_DUAL_COPYPLAN_FUSED"} else {"Fast"},
                    "windows":windows,
                    "strata":strata,
                    ...
                    "decision":match (comparator,strict_win) {
                        (Arm::Retained,true)=>"direct_bk16_beats_retained",
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDirectD768InBk16FinalDecisionV1",
                "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                "candidate":[DIRECT,REDUCER],
