//! Discovery only: transpose X, then one M128N64/BK16/S2 CTA keeps both exact
//! SplitM chunks in independent register accumulators and finalizes in the
//! retained FP64 order. No dispatcher admission or production changes.

#[path = "support/triad_f32_tn_copyplan_dual_chunk_m128n64_bk16_source.rs"]
...
const CANDIDATE_MAX_REGISTERS: u32 = 128;
const CANDIDATE_STATIC_SHARED: usize = 24_576;
const CANDIDATE_MIN_OCCUPANCY: u32 = 2;
...
impl Cell {
    ...
    fn candidate_params(self, alpha: f32) -> Result<CandidateParams, String> {
    ...
    fn candidate_grid(self) -> (u32, u32, u32) {
        (
            (self.k_out.div_ceil(128) * self.n_out.div_ceil(64)) as u32,
            1,
            1,
        )
    }
...
#[test]
fn tn_d768_in_maps_to_exact_two_node_m128n64_bk16_dual_pipeline() {
    assert_eq!(
    ...
    assert_eq!(tail.candidate_params(1.0).unwrap().k1, 1023);
    ...
    let dual_raw = candidate_source::compose_raw_source();
    let dual_fused = candidate_source::compose_fused_source();
    assert_eq!(dual_raw.matches(candidate_source::RAW_SYMBOL).count(), 1);
    assert_eq!(
        dual_fused.matches(candidate_source::FUSED_SYMBOL).count(),
        1
    );
    assert_eq!(
        (size_of::<CandidateParams>(), align_of::<CandidateParams>()),
        (32, 4)
    );
    assert_eq!(
        (
            CANDIDATE_MAX_REGISTERS,
            CANDIDATE_STATIC_SHARED,
            CANDIDATE_MIN_OCCUPANCY
        ),
        (128, 24_576, 2)
    );
}
...
#[test]
fn cuda_harness_routes_fused_candidate_and_stages_retained_before_fast() {
    let harness =
        include_str!("gemm_bi_scalar_tn_copyplan_dual_chunk_m128n64_bk16_d768_in_discovery.rs");
    assert!(harness.contains("Arm::Candidate => launch_candidate_fused(runtime, fixture)"));
    assert!(harness.contains("Arm::Retained => launch_retained_dual_fused(runtime, fixture)"));
    assert!(harness.contains("Arm::Auto => gpu_gemm_bi_backward_dw_grad("));
    assert!(harness.contains("launch_candidate_raw(runtime, fixture).map_err(&harness)?"));
    assert!(harness.contains("&[transpose.node, fused.node]"));
    assert!(harness.contains("\"M128N64 BK16 dual-chunk\""));
    assert!(harness.contains("\"retained dual-chunk\""));
    assert!(harness.contains("\"candidate_nodes\":[TRANSPOSE,CANDIDATE_FUSED]"));
    assert!(harness.contains("\"retained_nodes\":[TRANSPOSE,RETAINED_DUAL_FUSED]"));
    assert!(harness.contains("let mut comparators = vec![(Arm::Retained, 3_usize)]"));
    assert!(harness.contains("comparators.push((Arm::Retained, 7));"));
    assert!(harness.contains("comparators.push((Arm::Fast, 7));"));
    assert!(harness.contains("check_exact_case(\n            &runtime,\n            &mut auto_proof,\n            candidate,\n            Arm::Auto,"));
    assert!(harness.contains("registers > CANDIDATE_MAX_REGISTERS as i32"));
    assert!(harness.contains("shared as usize != CANDIDATE_STATIC_SHARED"));
    assert!(harness.contains("occupancy < CANDIDATE_MIN_OCCUPANCY"));
    assert!(
...
mod splitm_cuda_suite {
    ...
    const CANDIDATE_RAW: &str = candidate_source::RAW_SYMBOL;
    const CANDIDATE_FUSED: &str = candidate_source::FUSED_SYMBOL;
    ...
    unsafe impl DeviceRepr for CandidateParams {}
    ...
    impl Fixture {
        ...
        fn validate_transposed_unchanged(&self, ctx: &GpuCtx) -> Result<(), String> {
            self.transposed.unchanged(ctx)
        }
    ...
    struct Runtime {
        ...
        _candidate_raw_module: Arc<CudaModule>,
        _candidate_fused_module: Arc<CudaModule>,
        _retained_dual_fused_module: Arc<CudaModule>,
        raw_fixed: CudaFunction,
        candidate_raw: CudaFunction,
        candidate_fused: CudaFunction,
        retained_dual_fused: CudaFunction,
        raw_source_sha: String,
        candidate_raw_source_sha: String,
        candidate_fused_source_sha: String,
    ...
    fn ptxas_used_resources(line: &str) -> Option<(u32, u32)> {
        if !line.contains("Used ") || !line.contains(" registers") {
            return None;
        }
        let registers = metric_before(line, " registers")?;
        let shared = metric_before(line, " bytes smem").unwrap_or(0);
        Some((registers, shared))
    }
    ...
    #[test]
    fn ptxas_used_resource_parser_is_fail_closed() {
        assert_eq!(
            ptxas_used_resources(
                "ptxas info    : Used 127 registers, used 2 barriers, 24576 bytes smem, 392 bytes cmem[0]"
            ),
            Some((127, 24_576))
        );
        assert_eq!(ptxas_used_resources("Used 127 registers"), Some((127, 0)));
    }
    ...
    fn validate_ptxas_and_sass(symbol: &str, ptx: &str) -> Result<(), String> {
        ...
        let result = (|| {
            ...
            for metric in [
                ...
                if values != [0] {
                    return Err(format!(
                        "{symbol} nonzero or ambiguous {metric}: {values:?}"
                    ));
                }
            }
            let used = block
                .lines()
                .filter_map(ptxas_used_resources)
                .collect::<Vec<_>>();
            if used.len() != 1
                || used[0].0 > CANDIDATE_MAX_REGISTERS
                || used[0].1 as usize != CANDIDATE_STATIC_SHARED
            {
                return Err(format!(
                    "{symbol} ptxas hard resource stop: expected one <= {}-register/{}B-smem record, got {used:?}",
                    CANDIDATE_MAX_REGISTERS, CANDIDATE_STATIC_SHARED
                ));
            }
            ...
            if symbol == CANDIDATE_FUSED && (!entry.contains("DADD") || !entry.contains("DMUL")) {
                return Err(format!("{symbol} SASS omitted exact FP64 DADD or DMUL"));
            }
            ...
            println!(
                ...
                json!({
                    ...
                    "ffma":true,
                    "ldgsts":true,
                    "dadd_dmul":symbol != CANDIDATE_FUSED || (entry.contains("DADD") && entry.contains("DMUL")),
                    "stack_bytes":0,
                    "spill_store_bytes":0,
                    "spill_load_bytes":0,
                    "registers":used[0].0,
                    "registers_max":CANDIDATE_MAX_REGISTERS,
                    "static_shared_bytes":used[0].1,
                    "static_shared_expected":CANDIDATE_STATIC_SHARED,
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        let candidate_raw_source = candidate_source::compose_raw_source();
        let candidate_raw_source_sha =
            format!("{:x}", Sha256::digest(candidate_raw_source.as_bytes()));
        let candidate_raw_ptx = cudarc::nvrtc::compile_ptx_with_opts(
        ...
        .map_err(|error| format!("compile M128N64 BK16 dual-chunk raw: {error:?}"))?;
        validate_ptxas_and_sass(CANDIDATE_RAW, &candidate_raw_ptx.to_src())?;
        let candidate_raw_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(candidate_raw_ptx.to_src()))
            .map_err(|error| format!("load M128N64 BK16 dual-chunk raw module: {error:?}"))?;
        let candidate_raw = candidate_raw_module
            .load_function(CANDIDATE_RAW)
            .map_err(|error| format!("load {CANDIDATE_RAW}: {error:?}"))?;
        let candidate_fused_source = candidate_source::compose_fused_source();
        let candidate_fused_source_sha =
            format!("{:x}", Sha256::digest(candidate_fused_source.as_bytes()));
        let candidate_fused_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            candidate_fused_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
                include_paths: cuda_include_paths(),
        ...
        .map_err(|error| format!("compile M128N64 BK16 dual-chunk fused: {error:?}"))?;
        validate_ptxas_and_sass(CANDIDATE_FUSED, &candidate_fused_ptx.to_src())?;
        let candidate_fused_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(candidate_fused_ptx.to_src()))
            .map_err(|error| format!("load M128N64 BK16 dual-chunk fused module: {error:?}"))?;
        let candidate_fused = candidate_fused_module
            .load_function(CANDIDATE_FUSED)
            .map_err(|error| format!("load {CANDIDATE_FUSED}: {error:?}"))?;
        ...
        Ok(Runtime {
            ...
            ctx,
            _raw_module: module,
            _candidate_raw_module: candidate_raw_module,
            _candidate_fused_module: candidate_fused_module,
            ...
            retained_dual_fused,
            raw_source_sha,
            candidate_raw_source_sha,
            candidate_fused_source_sha,
    ...
    fn launch_candidate_raw(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        if fixture.cell.batch == 0 {
            return Err("zero reduction has no M128 dual raw launch".into());
        }
        launch_transpose(runtime, fixture)?;
        let partial = fixture.candidate_partial.ptr();
        let x = fixture.transposed.ptr();
        let dy = fixture.dy.ptr();
        let params = fixture.cell.candidate_params(1.0)?;
        let mut builder = runtime.ctx.stream.launch_builder(&runtime.candidate_raw);
        builder.arg(&partial);
        builder.arg(&x);
        builder.arg(&dy);
        builder.arg(&params);
        unsafe { builder.launch(config(fixture.cell.candidate_grid(), (256, 1, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch {CANDIDATE_RAW}: {error:?}"))
    }

    fn launch_candidate_fused(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        if fixture.cell.batch == 0 {
        ...
        let x = fixture.transposed.ptr();
        let dy = fixture.dy.ptr();
        let params = fixture.cell.candidate_params(fixture.alpha)?;
        let mut builder = runtime.ctx.stream.launch_builder(&runtime.candidate_fused);
        builder.arg(&output);
        builder.arg(&x);
        builder.arg(&dy);
        builder.arg(&params);
        unsafe { builder.launch(config(fixture.cell.candidate_grid(), (256, 1, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch {CANDIDATE_FUSED}: {error:?}"))
    }
    ...
    fn launch(
        ...
        if fixture.cell.batch == 0 {
            return match arm {
                ...
                Arm::Retained | Arm::Auto => gpu_gemm_bi_backward_dw_grad(
                    &runtime.ctx,
                    &GradSlice::from_raw(
                        fixture.output.ptr(),
                        fixture.cell.k_out * fixture.cell.n_out,
        ...
        match arm {
            Arm::Candidate => launch_candidate_fused(runtime, fixture),
            Arm::Reference => {
    ...
    fn compare_candidate_raw_partials(
        runtime: &Runtime,
        fixture: &mut Fixture,
        corpus: &str,
    ) -> Result<(), RawPartialError> {
        ...
        fixture.reset(&runtime.ctx).map_err(&harness)?;
        launch_reference_partials(runtime, fixture).map_err(&harness)?;
        launch_candidate_raw(runtime, fixture).map_err(&harness)?;
        let reference = fixture
            .reference_partial
            .bits(&runtime.ctx)
            .map_err(&harness)?;
        ...
        for index in raw_plane_indices(fixture.cell).map_err(&harness)? {
                ...
                .position(|(a, b)| a != b)
            {
                let word = begin + relative;
                return Err(RawPartialError::Mismatch(format!(
                    "{corpus} {CANDIDATE_RAW} raw partial fc={index} m=[{},{}), word={relative}: {:08x} != {:08x}",
                    chunks[index].begin,
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnM128N64Bk16DualChunkRawPartialV1",
                    "corpus":corpus,
                    "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
    ...
    fn prove_candidate_raw_accumulators(runtime: &Runtime) -> Result<(), String> {
        for (corpus, exceptional) in FUSED_RAW_TARGET_CORPORA {
            let mut fixture = Fixture::new(&runtime.ctx, D768_IN, 1.0, exceptional)?;
            compare_candidate_raw_partials(runtime, &mut fixture, corpus)
    ...
    unsafe fn require_candidate_graph(graph: &CudaGraph, fixture: &Fixture) -> Result<(), String> {
        if fixture.cell.batch == 0 {
        ...
        if nodes.len() != 2 {
            return Err(format!(
                "M128N64 BK16 dual-chunk candidate has {} nodes, expected two",
                nodes.len()
            ));
        }
        let transpose = nodes
            .iter()
            .find(|node| node.symbol == TRANSPOSE)
            .ok_or_else(|| format!("M128N64 BK16 candidate omitted {TRANSPOSE}"))?;
        let fused = nodes
            .iter()
            .find(|node| node.symbol == CANDIDATE_FUSED)
            .ok_or_else(|| {
                format!("M128N64 BK16 dual-chunk candidate omitted {CANDIDATE_FUSED}")
            })?;
        require_launch(
            transpose,
            TRANSPOSE,
        ...
        if unsafe { graph_arg::<u64>(transpose, 0) }? != fixture.transposed.ptr()
            ...
            || unsafe { graph_arg::<i32>(transpose, 3) }? != fixture.cell.k_out as i32
        {
            return Err("M128N64 BK16 transpose ABI/arguments changed".into());
        }
        if unsafe { graph_arg::<u64>(fused, 0) }? != fixture.output.ptr()
            ...
            || unsafe { graph_arg::<CandidateParams>(fused, 3) }?
                != fixture.cell.candidate_params(fixture.alpha)?
        {
            return Err("M128N64 BK16 dual-chunk fused ABI/arguments changed".into());
        }
        unsafe {
            require_chain(
                graph,
                &[transpose.node, fused.node],
                "M128N64 BK16 dual-chunk",
            )
        }?;
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnM128N64Bk16DualChunkGraphV1",
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "nodes":[TRANSPOSE,CANDIDATE_FUSED],
                "edges":[[TRANSPOSE,CANDIDATE_FUSED]],
                "order":["transpose","m128n64_bk16_dual_fused_finalize"]
            })
        );
        Ok(())
    }
    ...
    fn output_after(
        ...
        match arm {
            ...
            Arm::Auto if fixture.cell.batch > 0 => {
                fixture.validate_transposed_unchanged(&runtime.ctx)?;
                fixture.validate_candidate_partial_unchanged(&runtime.ctx)?;
            }
    ...
    fn check_exact_case(
        ...
        unsafe {
            require_candidate_graph(&candidate_graph, fixture)?;
            match oracle {
                Arm::Auto if fixture.cell.batch > 0 => {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnM128N64Bk16DualChunkBitsV1",
                "case":label,
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
    ...
    fn candidate_resources(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        let registers = function
            .num_regs()
            .map_err(|error| format!("{symbol} regs: {error:?}"))?;
        let local = function
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnM128N64Bk16DualChunkHardResourceV1",
                "symbol":symbol,
                "threads":256,
                "registers":registers,
                "registers_max":CANDIDATE_MAX_REGISTERS,
                ...
                "dynamic_shared_bytes":0,
                "occupancy":occupancy,
                "occupancy_min":CANDIDATE_MIN_OCCUPANCY
            })
        );
        if registers > CANDIDATE_MAX_REGISTERS as i32
            || local != 0
            || shared as usize != CANDIDATE_STATIC_SHARED
            || occupancy < CANDIDATE_MIN_OCCUPANCY
        {
            return Err(format!(
    ...
    #[test]
    #[ignore = "Ada CUDA13.2 discovery: exact F32 TN d768-in transpose + M128N64 BK16 dual-chunk fused"]
    fn ada_f32_tn_m128n64_bk16_dual_fused_d768_in_once3_once7_fast() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("f32-tn-m128n64-bk16-dual/pre", 2_000)?;
        } else {
            quiet.require_pre_context("f32-tn-m128n64-bk16-dual/pre")?;
        }
        let runtime = new_runtime()?;

        for (function, symbol) in [
            (&runtime.candidate_raw, CANDIDATE_RAW),
            (&runtime.candidate_fused, CANDIDATE_FUSED),
        ] {
            if let Err(reason) = candidate_resources(function, symbol) {
                println!(
                    "{}",
                    json!({
                        "schema":"MambaBiF32TnM128N64Bk16DualChunkResourceStopV1",
                        "candidate":[CANDIDATE_RAW,CANDIDATE_FUSED],
                        "reason":reason,
                ...
                if idle_resident_mode {
                    quiet.require_idle_resident("f32-tn-m128n64-bk16-dual/resource-stop", 256)?;
                } else {
                    quiet.verify_post_cohort("f32-tn-m128n64-bk16-dual/resource-stop")?;
                }
                return Ok(());
            }
        }

        // Fail closed if the real dispatcher no longer emits its exact
        // two-chunk AUTO plan for the canonical d768-in cell.
        let mut auto_dispatch_proof = Fixture::new(&runtime.ctx, D768_IN, 1.0, false)?;
        let auto_dispatch_graph = capture(
            &runtime,
            &mut auto_dispatch_proof,
            Arm::Auto,
            CandidateFixed::Production,
        )?;
        unsafe { require_auto_splitm_graph(&auto_dispatch_graph, &auto_dispatch_proof) }?;
        drop(auto_dispatch_graph);
        drop(auto_dispatch_proof);
        ...
        prove_candidate_raw_accumulators(&runtime)?;
        let fixed = fixed_function(&runtime, candidate)?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnM128N64Bk16DualChunkArtifactsV1",
                "artifacts":format!("{:?}",runtime.ctx.kernels.artifact_set_identity()),
                "raw_store_source_sha":runtime.raw_source_sha,
                "candidate_raw_source_sha":runtime.candidate_raw_source_sha,
                "candidate_fused_source_sha":runtime.candidate_fused_source_sha,
                "retained_dual_fused_source_sha":runtime.retained_dual_fused_source_sha,
                "candidate_nodes":[TRANSPOSE,CANDIDATE_FUSED],
        ...
        let (_, auto_repeated) = check_exact_case(
            &runtime,
            &mut auto_proof,
            candidate,
            Arm::Auto,
        ...
        if auto_repeated != exact_repeated {
            return Err("actual AUTO and retained dual-chunk exact outputs diverged".into());
        }
        ...
        compare_candidate_raw_partials(&runtime, &mut tail, "tail_chunk_dual_accumulators")
            .map_err(|error| error.to_string())?;
        ...
        compare_candidate_raw_partials(
            &runtime,
            &mut exceptional,
        ...
        // The transpose + fused kernel is timed as one logical GEMM. Resets,
        ...
        unsafe { require_candidate_graph(&candidate_graph, &target) }?;
        let mut retained_once3_win = None;
        let mut retained_once7_win = None;
        let mut fast_win = None;
        let mut comparators = vec![(Arm::Retained, 3_usize)];
        let mut comparator_index = 0;
        while comparator_index < comparators.len() {
            let (comparator, windows) = comparators[comparator_index];
            ...
            if idle_resident_mode {
                quiet.require_idle_resident("f32-tn-m128n64-bk16-dual/timed", 256)?;
            } else {
                quiet.require_cohort("f32-tn-m128n64-bk16-dual/timed")?;
            }
            ...
            for graph_path in [false, true] {
                for candidate_endpoints in [true, false] {
                    ...
                    println!(
                        "{}",
                        json!({
                            "schema":"MambaBiF32TnM128N64Bk16DualChunkScreenV1",
                            ...
                            "candidate_timed_nodes":[TRANSPOSE,CANDIDATE_FUSED],
            ...
            match (comparator, windows) {
                (Arm::Retained, 3) => {
                    retained_once3_win = Some(strict_win);
                    if strict_win {
                        comparators.push((Arm::Retained, 7));
                    }
                }
                (Arm::Retained, 7) => {
                    retained_once7_win = Some(strict_win);
                    if strict_win {
                        comparators.push((Arm::Fast, 7));
                    }
                }
                (Arm::Fast, 7) => fast_win = Some(strict_win),
                _ => unreachable!(),
            }
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnM128N64Bk16DualChunkDecisionV1",
                    "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                    "comparator":if comparator == Arm::Retained {"N64_G8_DUAL_COPYPLAN_FUSED"} else {"Fast"},
                    "windows":windows,
                    "strata":strata,
                    "strict_win":strict_win,
                    "decision":match (comparator,windows,strict_win) {
                        (Arm::Retained,3,true)=>"m128n64_bk16_dual_once3_beats_retained",
                        (Arm::Retained,3,false)=>"stop_once3_no_retry",
                        (Arm::Retained,7,true)=>"m128n64_bk16_dual_once7_beats_retained",
                        (Arm::Retained,7,false)=>"stop_once7_no_retry",
                        (Arm::Fast,7,true)=>"fast_qualified",
                        (Arm::Fast,7,false)=>"not_fast_qualified",
                        _=>unreachable!(),
                    },
                    "promotion":false
                })
        ...
        let retained_once3_win = retained_once3_win.ok_or("missing retained once3 decision")?;
        let retained_win = retained_once7_win.unwrap_or(false);
        let fast_win = fast_win.unwrap_or(false);
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnM128N64Bk16DualChunkFinalDecisionV1",
                "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                "candidate":CANDIDATE_FUSED,
                "retained_once3_win":retained_once3_win,
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("f32-tn-m128n64-bk16-dual/post", 256)?;
        } else {
