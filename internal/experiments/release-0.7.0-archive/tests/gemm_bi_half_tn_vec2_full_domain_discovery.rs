//! Test-only actual-Triad F16 TN d768-in full-domain fast-path discovery.
//! Production kernels and dispatch remain untouched.

#[path = "support/triad_half_tn_vec2_full_domain_source.rs"]
...
const SCOUT_THRESHOLD: f64 = 1.0;
const SCOUT_GEOMEAN_CAP: f64 = 0.995;
...
#[test]
fn target_2d_mapping_is_bijective_and_matches_retained_linear_order() {
    let mut seen = [false; 576];
    for block_y in 0..12 {
        for block_x in 0..48 {
            let (pid_m, pid_n) = candidate_source::tile_coordinate(block_x, block_y);
            let linear = (pid_m * 48 + pid_n) as usize;
            assert!(!seen[linear]);
            seen[linear] = true;
        }
    }
...
#[test]
fn source_adapter_changes_only_mapper_epilogue_and_export() {
    let retained = candidate_source::retained_source(PRODUCTION).unwrap();
    let candidate = candidate_source::candidate_source(PRODUCTION).unwrap();
    assert!(
        candidate.contains("void gemm_bi_tn_test_tc64_bk64_s2_regpipe_vec2_full_domain_##SUFFIX(")
    );
    for parameter in [
    ...
    assert_eq!(
        candidate
            .matches("gemm_bi_accumulate_float2_or_scalar(")
            .count(),
        1
    );
    assert_eq!(
...
fn geometric_mean_p50(strata: &[[f64; 2]]) -> f64 {
    (strata.iter().map(|[p50, _]| p50.ln()).sum::<f64>() / strata.len() as f64).exp()
}

#[test]
fn geometry_resources_and_gated_timing_policy_are_frozen() {
    assert_eq!(candidate_source::TARGET, (2_048, 768, 3_072));
    assert_eq!(candidate_source::TARGET_GRID, (48, 12, 1));
    assert_eq!(candidate_source::RETAINED_TARGET_GRID, (576, 1, 1));
    assert_eq!(candidate_source::BLOCK_THREADS, 128);
    assert_eq!(candidate_source::STATIC_SHARED_BYTES, 32_768);
    assert_eq!(candidate_source::REGISTER_CAP, 125);
    assert_eq!(candidate_source::REQUIRED_OCCUPANCY, 3);
    assert_eq!(candidate_source::TEXT_RATIO_CAP, 1.05);
    assert_eq!(candidate_source::EXPECTED_HMMA, 32);
    assert_eq!(candidate_source::EXPECTED_LDGSTS, 24);
    ...
    let passing_scout = [[0.989, 0.999]; 4];
    assert!(candidate_source::all_strata_below(
        &passing_scout,
        SCOUT_THRESHOLD
    ));
    assert!(geometric_mean_p50(&passing_scout) <= SCOUT_GEOMEAN_CAP);
    assert!(!candidate_source::all_strata_below(
        &[[0.98, 1.0]; 4],
        SCOUT_THRESHOLD
    ));
    ...
    assert_eq!(QUALIFICATION_THRESHOLD, FAST_THRESHOLD);
}
...
mod cuda_suite {
    ...
    use super::{
        FAST_THRESHOLD, QUALIFICATION_THRESHOLD, SCOUT_GEOMEAN_CAP, SCOUT_THRESHOLD, TimingPhase,
        geometric_mean_p50, may_run_fast, windows_for,
    };
    ...
    const TARGET_NONUNIT: Shape = Shape::new(2_048, 768, 3_072, -0.75, "d768_in_negative_alpha");
    const TARGET_EXCEPTIONAL: Shape = Shape::new(2_048, 768, 3_072, 1.0, "d768_in_exceptional");
    ...
    impl Shape {
        ...
        fn is_full_domain_target(self) -> bool {
            candidate_source::is_full_domain_shape(self.reduction, self.k_out, self.n)
        }

        fn launch_grid(self, arm: Arm) -> (u32, u32, u32) {
            if arm == Arm::Candidate && self.is_full_domain_target() {
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "vec2_full_domain",
    ...
    fn validate_ptxas_and_sass(symbol: &str, ptx: &str) -> Result<SassStats, String> {
        ...
        let result = (|| {
            ...
            let static_shared = metric(" bytes smem")?;
            for suffix in [
                " bytes stack frame",
                " bytes spill stores",
                " bytes spill loads",
            ...
            if registers > candidate_source::REGISTER_CAP as u32
                || static_shared != candidate_source::STATIC_SHARED_BYTES as u32
            {
                return Err(format!(
                    "{symbol} resource reject regs={registers}/{} static={static_shared}/{}",
            ...
            let instructions = entry
                ...
                .count();
            if hmma == 0 || ldgsts == 0 || instructions == 0 {
                return Err(format!("{symbol} SASS omitted HMMA, LDGSTS, or text"));
            }
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        let (candidate_ptx, candidate_source_sha) =
            compile_ptx_only(&candidate, "full-domain candidate compile-only")?;
        ...
        let candidate_sass = validate_ptxas_and_sass(&candidate_symbol, &candidate_ptx)?;
        let retained_sass = validate_ptxas_and_sass(&retained_symbol, &retained_ptx)?;
        let text_ratio = candidate_sass.text_bytes as f64 / retained_sass.text_bytes as f64;
        if candidate_sass.hmma != candidate_source::EXPECTED_HMMA
            || candidate_sass.ldgsts != candidate_source::EXPECTED_LDGSTS
            || candidate_sass.hmma != retained_sass.hmma
            || candidate_sass.ldgsts != retained_sass.ldgsts
            || candidate_sass.has_runtime_division
            || text_ratio > candidate_source::TEXT_RATIO_CAP
        {
            return Err(format!(
                "candidate SASS reject HMMA {}/{} LDGSTS {}/{} runtime_division={} text_ratio={text_ratio:.6}/{}",
                candidate_sass.hmma,
                retained_sass.hmma,
                candidate_sass.ldgsts,
                retained_sass.ldgsts,
                candidate_sass.has_runtime_division,
                candidate_source::TEXT_RATIO_CAP,
            ));
        }
        println!(
            "{{\"schema\":\"MambaBiHalfTnFullDomainCompileV1\",\"candidate_source_sha\":\"{candidate_source_sha}\",\"retained_source_sha\":\"{retained_source_sha}\",\"candidate_ptx_sha\":\"{:x}\",\"retained_ptx_sha\":\"{:x}\",\"candidate_cubin_sha\":\"{}\",\"retained_cubin_sha\":\"{}\",\"candidate_sass_sha\":\"{}\",\"retained_sass_sha\":\"{}\",\"candidate_registers\":{},\"retained_registers\":{},\"static_shared_bytes\":{},\"hmma\":{},\"ldgsts\":{},\"candidate_text_bytes\":{},\"retained_text_bytes\":{},\"text_ratio\":{text_ratio},\"runtime_division\":{}}}",
            ...
            candidate_sass.hmma,
            candidate_sass.ldgsts,
            candidate_sass.text_bytes,
            retained_sass.text_bytes,
            candidate_sass.has_runtime_division,
        );
        Ok(())
    }
    ...
    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_F16_TN_FULL_DOMAIN_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_F16_TN_FULL_DOMAIN_DISCOVERY=1".into());
        }
        let device = GpuDevice::new(0)?;
        let identity = device.identity();
        if identity.compute_capability != (8, 9) || identity.multiprocessor_count != 142 {
            return Err(format!(
                "half TN full-domain discovery requires CC8.9/142-SM Ada, got {:?}/{}",
        ...
        if compiler.nvrtc_version != (13, 2) || compiler.target.as_str() != "sm_89" {
            return Err(format!(
                "half TN full-domain discovery requires CUDA13.2/sm_89, got {compiler:?}"
            ));
        }
        let production = include_str!("../kernels/gemm_bi_triad/sm80.cu");
    ...
    fn launch(runtime: &Runtime, fixture: &Fixture, arm: Arm) -> Result<(), String> {
        ...
        match arm {
            Arm::Candidate | Arm::Retained => {
                ...
                let function = if arm == Arm::Candidate && shape.is_full_domain_target() {
                    &runtime.candidate
                } else {
                    &runtime.retained
                };
                ...
                unsafe {
                    builder.launch(LaunchConfig {
                        grid_dim: shape.launch_grid(arm),
    ...
    fn validate_kernel_graph(graph: &CudaGraph, fixture: &Fixture, arm: Arm) -> Result<(), String> {
        ...
        let expected = match arm {
            Arm::Candidate if fixture.shape.is_full_domain_target() => {
                format!("{}f16", candidate_source::SYMBOL_PREFIX)
            }
            Arm::Candidate => format!("{}f16", candidate_source::RETAINED_SYMBOL_PREFIX),
            Arm::Retained => format!("{}f16", candidate_source::RETAINED_SYMBOL_PREFIX),
            Arm::Fast => return Err("Fast graph is vendor-owned".into()),
        };
        if actual != expected
            || (params.gridDimX, params.gridDimY, params.gridDimZ) != fixture.shape.launch_grid(arm)
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function) in [
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfTnFullDomainResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"register_cap\":{},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":0,\"block_threads\":{},\"active_ctas\":{active},\"required_active_ctas\":{}}}",
                arm.name(),
                candidate_source::REGISTER_CAP,
                candidate_source::BLOCK_THREADS,
                candidate_source::REQUIRED_OCCUPANCY,
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnFullDomainBitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha_bits\":{},\"exceptional\":{exceptional},\"candidate_vs_retained_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.reduction,
            shape.k_out,
            shape.n,
    ...
    fn timed_pair(
        ...
        for window in 0..windows {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiHalfTnFullDomainTimingV1\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"windows\":{windows},\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw,
    ...
    fn finish_quiet(runtime: Runtime, quiet: &QuietGpu) -> Result<(), String> {
        drop(runtime);
        quiet
            .verify_post_cohort("half-tn-vec2-full-domain/post")
            .map(|_| ())
    }

    #[test]
    #[ignore = "requires exclusive 142-SM Ada and CUDA13.2"]
    fn ada_f16_tn_vec2_full_domain_d768_in_scout3_then_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("half-tn-vec2-full-domain/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("half-tn-vec2-full-domain/cohort")?;
        validate_resources(&runtime)?;
        exact_case(&runtime, TARGET, false)?;
        exact_case(&runtime, TARGET_NONUNIT, false)?;
        exact_case(&runtime, TARGET_EXCEPTIONAL, true)?;
        ...
        let scout_geomean_p50 = geometric_mean_p50(&scout);
        let scout_pass = candidate_source::all_strata_below(&scout, SCOUT_THRESHOLD)
            && scout_geomean_p50 <= SCOUT_GEOMEAN_CAP;
        println!(
            "{{\"schema\":\"MambaBiHalfTnFullDomainScoutDecisionV1\",\"candidate\":\"{}\",\"comparator\":\"{}\",\"windows\":{scout_windows},\"strict_p50_p95_threshold\":{SCOUT_THRESHOLD},\"geomean_p50\":{scout_geomean_p50},\"geomean_p50_cap\":{SCOUT_GEOMEAN_CAP},\"strata\":{:?},\"pass\":{scout_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            scout,
            if scout_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnFullDomainRetainedDecisionV1\",\"candidate\":\"{}\",\"comparator\":\"{}\",\"windows\":{qualification_windows},\"threshold\":{QUALIFICATION_THRESHOLD},\"strata\":{:?},\"pass\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_once7,
            if retained_pass {
        ...
        let fast_pass = candidate_source::all_strata_below(&fast_once7, FAST_THRESHOLD);
        let final_pass = retained_pass && fast_pass;
        println!(
            "{{\"schema\":\"MambaBiHalfTnFullDomainFinalDecisionV1\",\"shape\":[2048,768,3072],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"candidate_ptx_sha\":\"{}\",\"retained_ptx_sha\":\"{}\",\"scout_strata\":{:?},\"retained_once7_strata\":{:?},\"fast_once7_strata\":{:?},\"strict_retained_and_fast_p50_p95_lt_0_99\":{final_pass},\"decision\":\"{}\",\"promotion\":false}}",
