//! Discovery only: M64N32BK32/S2 GROUP_M8 fused-finalize candidate screened
//! against the committed M64N64BK32/S2 GROUP_M8 fused winner.
//! No dispatcher admission or production changes.

#[path = "support/triad_f32_tn_copyplan_fused_finalize_source.rs"]
mod fused_source;
#[path = "support/triad_f32_tn_copyplan_fused_finalize_n32_source.rs"]
mod n32_source;
...
const N32_MAX_REGISTERS: u32 = 128;
const N32_STATIC_SHARED: usize = 24_576;
const N32_MIN_OCCUPANCY: u32 = 4;
...
impl Cell {
    ...
    fn fixed_grid_n32(self) -> (u32, u32, u32) {
        (
            (self.k_out.div_ceil(64) * self.n_out.div_ceil(32)) as u32,
            1,
            1,
        )
    }
...
#[test]
fn tn_d768_in_maps_to_exact_three_node_fused_finalize_pipeline() {
    assert_eq!(D768_IN.fixed_grid_n32(), (1152, 1, 1));
    assert_eq!(
        (N32_MAX_REGISTERS, N32_STATIC_SHARED, N32_MIN_OCCUPANCY),
        (128, 24_576, 4)
    );
    assert_eq!(
    ...
    let n32_raw = n32_source::compose_raw_source().unwrap();
    let n32_fused = n32_source::compose_fused_source().unwrap();
    assert_eq!(n32_raw.matches(n32_source::N32_RAW_SYMBOL).count(), 1);
    assert_eq!(n32_fused.matches(n32_source::N32_FUSED_SYMBOL).count(), 1);
    assert!(n32_raw.contains("float acc[8][2];"));
    assert!(n32_fused.contains("float acc[8][2];"));
}
...
mod splitm_cuda_suite {
    ...
    const N32_RAW: &str = n32_source::N32_RAW_SYMBOL;
    const N32_FUSED: &str = n32_source::N32_FUSED_SYMBOL;
    ...
    impl CandidateFixed {
        ...
        fn grid(self, cell: Cell) -> (u32, u32, u32) {
            match self {
                Self::N32Raw => cell.fixed_grid_n32(),
                Self::Production | Self::RawStore => cell.fixed_grid(),
            }
        }
    }
    ...
    struct Runtime {
        ...
        _n32_raw_module: Arc<CudaModule>,
        _n32_fused_module: Arc<CudaModule>,
        raw_fixed: CudaFunction,
        fused: CudaFunction,
        n32_raw: CudaFunction,
        n32_fused: CudaFunction,
        raw_source_sha: String,
        fused_source_sha: String,
        n32_raw_source_sha: String,
        n32_fused_source_sha: String,
    }
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        let n32_raw_source = n32_source::compose_raw_source()?;
        let n32_raw_source_sha = format!("{:x}", Sha256::digest(n32_raw_source.as_bytes()));
        let n32_raw_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            n32_raw_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
        ...
        .map_err(|error| format!("compile N32 raw CopyPlan: {error:?}"))?;
        let n32_raw_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(n32_raw_ptx.to_src()))
            .map_err(|error| format!("load N32 raw CopyPlan module: {error:?}"))?;
        let n32_raw = n32_raw_module
            .load_function(N32_RAW)
            .map_err(|error| format!("load {N32_RAW}: {error:?}"))?;
        let n32_fused_source = n32_source::compose_fused_source()?;
        let n32_fused_source_sha = format!("{:x}", Sha256::digest(n32_fused_source.as_bytes()));
        let n32_fused_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            n32_fused_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
        ...
        .map_err(|error| format!("compile N32 fused CopyPlan: {error:?}"))?;
        let n32_fused_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(n32_fused_ptx.to_src()))
            .map_err(|error| format!("load N32 fused CopyPlan module: {error:?}"))?;
        let n32_fused = n32_fused_module
            .load_function(N32_FUSED)
            .map_err(|error| format!("load {N32_FUSED}: {error:?}"))?;
        Ok(Runtime {
            _device: device,
            ctx,
            _raw_module: module,
            _fused_module: fused_module,
            _n32_raw_module: n32_raw_module,
            _n32_fused_module: n32_fused_module,
            raw_fixed,
            fused,
            n32_raw,
            n32_fused,
    ...
    fn fixed_function(
        ...
        match candidate {
            ...
            CandidateFixed::N32Raw => Ok(&runtime.n32_raw),
        }
    }
    ...
    fn launch_retained_partials(
        ...
        for chunk in fixture.cell.chunks()? {
            ...
            builder.arg(&params);
            unsafe { builder.launch(config(candidate.grid(fixture.cell), (128, 1, 1), 0)) }
                .map_err(|error| {
                    format!("{} chunk {}: {error:?}", candidate.symbol(), chunk.begin)
                })?;
        }
        Ok(())
    }

    fn launch_fused_pipeline(
        runtime: &Runtime,
        fixture: &Fixture,
        ...
        unsafe { chunk0_builder.launch(config(chunk0_fixed.grid(fixture.cell), (128, 1, 1), 0)) }
        ...
        let mut fused_builder = runtime.ctx.stream.launch_builder(fused);
        ...
        unsafe { fused_builder.launch(config(chunk0_fixed.grid(fixture.cell), (128, 1, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch fused chunk1 {fused_symbol}: {error:?}"))
    }
    ...
    fn launch(
        ...
        match arm {
            Arm::Candidate => launch_fused_pipeline(
                runtime,
                fixture,
                CandidateFixed::N32Raw,
                &runtime.n32_fused,
                N32_FUSED,
            ),
            Arm::Reference => {
            ...
            Arm::Retained => {
                launch_fused_pipeline(runtime, fixture, candidate, &runtime.fused, FUSED)
            }
            Arm::Fast => {
    ...
    fn compare_raw_partials(
        ...
        for index in raw_plane_indices(fixture.cell).map_err(&harness)? {
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnFusedN32RawPartialV1",
                    "corpus":corpus,
        ...
        match production_exceptional {
            ...
            Err(RawPartialError::Mismatch(production_error)) => {
                println!(
                    "{}",
                    json!({
                        "schema":"MambaBiF32TnFusedN32N64RawStoreFallbackV1",
    ...
    fn prove_n32_raw_accumulator(runtime: &Runtime) -> Result<(), String> {
        for (corpus, exceptional) in FUSED_RAW_TARGET_CORPORA {
            let mut fixture = Fixture::new(&runtime.ctx, D768_IN, 1.0, exceptional)?;
            compare_raw_partials(runtime, &mut fixture, CandidateFixed::N32Raw, corpus)
                .map_err(|error| error.to_string())?;
        }
        Ok(())
    }
    ...
    unsafe fn require_fused_graph(
        ...
        if nodes.len() != 3 {
            return Err(format!(
                "fused candidate has {} nodes, expected three",
                nodes.len()
            ));
        }
        let transpose = nodes
        ...
        let fused = nodes
            .iter()
            .find(|node| node.symbol == fused_symbol)
            .ok_or_else(|| format!("fused pipeline omitted {fused_symbol}"))?;
        require_launch(
            transpose,
            TRANSPOSE,
            fixture.cell.transpose_grid(),
        ...
        require_launch(
            chunk0,
            chunk0_fixed.symbol(),
            chunk0_fixed.grid(fixture.cell),
            (128, 1, 1),
            0,
        )?;
        require_launch(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedN32GraphV1",
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "nodes":[TRANSPOSE,chunk0_fixed.symbol(),fused_symbol],
                "edges":[[TRANSPOSE,chunk0_fixed.symbol()],[chunk0_fixed.symbol(),fused_symbol]],
    ...
    unsafe fn require_fast_graph(graph: &CudaGraph) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiF32TnFusedN32GraphV1","arm":"Fast","node_count":count,"abi":"opaque","timing":"whole_graph"})
        );
        Ok(())
    }
    ...
    fn check_exact_case(
        ...
        unsafe {
            require_fused_graph(&candidate_graph, fixture, CandidateFixed::N32Raw, N32_FUSED)?;
            match oracle {
                Arm::Retained if fixture.cell.batch > 0 => {
                    require_fused_graph(&oracle_graph, fixture, candidate, FUSED)?
                }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedN32BitsV1",
                "case":label,
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
    ...
    fn n32_candidate_resources(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        let registers = function
            .num_regs()
            .map_err(|error| format!("{symbol} regs: {error:?}"))?;
        let local = function
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedN32HardResourceV1",
                "symbol":symbol,
                "threads":128,
                "registers":registers,
                "registers_max":N32_MAX_REGISTERS,
                "local_bytes":local,
                "static_shared_bytes":shared,
                "static_shared_expected":N32_STATIC_SHARED,
                "occupancy":occupancy,
                "occupancy_min":N32_MIN_OCCUPANCY
            })
        );
        if registers > N32_MAX_REGISTERS as i32
            || local != 0
            || shared as usize != N32_STATIC_SHARED
            || occupancy < N32_MIN_OCCUPANCY
        {
            return Err(format!(
    ...
    fn measure(
        ...
        if matches!(arm, Arm::Candidate | Arm::Retained) {
            fixture.validate_transpose(&runtime.ctx)?;
            fixture.validate_unused_chunk1_slab(&runtime.ctx)?;
        }
    ...
    #[test]
    #[ignore = "Ada CUDA13.2 discovery: exact F32 TN d768-in fused N32 vs N64"]
    fn ada_f32_tn_copyplan_fused_finalize_n32_d768_in_once7() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("f32-tn-fused-n32/pre", 1_800)?;
        } else {
            quiet.require_pre_context("f32-tn-fused-n32/pre")?;
        }
        let runtime = new_runtime()?;

        for (function, symbol) in [(&runtime.n32_raw, N32_RAW), (&runtime.n32_fused, N32_FUSED)] {
            if let Err(reason) = n32_candidate_resources(function, symbol) {
                println!(
                    "{}",
                    json!({
                        "schema":"MambaBiF32TnFusedN32ResourceStopV1",
                        "candidate":[N32_RAW,N32_FUSED],
                        "reason":reason,
                        "timing_started":false,
                        "decision":"stop_resource_gate",
                        "promotion":false
                    })
                );
                if idle_resident_mode {
                    quiet.require_idle_resident("f32-tn-fused-n32/resource-stop", 256)?;
                } else {
                    quiet.verify_post_cohort("f32-tn-fused-n32/resource-stop")?;
                }
                return Ok(());
            }
        }

        // Prove the committed N64 fused comparator before comparing a
        ...
        // Independently prove both raw partial planes for the exact chunk0
        // implementation selected below. The timed fused kernel never
        // materializes chunk1 and must also pass final exact-bit gates.
        let candidate = select_candidate(&runtime)?;
        prove_n32_raw_accumulator(&runtime)?;
        let fixed = fixed_function(&runtime, candidate)?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedN32ArtifactsV1",
                ...
                "n32_raw_source_sha":runtime.n32_raw_source_sha,
                "n32_fused_source_sha":runtime.n32_fused_source_sha,
                "candidate_nodes":[TRANSPOSE,N32_RAW,N32_FUSED],
                "retained_nodes":[TRANSPOSE,candidate.symbol(),FUSED],
                "candidate_grid":D768_IN.fixed_grid_n32(),
                "retained_grid":D768_IN.fixed_grid(),
        ...
        compare_raw_partials(
            ...
            &mut exceptional,
            CandidateFixed::N32Raw,
            "exceptional_tail_n32_accumulator",
        )
        .map_err(|error| error.to_string())?;
        check_exact_case(
            &runtime,
        ...
        // The whole three-node pipeline is timed as one logical GEMM. Resets,
        ...
        unsafe {
            require_fused_graph(&candidate_graph, &target, CandidateFixed::N32Raw, N32_FUSED)
        }?;
        ...
        while comparator_index < comparators.len() {
            ...
            unsafe {
                match comparator {
                    Arm::Retained => {
                        require_fused_graph(&comparator_graph, &target, candidate, FUSED)?
                    }
            ...
            if idle_resident_mode {
                quiet.require_idle_resident("f32-tn-fused-n32/timed", 256)?;
            } else {
                quiet.require_cohort("f32-tn-fused-n32/timed")?;
            }
            ...
            for graph_path in [false, true] {
                for candidate_endpoints in [true, false] {
                    ...
                    println!(
                        "{}",
                        json!({
                            "schema":"MambaBiF32TnFusedN32ScreenV1",
                            ...
                            "candidate_timed_nodes":[TRANSPOSE,N32_RAW,N32_FUSED],
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnFusedN32DecisionV1",
                    "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                    "comparator":if comparator == Arm::Retained {"N64_G8_FUSED"} else {"Fast"},
                    "strata":strata,
                    "strict_win":strict_win,
                    "decision":match (comparator,strict_win) {
                        (Arm::Retained,true)=>"n32_beats_n64_fused",
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedN32FinalDecisionV1",
                "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                "candidate":N32_FUSED,
                "retain_against_n64_fused":retained_win,
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("f32-tn-fused-n32/post", 256)?;
        } else {
            quiet.verify_post_cohort("f32-tn-fused-n32/post")?;
        }
        Ok(())
    }
}
