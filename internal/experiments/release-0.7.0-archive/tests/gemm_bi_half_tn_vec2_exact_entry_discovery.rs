//! Test-only actual-Triad F16 TN d768-in combined exact-entry discovery.
//! Production kernels and dispatch remain untouched.

#[path = "support/triad_half_tn_vec2_exact_entry_source.rs"]
...
#[test]
fn source_adapter_combines_exact_mapper_epilogue_stage_and_export() {
    ...
    assert!(
        candidate.contains("void gemm_bi_tn_test_tc64_bk64_s2_regpipe_vec2_exact_entry_##SUFFIX(")
    );
    for parameter in [
    ...
    assert_eq!(
        candidate_source::restore_retained_source(&candidate, PRODUCTION).unwrap(),
        retained
    );
    ...
    assert!(!candidate.contains("bool fast_stage"));
    assert!(!candidate.contains("gemm_bi_cp_async_valid_elems"));
    assert!(candidate.contains("cp.async.ca.shared.global [%0], [%1], 16;"));
}
...
fn geometry_resources_and_gated_timing_policy_are_frozen() {
    ...
    assert_eq!(candidate_source::TEXT_RATIO_CAP, 1.10);
    assert_eq!(candidate_source::EXPECTED_HMMA, 32);
    assert_eq!(candidate_source::EXPECTED_LDSM, 24);
    ...
    for threshold in [SCOUT_THRESHOLD, QUALIFICATION_THRESHOLD, FAST_THRESHOLD] {
...
mod cuda_suite {
    ...
    use super::{
        FAST_THRESHOLD, QUALIFICATION_THRESHOLD, SCOUT_THRESHOLD, TimingPhase, may_run_fast,
        windows_for,
    };
    ...
    impl Shape {
        ...
        fn is_exact_entry_target(self) -> bool {
            candidate_source::is_exact_target(self.reduction, self.k_out, self.n)
        }

        fn launch_grid(self, arm: Arm) -> (u32, u32, u32) {
            if arm == Arm::Candidate && self.is_exact_entry_target() {
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "vec2_exact_entry",
    ...
    fn validate_ptxas_and_sass(symbol: &str, ptx: &str) -> Result<SassStats, String> {
        ...
        let result = (|| {
            ...
            let ldsm = entry
                .lines()
                .filter(|line| line.contains("LDSM.16.MT88"))
                .count();
            ...
            let instructions = entry
                ...
                .count();
            if hmma == 0 || ldsm == 0 || ldgsts == 0 || instructions == 0 {
                return Err(format!("{symbol} SASS omitted HMMA, LDSM, LDGSTS, or text"));
            }
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        let (candidate_ptx, candidate_source_sha) =
            compile_ptx_only(&candidate, "exact-entry candidate compile-only")?;
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnExactEntryCompileV1\",\"candidate_source_sha\":\"{candidate_source_sha}\",\"retained_source_sha\":\"{retained_source_sha}\",\"candidate_ptx_sha\":\"{:x}\",\"retained_ptx_sha\":\"{:x}\",\"candidate_cubin_sha\":\"{}\",\"retained_cubin_sha\":\"{}\",\"candidate_sass_sha\":\"{}\",\"retained_sass_sha\":\"{}\",\"candidate_registers\":{},\"retained_registers\":{},\"static_shared_bytes\":{},\"hmma\":{},\"ldsm\":{},\"ldgsts\":{},\"candidate_text_bytes\":{},\"retained_text_bytes\":{},\"text_ratio\":{text_ratio},\"runtime_division\":{}}}",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_F16_TN_EXACT_ENTRY_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_F16_TN_EXACT_ENTRY_DISCOVERY=1".into());
        }
        let device = GpuDevice::new(0)?;
        let identity = device.identity();
        if identity.compute_capability != (8, 9) || identity.multiprocessor_count != 142 {
            return Err(format!(
                "half TN exact-entry discovery requires CC8.9/142-SM Ada, got {:?}/{}",
        ...
        if compiler.nvrtc_version != (13, 2) || compiler.target.as_str() != "sm_89" {
            return Err(format!(
                "half TN exact-entry discovery requires CUDA13.2/sm_89, got {compiler:?}"
            ));
        }
        let production = include_str!("../kernels/gemm_bi_triad/sm80.cu");
    ...
    fn launch(runtime: &Runtime, fixture: &Fixture, arm: Arm) -> Result<(), String> {
        ...
        match arm {
            Arm::Candidate | Arm::Retained => {
                ...
                let function = if arm == Arm::Candidate && shape.is_exact_entry_target() {
                    &runtime.candidate
                } else {
                    &runtime.retained
                };
    ...
    fn validate_kernel_graph(graph: &CudaGraph, fixture: &Fixture, arm: Arm) -> Result<(), String> {
        ...
        let expected = match arm {
            Arm::Candidate if fixture.shape.is_exact_entry_target() => {
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function) in [
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfTnExactEntryResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"register_cap\":{},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":0,\"block_threads\":{},\"active_ctas\":{active},\"required_active_ctas\":{}}}",
                arm.name(),
                candidate_source::REGISTER_CAP,
                candidate_source::BLOCK_THREADS,
                candidate_source::REQUIRED_OCCUPANCY,
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnExactEntryBitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha_bits\":{},\"exceptional\":{exceptional},\"candidate_vs_retained_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.reduction,
            shape.k_out,
            shape.n,
    ...
    fn timed_pair(
        ...
        for window in 0..windows {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiHalfTnExactEntryTimingV1\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"windows\":{windows},\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw,
    ...
    fn finish_quiet(runtime: Runtime, quiet: &QuietGpu) -> Result<(), String> {
        drop(runtime);
        quiet
            .verify_post_cohort("half-tn-vec2-exact-entry/post")
            .map(|_| ())
    }

    #[test]
    #[ignore = "requires exclusive 142-SM Ada and CUDA13.2"]
    fn ada_f16_tn_vec2_exact_entry_d768_in_scout3_then_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("half-tn-vec2-exact-entry/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("half-tn-vec2-exact-entry/cohort")?;
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnExactEntryScoutDecisionV1\",\"candidate\":\"{}\",\"comparator\":\"{}\",\"windows\":{scout_windows},\"strict_p50_p95_threshold\":{SCOUT_THRESHOLD},\"strata\":{:?},\"pass\":{scout_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            scout,
            if scout_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnExactEntryRetainedDecisionV1\",\"candidate\":\"{}\",\"comparator\":\"{}\",\"windows\":{qualification_windows},\"threshold\":{QUALIFICATION_THRESHOLD},\"strata\":{:?},\"pass\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_once7,
            if retained_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnExactEntryFinalDecisionV1\",\"shape\":[2048,768,3072],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"candidate_ptx_sha\":\"{}\",\"retained_ptx_sha\":\"{}\",\"scout_strata\":{:?},\"retained_once7_strata\":{:?},\"fast_once7_strata\":{:?},\"strict_retained_and_fast_p50_p95_lt_0_99\":{final_pass},\"decision\":\"{}\",\"promotion\":false}}",
