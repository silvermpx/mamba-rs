#[path = "support/triad_half_nt_m64n192_s3_bf16_source.rs"]
#[allow(dead_code)]
...
#[test]
fn candidate_source_has_unique_bf16_m64n192_geometry() {
    ...
    assert!(source.contains("sm89_test_half_nt_m64n192_s3::kernel<__nv_bfloat16>"));
    assert!(!source.contains("void gemm_bi_nt_test_fixed_s3_m64n192_f16"));

    let retained = candidate_source::measured_s3_source(SWIZZLE, S3)
        .expect("compose measured M64N128/S3 comparator");
...
mod cuda_suite {
    ...
    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_BF16_NT_M64N192_S3_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_BF16_NT_M64N192_S3_DISCOVERY=1".into());
        }
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function, shared, occupancy) in [
            ...
            println!(
                "{{\"schema\":\"MambaBiBf16NtM64N192S3ResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":{shared},\"active_ctas\":{active},\"required_active_ctas\":{occupancy}}}",
                arm.name()
            );
        }
        Ok(())
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiBf16NtM64N192S3BitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha\":{},\"exceptional\":{},\"candidate_vs_measured_s3_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.m,
            shape.k_out,
            shape.reduction,
    ...
    fn timed_pair(
        ...
        for window in 0..windows {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiBf16NtM64N192S3TimingV1\",\"phase\":\"{phase}\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw
    ...
    fn ada_bf16_nt_m64n192_bk64_s3_d768_out_scout_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("bf16-nt-m64n192-s3/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("bf16-nt-m64n192-s3/cohort")?;
        ...
        println!(
            "{{\"schema\":\"MambaBiBf16NtM64N192S3ScoutDecisionV1\",\"shape\":[2048,1536,768],\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{scout_pass},\"decision\":\"{}\"}}",
            scout_strata,
            if scout_pass {
                "advance_to_once7"
            } else {
        ...
        if !scout_pass {
            drop(runtime);
            quiet.verify_post_cohort("bf16-nt-m64n192-s3/post")?;
            return Ok(());
        }
        ...
        println!(
            "{{\"schema\":\"MambaBiBf16NtM64N192S3RetainedDecisionV1\",\"shape\":[2048,1536,768],\"candidate\":\"{}\",\"comparator\":\"{}\",\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_strata,
            if retained_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiBf16NtM64N192S3FinalDecisionV1\",\"shape\":[2048,1536,768],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"retained_strata\":{:?},\"fast_strata\":{:?},\"strict_fast_p50_p95_lt_0_99\":{fast_pass},\"decision\":\"{}\",\"promotion\":false}}",
