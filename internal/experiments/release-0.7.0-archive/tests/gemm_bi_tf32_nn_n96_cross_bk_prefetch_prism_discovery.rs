//! Test-only Ada TF32 NN canonical-Prism direct-N96 cross-BK prefetch discovery.
//! Production kernels and dispatch remain untouched.

#[path = "support/triad_tf32_nn_n96_cross_bk_prefetch_source.rs"]
#[allow(dead_code)]
...
#[test]
fn source_adapter_is_reversible_and_moves_only_safe_cross_bk_prefetches() {
    let retained = direct_retained_source();
    let candidate = candidate_source::candidate_source(FIXED_N96_SOURCE).unwrap();
    assert!(!candidate.contains("__grid_constant__"));
    assert!(candidate.contains("bool cross_bk_prefetch = has_next;"));
    assert!(candidate.contains("bool final_drain_boundary = !has_next && tile + 1 < tile_count;"));
    assert!(candidate.contains("for (int issue = 0; issue < 3; ++issue)"));
    assert!(candidate.contains("(int)(next * 32U), params.k, 3);"));
    assert!(candidate.contains("tf32n96_mma(fragments[1], acc);"));
...
#[test]
fn cross_bk_prefetch_excludes_the_unprotected_final_drain_boundary() {
    assert_eq!(candidate_source::cross_bk_prefetch_boundaries(0), 0);
    assert_eq!(candidate_source::cross_bk_prefetch_boundaries(1), 0);
    assert_eq!(candidate_source::cross_bk_prefetch_boundaries(2), 0);
    assert_eq!(candidate_source::cross_bk_prefetch_boundaries(3), 1);
    assert_eq!(candidate_source::cross_bk_prefetch_boundaries(12), 10);
}
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_TF32_NN_N96_CROSS_BK_PREFETCH_PRISM_DISCOVERY";
    ...
    const TAIL: Case = Case {
        label: "pipeline_tail_129x100x100",
        shape: Shape {
            m: 129,
            k: 100,
            n: 100,
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "n96_cross_bk_prefetch",
    ...
    fn ptxas_and_sass(symbol: &str, ptx: &str, shared: usize) -> Result<SassStats, String> {
        ...
        let result = (|| {
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiTriadTf32NnN96CrossBkPrefetchSassV1",
    ...
    fn compile_only_gate() -> Result<(), String> {
        ...
        let candidate = ptxas_and_sass(
            ...
            &compile_ptx(&candidate_body, "N96 cross-BK prefetch candidate")?,
            CANDIDATE_SHARED,
        )?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96CrossBkPrefetchCompileDecisionV1",
                ...
                "sass_distinct":sass_distinct,"text_ratio":text_ratio,"text_ratio_max":1.03,
        ...
        if !sass_distinct
            ...
            || retained.forbidden
            || candidate.text_bytes as f64 > retained.text_bytes as f64 * 1.03
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if cfg!(debug_assertions) {
            return Err("N96 cross-BK prefetch timing requires --release".into());
        }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96CrossBkPrefetchIdentityV1",
                ...
                "change":"cross_bk_fragment0_prefetch_before_current_mma3",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96CrossBkPrefetchResourceV1","symbol":symbol,
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32NnN96CrossBkPrefetchGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 1 {
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32NnN96CrossBkPrefetchGraphV1","arm":arm.name(),"symbol":symbol,"grid":expected_grid,"block":expected_config.block_dim,"shared":expected_config.shared_mem_bytes,"timing":"whole_graph"})
            );
        }
        Ok(())
    }
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32NnN96CrossBkPrefetchBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96CrossBkPrefetchTargetBitsV1",
    ...
    fn prepare_fast(runtime: &Runtime, case: Case, prepared: &mut Prepared) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96CrossBkPrefetchFastBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96CrossBkPrefetchScreenV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid(Arm::Candidate)?,
                "candidate":"n96_cross_bk_prefetch","comparator":comparator.name(),"path":path.name(),
    ...
    fn cuda132_compile_only_n96_cross_bk_prefetch_resources_and_sass() -> Result<(), String> {
        compile_only_gate()
    }

    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 NN N96 cross-BK prefetch Prism once3 -> once7 -> Fast"]
    fn ada_tf32_nn_n96_cross_bk_prefetch_prism_protocol() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-n96-cross-bk-prefetch/pre", 1_800)?;
        } else {
            quiet.require_pre_context("tf32-nn-n96-cross-bk-prefetch/pre")?;
        }
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-n96-cross-bk-prefetch/retained", 256)?;
        } else {
            quiet.require_cohort("tf32-nn-n96-cross-bk-prefetch/retained")?;
        }
        ...
        let fast_strata = if retained_win {
            ...
            if idle_resident_mode {
                quiet.require_idle_resident("tf32-nn-n96-cross-bk-prefetch/fast", 256)?;
            } else {
                quiet.require_cohort("tf32-nn-n96-cross-bk-prefetch/fast")?;
            }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96CrossBkPrefetchDecisionV1","cell":case.label,
        ...
        drop(prepared);
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-n96-cross-bk-prefetch/post", 256)?;
        } else {
            quiet.verify_post_cohort("tf32-nn-n96-cross-bk-prefetch/post")?;
        }
        Ok(())
    }
}
