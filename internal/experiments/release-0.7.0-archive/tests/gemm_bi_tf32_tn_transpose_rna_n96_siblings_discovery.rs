//! Test-only Ada TF32 TN d768-in and canonical-Prism pre-RNA transpose sibling discovery.
...
const D768_IN_TARGET: SiblingTarget = SiblingTarget {
    label: "d768_in_proj",
    shape: (2_048, 768, 3_072),
    gemm_grid: (192, 1, 1),
    transpose_grid: (24, 64, 1),
};

const PRISM_TARGET: SiblingTarget = SiblingTarget {
...
const fn sibling_targets() -> [SiblingTarget; 2] {
    [D768_IN_TARGET, PRISM_TARGET]
}

#[test]
fn sibling_targets_are_ordered_and_have_independently_derived_geometries() {
    assert_eq!(
        sibling_targets(),
        [
            SiblingTarget {
...
#[test]
fn adapter_moves_only_a_rna_into_transpose_and_is_reversible() {
    let retained = retained_source::candidate_source(FIXED_N96_SOURCE).unwrap();
    ...
    assert_eq!(
        candidate
            .matches(candidate_source::CANDIDATE_TRANSPOSE_SYMBOL)
            .count(),
        1
    );
    assert!(!candidate.contains(candidate_source::RETAINED_GEMM_SYMBOL));
    assert!(!candidate.contains(candidate_source::RETAINED_TRANSPOSE_SYMBOL));
    assert_eq!(
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_TF32_TN_TRANSPOSE_RNA_N96_SIBLINGS_DISCOVERY";
    ...
    impl Shape {
        ...
        fn transpose_stride(self) -> Result<usize, String> {
            retained_source::padded_stride(self.m)
        }
    ...
    const D768_IN: Case = Case {
        label: D768_IN_TARGET.label,
        shape: Shape {
            m: D768_IN_TARGET.shape.0,
            k: D768_IN_TARGET.shape.1,
            n: D768_IN_TARGET.shape.2,
        },
    };
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "pre_rna_transpose_n96",
                Self::Retained => "raw_transpose_n96",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        let retained_body = retained_source::candidate_source(FIXED_N96_SOURCE)?;
        ...
        let retained_transpose = retained_module
            .load_function(candidate_source::RETAINED_TRANSPOSE_SYMBOL)
            .map_err(|error| format!("load retained transpose: {error:?}"))?;
        let candidate_transpose = candidate_module
            .load_function(candidate_source::CANDIDATE_TRANSPOSE_SYMBOL)
        ...
        transpose_resource_gate(
            ...
            candidate_source::RETAINED_TRANSPOSE_SYMBOL,
        )?;
        transpose_resource_gate(
            &runtime.candidate_transpose,
            candidate_source::CANDIDATE_TRANSPOSE_SYMBOL,
        )?;
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96SiblingsIdentityV1",
                "retained_symbol":candidate_source::RETAINED_GEMM_SYMBOL,
                "candidate_symbol":candidate_source::CANDIDATE_GEMM_SYMBOL,
                "retained_transform_symbol":candidate_source::RETAINED_TRANSPOSE_SYMBOL,
                "candidate_transform_symbol":candidate_source::CANDIDATE_TRANSPOSE_SYMBOL,
                ...
                "change":"move_A_RNA_from_GEMM_loads_to_transpose_store",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96SiblingsResourceV1","symbol":symbol,
    ...
    fn transpose_resource_gate(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96SiblingsTransformResourceV1",
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32TnTransposeRnaN96SiblingsGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 2 {
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32TnTransposeRnaN96SiblingsGraphV1","arm":arm.name(),"nodes":2,"gemm_symbol":expected_gemm,"transpose_symbol":expected_transpose,"gemm_grid":case.shape.grid()?,"transpose_grid":case.shape.transpose_config()?.grid_dim,"timing":"whole_two_node_graph"})
            );
        }
        Ok(())
    }
    ...
    fn output_after(
        ...
        if arm != Arm::Fast && !fixture.exceptional {
            ...
            if arm == Arm::Candidate {
                ...
                retained_source::validate_transposed_words(
                    &input,
                    case.shape.m,
                    case.shape.k,
                    stride,
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32TnTransposeRnaN96SiblingsBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96SiblingsTargetBitsV1",
    ...
    fn prepare_fast(runtime: &Runtime, case: Case, prepared: &mut Prepared) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96SiblingsFastBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96SiblingsScreenV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid()?,
                "candidate":"pre_rna_transpose_n96","comparator":comparator.name(),"path":path.name(),
    ...
    fn run_target_protocol(
        ...
        let retained_label = format!("tf32-tn-transpose-rna-n96-siblings/{}/retained", case.label);
        let fast_label = format!("tf32-tn-transpose-rna-n96-siblings/{}/fast", case.label);
        let post_label = format!("tf32-tn-transpose-rna-n96-siblings/{}/post", case.label);
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96SiblingsDecisionV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid()?,
    ...
    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 TN transpose-RNA N96 d768-in + canonical Prism sequential once3 -> once7 -> Fast"]
    fn ada_tf32_tn_transpose_rna_n96_siblings_protocol() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-tn-transpose-rna-n96-siblings/pre", 1_800)?;
        } else {
            quiet.require_pre_context("tf32-tn-transpose-rna-n96-siblings/pre")?;
        }
        let runtime = new_runtime()?;
        assert_eq!(D768_IN.shape.grid()?, D768_IN_TARGET.gemm_grid);
        assert_eq!(
            D768_IN.shape.transpose_config()?.grid_dim,
            D768_IN_TARGET.transpose_grid
        );
        assert_eq!(PRISM.shape.grid()?, PRISM_TARGET.gemm_grid);
        assert_eq!(
        ...
        for case in [D768_IN, PRISM] {
            run_target_protocol(&runtime, &quiet, idle_resident_mode, case)?;
        }
        Ok(())
    }
}
