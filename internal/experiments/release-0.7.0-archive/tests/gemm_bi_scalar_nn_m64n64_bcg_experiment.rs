...
const CANDIDATE_SYMBOL: &str = "gemm_bi_nn_m64n64_bk16_s2_bcg_exp_v1";
const CUDA_SOURCE_FILE: &str = "gemm_bi_scalar_nn_m64n64_bcg_experiment.cu";
...
fn validate_exact_dims(dims: (usize, usize, usize)) -> Result<(), String> {
    if dims != DIMS {
        return Err(format!(
            "M64N64BCG post-production experiment only accepts {DIMS:?}, received {dims:?}"
        ));
    }
    Ok(())
}
...
fn ptxas_resource_contract(
    ...
    let registers = match (target, arm) {
        ...
        ("sm_80", ResourceArm::Candidate) => 123,
        ("sm_120", ResourceArm::Production) => 101,
        ("sm_120", ResourceArm::Candidate) => 101,
...
fn candidate_source_is_exact_one_owner_and_never_enters_production() {
    ...
    for required in [
        ...
        "__launch_bounds__(BCG_NN_M64N64_THREADS, 3)",
...
fn resource_contracts_are_arch_specific_strict_and_fail_closed() {
    ...
    for invalid in [vec![], vec![0], vec![100], vec![102], vec![101, 101]] {
...
fn windows_percentiles_and_promotion_thresholds_are_fail_closed() {
    ...
    assert!(validate_speedup(21, &[1.001; 21]).is_err());
...
mod cuda_experiment {
    ...
    fn compile_ptx(target: &'static str) -> Result<String, String> {
        ...
        cudarc::nvrtc::compile_ptx_with_opts(compose_source()?, options)
            ...
            .map_err(|error| format!("compile M64N64BCG post-production experiment: {error:?}"))
    }
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if identity.compute_capability != (12, 0) || identity.multiprocessor_count != 170 {
            return Err(format!(
                "M64N64BCG post-production experiment requires CC12.0/170SM, found CC{}.{} with {} SMs",
        ...
        let module = device
            ...
            .map_err(|error| format!("load M64N64BCG experiment module: {error:?}"))?;
        Ok(Runtime {
            production_ctx,
            stream,
            module,
    ...
    fn launch(
        ...
        {
            return Err("M64N64BCG launch operands or contiguous strides drifted".into());
        }
        ...
        {
            return Err("M64N64BCG launch requires non-null 16-byte-aligned C/A/B".into());
        }
    ...
    fn graph_identity(graph: &CudaGraph, arm: Arm) -> Result<[u8; 32], String> {
        ...
        digest.update(b"scalar-nn-m64n64-bcg-post-graph-args.v1");
    ...
    fn validate_driver_resources(kernels: &Kernels) -> Result<(), String> {
        ...
        for arm in [Arm::Production, Arm::Candidate] {
            ...
            eprintln!(
                "scalar_nn_m64n64-bcg resource symbol={} threads={} dynamic_shared={} static_shared={} registers={} local={} occupancy={}",
                arm.symbol(),
                config.block_dim.0,
    ...
    fn validate_ptxas_and_sass(target: &str, ptx: &str) -> Result<(), String> {
        ...
        let stem = format!("mamba-nn-m64n64-bcg-post-{target}-{nonce}");
        ...
        let result = (|| {
            ...
            for arm in [Arm::Production, Arm::Candidate] {
                ...
                eprintln!(
                    "scalar_nn_m64n64-bcg ptxas target={target} symbol={} registers={}",
                    arm.symbol(),
                    registers[0]
                );
    ...
    fn paired(
        ...
        let label = format!("scalar-nn-m64n64-bcg-post/{path:?}/{order:?}/{windows}");
        ...
        let body = (|| {
            ...
            eprintln!(
                "scalar_nn_m64n64-bcg path={path:?} order={order:?} windows={windows} production_us_p50={:.6} candidate_us_p50={:.6} speedup_p05={:.9} speedup_p50={:.9} speedup_p95={:.9}",
    ...
    fn run_timing(windows: usize) -> Result<(), String> {
        ...
        let label = format!("scalar-nn-m64n64-bcg-post-{windows}");
    ...
    fn candidate_screen_21_windows_abba_baab() -> Result<(), String> {
        ...
        let windows = exact_windows(
            std::env::var_os("MAMBA_RS_NN_M64N64BCG_POST_WINDOWS").as_deref(),
            SCREEN_WINDOWS,
        )?;
        run_timing(windows)
    }
