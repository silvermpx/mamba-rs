//! Test-only actual-Triad F16 TN d768-in eight-compute-warp discovery.
//! Production kernels and dispatch remain untouched.

#[path = "support/triad_half_tn_8warp_16x32_source.rs"]
...
#[test]
fn candidate_assigns_one_16x32_output_tile_to_each_of_eight_compute_warps() {
    let source = candidate_source::candidate_source(PRODUCTION).unwrap();
    assert!(source.contains("#define GEMM_BI_TC64_THREADS 256"));
    assert!(source.contains("int warpM = (warp / 2) * 16;"));
    assert!(source.contains("int warpN = (warp % 2) * 32;"));
    assert!(source.contains("float acc[1][4][4];"));
    assert!(source.contains("unsigned a_frag[2][1][4];"));
    assert!(source.contains("unsigned b_frag[2][4][2];"));
    assert_eq!(source.matches("for (int fm = 0; fm < 1; fm++)").count(), 5);
}

#[test]
fn output_ownership_and_dynamic_tensor_work_are_exact() {
    let mut owners = [[0_u8; 64]; 64];
    for warp in 0..8 {
        let (m0, n0) = candidate_source::warp_output_origin(warp);
        for m in m0..m0 + 16 {
            for n in n0..n0 + 32 {
                owners[m][n] += 1;
            }
    ...
    assert_eq!(8 * 1 * 4 * 4, 4 * 2 * 4 * 4);
    assert_eq!(8 * (1 + 4), 40);
    assert_eq!(4 * (2 + 4), 24);
}

#[test]
fn source_adapter_changes_only_warp_decomposition_and_export() {
    ...
    assert!(
        candidate.contains("void gemm_bi_tn_test_tc64_bk64_s2_regpipe_vec2_8warp16x32_##SUFFIX(")
    );
    for parameter in [
...
fn geometry_resources_and_gated_timing_policy_are_frozen() {
    ...
    assert_eq!(candidate_source::REGISTER_CAP, 104);
    assert_eq!(candidate_source::REQUIRED_OCCUPANCY, 2);
    assert_eq!(candidate_source::EXPECTED_HMMA, 16);
    assert_eq!(candidate_source::EXPECTED_LDSM, 20);
    ...
    assert_eq!(SCOUT_THRESHOLD, QUALIFICATION_THRESHOLD);
    assert_eq!(FAST_THRESHOLD, 0.99);
    assert!(candidate_source::all_strata_below(
        &[[0.98, 0.989]; 4],
        FAST_THRESHOLD
    ));
...
mod cuda_suite {
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "regpipe_vec2_8warp16x32",
                Self::Retained => "retained_regpipe_vec2",
                Self::Fast => "cublas_fast_f16",
            }
        }

        const fn block_threads(self) -> u32 {
            match self {
                Self::Candidate => candidate_source::BLOCK_THREADS,
                Self::Retained => 128,
                Self::Fast => 0,
            }
        }
    ...
    fn validate_ptxas_and_sass(
        ...
        let result = (|| {
            ...
            if hmma != expected_hmma || ldsm != expected_ldsm || ldgsts == 0 {
                return Err(format!(
                    "{symbol} SASS signature reject HMMA={hmma}/{expected_hmma} LDSM={ldsm}/{expected_ldsm} LDGSTS={ldgsts}"
                ));
            }
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        let (candidate_ptx, candidate_source_sha) =
            compile_ptx_only(&candidate, "eight-warp 16x32 candidate compile-only")?;
    ...
    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_F16_TN_8WARP_16X32_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_F16_TN_8WARP_16X32_DISCOVERY=1".into());
        }
        let device = GpuDevice::new(0)?;
        let identity = device.identity();
        if identity.compute_capability != (8, 9) || identity.multiprocessor_count != 142 {
            return Err(format!(
                "half TN eight-warp 16x32 discovery requires CC8.9/142-SM Ada, got {:?}/{}",
        ...
        if compiler.nvrtc_version != (13, 2) || compiler.target.as_str() != "sm_89" {
            return Err(format!(
                "half TN eight-warp 16x32 discovery requires CUDA13.2/sm_89, got {compiler:?}"
            ));
        }
        let production = include_str!("../kernels/gemm_bi_triad/sm80.cu");
            ...
            Arm::Candidate | Arm::Retained => {
                ...
                unsafe {
                    builder.launch(LaunchConfig {
                        ...
                        block_dim: (arm.block_threads(), 1, 1),
                        shared_mem_bytes: 0,
                    })
                }
                .map(|_| ())
    ...
    fn validate_kernel_graph(graph: &CudaGraph, fixture: &Fixture, arm: Arm) -> Result<(), String> {
        ...
        if actual != expected
            ...
            || (params.blockDimX, params.blockDimY, params.blockDimZ) != (arm.block_threads(), 1, 1)
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function, register_cap, required_occupancy) in [
            (
                Arm::Candidate,
                &runtime.candidate,
                candidate_source::REGISTER_CAP as i32,
                candidate_source::REQUIRED_OCCUPANCY as u32,
            ),
            (Arm::Retained, &runtime.retained, 128_i32, 3_u32),
        ] {
            ...
            let active = function
                .occupancy_max_active_blocks_per_multiprocessor(arm.block_threads(), 0, None)
                .map_err(|error| format!("{} occupancy: {error:?}", arm.name()))?;
            if !(1..=register_cap).contains(&registers)
                || local != 0
                || static_shared != candidate_source::STATIC_SHARED_BYTES as i32
                || max_threads < arm.block_threads() as i32
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfTnEightWarp16x32ResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"register_cap\":{},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":0,\"block_threads\":{},\"active_ctas\":{active},\"required_active_ctas\":{}}}",
                arm.name(),
                register_cap,
                arm.block_threads(),
                required_occupancy,
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnEightWarp16x32BitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha_bits\":{},\"exceptional\":{exceptional},\"candidate_vs_retained_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.reduction,
            shape.k_out,
            shape.n,
    ...
    fn timed_pair(
        ...
        for window in 0..windows {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiHalfTnEightWarp16x32TimingV1\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"windows\":{windows},\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw,
    ...
    fn finish_quiet(runtime: Runtime, quiet: &QuietGpu) -> Result<(), String> {
        drop(runtime);
        quiet
            .verify_post_cohort("half-tn-8warp-16x32/post")
            .map(|_| ())
    }

    #[test]
    #[ignore = "requires exclusive 142-SM Ada and CUDA13.2"]
    fn ada_f16_tn_regpipe_vec2_8warp16x32_d768_in_scout3_then_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("half-tn-8warp-16x32/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("half-tn-8warp-16x32/cohort")?;
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnEightWarp16x32ScoutDecisionV1\",\"candidate\":\"{}\",\"comparator\":\"{}\",\"windows\":{scout_windows},\"threshold\":{SCOUT_THRESHOLD},\"strata\":{:?},\"pass\":{scout_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            scout,
            if scout_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnEightWarp16x32RetainedDecisionV1\",\"candidate\":\"{}\",\"comparator\":\"{}\",\"windows\":{qualification_windows},\"threshold\":{QUALIFICATION_THRESHOLD},\"strata\":{:?},\"pass\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_once7,
            if retained_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnEightWarp16x32FinalDecisionV1\",\"shape\":[2048,768,3072],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"candidate_ptx_sha\":\"{}\",\"retained_ptx_sha\":\"{}\",\"scout_strata\":{:?},\"retained_once7_strata\":{:?},\"fast_once7_strata\":{:?},\"strict_fast_p50_p95_lt_0_99\":{fast_pass},\"decision\":\"{}\",\"promotion\":false}}",
