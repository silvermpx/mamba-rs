// Exact-F32 experiment: the production ascending-k FMA chain behind an 8x8
...
// production 64x64 NN chain. With S splits, unit (tile, s) accumulates its
...
#define WIDE_TM 8
#define WIDE_TN 8
#define WIDE_WARP_SIZE 32
#define WIDE_GROUP_M 16
#define WIDE_K_PIPE 2

struct SgbWideParams {
    float alpha;
    float beta;
    int m;
    int n;
...
static_assert(sizeof(SgbWideParams) == 32,
              "wide-microtile parameter ABI drift");
static_assert(alignof(SgbWideParams) == 4,
              "wide-microtile parameter alignment drift");
static_assert(__is_standard_layout(SgbWideParams),
              "wide-microtile parameters must remain standard layout");

enum { WIDE_OP_NN = 0, WIDE_OP_TN = 1, WIDE_OP_NT = 2 };

template <int Arrivals>
static __device__ __forceinline__ void wide_init_barrier(unsigned barrier) {
    asm volatile("mbarrier.init.shared::cta.b64 [%0], %1;"
                 :: "r"(barrier), "n"(Arrivals) : "memory");
}

static __device__ __forceinline__ void wide_wait_barrier(
...
template <int Bytes>
static __device__ __forceinline__ void wide_expect_transaction(
    unsigned barrier) {
    asm volatile(
...
static __device__ __forceinline__ void wide_tma_copy(
...
static __device__ __forceinline__ void wide_flag_raise(unsigned* flag) {
    asm volatile("st.release.gpu.global.u32 [%0], %1;"
                 :: "l"(flag), "r"(1u) : "memory");
}

static __device__ __forceinline__ unsigned wide_flag_read(
    const unsigned* flag) {
    unsigned value;
...
__device__ __forceinline__ void wide_microtile_body(
    float* __restrict__ C,
    const float* __restrict__ bias,
    float* __restrict__ partials,
    unsigned* __restrict__ flags,
    const SgbWideParams& params,
    const CUtensorMap& a_map,
    const CUtensorMap& b_map
) {
    constexpr int BK = WIDE_BK;
    constexpr int TM = WIDE_TM;
    constexpr int TN = WIDE_TN;
    constexpr int K_PIPE = WIDE_K_PIPE;
    ...
    constexpr int WARPS = THREADS / WIDE_WARP_SIZE;
    ...
    static_assert(LANE_COLUMNS * LANE_ROWS == WIDE_WARP_SIZE,
    ...
    static_assert(!NT_KVEC || OP == WIDE_OP_NT,
                  "the k-vector B fragment only exists for NT");

    extern __shared__ __align__(1024) float wide_smem[];
    float* stages = wide_smem;
    unsigned smem_base = __cvta_generic_to_shared(wide_smem);
    ...
    int num_pid_in_group = WIDE_GROUP_M * num_pid_n;
    int group_id = tile_id / num_pid_in_group;
    int first_pid_m = group_id * WIDE_GROUP_M;
    int group_size_m = min(num_pid_m - first_pid_m, WIDE_GROUP_M);
    int pid_m = first_pid_m + ((tile_id % num_pid_in_group) % group_size_m);
    int pid_n = (tile_id % num_pid_in_group) / group_size_m;

    int warp = threadIdx.x / WIDE_WARP_SIZE;
    int lane = threadIdx.x % WIDE_WARP_SIZE;
    ...
    if (threadIdx.x == 0) {
        ...
        for (int stage = 0; stage < K_PIPE; ++stage) {
            wide_init_barrier<1>(barrier_base + stage * 8);
        }
    ...
    auto produce = [&](int stage, int k_tile) {
        ...
        wide_expect_transaction<STAGE_BYTES>(barrier);
        if (OP == WIDE_OP_TN) {
            wide_tma_copy(destination, &a_map, pid_m * BM, k_tile * BK, barrier);
        } else {
            wide_tma_copy(destination, &a_map, k_tile * BK, pid_m * BM, barrier);
        }
        if (OP == WIDE_OP_NT) {
            wide_tma_copy(b_destination, &b_map, k_tile * BK, pid_n * BN, barrier);
        } else {
            wide_tma_copy(b_destination, &b_map, pid_n * BN, k_tile * BK, barrier);
        }
    };
    ...
    for (int step = 0; step < range; ++step) {
        ...
        wide_wait_barrier(barrier_base + stage * 8,
        ...
        for (int dot_index = 0; dot_index < BK; ++dot_index) {
            ...
            #pragma unroll
            for (int i = 0; i < TM; ++i) {
                float a = OP == WIDE_OP_TN
                    ? a_fragment[i][0]
                    : a_fragment[i][dot_index % 4];
                #pragma unroll
                for (int c = 0; c < TN; ++c) {
                    float b = (OP == WIDE_OP_NT && NT_KVEC)
    ...
    if (splits > 1) {
        ...
        if (!owner) {
            ...
            __syncthreads();
            if (threadIdx.x == 0) {
                wide_flag_raise(tile_flags + split);
            }
            return;
        }
        if (threadIdx.x == 0) {
            for (int s = 0; s < splits - 1; ++s) {
                while (wide_flag_read(tile_flags + s) == 0u) {
                }
            }
        }
        __syncthreads();
...
#define WIDE_DEFINE_KERNEL(NAME, OP, BM, BN, WM, WN, THREADS, MIN_BLOCKS, NT_KVEC) \
    ...
    void NAME(                                                                    \
        ...
        float* __restrict__ partials,                                             \
        unsigned* __restrict__ flags,                                             \
        const __grid_constant__ SgbWideParams params,                             \
        const __grid_constant__ CUtensorMap a_map,                                \
        const __grid_constant__ CUtensorMap b_map                                 \
    ) {                                                                           \
        assert(params.alpha == 1.0f || bias == nullptr);                          \
        wide_microtile_body<OP, BM, BN, WM, WN, THREADS, NT_KVEC>(                \
...
WIDE_DEFINE_KERNEL(gemm_bi_nn_m128n64_bk16_s2_micro8x8_exp, WIDE_OP_NN, 128, 64, 32, 64, 128, 3, false)
WIDE_DEFINE_KERNEL(gemm_bi_nn_m64n128_bk16_s2_micro8x8_exp, WIDE_OP_NN, 64, 128, 64, 32, 128, 3, false)
WIDE_DEFINE_KERNEL(gemm_bi_nn_m64n64_bk16_s2_micro8x8_exp, WIDE_OP_NN, 64, 64, 32, 64, 64, 5, false)
WIDE_DEFINE_KERNEL(gemm_bi_tn_m128n64_bk16_s2_micro8x8_exp, WIDE_OP_TN, 128, 64, 32, 64, 128, 3, false)
WIDE_DEFINE_KERNEL(gemm_bi_tn_m64n128_bk16_s2_micro8x8_exp, WIDE_OP_TN, 64, 128, 64, 32, 128, 3, false)
WIDE_DEFINE_KERNEL(gemm_bi_tn_m64n64_bk16_s2_micro8x8_exp, WIDE_OP_TN, 64, 64, 32, 64, 64, 5, false)
WIDE_DEFINE_KERNEL(gemm_bi_nt_m128n64_bk16_s2_micro8x8_exp, WIDE_OP_NT, 128, 64, 32, 64, 128, 3, false)
WIDE_DEFINE_KERNEL(gemm_bi_nt_m64n128_bk16_s2_micro8x8_exp, WIDE_OP_NT, 64, 128, 64, 32, 128, 3, false)
WIDE_DEFINE_KERNEL(gemm_bi_nt_m64n64_bk16_s2_micro8x8_exp, WIDE_OP_NT, 64, 64, 32, 64, 64, 5, false)
WIDE_DEFINE_KERNEL(gemm_bi_nt_m128n64_bk16_s2_micro8x8_kvec_exp, WIDE_OP_NT, 128, 64, 32, 64, 128, 2, true)
WIDE_DEFINE_KERNEL(gemm_bi_nt_m64n128_bk16_s2_micro8x8_kvec_exp, WIDE_OP_NT, 64, 128, 64, 32, 128, 2, true)
WIDE_DEFINE_KERNEL(gemm_bi_nt_m64n64_bk16_s2_micro8x8_kvec_exp, WIDE_OP_NT, 64, 64, 32, 64, 64, 4, true)

#undef WIDE_DEFINE_KERNEL
#undef WIDE_K_PIPE
#undef WIDE_GROUP_M
#undef WIDE_WARP_SIZE
#undef WIDE_TN
#undef WIDE_TM
#undef WIDE_BK
