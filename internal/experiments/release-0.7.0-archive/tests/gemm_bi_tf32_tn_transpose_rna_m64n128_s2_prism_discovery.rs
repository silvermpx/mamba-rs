//! Test-only Ada TF32 TN canonical-Prism M64N128/S2 higher-reuse discovery.
//! Production kernels and dispatch remain untouched.

#[path = "support/triad_tf32_tn_transpose_rna_m64n128_s2_source.rs"]
#[allow(dead_code)]
...
const PRISM_TARGET: Target = Target {
    ...
    candidate_grid: (96, 1, 1),
    retained_grid: (186, 1, 1),
...
fn prism_target_has_independently_derived_geometries() {
    ...
    assert_eq!(candidate_source::TILE, (64, 128, 32));
    assert_eq!(candidate_source::STAGES, 2);
    assert_eq!(candidate_source::ACCUMULATORS_PER_THREAD, 32);
    assert_eq!(2 * (64 * 32 + 32 * 128) * 4, 49_152);
}
...
fn rna_oracle_rounds_finite_ties_away_from_zero() {
    for (input, expected) in [
        ...
        assert_eq!(rna_source::tf32_rna_bits(input), expected);
    }
}
...
fn scratch_oracle_is_an_independent_rna_transpose_with_zero_padding() {
    ...
    assert!(rna_source::validate_rna_transposed_words(&input, 2, 3, 4, &expected).is_ok());
    let mut wrong = expected;
    wrong[4] ^= 0x2000;
    assert!(rna_source::validate_rna_transposed_words(&input, 2, 3, 4, &wrong).is_err());
}

#[test]
fn adapter_preserves_pre_rna_contract_and_changes_tile_materially() {
    let retained = candidate_source::retained_source(FIXED_N96_SOURCE).unwrap();
    ...
    assert!(candidate.contains("int warp_n = (warp & 3) * 32;"));
    assert!(candidate.contains("fragments.a[m_atom][0] = raw0;"));
    assert_eq!(
        candidate
            .matches("tf32m64n128_round(__float_as_uint(b_step")
            .count(),
        2
    );
    assert!(
        candidate.contains("tf32m64n128_round(tile[(int)threadIdx.x][(int)threadIdx.y + offset])")
    );
    assert_eq!(
...
#[test]
fn eight_warps_own_each_m64n128_output_exactly_once() {
    let mut ownership = vec![0_u8; 64 * 128];
    for warp in 0..8 {
        let warp_m = (warp >> 2) * 32;
        let warp_n = (warp & 3) * 32;
        for row in warp_m..warp_m + 32 {
            for column in warp_n..warp_n + 32 {
                ownership[row * 128 + column] += 1;
            }
        }
    }
...
#[test]
fn copy_slices_own_a_and_b_float4_vectors_exactly_once() {
    let mut a_ownership = vec![0_u8; 64 * 8];
    let mut b_ownership = vec![0_u8; 32 * 32];
    for thread in 0..256 {
        for slice in 0..2 {
        ...
        for slice in 0..4 {
            let linear = thread + slice * 256;
            b_ownership[(linear / 32) * 32 + (linear % 32)] += 1;
        }
    }
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_TF32_TN_TRANSPOSE_RNA_M64N128_S2_PRISM_DISCOVERY";
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "pre_rna_transpose_m64n128_s2_reuse",
                Self::Retained => "pre_rna_transpose_m64n64_s3_wave",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if cfg!(debug_assertions) {
            return Err("TN pre-RNA transpose M64N128/S2 timing requires --release".into());
        }
        ...
        {
            return Err(format!("wrong M64N128/S2 discovery compiler: {compiler:?}"));
        }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N128S2PrismIdentityV1",
                ...
                "change":"M64N64_S3_grid186_to_M64N128_S2_grid96_higher_reuse",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N128S2PrismResourceV1","symbol":symbol,
    ...
    fn transpose_resource_gate(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N128S2PrismTransformResourceV1",
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32TnTransposeRnaM64N128S2PrismGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 2 {
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32TnTransposeRnaM64N128S2PrismGraphV1","arm":arm.name(),"nodes":2,"gemm_symbol":expected_gemm,"transpose_symbol":expected_transpose,"gemm_grid":case.shape.grid(arm)?,"transpose_grid":case.shape.transpose_config()?.grid_dim,"timing":"whole_two_node_graph"})
            );
        }
        Ok(())
    }
    ...
    fn output_after(
        ...
        if arm != Arm::Fast && !fixture.exceptional {
            ...
            rna_source::validate_rna_transposed_words(
                &input,
                case.shape.m,
                case.shape.k,
                stride,
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32TnTransposeRnaM64N128S2PrismBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N128S2PrismTargetBitsV1",
    ...
    fn prepare_fast(runtime: &Runtime, case: Case, prepared: &mut Prepared) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N128S2PrismFastBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N128S2PrismScreenV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid(Arm::Candidate)?,
                "candidate":"pre_rna_transpose_m64n128_s2_reuse","comparator":comparator.name(),"path":path.name(),
    ...
    fn run_target_protocol(
        ...
        let retained_label = format!(
            "tf32-tn-transpose-rna-m64n128-s2-prism/{}/retained",
            case.label
        );
        let fast_label = format!("tf32-tn-transpose-rna-m64n128-s2-prism/{}/fast", case.label);
        let post_label = format!("tf32-tn-transpose-rna-m64n128-s2-prism/{}/post", case.label);
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N128S2PrismDecisionV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid(Arm::Candidate)?,
    ...
    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 TN pre-RNA M64N128/S2 canonical Prism once3 -> once7 -> Fast"]
    fn ada_tf32_tn_transpose_rna_m64n128_s2_prism_protocol() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-tn-transpose-rna-m64n128-s2-prism/pre", 1_800)?;
        } else {
            quiet.require_pre_context("tf32-tn-transpose-rna-m64n128-s2-prism/pre")?;
        }
