...
const CANDIDATE_SYMBOL: &str = "gemm_bi_nn_splitk32_m64n64_partial_exp";
const PRODUCTION_PARTIAL_SYMBOL: &str = "gemm_bi_nn_splitk32_partial";
const REDUCER_SYMBOL: &str = "gemm_bi_splitk_reduce";
const CANDIDATE_SOURCE: &str = include_str!("gemm_bi_scalar_nn_splitk_m64n64_tournament.cu");
const HARNESS_SOURCE: &str = include_str!("gemm_bi_scalar_nn_splitk_m64n64_tournament.rs");
...
const CANDIDATE_SHARED: u32 = 17_408;
const OFFICIAL_WINDOWS: usize = 101;
const MIN_P05_SPEEDUP: f64 = 1.02;
const MIN_P50_SPEEDUP: f64 = 1.05;
...
fn qualification_windows(value: Option<&str>) -> Result<usize, String> {
    ...
    if windows != OFFICIAL_WINDOWS {
        return Err(format!(
            "official qualification requires exactly {OFFICIAL_WINDOWS} windows, found {windows}"
        ));
    }
    Ok(windows)
}
...
fn validate_performance_gate(order: &str, p05: f64, p50: f64, p95: f64) -> Result<(), String> {
    ...
    if p05 < MIN_P05_SPEEDUP || p50 < MIN_P50_SPEEDUP {
        return Err(format!(
            "M64N64 Split-K candidate loses the {order} gate: p05={p05:.9} p50={p50:.9}; required p05>={MIN_P05_SPEEDUP:.2} and p50>={MIN_P50_SPEEDUP:.2}"
        ));
    }
    Ok(())
}
...
fn timing_gates_reject_invalid_samples_thresholds_and_tiny_windows() {
    ...
    for value in ["0", "1", "20", "21", "100", "102", "bad"] {
    ...
    assert!(validate_performance_gate("test", 1.02, 1.05, 1.10).is_ok());
    assert!(validate_performance_gate("test", 1.019, 1.05, 1.10).is_err());
    assert!(validate_performance_gate("test", 1.02, 1.049, 1.10).is_err());
}
...
fn candidate_compiles_and_meets_sm89_sm120_ptxas_resources() {
    for (arch, sm) in [("compute_89", "sm_89"), ("compute_120", "sm_120")] {
        ...
        let stem = format!("mamba-rs-nn-splitk-m64-{}-{sm}", std::process::id());
        ...
        eprintln!("NN Split-K M64N64 resources {sm}:\n{log}");
...
mod cuda_tournament {
    ...
    impl Arm {
        fn name(self) -> &'static str {
            match self {
                ...
                Self::Candidate => "candidate_splitk32_m64n64",
            }
        }
    }
    ...
    fn check_resources(kernel: &Kernel, contract: ResourceContract) -> Result<(), String> {
        ...
        eprintln!(
            "nn_splitk_m64 resource symbol={} threads={} registers={} local={} static_shared={} dynamic_shared={} occupancy={}",
            kernel.symbol,
            threads,
            registers,
            local,
    ...
    fn paired(
        ...
        let production_iterations = calibrated_iterations(runtime, fixture, Arm::Production)?;
        let candidate_iterations = calibrated_iterations(runtime, fixture, Arm::Candidate)?;
        ...
        for (order, baseline_first) in [("ABBA", true), ("BAAB", false)] {
            ...
            for _ in 0..windows {
                let (p0, p1, c0, c1) = if baseline_first {
                    let p0 = measure(runtime, fixture, Arm::Production, production_iterations)?;
                    let c0 = measure(runtime, fixture, Arm::Candidate, candidate_iterations)?;
                    let c1 = measure(runtime, fixture, Arm::Candidate, candidate_iterations)?;
                    let p1 = measure(runtime, fixture, Arm::Production, production_iterations)?;
                    (p0, p1, c0, c1)
                } else {
                    let c0 = measure(runtime, fixture, Arm::Candidate, candidate_iterations)?;
                    let p0 = measure(runtime, fixture, Arm::Production, production_iterations)?;
            ...
            eprintln!(
                "nn_splitk_m64 baseline={} candidate={} order={} windows={} production_us_p50={:.6} candidate_us_p50={:.6} speedup_p05={:.9} speedup_p50={:.9} speedup_p95={:.9} production_iterations={} candidate_iterations={}",
                Arm::Production.name(),
                Arm::Candidate.name(),
                order,
                windows,
    ...
    fn candidate_vs_production_101_window_abba_baab() -> Result<(), String> {
        ...
        let value = match std::env::var("MAMBA_RS_NN_SPLITK_M64N64_WINDOWS") {
            Ok(value) => Some(value),
            Err(std::env::VarError::NotPresent) => None,
            Err(std::env::VarError::NotUnicode(_)) => {
                return Err("MAMBA_RS_NN_SPLITK_M64N64_WINDOWS is not Unicode".into());
            }
        };
