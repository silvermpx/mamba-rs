//! Test-only canonical Ada TF32 NT prism_in_proj A-ldmatrix full-domain discovery.

#[path = "support/triad_tf32_nt_a_ldmatrix_prism_full_domain_source.rs"]
...
#[test]
fn canonical_prism_geometry_has_216_full_ctas_and_guarded_edges() {
    assert_eq!(candidate_source::TARGET, (4_621, 384, 1_928));
    assert_eq!(candidate_source::TARGET_GRID, (6, 37, 1));
    assert_eq!(candidate_source::FULL_M_CTAS, 216);
    assert!(candidate_source::full_m_cta(35));
    assert!(!candidate_source::full_m_cta(36));
    assert!(candidate_source::full_k_stage(59));
    assert!(!candidate_source::full_k_stage(60));
...
#[test]
fn source_transform_is_reversible_and_keeps_retained_math() {
    let retained = candidate_source::retained_source(PRODUCTION, COMPACT_LAYOUT).unwrap();
    let candidate = candidate_source::candidate_source(PRODUCTION, COMPACT_LAYOUT).unwrap();
    assert_eq!(
        candidate_source::restore_retained_source(&candidate).unwrap(),
        retained
    );
    assert_eq!(candidate.matches(candidate_source::SYMBOL).count(), 2);
    assert!(!candidate.contains(candidate_source::RETAINED_SYMBOL));
    assert_eq!(
        candidate
    ...
    assert!(candidate.contains("gemm_bi_tf32_compute_stage<"));
    assert!(candidate.contains("gemm_bi_tf32_epilogue<Op>"));
    assert!(candidate.contains("canonical_prism ? (int)blockIdx.y * BM"));
    assert!(candidate.contains("full_output && next + 1 < tile_count"));
}
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_TF32_NT_A_LDMATRIX_PRISM_FULL_DOMAIN_DISCOVERY";
    const OPS: usize = 20;
    ...
    impl Shape {
        ...
        fn grid_for(self, arm: Arm) -> Result<(u32, u32, u32), String> {
            if arm == Arm::Candidate && self == TARGET {
                Ok(candidate_source::TARGET_GRID)
            } else {
                self.grid()
            }
        }
    ...
    const TARGET: Shape = Shape::new(4_621, 384, 1_928, 1.0, "canonical_prism_in_proj");
    const ALIGNED_TAIL: Shape = Shape::new(129, 67, 72, -0.75, "aligned_tail");
    const MISALIGNED_TAIL: Shape = Shape::new(67, 69, 71, -0.75, "misaligned_tail");
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "a_ldmatrix_prism_full_domain",
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        let (candidate_ptx, candidate_source_sha) =
            compile_ptx_only(&candidate, "canonical Prism TF32 NT full-domain candidate")?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixPrismFullDomainCompileV1",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if cfg!(debug_assertions) {
            return Err("canonical Prism TF32 NT full-domain timing requires --release".into());
        }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixPrismFullDomainIdentityV1",
                ...
                "change":"2d_grid_and_unpredicated_full_copy_for_216_full_ctas_and_first_60_k_stages",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixPrismFullDomainResourceV1",
    ...
    fn graph_identity(graph: &CudaGraph, shape: Shape, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32NtALdmatrixPrismFullDomainGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 1 {
            ...
            if symbol != expected_symbol
                || (params.gridDimX, params.gridDimY, params.gridDimZ) != shape.grid_for(arm)?
    ...
    fn check_exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixPrismFullDomainBitsV1",
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixPrismFullDomainScreenV1",
                "cell":TARGET.label,"shape":[TARGET.m,TARGET.k_out,TARGET.reduction],
                "grid":TARGET.grid_for(Arm::Candidate)?,"candidate":Arm::Candidate.name(),
    ...
    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; canonical Prism TF32 NT full-domain once3/once7"]
    fn ada_tf32_nt_a_ldmatrix_prism_full_domain_scout3_then_once7() -> Result<(), String> {
        ...
        quiet.require_pre_context("tf32-nt-a-ldmatrix-prism-full-domain/pre")?;
        let runtime = new_runtime()?;
        assert_eq!(
            TARGET.grid_for(Arm::Candidate)?,
            candidate_source::TARGET_GRID
        );
        assert_eq!(TARGET.grid_for(Arm::Retained)?, (222, 1, 1));
        check_exact_case(&runtime, TARGET, false)?;
        check_exact_case(&runtime, TARGET, true)?;
        check_exact_case(&runtime, ALIGNED_TAIL, false)?;
        check_exact_case(&runtime, MISALIGNED_TAIL, false)?;
        check_exact_case(&runtime, EXCEPTION, true)?;
        check_exact_case(&runtime, K0, false)?;
        let mut prepared = prepare_target(&runtime)?;
        quiet.require_cohort("tf32-nt-a-ldmatrix-prism-full-domain/scout")?;
        ...
        if !scout_pass {
            println!(
                "{}",
                json!({
                    "schema":"MambaBiTriadTf32NtALdmatrixPrismFullDomainDecisionV1",
                    "cell":TARGET.label,"retained_scout":scout,
                    "decision":"stop_scout_retained_loss","promotion":false,
                })
            );
            quiet.verify_post_cohort("tf32-nt-a-ldmatrix-prism-full-domain/scout-post")?;
            return Ok(());
        }
        quiet.require_cohort("tf32-nt-a-ldmatrix-prism-full-domain/qualification")?;
        ...
        if !may_run_fast(&retained) {
            ...
            quiet.verify_post_cohort("tf32-nt-a-ldmatrix-prism-full-domain/retained-post")?;
            return Ok(());
        }
        ...
        quiet.verify_post_cohort("tf32-nt-a-ldmatrix-prism-full-domain/post")?;
        Ok(())
    }
}
