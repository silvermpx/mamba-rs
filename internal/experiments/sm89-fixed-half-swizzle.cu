// OWN ignored Ada experiment: packed/XOR staging twin of measured pipe_vec.
...
// No production dispatch or frozen source composition includes this file.
...
void gemm_bi_nn_tc128_sm89_exp_pipe_vec_swizzle_##SUFFIX(                   \
    TYPE* __restrict__ C, const TYPE* __restrict__ A,                       \
    const TYPE* __restrict__ B, const float* __restrict__ bias,             \
    float alpha, float beta, int M, int N, int K, int lda, int ldb, int ldc) { \
    sm89_fixed_half_swizzle::kernel<TYPE>(C, A, B, bias, alpha, beta,        \
        M, N, K, lda, ldb, ldc);                                          \
}
