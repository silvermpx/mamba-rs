#!/usr/bin/env bash
# Set B alone: inference step on the inference GEMM family (old `Fixed`,
# release `Inference`), f32 and bf16, as two mirrored blocks. Rebuilds the two
# example binaries first because the adapter changed after the full run.
set -uo pipefail
source /root/triad-env.sh
OLD=/root/mamba-rs-main-d8f2efbe
NEW=/root/mamba-rs-new-c65c1c85
out=/root/monolith-baseline-setb-$(date -u +%Y%m%dT%H%M%SZ); mkdir -p "$out"
...
log "build old"
(cd $OLD && CARGO_TARGET_DIR=$OLD/target cargo build --release --features cuda --example m1_inference_family_baseline > "$out/build-old.log" 2>&1) || { log "build old FAILED"; exit 2; }
log "build new"
(cd $NEW && CARGO_TARGET_DIR=$NEW/target cargo build --release --features cuda --example m1_inference_family_baseline > "$out/build-new.log" 2>&1) || { log "build new FAILED"; exit 2; }
{
  echo "date $(date -u +%FT%TZ)"; nvidia-smi --query-gpu=name,driver_version,uuid --format=csv,noheader; nvcc --version | tail -1
  echo "old m1_inference_family_baseline $(sha256sum $OLD/examples/m1_inference_family_baseline.rs | cut -c1-64)"
  echo "new m1_inference_family_baseline $(sha256sum $NEW/examples/m1_inference_family_baseline.rs | cut -c1-64)"
  sha256sum $OLD/target/release/examples/m1_inference_family_baseline $NEW/target/release/examples/m1_inference_family_baseline
} > "$out/identity.txt"
quiet() {
...
# The release bench with its two opt-in arms (deterministic TF32 for f32,
# stream-K for the half weight gradient); new tree only, twice, after set B.
log "build new bench"
(cd $NEW && CARGO_TARGET_DIR=$NEW/target cargo bench --features cuda --bench gemm_bi_trainer_step_bench --no-run >> "$out/build-new.log" 2>&1) || { log "build new bench FAILED"; exit 2; }
newbench="$(ls -t $NEW/target/release/deps/gemm_bi_trainer_step_bench-* | grep -v '\.d$' | head -1)"
echo "new bench $(sha256sum $NEW/benches/gemm_bi_trainer_step_bench.rs | cut -c1-64)" >> "$out/identity.txt"
sha256sum "$newbench" >> "$out/identity.txt"
arm C2 1 1 new "$newbench" bench_gemm_bi_vs_tf32
arm C2 1 2 new "$newbench" bench_gemm_bi_vs_tf32
log "done C2"
log "done"
echo "$out"
