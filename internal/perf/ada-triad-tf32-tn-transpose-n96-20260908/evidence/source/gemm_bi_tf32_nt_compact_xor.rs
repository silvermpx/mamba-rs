...
impl CandidateVariant {
    ...
    const fn compares_fast_tn(self) -> bool {
        self.has_a_transform() || matches!(self, Self::TnSingleBarrierPrism)
    }
    ...
    const fn grid_dim(self) -> (u32, u32, u32) {
        ...
        let (rows, columns, tile_m, tile_n) = if self.is_transpose_n96() {
            (dims.1, dims.2, 128, 96)
        } else if self.is_tn() {
            (dims.1, dims.2, BM, BN)
        } else {
            (dims.0, dims.1, BM, BN)
...
mod cuda_suite {
    ...
    impl Candidate {
        ...
        fn new_reusing(
            ...
            let (rows, columns, tile_m, tile_n) = if variant.is_transpose_n96() {
                (dims.1, dims.2, 128, 96)
