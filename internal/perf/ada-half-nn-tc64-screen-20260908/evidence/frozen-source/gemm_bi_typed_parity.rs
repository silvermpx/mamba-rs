...
use triad_half_tile_screen::{
    BracketOrder, candidate_over_reference, percentile as ada_percentile, retain_decision,
};
...
fn ada_half_symbol(schedule: NnSchedule, dtype: WeightDtype) -> &'static str {
    match (schedule, dtype) {
        (NnSchedule::Tile64, WeightDtype::Bf16) => "gemm_bi_nn_tc64_bf16",
        (NnSchedule::Tile64, WeightDtype::F16) => "gemm_bi_nn_tc64_f16",
        (NnSchedule::Tile128, WeightDtype::Bf16) => "gemm_bi_nn_tc_bf16",
        (NnSchedule::Tile128, WeightDtype::F16) => "gemm_bi_nn_tc_f16",
        _ => panic!("unsupported Ada half discovery arm {schedule:?}/{dtype:?}"),
    }
}

fn ada_half_function(t: &Ctx, schedule: NnSchedule, dtype: WeightDtype) -> &CudaFunction {
    match schedule {
        NnSchedule::Tile64 => t.ctx.kernels.gemm_bi_nn_tc64_typed.get(dtype),
        NnSchedule::Tile128 => t.ctx.kernels.gemm_bi_nn_tc_typed.get(dtype),
        NnSchedule::Thin16 => panic!("Thin16 is outside the Ada half tile screen"),
    }
}
...
impl AdaHalfFixture {
    ...
    fn output(&self, schedule: NnSchedule) -> &TypedSubview {
        match schedule {
            NnSchedule::Tile64 => &self.candidate,
            NnSchedule::Tile128 => &self.reference,
            NnSchedule::Thin16 => unreachable!(),
        }
    }

    fn operands(&self, schedule: NnSchedule) -> gemm_bi_triad::TcFwdOperands {
        tc_nn_operands(
            self.dtype,
            self.output(schedule).ptr(),
            self.a.ptr(),
            self.b.ptr(),
        )
    }

    fn reset_and_validate_inputs(&self, t: &Ctx, schedule: NnSchedule) -> Result<(), String> {
        self.a.upload_logical(t, &self.a_values);
        self.b.upload_logical(t, &self.b_values);
        self.output(schedule).upload_logical(t, &self.output_seed);
        t.ctx
            .stream
            .synchronize()
...
fn ada_half_resource_gate(t: &Ctx, schedule: NnSchedule, dtype: WeightDtype) -> Result<(), String> {
    let function = ada_half_function(t, schedule, dtype);
    let (threads, expected_static, dynamic_shared) = match schedule {
        NnSchedule::Tile64 => (128_u32, 36_864_i32, 0_usize),
        NnSchedule::Tile128 => (256, 0, 71_680),
        NnSchedule::Thin16 => unreachable!(),
    };
    ...
    let symbol = ada_half_symbol(schedule, dtype);
    println!(
        "{{\"schema\":\"MambaBiHalfNnTileAdaDiscoveryResourceV1\",\"dtype\":\"{dtype:?}\",\"symbol\":\"{symbol}\",\"threads\":{threads},\"registers\":{registers},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":{dynamic_shared},\"max_threads\":{max_threads},\"occupancy\":{occupancy},\"required_occupancy\":1}}"
    );
    if registers <= 0
        || local != 0
    ...
    {
        return Err(format!(
            "Ada half {symbol} resource floor failed: regs={registers} local={local} static={static_shared}/{expected_static} max_threads={max_threads}/{threads} occupancy={occupancy}/1"
        ));
    }
    Ok(())
}
...
fn capture_ada_half(
    ...
    let operands = fixture.operands(schedule);
    unsafe {
...
fn launch_ada_half_path(
    ...
    match path {
        AdaHalfPath::Eager => enqueue_tc_nn(
            t,
            schedule,
            &fixture.operands(schedule),
            ADA_HALF_D768_IN,
            (768, 3_072, 3_072),
            0.0,
        ),
...
fn observe_ada_half(
    ...
    fixture.reset_and_validate_inputs(t, schedule)?;
    let start = t
        .ctx
        .stream
        .record_event(Some(sys::CUevent_flags::CU_EVENT_DEFAULT))
        .map_err(|error| format!("Ada half timing start: {error:?}"))?;
    launch_ada_half_path(t, fixture, schedule, graph, path)?;
    let end = t
        .ctx
        .stream
        .record_event(Some(sys::CUevent_flags::CU_EVENT_DEFAULT))
    ...
    let bits = fixture.output(schedule).logical_bits(t);
...
fn ada_half_screen_stratum(
    ...
    let arms = match order {
        BracketOrder::Abba => [
            (NnSchedule::Tile64, candidate_graph),
            (NnSchedule::Tile128, reference_graph),
            (NnSchedule::Tile128, reference_graph),
            (NnSchedule::Tile64, candidate_graph),
        ],
        BracketOrder::Baab => [
    ...
    for _ in 0..ADA_HALF_WINDOWS {
        let mut raw = [0.0; 4];
        for (index, (schedule, graph)) in arms.into_iter().enumerate() {
            raw[index] = observe_ada_half(t, fixture, schedule, graph, path, Some(expected))?.0;
        }
    ...
    println!(
        "{{\"schema\":\"MambaBiHalfNnTileAdaDiscoveryScreenV1\",\"dtype\":\"{:?}\",\"cell\":\"d768_in_proj\",\"shape\":[2048,768,3072],\"candidate\":\"forced_tc64\",\"candidate_symbol\":\"{}\",\"comparator\":\"forced_tc128\",\"comparator_symbol\":\"{}\",\"path\":\"{}\",\"order\":\"{order_name}\",\"windows\":{ADA_HALF_WINDOWS},\"warmups_per_arm\":{ADA_HALF_WARMUPS},\"logical_gemms_per_observation\":1,\"raw_observations_us\":{observations_json},\"ratio_direction\":\"candidate_over_forced_tc128\",\"ratio_p50\":{p50:.9},\"ratio_p95\":{p95:.9}}}",
        fixture.dtype,
        ada_half_symbol(NnSchedule::Tile64, fixture.dtype),
        ada_half_symbol(NnSchedule::Tile128, fixture.dtype),
        path.name(),
    );
    Ok([p50, p95])
}
...
fn ada_half_nn_d768_in_tc64_vs_tc128_discovery_once7() -> Result<(), String> {
    ...
    let _pre = quiet.require_pre_context("typed-nn-d768-in-ada/pre-context")?;
    let t = Ctx::new_ada()?;
    let _cohort = quiet.require_cohort("typed-nn-d768-in-ada/cohort")?;
    let mut decisions = Vec::new();
    for dtype in [WeightDtype::F16, WeightDtype::Bf16] {
        ada_half_resource_gate(&t, NnSchedule::Tile64, dtype)?;
        ada_half_resource_gate(&t, NnSchedule::Tile128, dtype)?;
        let fixture = AdaHalfFixture::new(&t, dtype);
        let candidate_graph = capture_ada_half(&t, &fixture, NnSchedule::Tile64)?;
        let reference_graph = capture_ada_half(&t, &fixture, NnSchedule::Tile128)?;
        validate_single_node_graph(&candidate_graph, "TC64")?;
        ...
        for path in [AdaHalfPath::Eager, AdaHalfPath::Graph] {
            for repeat in 0..2 {
                for (schedule, graph) in [
                    (NnSchedule::Tile128, &reference_graph),
                    (NnSchedule::Tile64, &candidate_graph),
                ] {
                    let bits =
                        observe_ada_half(&t, &fixture, schedule, graph, path, Some(&reference))?.1;
                    println!(
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNnTileAdaDiscoveryDecisionV1\",\"dtype\":\"{dtype:?}\",\"cell\":\"d768_in_proj\",\"shape\":[2048,768,3072],\"candidate\":\"forced_tc64\",\"comparator\":\"forced_tc128\",\"strata_order\":[\"eager/ABBA\",\"eager/BAAB\",\"graph/ABBA\",\"graph/BAAB\"],\"strata\":{:?},\"ratio_direction\":\"candidate_over_forced_tc128\",\"retain\":{retain},\"decision\":\"{}\",\"promotion\":false}}",
            strata,
            if retain {
    ...
    drop(t);
    quiet.verify_post_cohort("typed-nn-d768-in-ada/post")?;
