// clang++ -std=c++17 -O2 internal/experiments/sm89-fixed-half-swizzle-layout-test.cpp -o /tmp/sm89-fixed-half-swizzle-layout-test
// Pure host: exercises the exact constexpr mapping helpers used by the twin.
// Catches missing/wrong XOR, add-instead-of-XOR K issue, incorrect B stage
// advance, stage overlap, chunk fragmentation, and ldmatrix lane permutation.
...
int main() try {
    ...
    for (int stage = 0; stage < 3; ++stage) for (int warp = 0; warp < 8; ++warp)
    for (int atom = 0; atom < 4; ++atom) for (int issue = 0; issue < 4; ++issue) {
        ...
        // PTX .x4 suppliers: rows of matrix j are lanes 8*j..8*j+7.
        // The register in receiver lane t contains columns 2*(t%4)+{0,1}
        // of row t/4. Expected coordinates are independent logical A values.
        ...
        // PTX .x2.trans transposes each 8x8 matrix after using its row
        // suppliers; the original row now comes from receiver column.
        ...
        for (int e = 0; e < 2; ++e) {
            ...
            require(shared[(3 + stage) * l::kStageElements + index] ==
                stage * 100000 + 20000 + k * 128 + column,
                "ldmatrix.x2.trans B fragment changed logical value or lane");
            ++fragment_halfs;
        }
        // Each 8-row/128B ldmatrix matrix needs all 32 distinct banks.
        // This fails for plain packed rows, even though those are bijective.
    ...
    std::printf("{\"schema\":\"Sm89HalfSwizzleS3HostV1\",\"all_passed\":true,\"copied_chunks\":%zu,\"checked_fragment_halfs\":%zu,\"conflict_free_ldmatrix_groups\":%zu,\"staging_bytes\":98304,\"dynamic_shared_bytes\":%d}\n",
        chunks, fragment_halfs, bank_groups, 98304);
    return 0;
