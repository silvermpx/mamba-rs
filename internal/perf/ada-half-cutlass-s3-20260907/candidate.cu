...
#pragma unroll
            for (int issue = 0; issue < 4; ++issue) {
                if (issue < 3)
                    load_fragments(a_read, b_read, issue + 1, offsets, fragments[(issue + 1) & 1]);
