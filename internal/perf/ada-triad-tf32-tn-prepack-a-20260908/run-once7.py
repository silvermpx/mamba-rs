#!/usr/bin/env python3
"""Run the reviewed CUDA 13.2 TF32 TN A-prepack two-cell once7 screen."""
...
EVIDENCE = Path("/root/evidence-ada-triad-tf32-tn-prepack-a-20260908")
RUN = EVIDENCE / "once7-cuda132-prism-only"
...
BINARY = Path(
    ...
    "release/deps/gemm_bi_tf32_nt_compact_xor-fa0042152294a197"
)
BINARY_SHA = "4d1e2460c0a3fd5405687b500455a62fb9e3fc3bb57eb20b568bb5142af0891f"
TEST = "cuda_suite::ada_tf32_tn_prepack_a_prism_discovery_once7"
...
def main() -> int:
    ...
    env_runner = load("tf32_tn_prepack_env", ENV_RUNNER)
    telemetry_runner = load("tf32_tn_prepack_telemetry", TELEMETRY_RUNNER)
    ...
    listed = {
        ...
        for line in (EVIDENCE / "test-list-prism-only.log").read_text().splitlines()
    ...
    counts = {
        "pack_resource": text.count('"schema":"MambaBiTf32TnPrepackAResourceV1"'),
        "candidate_resource": text.count('"schema":"MambaBiTf32TnDiscoveryResourceV1"'),
        "auto_screen": text.count('"schema":"MambaBiTf32TnPrepackADiscoveryScreenV1"'),
        "fast_screen": text.count('"schema":"MambaBiTf32TnPrepackAFastScreenV1"'),
        "decision": text.count('"schema":"MambaBiTf32TnPrepackADiscoveryDecisionV1"'),
        "k0": text.count("TN_K0_DIRECT_ORACLE_PASS op=TN"),
    }
    expected = {
        "pack_resource": 1,
    ...
    receipt = {
        "schema": "MambaBiTf32TnPrepackATwoCellRunReceiptV1",
        "args": args,
        "started_utc": started,
