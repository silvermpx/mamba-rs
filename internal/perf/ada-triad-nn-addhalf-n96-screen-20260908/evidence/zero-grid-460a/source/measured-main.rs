...
impl GuardedF32 {
    fn new(stream: &Arc<CudaStream>, active: Vec<f32>) -> Result<Self, String> {
        Self::new_with_leading_guard(stream, active, GUARD)
    }

    fn new_with_leading_guard(
        stream: &Arc<CudaStream>,
        active: Vec<f32>,
        leading_guard: usize,
    ) -> Result<Self, String> {
        let len = active.len();
        let mut baseline = vec![f32::from_bits(GUARD_BITS); leading_guard + len + GUARD];
        baseline[leading_guard..leading_guard + len].copy_from_slice(&active);
        Ok(Self {
            buffer: GpuBuffer::from_cpu(stream, &baseline)?,
            baseline,
            offset: leading_guard,
            len,
        })
    }
...
mod triad_nn_add_half_screen {
    ...
    const ALIGNED_LOGICAL_BYTES: usize = 256;
    const ALIGNED_LOGICAL_GUARD: usize = 64;
    const NEW_CELL_WARMUPS: usize = 8;
    const NEW_CELL_GEMMS_PER_OBSERVATION: usize = 20;
    ...
    struct FastGapCell {
        ...
        leading_guard: usize,
        warmups: usize,
        gemms_per_observation: usize,
    }

    const D768_OUT_FAST_GAP: FastGapCell = FastGapCell {
        cell: "d768_out_proj",
        case: TARGET,
        leading_guard: GUARD,
        warmups: WARMUPS,
        gemms_per_observation: 1,
    };

    const D768_IN_FAST_GAP: FastGapCell = FastGapCell {
        ...
        leading_guard: ALIGNED_LOGICAL_GUARD,
        warmups: NEW_CELL_WARMUPS,
        gemms_per_observation: NEW_CELL_GEMMS_PER_OBSERVATION,
    };

    const PRISM_FAST_GAP: FastGapCell = FastGapCell {
    ...
    fn validate_logical_alignment(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        for (label, buffer) in [
            ("A", &fixture.a),
            ("B", &fixture.b),
            ("bias", &fixture.bias),
            ("candidate C", &fixture.candidate),
            ("current C", &fixture.production),
            ("Fast C", &fixture.fast),
            (
                "AUTO A",
                fixture.auto_a.as_ref().expect("Triad NN AUTO A fixture"),
            ),
            (
                "AUTO B",
                fixture.auto_b.as_ref().expect("Triad NN AUTO B fixture"),
            ),
            (
                "AUTO C",
                fixture
            ...
            let ptr = buffer.ptr(&runtime.ctx.stream);
            if ptr % ALIGNED_LOGICAL_BYTES as u64 != 0 {
                return Err(format!(
                    "Triad NN N96 {label} logical pointer 0x{ptr:x} is not {ALIGNED_LOGICAL_BYTES}-byte aligned"
                ));
            }
        }
        Ok(())
    ...
    fn fixture_with_values(
        ...
        fixture_with_values_and_guard(runtime, case, a_values, b_values, GUARD)
    }

    fn fixture_with_values_and_guard(
        runtime: &Runtime,
        case: Case,
        a_values: Vec<f32>,
        b_values: Vec<f32>,
        ...
        Ok(Fixture {
            a: GuardedF32::new_with_leading_guard(&runtime.ctx.stream, a_values, leading_guard)?,
            b: GuardedF32::new_with_leading_guard(&runtime.ctx.stream, b_values, leading_guard)?,
            bias: GuardedF32::new_with_leading_guard(
                &runtime.ctx.stream,
                vec![0.0; case.shape.n],
                leading_guard,
            )?,
            candidate: GuardedF32::new_with_leading_guard(
                &runtime.ctx.stream,
                vec![f32::from_bits(POISON_BITS); output_len],
                leading_guard,
            )?,
            production: GuardedF32::new_with_leading_guard(
                &runtime.ctx.stream,
                vec![f32::from_bits(POISON_BITS); output_len],
                leading_guard,
            )?,
            fast: GuardedF32::new_with_leading_guard(
                &runtime.ctx.stream,
                vec![f32::from_bits(POISON_BITS); output_len],
                leading_guard,
            )?,
    ...
    fn ordinary_fixture(runtime: &Runtime, case: Case) -> Result<Fixture, String> {
        ordinary_fixture_with_guard(runtime, case, GUARD)
    }

    fn ordinary_fixture_with_guard(
        runtime: &Runtime,
        case: Case,
        leading_guard: usize,
    ) -> Result<Fixture, String> {
        fixture_with_values_and_guard(
            runtime,
            case,
    ...
    fn exceptional_fixture(runtime: &Runtime) -> Result<Fixture, String> {
        exceptional_fixture_with_guard(runtime, GUARD)
    }

    fn exceptional_fixture_with_guard(
        runtime: &Runtime,
        leading_guard: usize,
    ) -> Result<Fixture, String> {
        ...
        fixture_with_values_and_guard(runtime, TAIL, a, b, leading_guard)
    }
    ...
    fn check_pair(
        ...
        let actual_auto = run_eager(runtime, &mut fixture, case, TriadArm::ActualAuto)?;
        ...
        let auto_graph = capture_arm(runtime, &mut fixture, case, TriadArm::ActualAuto)?;
    ...
    fn report_actual_auto_identity(runtime: &Runtime, case: Case) -> Result<AutoIdentity, String> {
        ...
        let node = evidence
            .nodes()
            .first()
            .ok_or_else(|| "Triad NN AUTO has no physical node".to_owned())?;
        ...
        Ok(AutoIdentity {
            symbol,
            tile: (tile_m as usize, tile_n as usize),
            grid: node.launch.grid_dim,
            block: node.launch.block_dim,
            shared_bytes: node.launch.shared_mem_bytes,
        })
    }

    fn assert_auto_graph(graph: &CudaGraph, expected: AutoIdentity) -> Result<(), String> {
        let mut count = 0usize;
        if unsafe { sys::cuGraphGetNodes(graph.cu_graph(), std::ptr::null_mut(), &mut count) }
            != sys::CUresult::CUDA_SUCCESS
            || count != 1
        {
            return Err(format!(
                "Triad NN AUTO graph node count {count}, expected one"
            ));
        }
        let mut nodes = vec![std::ptr::null_mut(); count];
        if unsafe { sys::cuGraphGetNodes(graph.cu_graph(), nodes.as_mut_ptr(), &mut count) }
            != sys::CUresult::CUDA_SUCCESS
        {
            return Err("query Triad NN AUTO graph nodes failed".into());
        }
        let mut params: sys::CUDA_KERNEL_NODE_PARAMS = unsafe { std::mem::zeroed() };
        if unsafe { sys::cuGraphKernelNodeGetParams_v2(nodes[0], &mut params) }
            != sys::CUresult::CUDA_SUCCESS
        {
            return Err("query Triad NN AUTO graph parameters failed".into());
        }
        let mut name = std::ptr::null();
        if unsafe { sys::cuFuncGetName(&mut name, params.func) } != sys::CUresult::CUDA_SUCCESS
            || name.is_null()
        {
            return Err("query Triad NN AUTO graph symbol failed".into());
        }
        let symbol = unsafe { CStr::from_ptr(name) }
            .to_str()
            .map_err(|error| format!("Triad NN AUTO symbol UTF-8: {error}"))?;
        ...
        if symbol != expected.symbol
            || actual_grid != expected.grid
            || actual_block != expected.block
            || params.sharedMemBytes != expected.shared_bytes
        {
            return Err(format!(
                "Triad NN AUTO graph changed: symbol={symbol} grid={actual_grid:?} block={actual_block:?} shared={} expected_symbol={} expected_tile={:?} expected_grid={:?} expected_block={:?} expected_shared={}",
                params.sharedMemBytes,
                expected.symbol,
                expected.tile,
                expected.grid,
                expected.block,
                expected.shared_bytes,
            ));
        }
        Ok(())
    }
    ...
    fn assert_fast_graph(graph: &CudaGraph) -> Result<String, String> {
        ...
        if unsafe { sys::cuGraphGetNodes(graph.cu_graph(), std::ptr::null_mut(), &mut node_count) }
            != sys::CUresult::CUDA_SUCCESS
        {
            return Err("query cuBLAS Fast TF32 graph nodes failed".into());
        }
        if node_count == 0 {
            return Err("cuBLAS Fast TF32 graph is empty".into());
        }
        let graph_label = "cublas_fast_tf32_whole_graph";
        println!(
            "{{\"schema\":\"MambaBiTriadNnN96FastGapBindingV1\",\"graph_label\":\"{graph_label}\",\"graph_node_count\":{node_count},\"compute\":\"CUBLAS_COMPUTE_32F_FAST_TF32\",\"algorithm\":\"CUBLAS_GEMM_DEFAULT\",\"alpha\":1.0,\"beta\":0.0,\"bias\":false}}"
        );
        Ok(graph_label.to_owned())
    }
    ...
    fn prepare_fast_gap_target(
        runtime: &Runtime,
        target: FastGapCell,
    ) -> Result<FastGapTarget, String> {
        let mut fixture = ordinary_fixture_with_guard(runtime, target.case, target.leading_guard)?;
        if target.leading_guard == ALIGNED_LOGICAL_GUARD {
            validate_logical_alignment(runtime, &fixture)?;
        }
        let current = run_eager(runtime, &mut fixture, target.case, TriadArm::CurrentWide)?;
        let candidate = run_eager(runtime, &mut fixture, target.case, TriadArm::Candidate)?;
        let actual_auto = run_eager(runtime, &mut fixture, target.case, TriadArm::ActualAuto)?;
        ...
        let auto_identity = report_actual_auto_identity(runtime, target.case)?;
        let fast_bits = run_eager(runtime, &mut fixture, target.case, TriadArm::FastTf32)?;
        ...
        let candidate_graph = capture_arm(runtime, &mut fixture, target.case, TriadArm::Candidate)?;
        let auto_graph = capture_arm(runtime, &mut fixture, target.case, TriadArm::ActualAuto)?;
        let fast_graph = capture_arm(runtime, &mut fixture, target.case, TriadArm::FastTf32)?;
        assert_n96_graph(
            &candidate_graph,
            target.case.shape,
            triad_nn_n96_source::TRIAD_NN_N96_SYMBOL,
        ...
        for repeat in 0..2 {
            for (arm, graph) in [(TriadArm::Candidate, &candidate_graph)] {
                if run_eager(runtime, &mut fixture, target.case, arm)? != current
            ...
            if run_eager(runtime, &mut fixture, target.case, TriadArm::ActualAuto)? != actual_auto
                || run_graph(runtime, &mut fixture, TriadArm::ActualAuto, &auto_graph)?
                    != actual_auto
            {
                return Err(format!(
            ...
            if run_eager(runtime, &mut fixture, target.case, TriadArm::FastTf32)? != fast_bits
        ...
        println!(
            "{{\"schema\":\"MambaBiTriadNnN96FastGapBitsV1\",\"cell\":\"{}\",\"case\":\"ordinary_target\",\"shape\":[{},{},{}],\"logical_alignment_bytes\":{},\"exact_arms\":[\"candidate\",\"current_wide\",\"actual_auto\"],\"actual_auto_symbol\":\"{}\",\"fast_contract\":\"finite_nonzero_self_consistency_only\",\"eager_repeats\":2,\"graph_repeats\":2,\"passed\":true}}",
            target.cell,
            target.case.shape.m,
            target.case.shape.k,
            target.case.shape.n,
            if target.leading_guard == ALIGNED_LOGICAL_GUARD { ALIGNED_LOGICAL_BYTES } else { 128 },
            auto_identity.symbol,
        );
        Ok(FastGapTarget {
            fixture,
            ...
            fast_bits,
            auto_symbol: auto_identity.symbol.to_owned(),
            fast_symbol,
        })
    }
    ...
    fn measure_one(
        ...
        let start = runtime
            ...
            .record_event(Some(sys::CUevent_flags::CU_EVENT_DEFAULT))
            .map_err(|error| format!("record {} start: {error:?}", arm.name()))?;
        for _ in 0..target.gemms_per_observation {
            match path {
                Path::Eager => launch(runtime, fixture, target.case, arm)?,
                Path::Graph => graph
                    .launch()
                    .map_err(|error| format!("launch {} graph: {error:?}", arm.name()))?,
            }
        ...
        let elapsed_us = f64::from(
            ...
            / target.gemms_per_observation as f64;
    ...
    fn fast_gap_stratum(
        ...
        for _ in 0..WINDOWS {
            ...
            for (index, (arm, graph, expected)) in arms.into_iter().enumerate() {
                observation[index] =
                    measure_one(runtime, fixture, target, arm, graph, path, expected)?;
            }
            ratios.push(triad_nn_n96_source::candidate_over_auto_ratio(
                order,
                observation,
        ...
        println!(
            "{{\"schema\":\"MambaBiTriadNnN96FastGapTimingV1\",\"cell\":\"{}\",\"shape\":[{},{},{}],\"candidate\":\"add_half_n96\",\"comparator\":\"{}\",\"candidate_symbol\":\"{}\",\"comparator_symbol\":\"{comparator_symbol}\",\"path\":\"{}\",\"order\":\"{}\",\"windows\":{WINDOWS},\"warmups_per_arm\":{},\"logical_gemms_per_observation\":{},\"raw_observations_us\":{},\"ratio_direction\":\"candidate_over_comparator\",\"ratio_p50\":{p50:.9},\"ratio_p95\":{p95:.9}}}",
            target.cell,
            target.case.shape.m,
            target.case.shape.k,
            target.case.shape.n,
    ...
    fn run_fast_gap_cell(target: FastGapCell, check_focused_cases: bool) -> Result<(), String> {
        assert!(
            !cfg!(debug_assertions),
            "Triad NN N96 Fast-gap screen requires --release"
        );
        let runtime = new_runtime()?;
        if check_focused_cases {
            check_pair(
                &runtime,
                TAIL,
                ordinary_fixture_with_guard(&runtime, TAIL, target.leading_guard)?,
                "tail",
            )?;
            check_pair(
                &runtime,
                TAIL,
                exceptional_fixture_with_guard(&runtime, target.leading_guard)?,
                "exceptional",
            )?;
            check_pair(
                &runtime,
                K0,
                ordinary_fixture_with_guard(&runtime, K0, target.leading_guard)?,
                "k0_null_ab",
            )?;
        }
        let FastGapTarget {
            ...
            auto_symbol,
            fast_symbol,
        } = prepare_fast_gap_target(&runtime, target)?;
        ...
        println!(
            "{{\"schema\":\"MambaBiTriadNnN96FastGapDecisionV1\",\"cell\":\"{}\",\"shape\":[{},{},{}],\"candidate\":\"add_half_n96\",\"strata_order\":[\"eager/ABBA\",\"eager/BAAB\",\"graph/ABBA\",\"graph/BAAB\"],\"strata_fields\":[\"ratio_p50\",\"ratio_p95\"],\"actual_auto_strata\":{},\"fast_tf32_strata\":{},\"ratio_direction\":\"candidate_over_comparator\",\"logical_gemms_per_observation\":{},\"retain_against_actual_auto\":{retain},\"fast_comparison_valid\":{fast_valid},\"fast_win\":{fast_win},\"fast_is_exact_golden\":false,\"promotion\":false}}",
            target.cell,
            target.case.shape.m,
            target.case.shape.k,
            target.case.shape.n,
    ...
    fn triad_nn_d768_out_add_half_n96_fast_gap_once7() -> Result<(), String> {
        run_fast_gap_cell(D768_OUT_FAST_GAP, false)
    }

    #[test]
    #[ignore = "requires an exclusive quiet CC8.9/142-SM CUDA13.2 GPU; set MAMBA_TRIAD_NN_N96_DISCOVERY=1"]
    fn triad_nn_d768_in_add_half_n96_fast_gap_once7() -> Result<(), String> {
        run_fast_gap_cell(D768_IN_FAST_GAP, true)
    }

    #[test]
    #[ignore = "requires an exclusive quiet CC8.9/142-SM CUDA13.2 GPU; set MAMBA_TRIAD_NN_N96_DISCOVERY=1"]
    fn triad_nn_prism_add_half_n96_fast_gap_once7() -> Result<(), String> {
        run_fast_gap_cell(PRISM_FAST_GAP, true)
    }
}
