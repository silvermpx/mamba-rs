...
evidence=/root/evidence-ada-half-s3-force-20260907/cuda${tag}-continuation1
mkdir -p /root/evidence-ada-half-s3-force-20260907 || exit $?
mkdir "$evidence" || exit $?
test -d "$MAMBA_RS_KERNEL_CACHE" || exit 98
run() {
    local label=$1
    shift
    date -u +%FT%TZ
...
if [ "$tag" = 132 ]; then
    ...
    run retained-exact "${testcmd[@]}" --test gemm_bi_fixed_sm89_exact_n64 -- --include-ignored --skip fixed_sm89_exact_n64_auto_prefix_view_graph_bits --test-threads=1 --nocapture || exit $?
fi
...
run artifact-proof python3 analyze-artifact.py "$fixed_cache" "$evidence/fixed.ptx" "$evidence/sass.log" "$evidence/resources.log" "$evidence/ptxas.log" 188 || exit $?
run identities-final bash -euo pipefail -c 'find src kernels tests -type f -print0 | sort -z | xargs -0 sha256sum; echo EXIT_source_hashes=0; sha256sum Cargo.toml Cargo.lock; echo EXIT_build_input_hashes=0; find "$CARGO_TARGET_DIR/release/deps" -maxdepth 1 -type f -executable -print0 | sort -z | xargs -0 sha256sum; echo EXIT_binary_hashes=0; find "$MAMBA_RS_KERNEL_CACHE" -type f -print0 | sort -z | xargs -0 sha256sum; echo EXIT_cache_hashes=0' || exit $?
date -u +%FT%TZ
