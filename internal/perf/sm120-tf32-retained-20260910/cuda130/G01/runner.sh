...
packet=${3:-existing23}
label=${toolkit/./}
case "$packet" in existing23|g10|all24) ;; *) exit 2 ;; esac
evidence_dir=/root/sm120-tf32-retained-cuda${label}-${packet}-20260910
test ! -e "$evidence_dir"
...
binary=$(find "$CARGO_TARGET_DIR/release/deps" -maxdepth 1 -type f -executable -name "$test_target-*" | head -n 1)
test -n "$binary"
