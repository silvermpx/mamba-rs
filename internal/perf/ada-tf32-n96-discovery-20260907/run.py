...
def context_for(operation):
    ...
    if operation in ("timing7", "timing21"):
        context["env"]["MAMBA_FIXED_N96_WINDOWS"] = operation.removeprefix("timing")
...
def wait_for_quiet(directory, context):
    ...
    for _ in range(60):
        ...
        time.sleep(0.5)
    (directory / "drain.json").write_text(json.dumps(records, indent=2) + "\n")
    raise RuntimeError("GPU lane did not drain to quiet/noapps")


def main():
    if (
        len(sys.argv) != 3
        or sys.argv[1] not in ("correctness", "timing7", "timing21")
        or re.fullmatch(r"[a-z0-9-]+", sys.argv[2]) is None
    ):
        raise SystemExit("usage: run.py correctness|timing7|timing21 ATTEMPT")
