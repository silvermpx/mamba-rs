#!/usr/bin/env python3
"""Run one isolated CUDA13.2 W4 arm with strict lane telemetry."""

import hashlib
import importlib.util
import json
from pathlib import Path
import re
import subprocess
import sys
import time


ROOT = Path("/root/mamba-ada-exact-toolkit-auto-20260907")
TASK8 = Path("/root/evidence-ada-exact-toolkit-auto-20260907")
EVIDENCE = Path("/root/evidence-ada-tf32-w4-discovery-20260907")


def sha(path):
    digest = hashlib.sha256()
    with Path(path).open("rb") as source:
        for chunk in iter(lambda: source.read(1024 * 1024), b""):
            digest.update(chunk)
    return digest.hexdigest()


def task8_runner():
    spec = importlib.util.spec_from_file_location("task8_runner", TASK8 / "run.py")
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load accepted Task8 runner")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def context_for(operation):
    task8 = task8_runner()
    context = task8.env_for("13.2", "final2")
    context = {**context, "env": context["env"].copy()}
    context["env"]["MAMBA_FIXED_W4_DISCOVERY"] = "1"
    if operation.startswith("timing21-"):
        context["env"]["MAMBA_FIXED_W4_WINDOWS"] = "21"
    return task8, context


def wait_for_quiet(directory, context):
    records = []
    for _ in range(60):
        gpu = subprocess.run(
            [
                "/usr/bin/nvidia-smi",
                "--query-gpu=utilization.gpu,utilization.memory",
                "--format=csv,noheader",
            ],
            env=context["env"],
            text=True,
            capture_output=True,
        )
        apps = subprocess.run(
            [
                "/usr/bin/nvidia-smi",
                "--query-compute-apps=pid,gpu_uuid,process_name",
                "--format=csv,noheader",
            ],
            env=context["env"],
            text=True,
            capture_output=True,
        )
        record = {
            "gpu": gpu.stdout,
            "gpu_stderr": gpu.stderr,
            "gpu_exit": gpu.returncode,
            "apps": apps.stdout,
            "apps_stderr": apps.stderr,
            "apps_exit": apps.returncode,
        }
        records.append(record)
        if (
            gpu.returncode == 0
            and apps.returncode == 0
            and gpu.stdout.strip() == "0 %, 0 %"
            and not apps.stdout.strip()
        ):
            (directory / "drain.json").write_text(json.dumps(records, indent=2) + "\n")
            return
        time.sleep(0.5)
    (directory / "drain.json").write_text(json.dumps(records, indent=2) + "\n")
    raise RuntimeError("GPU lane did not drain")


def main():
    operations = {
        "correctness-n96": "w4_n96_small_bits_and_resources",
        "correctness-n128": "w4_n128_small_bits_and_resources",
        "timing7-n96": "w4_n96_short_timing",
        "timing7-n128": "w4_n128_short_timing",
        "timing21-n96": "w4_n96_short_timing",
        "timing21-n128": "w4_n128_short_timing",
    }
    if (
        len(sys.argv) != 3
        or sys.argv[1] not in operations
        or re.fullmatch(r"[a-z0-9-]+", sys.argv[2]) is None
    ):
        raise SystemExit("usage: run.py OP ATTEMPT")
    operation, attempt = sys.argv[1:]
    task8, context = context_for(operation)
    directory = EVIDENCE / f"cuda132-{attempt}"
    directory.mkdir(mode=0o700, parents=True)
    binary = task8.find_binary(context, "gemm_bi_fixed_tf32_w4_discovery")
    sources = [
        ROOT / "tests/gemm_bi_fixed_tf32_w4_discovery.rs",
        ROOT / "tests/gemm_bi_fixed_tf32_w4_discovery.cu",
        ROOT / "tests/gemm_bi_fixed_tf32_n96_discovery.cu",
    ]
    binding = {
        "schema": "MambaBiFixedTf32W4DiscoveryBindingV1",
        "operation": operation,
        "toolkit": "13.2",
        "feature": context["feature"],
        "binary": str(binary),
        "binary_sha256": sha(binary),
        "sources": {str(path.relative_to(ROOT)): sha(path) for path in sources},
        "tools": {
            str(Path(__file__)): sha(__file__),
            str(TASK8 / "run.py"): sha(TASK8 / "run.py"),
        },
    }
    (directory / "binding.json").write_text(json.dumps(binding, indent=2) + "\n")
    task8.telemetry("PRE", directory, context)
    code = task8.command(
        [
            str(binary),
            "--ignored",
            "--exact",
            operations[operation],
            "--nocapture",
            "--test-threads=1",
        ],
        directory / "test.log",
        context,
    )
    task8.telemetry("POST", directory, context)
    wait_for_quiet(directory, context)
    task8.telemetry("RELEASE", directory, context)
    (directory / "result.json").write_text(
        json.dumps({"operation": operation, "attempt": attempt, "exit": code}, indent=2)
        + "\n"
    )
    raise SystemExit(code)


if __name__ == "__main__":
    main()
