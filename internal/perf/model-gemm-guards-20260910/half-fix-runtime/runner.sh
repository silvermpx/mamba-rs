...
built_evidence=${2:?original build evidence}
evidence_dir=${3:?new evidence directory}
required_tests=${4:?selected exact tests}
test ! -e "$evidence_dir"
...
contract_binary=$(sed -n 's/^  Executable tests\/gemm_bi_tf32_contract.rs (\(.*\))$/\1/p' "$built_evidence/build.log")
idle_preflight() {
...
while read -r target lane test_name extra; do
  ...
  case "$lane" in gpu) idle_preflight "$test_name" ;; host) ;; *) exit 2 ;; esac
  set +e
...
test "$completed" -eq "$(wc -l < "$required_tests")"
