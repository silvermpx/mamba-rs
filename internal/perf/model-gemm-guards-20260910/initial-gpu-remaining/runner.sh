...
while read -r target lane test_name extra; do
  ...
  case "$test_name" in
    mamba3_siso::gpu::inference::model_gemm_manifest_tests::m3_capture_requires_successful_public_eager_manifest|mamba_ssm::gpu::graph_capture::model_gemm_guard_tests::deterministic_inference_half_work_rejects_empty_graph_plan|mamba_ssm::gpu::blas::vendor_gemm_test::vendor_gemm_counter_observes_both_modes_and_denies_before_ffi|mamba_ssm::gpu::graph_capture::model_gemm_guard_tests::model_graph_guard_zero_vendor_and_poison_controls|mamba_ssm::gpu::inference::model_gemm_manifest_tests::m1_model_manifests_replay_all_paths_without_vendor_gemm) continue ;;
  esac
  test -z "$extra"
  case "$target" in
