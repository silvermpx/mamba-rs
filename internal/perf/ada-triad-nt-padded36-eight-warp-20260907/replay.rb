...
manifest = json(mp)
check(build['exit'] == 0 && build['expected_test_listed'], 'build/list')
check(sha(mp) == build['source_manifest_sha256'], 'manifest binding')
check(manifest['count'] == 372 && manifest['sources'].size == 372, '372 sources')
manifest['sources'].each { |p,h| check(sha(p) == h, "source #{p}") }
dir = File.join(BASE,'once7-eight-warp-cuda132')
...
records.each do |r|
  check(r['variant'] == 'padded_eight_warp' &&
        r['symbol'] == 'gemm_bi_nt_test_padded_eight_warp_sm80_mma_tf32_v1_m128n64_bk32_s3' &&
...
check(!retain && decision['retain'] == false && decision['promotion'] == false &&
      decision['decision'] == 'stop_no_retry', 'STOP no promotion')
...
puts 'PASS:372 source hashes, one exact real test,28 raw brackets,4 paired strata,STOP.'
