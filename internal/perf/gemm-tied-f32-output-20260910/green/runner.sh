...
cargo test --locked --release --features cuda,hf --test gemm_tied_f32_output --no-run > "$evidence_dir/routing-build.log" 2>&1
...
test "$(grep -c ': test$' "$evidence_dir/routing-tests.list")" -ge 2
...
for test_name in "$@"; do
  nvidia-smi --query-gpu=utilization.gpu,memory.free --format=csv,noheader,nounits > "$evidence_dir/$test_name.preflight.csv"
  awk -F, 'NF != 2 || $1+0 > 1 || $2+0 < 2048 {exit 1}' "$evidence_dir/$test_name.preflight.csv"
