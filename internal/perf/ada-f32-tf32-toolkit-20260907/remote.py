#!/usr/bin/env python3
"""Run the remote Task7 wrapper and bind its wrapper/outer-SSH transcript."""
import hashlib
import json
...
REMOTE_EVIDENCE = "/root/evidence-ada-f32-tf32-toolkit-20260907"


def main():
    toolkit, attempt, family, stage, literals, *screen = sys.argv[1:]
    if len(screen) > 1:
        raise ValueError("at most one screen path")
    command = [
        "python3", f"{REMOTE_EVIDENCE}/run.py", "run", toolkit, attempt,
        family, stage, literals, *screen,
    ]
    transcript = LOCAL / f"cuda{toolkit.replace('.', '')}-{attempt}-ssh.log"
    ...
    closure_path = LOCAL / f"cuda{toolkit.replace('.', '')}-{attempt}-outer.json"
    closure_path.write_text(json.dumps(closure, indent=2) + "\n")
    remote = f"ada:{REMOTE_EVIDENCE}/cuda{toolkit.replace('.', '')}-{attempt}/"
