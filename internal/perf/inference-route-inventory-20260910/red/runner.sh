...
cargo test --locked --release --features cuda --test gemm_inference_route_inventory --no-run > "$evidence_dir/build.log" 2>&1
...
for test_name in "$@"; do
  ...
  test "$test_result" = 101
  grep -F 'Inference NN launch must be inventoried' "$evidence_dir/$test_name.log"
  grep -F 'test result: FAILED. 0 passed; 1 failed;' "$evidence_dir/$test_name.log"
done
