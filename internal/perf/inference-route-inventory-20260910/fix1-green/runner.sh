...
if test "$expected_exit" -eq 0; then
  ...
  for companion in inference_arch_rung_decision_preserves_nonempty_cold_guard_and_probe_verdict cold_architecture_probe_is_rejected_before_recording_or_capture; do
    exact_name="mamba_ssm::gpu::gemm_bi_inference::$companion"
    grep -Fx "$exact_name: test" "$evidence_dir/lib-tests.list"
    "$lib_binary" "$exact_name" --exact --nocapture --test-threads=1 > "$evidence_dir/$companion.log" 2>&1
    grep -F 'test result: ok. 1 passed; 0 failed; 0 ignored;' "$evidence_dir/$companion.log"
  done
else
