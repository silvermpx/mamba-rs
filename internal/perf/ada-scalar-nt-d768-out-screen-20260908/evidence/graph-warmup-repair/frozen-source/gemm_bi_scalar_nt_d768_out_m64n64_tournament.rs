...
mod cuda_tournament {
    ...
    use super::{
        AdaBracketOrder, PRODUCTION_TRANSPOSE_SOURCE, TEST_SOURCE, TRANSPOSE_32X16,
        ada_candidate_over_auto, ada_percentile, ada_retain_decision, fixed_full_mantissa,
    };
    ...
    fn validate_candidate_identity_with_transpose16(
        ...
        let (transpose_symbol, transpose_block) = match arm {
            ...
            Arm::Transpose16 => (transpose16_symbol, (32, 16, 1)),
    ...
    fn prepare_ada_pair(
        ...
        check_ada_resources(&runtime.m64, 128, 0, M64_SHARED)?;
        ...
        let candidate_graph = capture_arm(runtime, &mut fixture, Arm::Transpose16)?;
        let eager_identity = eager_candidate_identity(runtime, &fixture, Arm::Transpose16);
        ...
        for path in [AdaPath::Eager, AdaPath::Graph] {
            for repeat in 0..2 {
                for (arm, graph) in [
                    ...
                    (Arm::Transpose16, &candidate_graph),
                ] {
