...
dir = File.join(BASE,'once7-compact-eight-warp-s2-cuda132')
...
records.each do |r|
  check(r['variant'] == 'compact_eight_warp_s2' &&
        r['symbol'] == 'gemm_bi_nt_test_compact_eight_warp_sm80_mma_tf32_v1_m128n64_bk32_s2' &&
        r['dynamic_shared_bytes'] == 49152, 'record identity')
end
...
check(resource.values_at('registers','local_bytes','static_shared_bytes','max_dynamic_shared_bytes','max_threads','occupancy') ==
      [121,0,0,49152,256,2], 'resources')
check(resource['required_occupancy'] == 2, 'required occupancy')
