...
direct_pair_validate_gpu_snapshot() {
    local phase=$1
    local gpu_output=$2
    ...
    if ! printf '%s\n' "$gpu_output" | grep -Eq 'GPU-d1edd7be-e88d-aed6-047d-622163306f0e, NVIDIA RTX 6000 Ada Generation, 8\.9,'; then
        echo "${phase}_GPU_WRONG_IDENTITY"
        return 71
    fi
    case "$phase" in
        PRE)
            if ! printf '%s\n' "$gpu_output" | grep -Eq 'GPU-d1edd7be-e88d-aed6-047d-622163306f0e, NVIDIA RTX 6000 Ada Generation, 8\.9, 0 %, 0 %,'; then
                echo "${phase}_GPU_NOT_QUIET"
                return 71
            fi
            ;;
        POST) ;;
        *)
            echo "unknown telemetry phase $phase" >&2
            return 64
            ;;
    esac
}
...
telemetry() {
    ...
    printf '%s\n' "$gpu_output"
    direct_pair_validate_gpu_snapshot "$phase" "$gpu_output" "$smi_exit" || snapshot_exit=$?
    if [ "$snapshot_exit" -ne 0 ]; then
        smi_exit=$snapshot_exit
    fi
...
echo "RUN_STAGE=direct-pair TOOLKIT=$toolkit WINDOWS=$windows WRAPPER=v2"
