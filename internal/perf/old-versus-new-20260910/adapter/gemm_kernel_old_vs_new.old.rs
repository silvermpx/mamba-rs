...
mod bench {
    use mamba_rs::mamba_ssm::gpu::blas::{
        TypedPtr, gpu_gemm_ex_backward_dx_typed, gpu_gemm_typed_forward_raw,
        gpu_sgemm_backward_dw_grad, gpu_sgemm_backward_dw_grad_typed, gpu_sgemm_backward_dx_raw,
    ...
    fn make_ctx(device: &GpuDevice, arm: Arm, family: BiGemmFamily, tensor_cores: bool) -> GpuCtx {
        ...
        match arm {
            ...
            // f32 runs cuBLAS sgemm under the handle's TF32 math (the old
            // default); half runs GemmEx with COMPUTE_32F.
            Arm::Fast => {
                ctx.set_batch_invariant(false);
                ctx.set_fast_gemm(true);
            }
            // f32 runs sgemm under default math (full f32); half runs GemmEx
            // with COMPUTE_32F_PEDANTIC, the old default.
            Arm::Pedantic => {
                ctx.set_batch_invariant(false);
                ctx.set_fast_gemm(false);
                ctx.disable_tf32();
        ...
        fn launch(&mut self, ctx: &GpuCtx, dtype: WeightDtype, op: Op) {
            ...
            match (op, dtype) {
                (Op::Nn, WeightDtype::F32) => {
                    ...
                    gpu_sgemm_forward_raw(ctx, y, x, self.w.ptr(ctx), None, dims).expect("nn f32");
                }
                (Op::Nn, _) => {
                ...
                (Op::Tn, WeightDtype::F32) => {
                    ...
                    gpu_sgemm_backward_dw_grad(ctx, &dw, dy, x, self.m, self.k, self.n).expect("tn f32");
                }
                (Op::Tn, _) => {
                    let dw = GradSlice::from_raw(self.dw.cached_ptr(), self.k * self.n);
                ...
                (Op::Nt, WeightDtype::F32) => {
                    ...
                    let Operand::F32(dy) = &self.dy else { unreachable!() };
                    gpu_sgemm_backward_dx_raw(ctx, dx, dy, self.w.ptr(ctx), self.m, self.k, self.n)
                        .expect("nt f32");
                }
                (Op::Nt, _) => {
    ...
    pub fn run() {
        ...
        println!("DEVICE ordinal 0 (the old tree exposes no identity accessor)");
        run_family(
            &device,
            &Family {
                label: "triad",
