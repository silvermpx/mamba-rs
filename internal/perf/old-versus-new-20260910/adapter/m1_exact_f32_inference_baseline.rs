//! End-to-end exact-F32 Mamba-1 inference latency on the Triad family, at
...
//!
//! The same file is dropped into `examples/` of both endpoints, the unchanged
//! old main and the reviewed release commit, and built with `--features cuda`.
...
mod baseline {
    ...
    fn cpu_reference_matches(
        ...
        let cpu = MambaBackbone::init(cfg, input_dim, SEED);
    ...
    pub fn run() {
        ...
        for &b in &BATCHES {
            ...
            let mut eager_bb = GpuMambaBackbone::new(0, &weights, cfg, input_dim, b).unwrap();
            ...
            let repeat_digests = fixed_steps(&mut eager_bb, &input, &mut output, "eager repeat");
            assert_eq!(
                eager_digests, repeat_digests,
                "batch {b}: eager bits changed between repeats"
            );
            for (step, digest) in eager_digests.iter().enumerate() {
                println!("DIGEST arm=eager batch={b} step={step} fnv1a={digest:016x}");
            }
            cpu_reference_matches(cfg, input_dim, b, &input, &eager_outputs);

            let mut graph_bb = GpuMambaBackbone::new(0, &weights, cfg, input_dim, b).unwrap();
