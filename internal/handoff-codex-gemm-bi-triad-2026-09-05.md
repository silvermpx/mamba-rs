...
`internal/perf/ada-triad-tf32-nt-a-ldmatrix-sliced-prism-20260909/report.md`.

CORRECTION 2026-09-09, Fixed/inference rather than training Triad: the
test-only F16 NN d768-in M128N96/BK64/S3 eight-warp candidate passes
target/M/N/K-tail/exception/K0 exactness,
...
p95 `1.011727`; do not claim a Fast or Triad win or production promotion. Its
`gemm_bi_nn_fixed_sm89_*` symbol and `FixedSm89HalfSwizzleParams` ABI prove the
family boundary. The
corrected compile-only source labels now match runtime composed-source hashes
without a PTX/CUBIN/SASS change. Evidence:
`internal/perf/ada-triad-half-nn-n96-s3-20260909/report.md`.

Latest 2026-09-09 BF16 TN d768-in BK32/S3 stop: the compact regpipe+vec2
candidate passes exact bits and improves resources to120regs/shared24576/
occupancy4 from retained125regs/shared32768/occupancy3, but loses every once3
retained stratum by27.8–30.4%. Stop before once7/Fast and do not retry this
unchanged mechanism. Evidence:
`internal/perf/ada-triad-half-tn-bk32-s3-regpipe-vec2-20260909/report.md`.
