//! Discovery only: replace the retained d768-in 32x16 transpose with the
//! canonical NVIDIA 32x8/four-elements-per-thread transpose. The retained
//! exact-F32 dual-chunk fused GEMM remains unchanged.

#[path = "support/triad_f32_transpose_32x8_source.rs"]
...
const CANDIDATE_MAX_REGISTERS: u32 = 32;
const CANDIDATE_STATIC_SHARED: usize = 4_224;
const CANDIDATE_MIN_OCCUPANCY: u32 = 4;
...
#[test]
fn tn_d768_in_maps_to_isolated_32x8_transpose_pipeline() {
    assert_eq!(
    ...
    assert_eq!(tail.retained_dual_params(1.0).unwrap().k1, 1023);
    ...
    let candidate = candidate_source::compose_source();
    assert_eq!(candidate.matches(candidate_source::SYMBOL).count(), 1);
    assert!(candidate.contains("TRANSPOSE_BLOCK_ROWS 8"));
    assert!(candidate.contains("float tile[32][33]"));
    assert_eq!(
        (
...
#[test]
fn cuda_harness_isolates_transpose_and_stages_retained_before_fast() {
    let harness = include_str!("gemm_bi_scalar_tn_transpose_32x8_d768_in_discovery.rs");
    assert!(harness.contains("Arm::Candidate => launch_candidate_pipeline(runtime, fixture)"));
    ...
    assert!(harness.contains("\"candidate 32x8 transpose + retained fused\""));
    assert!(harness.contains("\"retained dual-chunk\""));
    assert!(harness.contains("\"candidate_nodes\":[CANDIDATE_TRANSPOSE,RETAINED_DUAL_FUSED]"));
    ...
    for forbidden in ["LDL", "STL", "ATOM", "REDUX", "FFMA", "DADD", "DMUL"] {
...
mod splitm_cuda_suite {
    ...
    const CANDIDATE_TRANSPOSE: &str = candidate_source::SYMBOL;
    ...
    struct Runtime {
        ...
        _candidate_transpose_module: Arc<CudaModule>,
        ...
        candidate_transpose_source_sha: String,
    ...
    fn ptxas_used_resource_parser_is_fail_closed() {
        assert_eq!(
            ptxas_used_resources(
                "ptxas info    : Used 31 registers, used 1 barriers, 4224 bytes smem, 392 bytes cmem[0]"
            ),
            Some((31, 4_224))
        );
    ...
    fn validate_ptxas_and_sass(symbol: &str, ptx: &str) -> Result<(), String> {
        let stem = format!("mamba-transpose-32x8-{}", std::process::id());
        ...
        let result = (|| {
            ...
            for required in [" LDG", " STG", " LDS", " STS", " BAR"] {
                if !entry.contains(required) {
                    return Err(format!("{symbol} SASS omitted {required}"));
                }
            }
            for forbidden in [
                " LDL", " STL", " ATOM", " RED", " REDUX", " FFMA", " DADD", " DMUL",
            ] {
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnTranspose32x8SassV1",
                    "symbol":symbol,
                    "target":"sm_89",
                    "required":["LDG","STG","LDS","STS","BAR"],
                    "stack_bytes":0,
                    "spill_store_bytes":0,
                    "spill_load_bytes":0,
                    "registers":used[0].0,
                    ...
                    "forbidden":["LDL","STL","ATOM","RED","REDUX","FFMA","DADD","DMUL"]
                })
            );
            Ok(())
        })();
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        let candidate_transpose_source = candidate_source::compose_source();
        let candidate_transpose_source_sha = format!(
            "{:x}",
            Sha256::digest(candidate_transpose_source.as_bytes())
        );
        let candidate_transpose_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            candidate_transpose_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
                include_paths: cuda_include_paths(),
        ...
        .map_err(|error| format!("compile 32x8 transpose: {error:?}"))?;
        validate_ptxas_and_sass(CANDIDATE_TRANSPOSE, &candidate_transpose_ptx.to_src())?;
        let candidate_transpose_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(
                candidate_transpose_ptx.to_src(),
            ))
            .map_err(|error| format!("load 32x8 transpose module: {error:?}"))?;
        let candidate_transpose = candidate_transpose_module
            .load_function(CANDIDATE_TRANSPOSE)
            .map_err(|error| format!("load {CANDIDATE_TRANSPOSE}: {error:?}"))?;
        ...
        Ok(Runtime {
            ...
            ctx,
            _raw_module: module,
            _candidate_transpose_module: candidate_transpose_module,
            _retained_dual_fused_module: retained_dual_fused_module,
            raw_fixed,
            candidate_transpose,
            retained_dual_fused,
            raw_source_sha,
            candidate_transpose_source_sha,
    ...
    fn launch_candidate_transpose(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        let output = fixture.transposed.ptr();
        let input = fixture.x.ptr();
        let rows = fixture.cell.batch as i32;
        let columns = fixture.cell.k_out as i32;
        ...
        builder.arg(&rows);
        builder.arg(&columns);
        unsafe { builder.launch(config(fixture.cell.transpose_grid(), (32, 8, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch {CANDIDATE_TRANSPOSE}: {error:?}"))
    }

    fn launch_retained_fused_only(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
    ...
    fn launch_candidate_pipeline(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        if fixture.cell.batch == 0 {
            return launch_zero(runtime, fixture);
        }
        launch_candidate_transpose(runtime, fixture)?;
        launch_retained_fused_only(runtime, fixture)
    }
    ...
    fn launch(
        ...
        match arm {
            Arm::Candidate => launch_candidate_pipeline(runtime, fixture),
            Arm::Reference => {
    ...
    unsafe fn require_candidate_graph(graph: &CudaGraph, fixture: &Fixture) -> Result<(), String> {
        ...
        if nodes.len() != 2 {
            return Err(format!(
                "32x8 transpose candidate has {} nodes, expected two",
                nodes.len()
            ));
        }
        let transpose = nodes
            .iter()
            .find(|node| node.symbol == CANDIDATE_TRANSPOSE)
            .ok_or_else(|| format!("candidate omitted {CANDIDATE_TRANSPOSE}"))?;
        let fused = nodes
            .iter()
            .find(|node| node.symbol == RETAINED_DUAL_FUSED)
            .ok_or_else(|| format!("candidate omitted {RETAINED_DUAL_FUSED}"))?;
        require_launch(
            transpose,
            CANDIDATE_TRANSPOSE,
            fixture.cell.transpose_grid(),
        ...
        if unsafe { graph_arg::<u64>(transpose, 0) }? != fixture.transposed.ptr()
            ...
            || unsafe { graph_arg::<i32>(transpose, 3) }? != fixture.cell.k_out as i32
        {
            return Err("candidate 32x8 transpose ABI/arguments changed".into());
        }
        ...
        {
            return Err("candidate retained fused ABI/arguments changed".into());
        }
        unsafe {
            require_chain(
                graph,
                &[transpose.node, fused.node],
                "candidate 32x8 transpose + retained fused",
            )
        }?;
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnTranspose32x8GraphV1",
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "nodes":[CANDIDATE_TRANSPOSE,RETAINED_DUAL_FUSED],
                "edges":[[CANDIDATE_TRANSPOSE,RETAINED_DUAL_FUSED]],
                "order":["transpose_32x8","retained_dual_chunk_fused_finalize"]
            })
        );
        Ok(())
    }
    ...
    fn check_exact_case(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnTranspose32x8BitsV1",
                "case":label,
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "alpha_bits":fixture.alpha.to_bits(),
                "candidate":CANDIDATE_TRANSPOSE,
    ...
    fn candidate_resources(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnTranspose32x8HardResourceV1",
                "symbol":symbol,
                "threads":256,
                "registers":registers,
                "registers_max":CANDIDATE_MAX_REGISTERS,
    ...
    #[test]
    #[ignore = "Ada CUDA13.2 discovery: exact F32 TN d768-in isolated 32x8 transpose"]
    fn ada_f32_tn_transpose_32x8_d768_in_once3_once7_fast() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("f32-tn-transpose-32x8/pre", 2_000)?;
        } else {
            quiet.require_pre_context("f32-tn-transpose-32x8/pre")?;
        }
        let runtime = new_runtime()?;

        for (function, symbol) in [(&runtime.candidate_transpose, CANDIDATE_TRANSPOSE)] {
            if let Err(reason) = candidate_resources(function, symbol) {
                println!(
                    "{}",
                    json!({
                        "schema":"MambaBiF32TnTranspose32x8ResourceStopV1",
                        "candidate":CANDIDATE_TRANSPOSE,
                        "reason":reason,
                ...
                if idle_resident_mode {
                    quiet.require_idle_resident("f32-tn-transpose-32x8/resource-stop", 256)?;
                } else {
                    quiet.verify_post_cohort("f32-tn-transpose-32x8/resource-stop")?;
                }
                return Ok(());
            }
        }
        ...
        let candidate = CandidateFixed::Production;
        resources(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnTranspose32x8ArtifactsV1",
                "artifacts":format!("{:?}",runtime.ctx.kernels.artifact_set_identity()),
                "candidate_transpose_source_sha":runtime.candidate_transpose_source_sha,
                "retained_dual_fused_source_sha":runtime.retained_dual_fused_source_sha,
                "candidate_nodes":[CANDIDATE_TRANSPOSE,RETAINED_DUAL_FUSED],
        ...
        while comparator_index < comparators.len() {
            ...
            if idle_resident_mode {
                quiet.require_idle_resident("f32-tn-transpose-32x8/timed", 256)?;
            } else {
                quiet.require_cohort("f32-tn-transpose-32x8/timed")?;
            }
            ...
            for graph_path in [false, true] {
                for candidate_endpoints in [true, false] {
                    ...
                    println!(
                        "{}",
                        json!({
                            "schema":"MambaBiF32TnTranspose32x8ScreenV1",
                            ...
                            "candidate_timed_nodes":[CANDIDATE_TRANSPOSE,RETAINED_DUAL_FUSED],
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnTranspose32x8DecisionV1",
                    "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                    "comparator":if comparator == Arm::Retained {"N64_G8_DUAL_COPYPLAN_FUSED"} else {"Fast"},
                    "windows":windows,
                    "strata":strata,
                    ...
                    "decision":match (comparator,windows,strict_win) {
                        (Arm::Retained,3,true)=>"transpose_32x8_once3_beats_retained",
                        (Arm::Retained,3,false)=>"stop_once3_no_retry",
                        (Arm::Retained,7,true)=>"transpose_32x8_once7_beats_retained",
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnTranspose32x8FinalDecisionV1",
                "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                "candidate":CANDIDATE_TRANSPOSE,
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("f32-tn-transpose-32x8/post", 256)?;
        } else {
