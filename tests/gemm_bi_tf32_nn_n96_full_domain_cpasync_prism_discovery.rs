//! Test-only Ada TF32 NN N96 full-domain unpredicated-cp.async discovery.
//! Production kernels and dispatch remain untouched.

#[path = "support/triad_tf32_nn_n96_full_domain_cpasync_source.rs"]
...
fn direct_retained_source() -> String {
    ...
    direct_source::compose_candidate_source(&triad).unwrap()
}

#[test]
fn source_adapter_adds_one_full_arm_and_restores_direct_parent() {
    let direct_retained = direct_retained_source();
    let candidate = candidate_source::compose_candidate_source(&direct_retained).unwrap();
    assert_eq!(
        candidate
            .matches("cp.async.cg.shared.global.L2::128B [%0], [%1], 16;\\n")
            .count(),
        1,
    );
    assert!(!candidate.contains("tf32n96_mainloop<"));
    assert_eq!(
        candidate
            .matches("tf32n96_stage_slice(\n                    plan")
            .count(),
        1,
    );
    assert_eq!(candidate.matches("full_copy_domain, issue").count(), 2);
...
#[test]
fn prism_full_domain_classification_is_literal_and_tail_safe() {
    assert!(candidate_source::full_domain(
        4_621, 384, 1_928, 0, 0, 384, 1_928, 0x1000, 0x2000,
    ));
    assert!(candidate_source::full_domain(
        4_621,
        384,
    ...
    assert!(!candidate_source::full_domain(
        4_621, 385, 1_928, 0, 0, 385, 1_928, 0x1000, 0x2000,
    ));
    assert!(!candidate_source::full_domain(
        4_621, 384, 1_928, 0, 0, 384, 1_928, 0x1004, 0x2000,
    ));
}

#[test]
fn transform_rejects_non_parent_and_duplicate_seams() {
    assert!(candidate_source::compose_candidate_source("not the frozen parent").is_err());
    let direct_retained = direct_retained_source();
    assert!(
        candidate_source::compose_candidate_source(&(direct_retained.clone() + &direct_retained))
            .is_err()
    );
}
...
fn fast_screen_requires_four_strict_retained_strata() {
    ...
    assert!(strict_fast_win(&[[0.99, 0.999]; 4]));
    assert!(!strict_fast_win(&[[0.99, 1.0]; 4]));
}
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_NN_N96_FULL_DOMAIN_CPASYNC_DISCOVERY";
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "full_domain_cpasync_n96",
    ...
    fn ptxas_and_sass(symbol: &str, ptx: &str) -> Result<SassStats, String> {
        let stem = format!(
            "mamba-n96-full-domain-{}-{}",
            std::process::id(),
            symbol.len()
        );
        ...
        let result = (|| {
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiTriadTf32NnN96FullDomainCpasyncSassV1",
    ...
    fn compile_only_gate() -> Result<(), String> {
        let triad = retained_source::compose_triad_nn_n96_source(FIXED_N96_SOURCE)?;
        let retained_body = direct_source::compose_candidate_source(&triad)?;
        ...
        let candidate = ptxas_and_sass(
            ...
            &compile_ptx(&candidate_body, "full-domain candidate")?,
        )?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96FullDomainCpasyncCompileDecisionV1",
        ...
        if candidate.registers == 0
            ...
            || candidate.forbidden
            || text_ratio > 1.20
            || !(candidate.hmma == retained.hmma || candidate.hmma == retained.hmma * 2)
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if cfg!(debug_assertions) {
            return Err("N96 full-domain cp.async timing requires --release".into());
        }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96FullDomainCpasyncIdentityV1",
                ...
                "change":"full_domain_unpredicated_cp_async",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96FullDomainCpasyncResourceV1","symbol":symbol,
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32NnN96FullDomainCpasyncGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 1 {
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32NnN96FullDomainCpasyncGraphV1","arm":arm.name(),"symbol":symbol,"grid":expected_grid,"block":[256,1,1],"shared":SHARED,"timing":"whole_graph"})
            );
        }
        Ok(())
    }
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32NnN96FullDomainCpasyncBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96FullDomainCpasyncTargetBitsV1",
    ...
    fn prepare_fast(runtime: &Runtime, case: Case, prepared: &mut Prepared) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96FullDomainCpasyncFastBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96FullDomainCpasyncScreenV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid()?,
                "candidate":"full_domain_cpasync_n96","comparator":comparator.name(),"path":path.name(),
    ...
    fn cuda132_compile_only_full_domain_cpasync_resources_and_sass() -> Result<(), String> {
        compile_only_gate()
    }

    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 NN N96 full-domain cp.async Prism once7"]
    fn ada_tf32_nn_n96_full_domain_cpasync_prism_once7() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-n96-full-domain-cpasync/pre", 1_800)?;
        } else {
            quiet.require_pre_context("tf32-nn-n96-full-domain-cpasync/pre")?;
        }
        ...
        for case in [PRISM] {
            ...
            if idle_resident_mode {
                quiet.require_idle_resident("tf32-nn-n96-full-domain-cpasync/retained", 256)?;
            } else {
                quiet.require_cohort("tf32-nn-n96-full-domain-cpasync/retained")?;
            }
            ...
            if retained_win {
                ...
                if idle_resident_mode {
                    quiet.require_idle_resident("tf32-nn-n96-full-domain-cpasync/fast", 256)?;
                } else {
                    quiet.require_cohort("tf32-nn-n96-full-domain-cpasync/fast")?;
                }
                for path in [Path::Eager, Path::Graph] {
                    for order in [
                        retained_source::BracketOrder::Abba,
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiTriadTf32NnN96FullDomainCpasyncDecisionV1","cell":case.label,
                    "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid()?,
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-n96-full-domain-cpasync/post", 256)?;
        } else {
            quiet.verify_post_cohort("tf32-nn-n96-full-domain-cpasync/post")?;
        }
        Ok(())
    }
}
