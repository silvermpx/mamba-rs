#[path = "support/triad_half_nt_m64n128_s2_source.rs"]
...
fn source_adapter_is_reversible_and_freezes_the_two_stage_contract() {
    let measured = candidate_source::measured_s3_source(SWIZZLE, S3).unwrap();
    ...
    assert!(candidate.contains("static constexpr int kSharedBytes = 49152;"));
    assert!(candidate.contains("int write_buf = read_buf ^ 1;"));
    assert!(candidate.contains("if (next) read_buf = write_buf;"));
    assert!(candidate.contains("int next_k = (kt + 1) * 64;"));
    assert!(!candidate.contains("int next_k = (kt + 2) * 64;"));
    assert!(!candidate.contains("int write_buf = 2;"));
    assert_eq!(candidate.matches("cp.async.commit_group;").count(), 2);
    assert_eq!(candidate.matches("cp.async.wait_group 0;").count(), 1);
    assert_eq!(candidate.matches("nt_m64n128_copy_slice").count(), 6);
    assert!(candidate.contains(candidate_source::SYMBOL_PREFIX));
}

#[test]
fn d768_out_grid_and_strict_retained_first_policy_are_frozen() {
    let (m, k_out, reduction) = candidate_source::D768_OUT;
    assert_eq!((m, k_out, reduction), (2_048, 1_536, 768));
    assert_eq!(m.div_ceil(64) * k_out.div_ceil(128), 384);
...
#[test]
fn two_stage_ring_never_reads_the_write_stage_and_finishes_on_the_last_tile() {
    for tile_count in 1..=17 {
        let mut resident = [None, None];
        resident[0] = Some(0usize);
        let mut read_buf = 0usize;
        for tile in 0..tile_count {
            assert_eq!(resident[read_buf], Some(tile));
            let next = tile + 1 < tile_count;
            let write_buf = read_buf ^ 1;
            assert_ne!(read_buf, write_buf);
            if next {
                resident[write_buf] = Some(tile + 1);
                read_buf = write_buf;
            }
        }
        assert_eq!(resident[read_buf], Some(tile_count - 1));
    }
}
...
mod cuda_suite {
    ...
    const TAIL: Shape = Shape::new(67, 131, 69, -0.75, "tail_negative_alpha");
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "m64n128_bk64_s2_pingpong",
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        if !candidate_ptx
            .to_src()
            .contains("gemm_bi_nt_test_fixed_s2_m64n128_f16")
            || !retained_ptx
                .to_src()
    ...
    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_F16_NT_M64N128_S2_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_F16_NT_M64N128_S2_DISCOVERY=1".into());
        }
        let device = GpuDevice::new(0)?;
        if device.compute_capability != (8, 9) || device.multiprocessor_count() != 142 {
            return Err(format!(
                "M64N128/S2 discovery requires 142-SM Ada, got {:?}/{}",
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function, shared, occupancy) in [
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfNtM64N128S2ResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":{shared},\"active_ctas\":{active},\"required_active_ctas\":{occupancy}}}",
                arm.name()
            );
        }
        Ok(())
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N128S2BitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha\":{},\"exceptional\":{},\"candidate_vs_measured_s3_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.m,
            shape.k_out,
            shape.reduction,
    ...
    fn timed_pair(
        ...
        for window in 0..WINDOWS {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiHalfNtM64N128S2TimingV1\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw
    ...
    fn ada_f16_nt_m64n128_bk64_s2_pingpong_d768_out_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("half-nt-m64n128-s2/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("half-nt-m64n128-s2/cohort")?;
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N128S2RetainedDecisionV1\",\"shape\":[2048,1536,768],\"candidate\":\"{}\",\"comparator\":\"{}\",\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_strata,
            if retained_pass {
        ...
        if !retained_pass {
            drop(runtime);
            quiet.verify_post_cohort("half-nt-m64n128-s2/post")?;
            return Ok(());
        }
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N128S2FinalDecisionV1\",\"shape\":[2048,1536,768],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"retained_strata\":{:?},\"fast_strata\":{:?},\"strict_fast_p50_p95_lt_0_99\":{fast_pass},\"decision\":\"{}\",\"promotion\":false}}",
