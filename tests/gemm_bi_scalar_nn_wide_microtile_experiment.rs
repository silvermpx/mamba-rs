...
const CUDA_SOURCE_FILE: &str = "gemm_bi_scalar_nn_wide_microtile_experiment.cu";
...
/// One experimental schedule: its symbol, block geometry, and the resource
/// pins the box has to reproduce before any timing counts.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
const CANDIDATES: [CandidateSpec; 3] = [
    CandidateSpec {
...
impl CandidateSpec {
    fn grid(self) -> u32 {
        let rows = DIMS.0.div_ceil(self.tile.0);
        let columns = DIMS.2.div_ceil(self.tile.1);
        (rows * columns) as u32
    }
...
fn validate_exact_dims(dims: (usize, usize, usize)) -> Result<(), String> {
    if dims != DIMS {
        return Err(format!(
            "wide-microtile experiment only accepts {DIMS:?}, received {dims:?}"
        ));
    }
    Ok(())
}
...
fn candidate_source_is_exact_one_owner_and_never_enters_production() {
    ...
    for candidate in CANDIDATES {
        assert_eq!(
            source
                .matches(&format!("void {}(", candidate.symbol))
                .count(),
            1,
    ...
    for candidate in CANDIDATES {
        assert!(!modules.contains(candidate.symbol));
    }
...
#[test]
fn candidate_geometry_covers_the_cell_and_stays_distinct() {
    assert_eq!(PRODUCTION_SPEC.grid(), 1_536);
    ...
    assert_eq!(CANDIDATES[0].grid(), 768);
    assert_eq!(CANDIDATES[1].grid(), 384);
    assert_eq!(CANDIDATES[2].grid(), 768);
}
...
fn resource_observations_are_strict_and_fail_closed() {
    let ptxas = PtxasResourceContract { registers: 94 };
...
mod cuda_experiment {
    ...
    use super::{
        CANDIDATES, CandidateSpec, DIMS, EXCEPTIONAL_CASES, OFFICIAL_WINDOWS, PRODUCTION_SPEC,
        PRODUCTION_SYMBOL, SCREEN_WINDOWS, exact_windows, percentile, read_candidate_source,
        speedup_stats, validate_driver_resource_observation, validate_exact_dims,
    ...
    impl Arm {
        fn all() -> impl Iterator<Item = Arm> {
            std::iter::once(Arm::Production).chain((0..CANDIDATES.len()).map(Arm::Candidate))
        }

        fn spec(self) -> CandidateSpec {
            match self {
                Self::Production => PRODUCTION_SPEC,
                Self::Candidate(index) => CANDIDATES[index],
            }
        }
    ...
    impl Kernels {
        fn function(&self, arm: Arm) -> &CudaFunction {
            match arm {
                ...
                Arm::Candidate(index) => &self.candidates[index],
            }
        }
    }
    ...
    impl Fixture {
        fn output(&self, arm: Arm) -> &GuardedBuffer {
            match arm {
                ...
                Arm::Candidate(_) => &self.candidate_output,
            }
        }
        ...
        fn output_mut(&mut self, arm: Arm) -> &mut GuardedBuffer {
            match arm {
                ...
                Arm::Candidate(_) => &mut self.candidate_output,
            }
        }
    }
    ...
    fn production_holder(runtime: &Runtime) -> Result<QualifiedPhysicalLaunch<'_>, String> {
        ...
        let config = Arm::Production.config();
        if evidence.launch_count() != 1
            || nodes.len() != 1
            || nodes[0].symbol != PRODUCTION_SYMBOL
            || nodes[0].module_kind != ModuleKind::TriadScalar
            || nodes[0].launch.grid_dim != config.grid_dim
            || nodes[0].launch.block_dim != config.block_dim
            || nodes[0].launch.shared_mem_bytes != config.shared_mem_bytes
    ...
    fn load_kernels(runtime: &Runtime) -> Result<Kernels, String> {
        ...
        for index in 0..CANDIDATES.len() {
            candidates.push(load(Arm::Candidate(index))?);
        }
        Ok(Kernels {
    ...
    fn launch(
        ...
        {
            return Err("wide-microtile launch operands or contiguous strides drifted".into());
        }
    ...
    fn validate_correctness(
        ...
        for index in 0..CANDIDATES.len() {
            let arm = Arm::Candidate(index);
            let graph = capture(runtime, kernels, &fixture, arm)?;
            if graph_identity(&graph, arm)? == production_digest {
                return Err(format!(
                    "production and {} physical argument digests collided",
                    arm.symbol()
                ));
            }
        ...
        let arms = std::iter::once((Arm::Production, &production_graph)).chain(
            candidate_graphs
                .iter()
                .enumerate()
                .map(|(index, graph)| (Arm::Candidate(index), graph)),
        );
                ...
                for repeat in 0..CORRECTNESS_REPEATS {
                    ...
                    if actual != reference {
                        let differing = actual
                            ...
                            .count();
                        return Err(format!(
                            "{} {path:?} repeat {repeat} kind {kind} differs from production bits in {differing} elements",
                            arm.symbol()
                        ));
                    }
    ...
    fn validate_driver_resources(kernels: &Kernels) -> Result<(), String> {
        ...
        for arm in Arm::all() {
            ...
            eprintln!(
                "scalar_nn_wide_microtile resource symbol={} grid={} threads={} dynamic_shared={} static_shared={} registers={} local={} occupancy={}",
                arm.symbol(),
                config.grid_dim.0,
                config.block_dim.0,
    ...
    fn validate_ptxas_and_sass(target: &str, ptx: &str) -> Result<(), String> {
        ...
        let stem = format!("mamba-nn-wide-microtile-{target}-{nonce}");
        ...
        let result = (|| {
            ...
            for arm in Arm::all() {
                ...
                eprintln!(
                    "scalar_nn_wide_microtile ptxas target={target} symbol={} registers={registers:?} stack_spill_stores_loads={spills:?} sass: {}",
                    arm.symbol(),
                    opcode_census(entry)
                );
                let contract = arm.spec().ptxas(target)?;
                ...
                for forbidden in [" LDL", " STL", " ATOM", " RED", " REDUX"] {
                    if entry.contains(forbidden) {
                        failures.push(format!("{} SASS contains {forbidden}", arm.symbol()));
                    }
                }
            }
            if failures.is_empty() {
    ...
    struct TimingFixture {
        ...
        candidate_graphs: Vec<CudaGraph>,
    }
    ...
    fn measure(
        ...
        for _ in 0..iterations {
            match (arm, path) {
                ...
                (Arm::Candidate(index), PathKind::Graph) => timing.candidate_graphs[index]
                    .launch()
                    .map_err(|error| format!("launch {} graph: {error:?}", arm.symbol()))?,
            }
        }
    ...
    fn adjacent(
        ...
        let sequence = match order {
            Order::Abba => [Arm::Production, candidate, candidate, Arm::Production],
            Order::Baab => [candidate, Arm::Production, Arm::Production, candidate],
        };
        ...
        for arm in sequence {
            ...
            match arm {
                ...
                Arm::Candidate(_) => candidate_us.push(value),
            }
        }
        Ok((
            production.iter().sum::<f64>() / 2.0,
            candidate_us.iter().sum::<f64>() / 2.0,
        ))
    }

    /// One paired cohort for one candidate. Reports the p50 of both arms and
    /// the speedup percentiles, then returns the p05/p50/p95 speedup so the
    /// caller decides whether the best candidate clears the promotion bar.
    fn paired(
        runtime: &Runtime,
        kernels: &Kernels,
        timing: &TimingFixture,
        ...
        let candidate_probe = measure(runtime, kernels, timing, candidate, path, 4)?;
        ...
        let label = format!(
            "scalar-nn-wide-microtile/{}/{path:?}/{order:?}/{windows}",
            candidate.symbol()
        );
        ...
        let body = (|| {
            ...
            for _ in 0..windows {
                let (production, measured) =
                    adjacent(runtime, kernels, timing, candidate, path, order, iterations)?;
            ...
            eprintln!(
                "scalar_nn_wide_microtile candidate={} path={path:?} order={order:?} windows={windows} production_us_p50={production_p50:.6} candidate_us_p50={candidate_p50:.6} production_tflops={:.2} candidate_tflops={:.2} speedup_p05={:.9} speedup_p50={:.9} speedup_p95={:.9}",
                candidate.symbol(),
    ...
    fn run_timing(windows: usize) -> Result<(), String> {
        ...
        let label = format!("scalar-nn-wide-microtile-{windows}");
        ...
        let body = (|| {
            ...
            for index in 0..CANDIDATES.len() {
                let candidate = Arm::Candidate(index);
                let mut worst_p50 = f64::INFINITY;
                for path in [PathKind::Eager, PathKind::Graph] {
                    for order in [Order::Abba, Order::Baab] {
                        let stats = paired(
                            &runtime, &kernels, &timing, candidate, path, order, windows, &quiet,
                        )?;
                        worst_p50 = worst_p50.min(stats.1);
                    }
                }
                if best.is_none_or(|(_, p50)| worst_p50 > p50) {
                    best = Some((candidate, worst_p50));
                }
            }
            let (candidate, p50) = best.ok_or_else(|| "no candidate was timed".to_string())?;
            eprintln!(
                "scalar_nn_wide_microtile best={} worst_cohort_speedup_p50={p50:.9}",
                candidate.symbol()
            );
            validate_speedup(windows, &vec![p50; windows]).map(drop)
        })();
        super::verify_post_cohort_even_on_error(body, quiet.verify_post_cohort(&label).map(drop))
    }
    ...
    fn candidate_screen_21_windows_abba_baab() -> Result<(), String> {
        ...
        let windows = exact_windows(
            std::env::var_os("MAMBA_RS_NN_WIDE_MICROTILE_WINDOWS").as_deref(),
            SCREEN_WINDOWS,
        )?;
        run_timing(windows)
    }
