//! Test-only Ada TF32 NN canonical-Prism M96N96/S2 direct-epilogue discovery.
//! Production kernels and dispatch remain untouched.

#[path = "support/triad_tf32_nn_m96n96_s2_direct_source.rs"]
#[allow(dead_code)]
...
#[test]
fn source_adapter_builds_reversible_s2_body_without_stopped_mechanisms() {
    let direct_retained = direct_retained_source();
    let candidate = candidate_source::candidate_source(FIXED_N96_SOURCE).unwrap();
    assert!(!candidate.contains("full_copy_domain"));
    assert!(!candidate.contains("gbf_tf32_copy_ca("));
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_TF32_NN_M96N96_S2_DIRECT_PRISM_DISCOVERY";
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "direct_epilogue_m96n96_s2",
    ...
    fn ptxas_and_sass(symbol: &str, ptx: &str, shared: usize) -> Result<SassStats, String> {
        ...
        let result = (|| {
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiTriadTf32NnM96N96S2DirectSassV1",
    ...
    fn compile_only_gate() -> Result<(), String> {
        ...
        let candidate = ptxas_and_sass(
            ...
            &compile_ptx(&candidate_body, "M96N96/S2 direct candidate")?,
            CANDIDATE_SHARED,
        )?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2DirectCompileDecisionV1",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if cfg!(debug_assertions) {
            return Err("M96N96/S2 direct timing requires --release".into());
        }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2DirectIdentityV1",
                ...
                "change":"M128N96_S3_occupancy1_to_M96N96_S2_occupancy2",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2DirectResourceV1","symbol":symbol,
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32NnM96N96S2DirectGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 1 {
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32NnM96N96S2DirectGraphV1","arm":arm.name(),"symbol":symbol,"grid":expected_grid,"block":expected_config.block_dim,"shared":expected_config.shared_mem_bytes,"timing":"whole_graph"})
            );
        }
        Ok(())
    }
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32NnM96N96S2DirectBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2DirectTargetBitsV1",
    ...
    fn prepare_fast(runtime: &Runtime, case: Case, prepared: &mut Prepared) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2DirectFastBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2DirectScreenV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid(Arm::Candidate)?,
                "candidate":"direct_epilogue_m96n96_s2","comparator":comparator.name(),"path":path.name(),
    ...
    fn cuda132_compile_only_m96n96_s2_direct_resources_and_sass() -> Result<(), String> {
        compile_only_gate()
    }

    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 NN M96N96/S2 direct Prism once3 -> once7 -> Fast"]
    fn ada_tf32_nn_m96n96_s2_direct_prism_protocol() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-m96n96-s2-direct/pre", 1_800)?;
        } else {
            quiet.require_pre_context("tf32-nn-m96n96-s2-direct/pre")?;
        }
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-m96n96-s2-direct/retained", 256)?;
        } else {
            quiet.require_cohort("tf32-nn-m96n96-s2-direct/retained")?;
        }
        ...
        let fast_strata = if retained_win {
            ...
            if idle_resident_mode {
                quiet.require_idle_resident("tf32-nn-m96n96-s2-direct/fast", 256)?;
            } else {
                quiet.require_cohort("tf32-nn-m96n96-s2-direct/fast")?;
            }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2DirectDecisionV1","cell":case.label,
        ...
        drop(prepared);
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-m96n96-s2-direct/post", 256)?;
        } else {
            quiet.verify_post_cohort("tf32-nn-m96n96-s2-direct/post")?;
        }
        Ok(())
    }
}
