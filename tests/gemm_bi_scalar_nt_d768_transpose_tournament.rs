...
const TEST_SOURCE: &str = include_str!("../kernels/gemm_bi_triad/scalar_nt_d768_transpose.cu");
...
mod cuda_tournament {
    ...
    const PRODUCTION_TRANSPOSE: &str = "gemm_bi_transpose_f32_2d";
    ...
    const BASELINE_MIN_SPEEDUP: f64 = 1.20;
    ...
    fn validate_candidate_identity(nodes: &[CandidateNodeIdentity]) -> Result<(), String> {
        ...
        let expected_configs = [
            ((96, 24, 1), (32, 16, 1), 0),
            ((384, 1, 1), (128, 1, 1), M64_SHARED as u32),
        ];
    ...
    fn validate_qualified_environment(
        ...
        {
            return Err(format!(
                "d768 tournament requires the qualified compute_120 NVRTC 13.2 TriadScalar artifact domain: cc={compute_capability:?} sms={multiprocessor_count} target={nvrtc_target} compiler={compiler:?} artifact={artifact:?}"
            ));
        }
        Ok(())
    }
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if identity.compute_capability != (12, 0) || identity.multiprocessor_count != 170 {
            return Err(format!(
                "d768 tournament requires CC12.0/170 SM, found {:?}/{} SM",
    ...
    fn new_fixture(runtime: &Runtime) -> Result<Fixture, String> {
        let (m, k_out, n) = DIMS;
        let a_expected = values(m * n, 0xa768_0001);
        let b_values = values(k_out * n, 0xb768_0002);
        let output = vec![0.0; m * k_out];
        Ok(Fixture {
            a: GpuBuffer::from_cpu(&runtime.ctx.stream, &a_expected)?,
            a_expected,
    ...
    fn digest_arguments(arguments: &[&[u8]]) -> [u8; 32] {
        ...
        digest.update(b"nt-d768-candidate-arguments.v1");
    ...
    fn check_resources(kernel: &Kernel, contract: ResourceContract) -> Result<(), String> {
        ...
        eprintln!(
            "nt_d768 resource symbol={} threads={} registers={} local_bytes={} static_shared_bytes={} dynamic_shared_bytes={} active_blocks={}",
            kernel.symbol,
            threads,
            registers,
            local,
    ...
    fn validate_physical_production(runtime: &Runtime) -> Result<(), String> {
        ...
        if nodes.len() != 2
            ...
            || nodes[0].launch.grid_dim != (96, 24, 1)
            || nodes[0].launch.block_dim != (32, 16, 1)
            || nodes[0].launch.shared_mem_bytes != 0
            || nodes[1].symbol != M64_SYMBOL
            || nodes[1].launch.grid_dim != (384, 1, 1)
            || nodes[1].launch.block_dim != (128, 1, 1)
            || nodes[1].launch.shared_mem_bytes != M64_SHARED as u32
        ...
        {
            return Err(format!("production d768 route identity changed: {nodes:?}"));
        }
        if !qualified.evidence().eager_graph_equal() {
    ...
    fn candidate_identity_gate_rejects_count_order_zero_and_collisions() {
        ...
        let mutations: [fn(&mut [CandidateNodeIdentity; 2]); 3] = [
    ...
    fn d768_transpose_candidates_are_exact_and_graph_stable() -> Result<(), String> {
        ...
        let a = fixture.a.to_cpu(&runtime.ctx.stream)?;
        if a.iter()
            .zip(&fixture.a_expected)
            .any(|(a, b)| a.to_bits() != b.to_bits())
        {
            return Err("A changed".into());
        }
        fixture
            .scratch
    ...
    fn paired(
        ...
        for (order, baseline_first) in [("ABBA", true), ("BAAB", false)] {
            ...
            eprintln!(
                "nt_d768 baseline={} candidate={} order={} windows={} baseline_us_p50={:.6} candidate_us_p50={:.6} speedup_p05={:.9} speedup_p50={:.9} speedup_p95={:.9}",
                baseline.name(),
                candidate.name(),
                order,
                windows,
            ...
            if baseline == Arm::Transpose32 {
                if speedup_p50 <= 1.0 {
                    return Err(format!(
                        "Transpose16 did not beat Transpose32 in {order}: p50={speedup_p50:.9}"
                    ));
                }
            } else if speedup_p05 < BASELINE_MIN_SPEEDUP {
                return Err(format!(
                    "candidate speedup over {} in {order} fell below {:.2}x: p05={speedup_p05:.9}",
                    baseline.name(),
    ...
    fn d768_transpose_tournament_abba_baab() -> Result<(), String> {
        let runtime = new_runtime()?;
        let mut fixture = new_fixture(&runtime)?;
        for arm in [Arm::Production, Arm::GenericNt, Arm::Transpose32] {
            for _ in 0..5 {
                launch_arm(&runtime, &mut fixture, arm)?;
            }
        }
        ...
        for arm in [Arm::Transpose32, Arm::Production] {
            let iterations = calibrated_iterations(&runtime, &mut fixture, arm)?;
            let samples = (0..11)
                .map(|_| measure(&runtime, &mut fixture, arm, iterations))
                .collect::<Result<Vec<_>, _>>()?;
            let p50 = percentile(&samples, 0.50);
            eprintln!("nt_d768 screen arm={} p50_us={p50:.6}", arm.name());
            screen_p50.push((arm, p50));
        }
        if screen_p50[1].1 >= screen_p50[0].1 {
            return Err(format!(
                "production d768 route did not beat Transpose32: {:?}",
                screen_p50
            ));
        }
        let winner = Arm::Production;
        let windows = std::env::var("MAMBA_RS_NT_D768_WINDOWS")
            .ok()
            .and_then(|value| value.parse().ok())
            .unwrap_or(101);
        paired(&runtime, &mut fixture, Arm::GenericNt, winner, windows)?;
