//! Test-only Ada TF32 NN N96 direct-epilogue discovery.
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_NN_N96_DIRECT_EPILOGUE_DISCOVERY";
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "direct_epilogue_n96",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96DirectEpilogueIdentityV1",
                ...
                "change":"shared_epilogue_to_direct_float2",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96DirectEpilogueResourceV1","symbol":symbol,
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32NnN96DirectEpilogueGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 1 {
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32NnN96DirectEpilogueGraphV1","arm":arm.name(),"symbol":symbol,"grid":expected_grid,"block":[256,1,1],"shared":SHARED,"timing":"whole_graph"})
            );
        }
        Ok(())
    }
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32NnN96DirectEpilogueBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96DirectEpilogueTargetBitsV1",
    ...
    fn prepare_fast(runtime: &Runtime, case: Case, prepared: &mut Prepared) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96DirectEpilogueFastBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96DirectEpilogueScreenV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid()?,
                "candidate":"direct_epilogue_n96","comparator":comparator.name(),"path":path.name(),
    ...
    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 NN N96 direct-epilogue once7"]
    fn ada_tf32_nn_n96_direct_epilogue_d768_out_prism_once7() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-n96-direct-epilogue/pre", 1_800)?;
        } else {
            quiet.require_pre_context("tf32-nn-n96-direct-epilogue/pre")?;
        }
        ...
        for case in [D768_OUT, PRISM] {
            ...
            if idle_resident_mode {
                quiet.require_idle_resident("tf32-nn-n96-direct-epilogue/retained", 256)?;
            } else {
                quiet.require_cohort("tf32-nn-n96-direct-epilogue/retained")?;
            }
            ...
            if retained_win {
                ...
                if idle_resident_mode {
                    quiet.require_idle_resident("tf32-nn-n96-direct-epilogue/fast", 256)?;
                } else {
                    quiet.require_cohort("tf32-nn-n96-direct-epilogue/fast")?;
                }
                for path in [Path::Eager, Path::Graph] {
                    for order in [
                        retained_source::BracketOrder::Abba,
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiTriadTf32NnN96DirectEpilogueDecisionV1","cell":case.label,
                    "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid()?,
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-n96-direct-epilogue/post", 256)?;
        } else {
            quiet.verify_post_cohort("tf32-nn-n96-direct-epilogue/post")?;
        }
        Ok(())
    }
}
