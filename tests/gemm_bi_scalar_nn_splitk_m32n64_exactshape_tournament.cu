// Test-only exact-shape specialization of the production M32N64/BK32
// Split-K partial. The output layout and ascending 32-FFMA chain are unchanged.
...
void gemm_bi_nn_splitk32_m32n64_exactshape_exp_v1(
    ...
    // This uniform guard runs before any asynchronous copy or barrier. The hot
    // path below uses only compile-time extents and contiguous fixed strides.
    ...
    // This is the production grouped mapping reduced for M_TILES=4, N_TILES=2.
...
#pragma unroll
    ...
    // Exactly the production accumulator tree: k_offset+0 through k_offset+31.
#pragma unroll
