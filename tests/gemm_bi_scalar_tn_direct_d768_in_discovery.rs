//! Discovery only: one direct-TN exact raw-partial launch plus the unchanged
//! production split-M reducer. No dispatcher admission or production changes.

#[path = "support/triad_f32_tn_direct_copyplan_source.rs"]
mod direct_source;
...
#[test]
fn tn_d768_in_maps_to_direct_two_node_and_retained_four_node_pipelines() {
    assert_eq!(
    ...
    assert_eq!(D768_IN.retained_node_count(), 4);
    assert_eq!(
        Cell {
            batch: 0,
            k_out: 68,
...
mod splitm_cuda_suite {
    ...
    #[test]
    #[ignore = "Ada CUDA13.2 discovery: exact F32 TN d768-in direct Split-M"]
    fn ada_f32_tn_direct_d768_in_discovery_once7() -> Result<(), String> {
        ...
        quiet.require_pre_context("f32-tn-direct/pre")?;
        let runtime = new_runtime()?;
        validate_fast_handle(&runtime.ctx)?;

        // The successor is meaningful only against this exact retained plan.
        // Prove its four-node identity and packed arguments before comparing a
        ...
        unsafe {
            require_retained_graph(&auto_proof_graph, &auto_proof, CandidateFixed::Production)
        }?;

        // No timing is reached unless direct raw partials match production for
        // both finite full-mantissa and exceptional payload corpora.
        let candidate = select_candidate(&runtime)?;
        let fixed = fixed_function(&runtime, candidate)?;
        resources(fixed, candidate.symbol(), 128, 32_768)?;
        resources(&runtime.direct, DIRECT, 128, 32_768)?;
        resources(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDirectArtifactsV1",
        ...
        for comparator in [Arm::Retained, Arm::Fast] {
            ...
            quiet.require_cohort("f32-tn-direct/timed")?;
            ...
            for graph_path in [false, true] {
                for candidate_endpoints in [true, false] {
                    ...
                    println!(
                        "{}",
                        json!({
                            "schema":"MambaBiF32TnDirectScreenV1",
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnDirectDecisionV1",
                    "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                    "comparator":format!("{comparator:?}"),
                    "strata":strata,
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDirectFinalDecisionV1",
                "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                "candidate":DIRECT,
        ...
        quiet.verify_post_cohort("f32-tn-direct/post")?;
        Ok(())
    }
}
