//! Discovery only: retained transpose plus one rolled N64-G8 CopyPlan mainloop
//! that executes both exact SplitM chains and finalizes.
//! No dispatcher admission or production changes.
...
mod retained_dual_source;
#[path = "support/triad_f32_tn_copyplan_dual_chunk_rolled_source.rs"]
mod rolled_source;
...
const ROLLED_DUAL_MAX_REGISTERS: u32 = 168;
const ROLLED_DUAL_STATIC_SHARED: usize = 32_768;
const ROLLED_DUAL_MIN_OCCUPANCY: u32 = 3;
const ROLLED_MAX_TEXT_RATIO: f64 = 0.70;
const ROLLED_MAX_STATIC_FFMA_RATIO: f64 = 0.60;
const FUSED_RAW_TARGET_CORPORA: [(&str, bool); 2] =
    [("full_mantissa", false), ("exceptional_payload", true)];
...
#[derive(Clone, Copy, Debug, PartialEq)]
#[repr(C)]
struct RetainedDualChunkParams {
    alpha: f32,
    m: i32,
    n: i32,
    k0: i32,
...
impl Cell {
    ...
    fn rolled_dual_params(self, alpha: f32) -> Result<RolledDualChunkParams, String> {
        let chunks = self.chunks()?;
        Ok(RolledDualChunkParams {
            alpha,
            m: self.k_out as i32,
            n: self.n_out as i32,
            k0: chunks[0].len as i32,
            k1: chunks[1].len as i32,
            lda: self.batch as i32,
            ldb: self.n_out as i32,
            ldc: self.n_out as i32,
        })
    ...
    fn retained_dual_params(self, alpha: f32) -> Result<RetainedDualChunkParams, String> {
        let chunks = self.chunks()?;
        Ok(RetainedDualChunkParams {
            alpha,
            m: self.k_out as i32,
            n: self.n_out as i32,
            k0: chunks[0].len as i32,
...
fn raw_plane_indices(cell: Cell) -> Result<Vec<usize>, String> {
    Ok((0..cell.chunks()?.len()).collect())
}

#[test]
fn tn_d768_in_maps_to_exact_two_node_rolled_dual_chunk_pipeline() {
    assert_eq!(
        (size_of::<FixedParams>(), align_of::<FixedParams>()),
        (32, 4)
    );
    assert_eq!(
        D768_IN.retained_dual_params(-0.75).unwrap(),
        RetainedDualChunkParams {
            alpha: -0.75,
            m: 768,
            n: 3072,
            k0: 1024,
    ...
    assert_eq!(D768_IN.candidate_node_count(), 2);
    assert_eq!(
        D768_IN.rolled_dual_params(-0.75).unwrap(),
        RolledDualChunkParams {
            alpha: -0.75,
            m: 768,
            n: 3072,
            k0: 1024,
    ...
    assert_eq!(tail.rolled_dual_params(1.0).unwrap().k1, 1023);
    ...
    let dual_raw = rolled_source::compose_raw_source().unwrap();
    let dual_fused = rolled_source::compose_fused_source().unwrap();
    assert_eq!(
        dual_raw
            .matches(rolled_source::ROLLED_DUAL_RAW_SYMBOL)
            .count(),
        1
    );
    assert_eq!(
        dual_fused
            .matches(rolled_source::ROLLED_DUAL_FUSED_SYMBOL)
            .count(),
        1
    );
    assert_eq!(
        (
            size_of::<RolledDualChunkParams>(),
            align_of::<RolledDualChunkParams>()
        ),
        (32, 4)
    );
    assert_eq!(
        (
            ROLLED_DUAL_MAX_REGISTERS,
            ROLLED_DUAL_STATIC_SHARED,
            ROLLED_DUAL_MIN_OCCUPANCY,
            ROLLED_MAX_TEXT_RATIO,
            ROLLED_MAX_STATIC_FFMA_RATIO,
        ),
        (168, 32_768, 3, 0.70, 0.60)
    );
}

#[test]
fn candidate_selection_requires_independent_proof_of_both_partial_planes() {
    assert_eq!(raw_plane_indices(D768_IN).unwrap(), vec![0, 1]);
}

#[test]
fn fused_chunk1_requires_unconditional_raw_store_target_corpora() {
    assert_eq!(
        FUSED_RAW_TARGET_CORPORA,
        [("full_mantissa", false), ("exceptional_payload", true)]
    );
}
...
#[test]
fn cuda_harness_routes_rolled_candidate_and_screens_fast_only_after_five_percent_win() {
    let harness = include_str!("gemm_bi_scalar_tn_copyplan_dual_chunk_rolled_d768_in_discovery.rs");
    assert!(harness.contains("Arm::Candidate => launch_rolled_dual_fused(runtime, fixture)"));
    assert!(harness.contains("Arm::Retained => launch_retained_dual_fused(runtime, fixture)"));
    assert!(harness.contains("launch_rolled_dual_raw(runtime, fixture).map_err(&harness)?"));
    assert!(
        harness
            .contains("require_chain(graph, &[transpose.node, fused.node], \"rolled dual-chunk\")")
    );
    assert!(
        harness.contains(
            "require_chain(graph, &[transpose.node, fused.node], \"retained dual-chunk\")"
        )
    );
    assert!(harness.contains("\"retained_nodes\":[TRANSPOSE,RETAINED_DUAL_FUSED]"));
    assert!(
        harness.contains("if strict_win {\n                        comparators.push(Arm::Fast);")
    );
    assert!(harness.contains("registers > ROLLED_DUAL_MAX_REGISTERS as i32"));
    assert!(harness.contains("occupancy < ROLLED_DUAL_MIN_OCCUPANCY"));
    assert!(harness.contains("text_ratio > ROLLED_MAX_TEXT_RATIO"));
    assert!(harness.contains("ffma_ratio > ROLLED_MAX_STATIC_FFMA_RATIO"));
    assert!(harness.contains("values[0] < 0.95 && values[1] < 0.95"));
    assert!(harness.contains(
        "let forbidden_memory_or_reduction = [\" LDL\", \" STL\", \" ATOM\", \" RED\", \" REDUX\"]"
    ));
}
...
mod splitm_cuda_suite {
    ...
    const RETAINED_DUAL_FUSED: &str = retained_dual_source::DUAL_FUSED_SYMBOL;
    const ROLLED_DUAL_RAW: &str = rolled_source::ROLLED_DUAL_RAW_SYMBOL;
    const ROLLED_DUAL_FUSED: &str = rolled_source::ROLLED_DUAL_FUSED_SYMBOL;
    ...
    unsafe impl DeviceRepr for RolledDualChunkParams {}
    unsafe impl DeviceRepr for RetainedDualChunkParams {}
    ...
    impl Fixture {
        ...
        fn validate_candidate_partial_unchanged(&self, ctx: &GpuCtx) -> Result<(), String> {
            self.candidate_partial.unchanged(ctx)
        }
    }
    ...
    struct Runtime {
        ...
        _rolled_dual_raw_module: Arc<CudaModule>,
        _rolled_dual_fused_module: Arc<CudaModule>,
        _retained_dual_fused_module: Arc<CudaModule>,
        raw_fixed: CudaFunction,
        rolled_dual_raw: CudaFunction,
        rolled_dual_fused: CudaFunction,
        retained_dual_fused: CudaFunction,
        raw_source_sha: String,
        rolled_dual_raw_source_sha: String,
        rolled_dual_fused_source_sha: String,
        retained_dual_fused_source_sha: String,
    }
    ...
    fn sass_entry<'a>(sass: &'a str, symbol: &str) -> Result<&'a str, String> {
        ...
        let (start, marker_len) = if let Some(start) = sass.find(&function_marker) {
            ...
            let function_lines = sass
                .lines()
                .filter(|line| line.contains("Function") || line.contains(".text."))
                .take(24)
                .collect::<Vec<_>>();
            return Err(format!(
                "nvdisasm omitted {symbol}; visible function lines: {function_lines:?}"
            ));
        };
        let tail = &sass[start..];
    ...
    struct SassStats {
        ...
        forbidden_memory_or_reduction: bool,
    }

    fn validate_ptxas_and_sass(symbol: &str, ptx: &str) -> Result<SassStats, String> {
        let stem = format!("mamba-dual-chunk-{}-{}", std::process::id(), symbol.len());
        ...
        let result = (|| {
            ...
            let stack_bytes = unique_metric(" bytes stack frame")?;
            let spill_store_bytes = unique_metric(" bytes spill stores")?;
            let spill_load_bytes = unique_metric(" bytes spill loads")?;
            let registers = block
                .lines()
                .find_map(|line| metric_before(line, " registers"))
                .ok_or_else(|| format!("ptxas omitted {symbol} register count"))?;
            let static_shared = block
                .lines()
                .find_map(|line| metric_before(line, " bytes smem"))
                .ok_or_else(|| format!("ptxas omitted {symbol} static shared count"))?;
            ...
            let forbidden_memory_or_reduction = [" LDL", " STL", " ATOM", " RED", " REDUX"]
                .into_iter()
                .any(|forbidden| entry.contains(forbidden));
            let static_instructions = entry
                .lines()
                .filter(|line| line.trim_start().starts_with("/*"))
                .count();
            if static_instructions == 0 {
                return Err(format!("{symbol} SASS instruction count is empty"));
            }
            let stats = SassStats {
                // Ada SASS instructions are 128-bit wide; instruction count is
                // therefore the executable text footprint in 16-byte units.
                text_bytes: static_instructions * 16,
                static_ffma: entry.lines().filter(|line| line.contains("FFMA")).count(),
                registers,
                static_shared,
                stack_bytes,
                spill_store_bytes,
                spill_load_bytes,
                forbidden_memory_or_reduction,
            };
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnDualChunkSassV1",
                    "symbol":symbol,
                    "target":"sm_89",
                    "static_ffma":stats.static_ffma,
                    "text_bytes":stats.text_bytes,
                    "registers":stats.registers,
                    "static_shared_bytes":stats.static_shared,
                    "ldgsts":true,
                    "stack_bytes":stats.stack_bytes,
                    "spill_store_bytes":stats.spill_store_bytes,
                    "spill_load_bytes":stats.spill_load_bytes,
                    "forbidden_memory_or_reduction":stats.forbidden_memory_or_reduction,
                    "forbidden":["LDL","STL","ATOM","RED","REDUX"]
                })
            );
            Ok(stats)
        })();
    ...
    fn compile_source(source: String, label: &str) -> Result<(String, String), String> {
        ...
        Ok((source_sha, ptx.to_src()))
    }

    fn enforce_code_footprint_gate(
        kind: &str,
        candidate: SassStats,
        retained: SassStats,
    ) -> Result<(), String> {
        let text_ratio = candidate.text_bytes as f64 / retained.text_bytes as f64;
        let ffma_ratio = candidate.static_ffma as f64 / retained.static_ffma as f64;
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnRolledDualChunkCodeFootprintV1",
                "kind":kind,
                "candidate_text_bytes":candidate.text_bytes,
                "retained_text_bytes":retained.text_bytes,
                "text_ratio":text_ratio,
                "text_ratio_max":ROLLED_MAX_TEXT_RATIO,
                "candidate_static_ffma":candidate.static_ffma,
                "retained_static_ffma":retained.static_ffma,
                "ffma_ratio":ffma_ratio,
                "ffma_ratio_max":ROLLED_MAX_STATIC_FFMA_RATIO,
                "candidate_registers":candidate.registers,
                "candidate_static_shared_bytes":candidate.static_shared,
                "candidate_stack_bytes":candidate.stack_bytes,
                "candidate_spill_store_bytes":candidate.spill_store_bytes,
                "candidate_spill_load_bytes":candidate.spill_load_bytes,
                "candidate_forbidden_memory_or_reduction":candidate.forbidden_memory_or_reduction,
                "timing_started":false
            })
        );
        if text_ratio > ROLLED_MAX_TEXT_RATIO
            || ffma_ratio > ROLLED_MAX_STATIC_FFMA_RATIO
            || candidate.registers > ROLLED_DUAL_MAX_REGISTERS
            || candidate.static_shared as usize != ROLLED_DUAL_STATIC_SHARED
            || candidate.stack_bytes != 0
            || candidate.spill_store_bytes != 0
            || candidate.spill_load_bytes != 0
            || candidate.forbidden_memory_or_reduction
        {
            return Err(format!(
                "{kind} compile-only stop: text_ratio={text_ratio:.6}, ffma_ratio={ffma_ratio:.6}, regs={}, shared={}, stack={}, spills={}/{}, forbidden={}",
                candidate.registers,
    ...
    fn compile_only_code_footprint_gate() -> Result<(), String> {
        let (_, rolled_raw_ptx) = compile_source(
            rolled_source::compose_raw_source()?,
            "rolled dual-chunk raw",
        )?;
        let (_, retained_raw_ptx) = compile_source(
            retained_dual_source::compose_raw_source(),
            "retained dual-chunk raw",
        )?;
        let (_, rolled_fused_ptx) = compile_source(
            rolled_source::compose_fused_source()?,
            "rolled dual-chunk fused",
        )?;
        let (_, retained_fused_ptx) = compile_source(
            retained_dual_source::compose_fused_source(),
            "retained dual-chunk fused",
        )?;
        let rolled_raw = validate_ptxas_and_sass(ROLLED_DUAL_RAW, &rolled_raw_ptx)?;
        let retained_raw =
            validate_ptxas_and_sass(retained_dual_source::DUAL_RAW_SYMBOL, &retained_raw_ptx)?;
        let rolled_fused = validate_ptxas_and_sass(ROLLED_DUAL_FUSED, &rolled_fused_ptx)?;
        let retained_fused = validate_ptxas_and_sass(RETAINED_DUAL_FUSED, &retained_fused_ptx)?;
        enforce_code_footprint_gate("raw", rolled_raw, retained_raw)?;
        enforce_code_footprint_gate("fused", rolled_fused, retained_fused)
    }
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        let rolled_dual_raw_source = rolled_source::compose_raw_source()?;
        let rolled_dual_raw_source_sha =
            format!("{:x}", Sha256::digest(rolled_dual_raw_source.as_bytes()));
        let rolled_dual_raw_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            rolled_dual_raw_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
                include_paths: cuda_include_paths(),
        ...
        .map_err(|error| format!("compile rolled dual-chunk raw: {error:?}"))?;
        let rolled_dual_raw_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(rolled_dual_raw_ptx.to_src()))
            .map_err(|error| format!("load rolled dual-chunk raw module: {error:?}"))?;
        let rolled_dual_raw = rolled_dual_raw_module
            .load_function(ROLLED_DUAL_RAW)
            .map_err(|error| format!("load {ROLLED_DUAL_RAW}: {error:?}"))?;
        let rolled_dual_fused_source = rolled_source::compose_fused_source()?;
        let rolled_dual_fused_source_sha =
            format!("{:x}", Sha256::digest(rolled_dual_fused_source.as_bytes()));
        let rolled_dual_fused_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            rolled_dual_fused_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
                include_paths: cuda_include_paths(),
        ...
        .map_err(|error| format!("compile rolled dual-chunk fused: {error:?}"))?;
        let rolled_dual_fused_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(rolled_dual_fused_ptx.to_src()))
            .map_err(|error| format!("load rolled dual-chunk fused module: {error:?}"))?;
        let rolled_dual_fused = rolled_dual_fused_module
            .load_function(ROLLED_DUAL_FUSED)
            .map_err(|error| format!("load {ROLLED_DUAL_FUSED}: {error:?}"))?;
        let retained_dual_fused_source = retained_dual_source::compose_fused_source();
        let retained_dual_fused_source_sha = format!(
            "{:x}",
            Sha256::digest(retained_dual_fused_source.as_bytes())
        );
        let retained_dual_fused_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            retained_dual_fused_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
                include_paths: cuda_include_paths(),
        ...
        .map_err(|error| format!("compile retained dual-chunk fused: {error:?}"))?;
        let retained_dual_fused_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(
                retained_dual_fused_ptx.to_src(),
            ))
            .map_err(|error| format!("load retained dual-chunk fused module: {error:?}"))?;
        let retained_dual_fused = retained_dual_fused_module
            .load_function(RETAINED_DUAL_FUSED)
            .map_err(|error| format!("load {RETAINED_DUAL_FUSED}: {error:?}"))?;
        Ok(Runtime {
            _device: device,
            ctx,
            _raw_module: module,
            _rolled_dual_raw_module: rolled_dual_raw_module,
            _rolled_dual_fused_module: rolled_dual_fused_module,
            _retained_dual_fused_module: retained_dual_fused_module,
            raw_fixed,
            rolled_dual_raw,
            rolled_dual_fused,
            retained_dual_fused,
            raw_source_sha,
            rolled_dual_raw_source_sha,
            rolled_dual_fused_source_sha,
            retained_dual_fused_source_sha,
        })
    }
    ...
    fn launch_rolled_dual_raw(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        if fixture.cell.batch == 0 {
            return Err("zero reduction has no rolled dual raw launch".into());
        }
        launch_transpose(runtime, fixture)?;
        let partial = fixture.candidate_partial.ptr();
        let transposed = fixture.transposed.ptr();
        let dy = fixture.dy.ptr();
        let params = fixture.cell.rolled_dual_params(1.0)?;
        let mut builder = runtime.ctx.stream.launch_builder(&runtime.rolled_dual_raw);
        builder.arg(&partial);
        builder.arg(&transposed);
        builder.arg(&dy);
        builder.arg(&params);
        unsafe { builder.launch(config(fixture.cell.fixed_grid(), (128, 1, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch {ROLLED_DUAL_RAW}: {error:?}"))
    }

    fn launch_rolled_dual_fused(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        if fixture.cell.batch == 0 {
            return launch_zero(runtime, fixture);
        }
        launch_transpose(runtime, fixture)?;
        let output = fixture.output.ptr();
        let transposed = fixture.transposed.ptr();
        let dy = fixture.dy.ptr();
        let params = fixture.cell.rolled_dual_params(fixture.alpha)?;
        let mut builder = runtime
            .ctx
            .stream
            .launch_builder(&runtime.rolled_dual_fused);
        builder.arg(&output);
        builder.arg(&transposed);
        builder.arg(&dy);
        builder.arg(&params);
        unsafe { builder.launch(config(fixture.cell.fixed_grid(), (128, 1, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch {ROLLED_DUAL_FUSED}: {error:?}"))
    }

    fn launch_retained_dual_fused(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        if fixture.cell.batch == 0 {
            return gpu_gemm_bi_backward_dw_grad(
                &runtime.ctx,
                &GradSlice::from_raw(
                    fixture.output.ptr(),
        ...
        let transposed = fixture.transposed.ptr();
        let dy = fixture.dy.ptr();
        let params = fixture.cell.retained_dual_params(fixture.alpha)?;
        let mut builder = runtime
            .ctx
            .stream
            .launch_builder(&runtime.retained_dual_fused);
        builder.arg(&output);
        builder.arg(&transposed);
        builder.arg(&dy);
        builder.arg(&params);
        unsafe { builder.launch(config(fixture.cell.fixed_grid(), (128, 1, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch {RETAINED_DUAL_FUSED}: {error:?}"))
    }
    ...
    fn launch(
        ...
        if fixture.cell.batch == 0 {
            return match arm {
                ...
                Arm::Retained => gpu_gemm_bi_backward_dw_grad(
                    &runtime.ctx,
                    &GradSlice::from_raw(
                        fixture.output.ptr(),
                        fixture.cell.k_out * fixture.cell.n_out,
        ...
        match arm {
            Arm::Candidate => launch_rolled_dual_fused(runtime, fixture),
            Arm::Reference => {
                launch_reference_partials(runtime, fixture)?;
                launch_reducer(runtime, fixture, fixture.reference_partial.ptr())
            }
            Arm::Retained => launch_retained_dual_fused(runtime, fixture),
            Arm::Fast => {
    ...
    fn compare_raw_partials(
        ...
        launch_retained_partials(runtime, fixture, candidate).map_err(&harness)?;
        ...
        let chunks = fixture.cell.chunks().map_err(&harness)?;
        for index in raw_plane_indices(fixture.cell).map_err(&harness)? {
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnFusedFinalizeRawPartialV1",
                    "corpus":corpus,
    ...
    fn compare_rolled_dual_raw_partials(
        runtime: &Runtime,
        fixture: &mut Fixture,
        corpus: &str,
    ) -> Result<(), RawPartialError> {
        ...
        fixture.reset(&runtime.ctx).map_err(&harness)?;
        launch_reference_partials(runtime, fixture).map_err(&harness)?;
        launch_rolled_dual_raw(runtime, fixture).map_err(&harness)?;
        let reference = fixture
            .reference_partial
            .bits(&runtime.ctx)
            .map_err(&harness)?;
        ...
        for index in raw_plane_indices(fixture.cell).map_err(&harness)? {
                ...
                .position(|(a, b)| a != b)
            {
                let word = begin + relative;
                return Err(RawPartialError::Mismatch(format!(
                    "{corpus} {ROLLED_DUAL_RAW} raw partial fc={index} m=[{},{}), word={relative}: {:08x} != {:08x}",
                    chunks[index].begin,
                    chunks[index].begin + chunks[index].len,
                    actual[word],
                    reference[word]
                )));
            }
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnRolledDualChunkRawPartialV1",
                    "corpus":corpus,
                    "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                    "candidate":ROLLED_DUAL_RAW,
                    "fc":index,
                    "m_begin":chunks[index].begin,
                    "m_end":chunks[index].begin+chunks[index].len,
                    "words":stride,
                    "oracle":PARTIAL,
                    "exact_bits":true
                })
    ...
    fn prove_rolled_dual_raw_accumulators(runtime: &Runtime) -> Result<(), String> {
        for (corpus, exceptional) in FUSED_RAW_TARGET_CORPORA {
            let mut fixture = Fixture::new(&runtime.ctx, D768_IN, 1.0, exceptional)?;
            compare_rolled_dual_raw_partials(runtime, &mut fixture, corpus)
    ...
    fn select_candidate(runtime: &Runtime) -> Result<CandidateFixed, String> {
        ...
        let production_exceptional = compare_raw_partials(
            runtime,
            &mut exceptional,
            CandidateFixed::Production,
            "exceptional_payload",
        ...
        // The fused chunk1 consumes the accumulator before the production
        // alpha=1 store. Prove that exact value unconditionally through the
        // raw-store clone, regardless of which implementation chunk0 selects.
        for (corpus, inject_exceptional) in FUSED_RAW_TARGET_CORPORA {
            let fixture = if inject_exceptional {
                &mut exceptional
            } else {
                &mut finite
            };
            compare_raw_partials(runtime, fixture, CandidateFixed::RawStore, corpus)
                .map_err(|error| error.to_string())?;
        }

        match production_exceptional {
            Ok(()) => Ok(CandidateFixed::Production),
            Err(RawPartialError::Mismatch(production_error)) => {
                println!(
                    "{}",
                    json!({
                        "schema":"MambaBiF32TnFusedFinalizeRawStoreFallbackV1",
    ...
    unsafe fn require_retained_graph(
        ...
        if nodes.len() != 4 {
            return Err(format!(
                "retained comparator has {} nodes, expected four",
                nodes.len()
            ));
        }
        let transpose = nodes
            ...
            .ok_or_else(|| format!("retained comparator omitted {TRANSPOSE}"))?;
        require_launch(
            transpose,
            TRANSPOSE,
        ...
        let reducer = nodes
            .iter()
            .find(|node| node.symbol == REDUCER)
            .ok_or_else(|| format!("retained comparator omitted {REDUCER}"))?;
        require_launch(
            reducer,
            REDUCER,
            fixture.cell.reducer_grid(),
        ...
        let first = ordered_fixed[0].ok_or_else(|| "missing retained chunk0".to_string())?;
        let second = ordered_fixed[1].ok_or_else(|| "missing retained chunk1".to_string())?;
        unsafe {
            require_chain(
                graph,
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnRetainedCopyPlanGraphV1",
                "arm":"Retained",
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
    ...
    unsafe fn require_rolled_dual_graph(
        graph: &CudaGraph,
        fixture: &Fixture,
    ) -> Result<(), String> {
        if fixture.cell.batch == 0 {
            return unsafe { require_retained_graph(graph, fixture, CandidateFixed::Production) };
        }
        let nodes = unsafe { observed_kernels(graph) }?;
        if nodes.len() != 2 {
            return Err(format!(
                "rolled dual-chunk candidate has {} nodes, expected two",
                nodes.len()
            ));
        }
        let transpose = nodes
            .iter()
            .find(|node| node.symbol == TRANSPOSE)
            .ok_or_else(|| format!("rolled dual-chunk candidate omitted {TRANSPOSE}"))?;
        let fused = nodes
            .iter()
            .find(|node| node.symbol == ROLLED_DUAL_FUSED)
            .ok_or_else(|| format!("rolled dual-chunk candidate omitted {ROLLED_DUAL_FUSED}"))?;
        require_launch(
            transpose,
            TRANSPOSE,
            fixture.cell.transpose_grid(),
        ...
        if unsafe { graph_arg::<u64>(transpose, 0) }? != fixture.transposed.ptr()
            ...
            || unsafe { graph_arg::<i32>(transpose, 3) }? != fixture.cell.k_out as i32
        {
            return Err("rolled dual-chunk transpose ABI/arguments changed".into());
        }
        if unsafe { graph_arg::<u64>(fused, 0) }? != fixture.output.ptr()
            || unsafe { graph_arg::<u64>(fused, 1) }? != fixture.transposed.ptr()
            || unsafe { graph_arg::<u64>(fused, 2) }? != fixture.dy.ptr()
            || unsafe { graph_arg::<RolledDualChunkParams>(fused, 3) }?
                != fixture.cell.rolled_dual_params(fixture.alpha)?
        {
            return Err("rolled dual-chunk fused ABI/arguments changed".into());
        }
        unsafe { require_chain(graph, &[transpose.node, fused.node], "rolled dual-chunk") }?;
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnRolledDualChunkGraphV1",
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "nodes":[TRANSPOSE,ROLLED_DUAL_FUSED],
                "edges":[[TRANSPOSE,ROLLED_DUAL_FUSED]],
                "order":["transpose","rolled_dual_chunk_fused_finalize"]
            })
        );
        Ok(())
    }

    unsafe fn require_retained_fused_graph(
        graph: &CudaGraph,
        fixture: &Fixture,
        candidate: CandidateFixed,
    ) -> Result<(), String> {
        ...
        if nodes.len() != 2 {
            return Err(format!(
                "retained dual-chunk pipeline has {} nodes, expected two",
                nodes.len()
            ));
        }
        let transpose = nodes
            .iter()
            .find(|node| node.symbol == TRANSPOSE)
            .ok_or_else(|| format!("retained dual-chunk omitted {TRANSPOSE}"))?;
        let fused = nodes
            .iter()
            .find(|node| node.symbol == RETAINED_DUAL_FUSED)
            .ok_or_else(|| format!("retained dual-chunk omitted {RETAINED_DUAL_FUSED}"))?;
        require_launch(
            transpose,
            TRANSPOSE,
            fixture.cell.transpose_grid(),
        ...
        if unsafe { graph_arg::<u64>(transpose, 0) }? != fixture.transposed.ptr()
            ...
            || unsafe { graph_arg::<i32>(transpose, 3) }? != fixture.cell.k_out as i32
        {
            return Err("retained dual-chunk transpose ABI/arguments changed".into());
        }
        if unsafe { graph_arg::<u64>(fused, 0) }? != fixture.output.ptr()
            || unsafe { graph_arg::<u64>(fused, 1) }? != fixture.transposed.ptr()
            || unsafe { graph_arg::<u64>(fused, 2) }? != fixture.dy.ptr()
            || unsafe { graph_arg::<RetainedDualChunkParams>(fused, 3) }?
                != fixture.cell.retained_dual_params(fixture.alpha)?
        {
            return Err("retained dual-chunk fused ABI/arguments changed".into());
        }
        unsafe { require_chain(graph, &[transpose.node, fused.node], "retained dual-chunk") }?;
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDualChunkRetainedGraphV1",
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "nodes":[TRANSPOSE,RETAINED_DUAL_FUSED],
                "edges":[[TRANSPOSE,RETAINED_DUAL_FUSED]],
                "order":["transpose","dual_chunk_fused_finalize"]
            })
        );
        Ok(())
    }
    ...
    unsafe fn require_fast_graph(graph: &CudaGraph) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiF32TnDualChunkGraphV1","arm":"Fast","node_count":count,"abi":"opaque","timing":"whole_graph"})
        );
        Ok(())
    }
    ...
    fn output_after(
        ...
        match arm {
            Arm::Candidate if fixture.cell.batch > 0 => {
                fixture.validate_transpose(&runtime.ctx)?;
                fixture.validate_candidate_partial_unchanged(&runtime.ctx)?;
            }
    ...
    fn check_exact_case(
        ...
        unsafe {
            require_rolled_dual_graph(&candidate_graph, fixture)?;
            match oracle {
                Arm::Retained if fixture.cell.batch > 0 => {
                    require_retained_fused_graph(&oracle_graph, fixture, candidate)?
                }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnRolledDualChunkBitsV1",
                "case":label,
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "alpha_bits":fixture.alpha.to_bits(),
                "candidate":ROLLED_DUAL_FUSED,
    ...
    fn resources(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedFinalizeResourceV1",
                "symbol":symbol,
                "threads":threads,
                "registers":registers,
                "local_bytes":local,
    ...
    fn rolled_dual_candidate_resources(
        function: &CudaFunction,
        symbol: &str,
    ) -> Result<(), String> {
        let registers = function
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnRolledDualChunkHardResourceV1",
                "symbol":symbol,
                "threads":128,
                "registers":registers,
                "registers_max":ROLLED_DUAL_MAX_REGISTERS,
                "local_bytes":local,
                "static_shared_bytes":shared,
                "static_shared_expected":ROLLED_DUAL_STATIC_SHARED,
                "dynamic_shared_bytes":0,
                "occupancy":occupancy,
                "occupancy_min":ROLLED_DUAL_MIN_OCCUPANCY
            })
        );
        if registers > ROLLED_DUAL_MAX_REGISTERS as i32
            || local != 0
            || shared as usize != ROLLED_DUAL_STATIC_SHARED
            || occupancy < ROLLED_DUAL_MIN_OCCUPANCY
        {
            return Err(format!(
    ...
    fn cuda132_compile_only_rolled_dual_chunk_code_footprint() -> Result<(), String> {
        match compile_only_code_footprint_gate() {
            Ok(()) => println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnRolledDualChunkCompileDecisionV1",
                    "decision":"continue_to_gpu_resource_gate",
                    "timing_started":false,
            ...
            Err(reason) => println!(
                ...
                json!({
                    "schema":"MambaBiF32TnRolledDualChunkCompileDecisionV1",
                    "decision":"stop_code_footprint_or_local_gate",
                    "reason":reason,
                    "timing_started":false,
                    "gpu_context_created":false
                })
    ...
    #[test]
    #[ignore = "Ada CUDA13.2 discovery: exact F32 TN d768-in two-node rolled dual-chunk fused"]
    fn ada_f32_tn_rolled_dual_chunk_fused_d768_in_once7() -> Result<(), String> {
        if cfg!(debug_assertions) {
            return Err("performance requires release".into());
        }
        if let Err(reason) = compile_only_code_footprint_gate() {
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnRolledDualChunkCompileStopV1",
                    "candidate":[ROLLED_DUAL_RAW,ROLLED_DUAL_FUSED],
                    "reason":reason,
                    "timing_started":false,
                    "decision":"stop_code_footprint_gate",
                    "promotion":false
                })
            );
            return Ok(());
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("f32-tn-rolled-dual-chunk/pre", 2_000)?;
        } else {
            quiet.require_pre_context("f32-tn-rolled-dual-chunk/pre")?;
        }
        let runtime = new_runtime()?;

        for (function, symbol) in [
            (&runtime.rolled_dual_raw, ROLLED_DUAL_RAW),
            (&runtime.rolled_dual_fused, ROLLED_DUAL_FUSED),
        ] {
            if let Err(reason) = rolled_dual_candidate_resources(function, symbol) {
                println!(
                    "{}",
                    json!({
                        "schema":"MambaBiF32TnRolledDualChunkResourceStopV1",
                        "candidate":[ROLLED_DUAL_RAW,ROLLED_DUAL_FUSED],
                        "reason":reason,
                        "timing_started":false,
                        "decision":"stop_resource_or_sass_gate",
                        "promotion":false
                    })
                );
                if idle_resident_mode {
                    quiet.require_idle_resident("f32-tn-rolled-dual-chunk/resource-stop", 256)?;
                } else {
                    quiet.verify_post_cohort("f32-tn-rolled-dual-chunk/resource-stop")?;
                }
                return Ok(());
            }
        }

        // Prove the committed two-node N64-G8 dual-chunk comparator before any
        // candidate output or timing observation.
        let mut retained_proof = Fixture::new(&runtime.ctx, D768_IN, 1.0, false)?;
        let retained_proof_graph = capture(
            &runtime,
            &mut retained_proof,
            Arm::Retained,
            CandidateFixed::Production,
        ...
        unsafe {
            require_retained_fused_graph(
                &retained_proof_graph,
                &retained_proof,
                CandidateFixed::Production,
            )
        }?;
        drop(retained_proof_graph);
        drop(retained_proof);

        // Select the exact retained chunk0 implementation and independently
        // prove both register chains of the new raw twin before finalization.
        let candidate = select_candidate(&runtime)?;
        prove_rolled_dual_raw_accumulators(&runtime)?;
        let fixed = fixed_function(&runtime, candidate)?;
        resources(fixed, candidate.symbol(), 128, 32_768)?;
        resources(
            &runtime.retained_dual_fused,
            RETAINED_DUAL_FUSED,
            128,
            32_768,
        )?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnRolledDualChunkArtifactsV1",
                "artifacts":format!("{:?}",runtime.ctx.kernels.artifact_set_identity()),
                "raw_store_source_sha":runtime.raw_source_sha,
                "rolled_dual_raw_source_sha":runtime.rolled_dual_raw_source_sha,
                "rolled_dual_fused_source_sha":runtime.rolled_dual_fused_source_sha,
                "retained_dual_fused_source_sha":runtime.retained_dual_fused_source_sha,
                "candidate_nodes":[TRANSPOSE,ROLLED_DUAL_FUSED],
                "retained_nodes":[TRANSPOSE,RETAINED_DUAL_FUSED],
        ...
        let (_, exact_repeated) =
            check_exact_case(&runtime, &mut target, candidate, Arm::Retained, "target")?;
        ...
        compare_rolled_dual_raw_partials(&runtime, &mut tail, "tail_chunk_dual_accumulators")
            .map_err(|error| error.to_string())?;
        ...
        compare_rolled_dual_raw_partials(
            &runtime,
            &mut exceptional,
            "exceptional_tail_dual_accumulators",
        )
        .map_err(|error| error.to_string())?;
        check_exact_case(
            &runtime,
        ...
        check_exact_case(&runtime, &mut zero, candidate, Arm::Retained, "k0")?;
        drop(zero);

        // The two-node transpose plus rolled kernel is timed as one logical GEMM. Resets,
        ...
        unsafe { require_rolled_dual_graph(&candidate_graph, &target) }?;
        let mut retained_win = None;
        let mut fast_win = None;
        let mut comparators = vec![Arm::Retained];
        let mut comparator_index = 0;
        while comparator_index < comparators.len() {
            let comparator = comparators[comparator_index];
            if comparator == Arm::Fast {
            ...
            unsafe {
                match comparator {
                    Arm::Retained => {
                        require_retained_fused_graph(&comparator_graph, &target, candidate)?
                    }
            ...
            if idle_resident_mode {
                quiet.require_idle_resident("f32-tn-rolled-dual-chunk/timed", 256)?;
            } else {
                quiet.require_cohort("f32-tn-rolled-dual-chunk/timed")?;
            }
            ...
            for graph_path in [false, true] {
                for candidate_endpoints in [true, false] {
                    ...
                    println!(
                        "{}",
                        json!({
                            "schema":"MambaBiF32TnRolledDualChunkScreenV1",
                            "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                            "comparator":if comparator == Arm::Retained {"N64_G8_DUAL_COPYPLAN_FUSED"} else {"Fast"},
                            ...
                            "candidate_timed_nodes":[TRANSPOSE,ROLLED_DUAL_FUSED],
            ...
            let strict_win = strata
                .iter()
                .all(|values| values[0] < 0.95 && values[1] < 0.95);
            match comparator {
                Arm::Retained => {
                    retained_win = Some(strict_win);
                    if strict_win {
                        comparators.push(Arm::Fast);
                    }
                }
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnRolledDualChunkDecisionV1",
                    "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                    "comparator":if comparator == Arm::Retained {"N64_G8_DUAL_COPYPLAN_FUSED"} else {"Fast"},
                    "strata":strata,
                    "strict_win":strict_win,
                    "decision":match (comparator,strict_win) {
                        (Arm::Retained,true)=>"rolled_dual_chunk_beats_retained",
                        (Arm::Retained,false)=>"stop_no_retry",
        ...
        let retained_win = retained_win.ok_or("missing retained decision")?;
        let fast_win = fast_win.unwrap_or(false);
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnRolledDualChunkFinalDecisionV1",
                "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                "candidate":ROLLED_DUAL_FUSED,
                "retain_against_n64_g8_fused":retained_win,
                "fast_screened":retained_win,
                "fast_win":fast_win,
                "decision":if retained_win {
                    if fast_win {"retain_fast_winner"} else {"retain_best_not_fast"}
                } else {"stop_no_retry"},
                "idle_resident_mode":idle_resident_mode,
                "promotion":false
            })
        );
        if idle_resident_mode {
            quiet.require_idle_resident("f32-tn-rolled-dual-chunk/post", 256)?;
        } else {
            quiet.verify_post_cohort("f32-tn-dual-chunk/post")?;
        }
        Ok(())
    }
}
