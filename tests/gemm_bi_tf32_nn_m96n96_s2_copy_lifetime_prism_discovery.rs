//! Test-only Ada TF32 NN canonical-Prism M96N96/S2 copy-lifetime discovery.
//! Production kernels and dispatch remain untouched.

#[path = "support/triad_tf32_nn_m96n96_s2_copy_lifetime_source.rs"]
#[allow(dead_code)]
...
#[test]
fn source_adapter_is_reversible_and_eliminates_only_persistent_copy_state() {
    ...
    assert!(!candidate.contains("tf32n96_copy_plan("));
    assert!(!candidate.contains("tf32n96_advance_plan("));
    assert!(candidate.contains("tf32n96_stage_slice_ondemand("));
...
fn prism_grids_and_resource_arithmetic_are_independent() {
    ...
    assert_eq!(candidate_source::REMOVED_PERSISTENT_PLAN_SLOTS, 37);
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_TF32_NN_M96N96_S2_COPY_LIFETIME_PRISM_DISCOVERY";
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "m96n96_s2_copy_lifetime",
    ...
    fn ptxas_and_sass(symbol: &str, ptx: &str, shared: usize) -> Result<SassStats, String> {
        ...
        let result = (|| {
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiTriadTf32NnM96N96S2CopyLifetimeSassV1",
    ...
    fn compile_only_gate() -> Result<(), String> {
        ...
        let candidate = ptxas_and_sass(
            ...
            &compile_ptx(&candidate_body, "M96N96/S2 copy-lifetime candidate")?,
            CANDIDATE_SHARED,
        )?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2CopyLifetimeCompileDecisionV1",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if cfg!(debug_assertions) {
            return Err("M96N96/S2 copy-lifetime timing requires --release".into());
        }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2CopyLifetimeIdentityV1",
                ...
                "change":"persistent_copy_plan_to_per_issue_scoped_addresses",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2CopyLifetimeResourceV1","symbol":symbol,
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32NnM96N96S2CopyLifetimeGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 1 {
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32NnM96N96S2CopyLifetimeGraphV1","arm":arm.name(),"symbol":symbol,"grid":expected_grid,"block":expected_config.block_dim,"shared":expected_config.shared_mem_bytes,"timing":"whole_graph"})
            );
        }
        Ok(())
    }
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32NnM96N96S2CopyLifetimeBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2CopyLifetimeTargetBitsV1",
    ...
    fn prepare_fast(runtime: &Runtime, case: Case, prepared: &mut Prepared) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2CopyLifetimeFastBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2CopyLifetimeScreenV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid(Arm::Candidate)?,
                "candidate":"m96n96_s2_copy_lifetime","comparator":comparator.name(),"path":path.name(),
    ...
    fn cuda132_compile_only_m96n96_s2_copy_lifetime_resources_and_sass() -> Result<(), String> {
        compile_only_gate()
    }

    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 NN M96N96/S2 copy-lifetime Prism once3 -> once7 -> Fast"]
    fn ada_tf32_nn_m96n96_s2_copy_lifetime_prism_protocol() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-m96n96-s2-copy-lifetime/pre", 1_800)?;
        } else {
            quiet.require_pre_context("tf32-nn-m96n96-s2-copy-lifetime/pre")?;
        }
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-m96n96-s2-copy-lifetime/retained", 256)?;
        } else {
            quiet.require_cohort("tf32-nn-m96n96-s2-copy-lifetime/retained")?;
        }
        ...
        let fast_strata = if retained_win {
            ...
            if idle_resident_mode {
                quiet.require_idle_resident("tf32-nn-m96n96-s2-copy-lifetime/fast", 256)?;
            } else {
                quiet.require_cohort("tf32-nn-m96n96-s2-copy-lifetime/fast")?;
            }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnM96N96S2CopyLifetimeDecisionV1","cell":case.label,
        ...
        drop(prepared);
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-nn-m96n96-s2-copy-lifetime/post", 256)?;
        } else {
            quiet.verify_post_cohort("tf32-nn-m96n96-s2-copy-lifetime/post")?;
        }
        Ok(())
    }
}
