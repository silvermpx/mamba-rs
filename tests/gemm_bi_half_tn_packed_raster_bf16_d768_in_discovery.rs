//! Test-only Ada BF16 TN d768-in discovery for a packed 4M-by-1N CTA raster.

#[path = "support/triad_half_tn_packed_raster_source.rs"]
#[allow(dead_code)]
...
#[test]
fn harness_scope_freezes_bf16_packed_raster_physics_and_retained_comparator() {
    assert_eq!(candidate_source::TARGET, (2_048, 768, 3_072));
    assert_eq!(candidate_source::TARGET_GRID, (4, 48, 3));
    assert_eq!(candidate_source::TARGET_TILE_COUNT, 576);
...
mod cuda_suite {
    ...
    use super::{QUALIFICATION_THRESHOLD, SCOUT_THRESHOLD, TimingPhase, may_run_fast, windows_for};
    ...
    const EXCEPTIONAL: Shape = Shape::new(64, 96, 64, 1.0, "exceptional_full_tile");
    ...
    impl Shape {
        ...
        fn candidate_grid(self) -> (u32, u32, u32) {
            if (self.reduction, self.k_out, self.n) == candidate_source::TARGET {
                candidate_source::TARGET_GRID
            } else {
                (self.retained_grid(), 1, 1)
            }
        }
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "packed_raster4_bk64_s2_regpipe_vec2",
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        let (candidate_ptx, candidate_source_sha) =
            compile_ptx_only(&candidate, "packed-raster candidate compile-only")?;
    ...
    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_BF16_TN_PACKED_RASTER_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_BF16_TN_PACKED_RASTER_DISCOVERY=1".into());
        }
        let device = GpuDevice::new(0)?;
        let identity = device.identity();
        if identity.compute_capability != (8, 9) || identity.multiprocessor_count != 142 {
            return Err(format!(
                "half TN packed-raster discovery requires CC8.9/142-SM Ada, got {:?}/{}",
        ...
        if compiler.nvrtc_version != (13, 2) || compiler.target.as_str() != "sm_89" {
            return Err(format!(
                "half TN packed-raster discovery requires CUDA13.2/sm_89, got {compiler:?}"
            ));
        }
        let production = include_str!("../kernels/gemm_bi_triad/sm80.cu");
            ...
            Arm::Candidate | Arm::Retained => {
                ...
                unsafe {
                    builder.launch(LaunchConfig {
                        grid_dim: if arm == Arm::Candidate {
                            shape.candidate_grid()
                        } else {
                            (shape.retained_grid(), 1, 1)
                        },
                        block_dim: (
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function) in [
            ...
            let (expected_static_shared, required_occupancy, threads, register_cap) = match arm {
                Arm::Candidate => (
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfTnPackedRasterResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"register_cap\":{},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":0,\"block_threads\":{},\"active_ctas\":{active},\"required_active_ctas\":{}}}",
                arm.name(),
                register_cap,
                threads,
                required_occupancy,
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnPackedRasterBitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha_bits\":{},\"exceptional\":{exceptional},\"candidate_vs_retained_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.reduction,
            shape.k_out,
            shape.n,
    ...
    fn timed_pair(
        ...
        for window in 0..windows {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiHalfTnPackedRasterTimingV1\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"windows\":{windows},\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw,
    ...
    fn finish_quiet(runtime: Runtime, quiet: &QuietGpu) -> Result<(), String> {
        drop(runtime);
        quiet
            .verify_post_cohort("half-tn-packed-raster/post")
            .map(|_| ())
    }

    #[test]
    #[ignore = "requires exclusive 142-SM Ada and CUDA13.2"]
    fn ada_bf16_tn_packed_raster_d768_in_scout3_then_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("half-tn-packed-raster/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("half-tn-packed-raster/cohort")?;
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnPackedRasterScoutDecisionV1\",\"candidate\":\"{}\",\"comparator\":\"{}\",\"windows\":{scout_windows},\"threshold\":{SCOUT_THRESHOLD},\"strata\":{:?},\"pass\":{scout_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            scout,
            if scout_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnPackedRasterRetainedDecisionV1\",\"candidate\":\"{}\",\"comparator\":\"{}\",\"windows\":{qualification_windows},\"threshold\":{QUALIFICATION_THRESHOLD},\"strata\":{:?},\"pass\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_once7,
            if retained_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnPackedRasterFinalDecisionV1\",\"shape\":[2048,768,3072],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"candidate_ptx_sha\":\"{}\",\"retained_ptx_sha\":\"{}\",\"scout_strata\":{:?},\"retained_once7_strata\":{:?},\"fast_once7_strata\":{:?},\"strict_fast_p50_p95_lt_0_99\":{fast_pass},\"decision\":\"{}\",\"promotion\":false}}",
