#[path = "support/triad_half_nt_m64n192_full_domain_source.rs"]
#[allow(dead_code)]
...
fn candidate_source_has_unique_m64n192_geometry() {
    ...
    assert!(source.contains(candidate_source::RETAINED_SYMBOL));
    assert!(source.contains("C, A, B, alpha, 2048, 768, 1536);"));
    assert!(candidate_source::is_exact_target(2_048, 1_536, 768));
    assert!(!candidate_source::is_exact_target(2_048, 1_535, 768));
}
...
fn candidate_resource_and_target_geometry_are_frozen() {
    ...
    assert_eq!(grid, candidate_source::TARGET_GRID);
}
...
mod cuda_suite {
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "m64n192_full_domain_bk64_s3",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_F16_NT_M64N192_FULL_DOMAIN_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_F16_NT_M64N192_FULL_DOMAIN_DISCOVERY=1".into());
        }
    ...
    fn launch(runtime: &Runtime, fixture: &Fixture, arm: Arm) -> Result<(), String> {
        ...
        match arm {
            Arm::Candidate | Arm::Retained => {
                ...
                let exact_candidate = arm == Arm::Candidate
                    && candidate_source::is_exact_target(shape.m, shape.k_out, shape.reduction);
                let function = if exact_candidate {
                    &runtime.candidate
                } else {
                    &runtime.retained
                };
    ...
    fn validate_kernel_graph(graph: &CudaGraph, fixture: &Fixture, arm: Arm) -> Result<(), String> {
        ...
        let expected_symbol = match arm {
            Arm::Candidate
                if candidate_source::is_exact_target(
                    fixture.shape.m,
                    fixture.shape.k_out,
                    fixture.shape.reduction,
                ) =>
            {
                candidate_source::SYMBOL.to_owned()
            }
            Arm::Candidate | Arm::Retained => candidate_source::RETAINED_SYMBOL.to_owned(),
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function, shared, occupancy) in [
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfNtM64N192FullDomainResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":{shared},\"active_ctas\":{active},\"required_active_ctas\":{occupancy}}}",
                arm.name()
            );
        }
        Ok(())
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N192FullDomainBitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha\":{},\"exceptional\":{},\"candidate_vs_measured_s3_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.m,
            shape.k_out,
            shape.reduction,
    ...
    fn timed_pair(
        ...
        for window in 0..windows {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiHalfNtM64N192FullDomainTimingV1\",\"phase\":\"{phase}\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw
    ...
    fn ada_f16_nt_m64n192_full_domain_d768_out_scout_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("half-nt-m64n192-full-domain/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("half-nt-m64n192-full-domain/cohort")?;
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N192FullDomainScoutDecisionV1\",\"shape\":[2048,1536,768],\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{scout_pass},\"decision\":\"{}\"}}",
            scout_strata,
            if scout_pass {
                "advance_to_once7"
            } else {
        ...
        if !scout_pass {
            drop(runtime);
            quiet.verify_post_cohort("half-nt-m64n192-full-domain/post")?;
            return Ok(());
        }
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N192FullDomainRetainedDecisionV1\",\"shape\":[2048,1536,768],\"candidate\":\"{}\",\"comparator\":\"{}\",\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_strata,
            if retained_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N192FullDomainFinalDecisionV1\",\"shape\":[2048,1536,768],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"retained_strata\":{:?},\"fast_strata\":{:?},\"strict_fast_p50_p95_lt_0_99\":{fast_pass},\"decision\":\"{}\",\"promotion\":false}}",
