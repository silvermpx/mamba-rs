//! Test-only Ada TF32 TN d768-out M64N64/S3 wave-quantization discovery.
...
const D768_OUT_TARGET: Target = Target {
    label: "d768_out_proj",
    shape: (2_048, 1_536, 768),
    candidate_grid: (288, 1, 1),
    retained_grid: (96, 1, 1),
    transpose_grid: (48, 64, 1),
};

#[test]
fn d768_out_target_has_wave_quantized_geometries() {
    assert_eq!(
        D768_OUT_TARGET,
        Target {
            label: "d768_out_proj",
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_TF32_TN_TRANSPOSE_RNA_M64N64_D768_OUT_DISCOVERY";
    ...
    const D768_OUT: Case = Case {
        label: D768_OUT_TARGET.label,
        shape: Shape {
            m: D768_OUT_TARGET.shape.0,
            k: D768_OUT_TARGET.shape.1,
            n: D768_OUT_TARGET.shape.2,
        },
    };
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N64D768OutIdentityV1",
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N64D768OutResourceV1","symbol":symbol,
    ...
    fn transpose_resource_gate(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N64D768OutTransformResourceV1",
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32TnTransposeRnaM64N64D768OutGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 2 {
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32TnTransposeRnaM64N64D768OutGraphV1","arm":arm.name(),"nodes":2,"gemm_symbol":expected_gemm,"transpose_symbol":expected_transpose,"gemm_grid":case.shape.grid(arm)?,"transpose_grid":case.shape.transpose_config()?.grid_dim,"timing":"whole_two_node_graph"})
            );
        }
        Ok(())
    }
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32TnTransposeRnaM64N64D768OutBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N64D768OutTargetBitsV1",
    ...
    fn prepare_fast(runtime: &Runtime, case: Case, prepared: &mut Prepared) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N64D768OutFastBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N64D768OutScreenV1","cell":case.label,
    ...
    fn run_target_protocol(
        ...
        let retained_label = format!(
            "tf32-tn-transpose-rna-m64n64-d768-out/{}/retained",
            case.label
        );
        let fast_label = format!("tf32-tn-transpose-rna-m64n64-d768-out/{}/fast", case.label);
        let post_label = format!("tf32-tn-transpose-rna-m64n64-d768-out/{}/post", case.label);
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaM64N64D768OutDecisionV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid(Arm::Candidate)?,
    ...
    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 TN pre-RNA M64N64/S3 d768-out once3 -> once7 -> Fast"]
    fn ada_tf32_tn_transpose_rna_m64n64_d768_out_protocol() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-tn-transpose-rna-m64n64-d768-out/pre", 1_800)?;
        } else {
            quiet.require_pre_context("tf32-tn-transpose-rna-m64n64-d768-out/pre")?;
        }
        let runtime = new_runtime()?;
        assert_eq!(
            D768_OUT.shape.grid(Arm::Candidate)?,
            D768_OUT_TARGET.candidate_grid
        );
        assert_eq!(
            D768_OUT.shape.grid(Arm::Retained)?,
            D768_OUT_TARGET.retained_grid
        );
        assert_eq!(
            D768_OUT.shape.transpose_config()?.grid_dim,
            D768_OUT_TARGET.transpose_grid
        );
        ...
        run_target_protocol(&runtime, &quiet, idle_resident_mode, D768_OUT)?;
        Ok(())
    }
}
