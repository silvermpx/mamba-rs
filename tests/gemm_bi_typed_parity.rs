...
#[path = "support/triad_half_tn_m64n128_regpipe_source.rs"]
mod triad_half_tn_m64n128_regpipe_source;
...
#[path = "support/triad_half_tn_regpipe_source.rs"]
mod triad_half_tn_regpipe_source;
#[path = "support/triad_half_tn_regpipe_vec2_sibling_plan.rs"]
mod triad_half_tn_regpipe_vec2_sibling_plan;
...
fn ada_half_symbol(arm: AdaHalfArm, dtype: WeightDtype) -> &'static str {
    match (arm, dtype) {
        ...
        (AdaHalfArm::FixedSm89Tc128Pipeline, WeightDtype::Bf16) => {
            "gemm_bi_nn_fixed_sm89_tc128_pipeline_v1_bf16"
        }
        (AdaHalfArm::FixedSm89Tc128Pipeline, WeightDtype::F16) => {
            "gemm_bi_nn_fixed_sm89_tc128_pipeline_v1_f16"
        }
        (AdaHalfArm::FixedSm89Tc128Swizzle, WeightDtype::Bf16) => {
            "gemm_bi_nn_fixed_sm89_tc128_swizzle_v1_bf16"
        }
        (AdaHalfArm::FixedSm89Tc128Swizzle, WeightDtype::F16) => {
...
fn ada_half_function(
    ...
    match arm {
        ...
        AdaHalfArm::FixedSm89Tc128Pipeline => t
            .ctx
            .kernels
            ...
            .ok_or_else(|| {
                t.ctx
                    ...
                    .clone()
                    .unwrap_or_else(|| "Fixed SM89 half pipeline holder is unavailable".into())
            }),
        AdaHalfArm::FixedSm89Tc128Swizzle => t
            .ctx
            .kernels
            ...
            .ok_or_else(|| {
                t.ctx
                    ...
                    .clone()
                    .unwrap_or_else(|| "Fixed SM89 half swizzle holder is unavailable".into())
            }),
...
impl AdaHalfFixture {
    ...
    fn output(&self, arm: AdaHalfArm) -> &TypedSubview {
        match arm {
            ...
            AdaHalfArm::PortableTc64
            | AdaHalfArm::FixedSm89Tc128Pipeline
            | AdaHalfArm::FixedSm89Tc128Swizzle
            | AdaHalfArm::FixedSm89Tc128S3 => &self.candidate,
        }
    }
...
fn validate_ada_half_tn_candidate_graph(
    graph: &CudaGraph,
    fixture: &AdaHalfTnFixture,
    candidate: &AdaHalfTnCandidate,
    expected_grid: u32,
    ...
    if nodes_result != sys::CUresult::CUDA_SUCCESS || count != 1 || node.is_null() {
        return Err(format!(
            "half TN candidate graph inventory changed: result={nodes_result:?} count={count}"
        ));
    }
    let mut params: sys::CUDA_KERNEL_NODE_PARAMS = unsafe { std::mem::zeroed() };
    let params_result = unsafe { sys::cuGraphKernelNodeGetParams_v2(node, &mut params) };
    if params_result != sys::CUresult::CUDA_SUCCESS {
        return Err(format!(
            "half TN candidate graph parameters: {params_result:?}"
        ));
    }
    let mut name = std::ptr::null();
    let name_result = unsafe { sys::cuFuncGetName(&mut name, params.func) };
    if name_result != sys::CUresult::CUDA_SUCCESS || name.is_null() {
        return Err(format!(
            "half TN candidate graph symbol: result={name_result:?} null={}",
            name.is_null()
        ));
    }
    let actual = unsafe { CStr::from_ptr(name) }
        .to_str()
        .map_err(|error| format!("half TN candidate graph symbol UTF-8: {error}"))?;
    let expected = candidate.symbol(fixture.dtype);
    let config = (
        (params.gridDimX, params.gridDimY, params.gridDimZ),
        (params.blockDimX, params.blockDimY, params.blockDimZ),
    ...
    if params.kernelParams.is_null() {
        return Err("half TN candidate graph arguments are null".into());
    }
    for index in 0..7 {
        if unsafe { *params.kernelParams.add(index) }.is_null() {
            return Err(format!(
                "half TN candidate graph argument {index} storage is null"
            ));
        }
    }
    Ok(())
...
fn validate_ada_half_nt_fixed_s3_graph(
    ...
    if nodes_result != sys::CUresult::CUDA_SUCCESS || count != 1 || node.is_null() {
        return Err(format!(
            "half NT Fixed S3 graph inventory changed: result={nodes_result:?} count={count}"
        ));
    }
    let mut params: sys::CUDA_KERNEL_NODE_PARAMS = unsafe { std::mem::zeroed() };
    let params_result = unsafe { sys::cuGraphKernelNodeGetParams_v2(node, &mut params) };
    if params_result != sys::CUresult::CUDA_SUCCESS {
        return Err(format!(
            "half NT Fixed S3 graph parameters: {params_result:?}"
        ));
    }
    let mut name = std::ptr::null();
    let name_result = unsafe { sys::cuFuncGetName(&mut name, params.func) };
    if name_result != sys::CUresult::CUDA_SUCCESS || name.is_null() {
        return Err(format!(
            "half NT Fixed S3 graph symbol: result={name_result:?} null={}",
            name.is_null()
        ));
    }
    let actual = unsafe { CStr::from_ptr(name) }
        .to_str()
        .map_err(|error| format!("half NT Fixed S3 graph symbol UTF-8: {error}"))?;
    let suffix = match dtype {
        WeightDtype::Bf16 => "bf16",
        WeightDtype::F16 => "f16",
        WeightDtype::F32 => return Err("half NT Fixed S3 graph requires half dtype".into()),
    };
    let expected = format!("{symbol_prefix}{suffix}");
    let config = (
        (params.gridDimX, params.gridDimY, params.gridDimZ),
        (params.blockDimX, params.blockDimY, params.blockDimZ),
        params.sharedMemBytes,
    ...
    if params.kernelParams.is_null() {
        return Err("half NT Fixed S3 graph arguments are null".into());
    }
    for index in 0..7 {
        if unsafe { *params.kernelParams.add(index) }.is_null() {
            return Err(format!(
                "half NT Fixed S3 graph argument {index} storage is null"
            ));
        }
    }
    Ok(())
            ...
            for repeat in 0..2 {
                for (arm, graph) in [
                    ...
                    let schedule = match arm {
                        ...
                        AdaHalfArm::FixedSm89Tc128Pipeline => "FixedSm89Tc128Pipeline",
                        AdaHalfArm::FixedSm89Tc128Swizzle => "FixedSm89Tc128Swizzle",
...
fn observe_ada_half_nn_aligned_candidate(
    ...
    match candidate {
        ...
        .map(|value| value.0),
        AdaHalfNnAlignedCandidate::Loaded(arm) => {
            observe_ada_half_many(t, fixture, arm, graph, path, gemms, Some(expected))
                .map(|value| value.0)
        }
...
fn observe_ada_half_nn_aligned_comparator(
    ...
    match comparator {
        ...
        .map(|value| value.0),
        AdaHalfNnAlignedComparator::FixedS3 => observe_ada_half_many(
            t,
            fixture,
            AdaHalfArm::FixedSm89Tc128S3,
            current_graph,
...
fn screen_ada_half_nn_aligned_pair(
    ...
    let schema = match candidate {
        ...
        AdaHalfNnAlignedCandidate::Loaded(_) => "MambaBiHalfNnLoadedAlignedScreenV1",
...
fn run_ada_half_nn_loaded_pipeline_swizzle_batch() -> Result<(), String> {
    assert!(
        !cfg!(debug_assertions),
        "loaded Ada half NN pipeline/swizzle discovery requires --release"
    );
    let quiet = QuietGpu::for_cuda_ordinal(0)?;
    let _pre = quiet.require_pre_context("half-nn-loaded-pipeline-swizzle/pre-context")?;
    let t = Ctx::new_ada()?;
    let compiler = t.ctx.kernels.compiler_identity();
    if compiler.nvrtc_version != (13, 2) {
        return Err(format!(
            "loaded Ada half NN pipeline/swizzle requires CUDA13.2, found {:?}",
            compiler.nvrtc_version
        ));
    }
    let _cohort = quiet.require_cohort("half-nn-loaded-pipeline-swizzle/cohort")?;
    let dtype = WeightDtype::F16;
    let cell = "d768_in_proj";
    let candidates = [
        AdaHalfArm::FixedSm89Tc128Pipeline,
        AdaHalfArm::FixedSm89Tc128Swizzle,
    ];
    for arm in [
        ...
        ada_half_resource_gate(&t, arm, dtype)?;
    }
    let fixture = AdaHalfFixture::new_with_guard(&t, dtype, ADA_HALF_D768_IN, 128);
    for (name, pointer) in [
        ("A", fixture.a.ptr()),
        ("B", fixture.b.ptr()),
        ...
        if pointer % 256 != 0 {
            return Err(format!(
                "loaded half NN {name} pointer is not 256-byte aligned"
            ));
        }
    }
    let reference_graph = capture_ada_half(&t, &fixture, AdaHalfArm::PortableTc128)?;
    let s3_graph = capture_ada_half(&t, &fixture, AdaHalfArm::FixedSm89Tc128S3)?;
    let candidate_graphs = [
        capture_ada_half(&t, &fixture, candidates[0])?,
        capture_ada_half(&t, &fixture, candidates[1])?,
    ];
    fixture.reset_fast(&t)?;
    enqueue_ada_half_nn_fast(&t, &fixture)?;
    t.ctx
        .stream
        .synchronize()
        .map_err(|error| format!("loaded half NN Fast warmup: {error:?}"))?;
    let fast_graph = capture_ada_half_nn_fast(&t, &fixture)?;
    validate_single_node_graph(&reference_graph, "loaded NN forced TC128")?;
    validate_single_node_graph(&s3_graph, "loaded NN retained S3")?;
    for (&candidate, graph) in candidates.iter().zip(&candidate_graphs) {
        validate_single_node_graph(graph, candidate.name())?;
    }
    validate_nonempty_graph(&fast_graph, "loaded NN native-half Fast")?;

    let reference_bits = observe_ada_half_many(
        &t,
        &fixture,
        AdaHalfArm::PortableTc128,
        &reference_graph,
    ...
    for path in [AdaHalfPath::Eager, AdaHalfPath::Graph] {
        for repeat in 0..2 {
            ...
            for (&candidate, graph) in candidates.iter().zip(&candidate_graphs) {
                ...
                println!(
                    "{{\"schema\":\"MambaBiHalfNnLoadedBitsV1\",\"dtype\":\"F16\",\"cell\":\"{cell}\",\"candidate\":\"{}\",\"path\":\"{}\",\"repeat\":{repeat},\"logical_gemms\":20,\"words\":{}}}",
                    candidate.name(),
                    path.name(),
                    reference_bits.len(),
                );
    ...
    let fast_bits =
        observe_ada_half_nn_fast_many(&t, &fixture, &fast_graph, AdaHalfPath::Eager, 20, None)?.1;
    if !fast_bits
        .iter()
        .all(|&word| f16::from_bits(word).to_f32().is_finite())
        || !fast_bits
            ...
            .any(|&word| f16::from_bits(word).to_f32() != 0.0)
    {
        return Err("loaded half NN Fast output is non-finite or all-zero".into());
    }
    for _ in 0..2 {
        observe_ada_half_nn_fast_many(
            &t,
    ...
    for (&candidate, candidate_graph) in candidates.iter().zip(&candidate_graphs) {
        let mut decisions = Vec::with_capacity(2);
        for comparator in [
            ...
            let comparator_bits = match comparator {
                AdaHalfNnAlignedComparator::FixedS3 => &reference_bits,
                AdaHalfNnAlignedComparator::Fast => &fast_bits,
                AdaHalfNnAlignedComparator::ForcedTc128 => unreachable!(),
            };
            let mut strata = Vec::with_capacity(4);
            for path in [AdaHalfPath::Eager, AdaHalfPath::Graph] {
                for order in [BracketOrder::Abba, BracketOrder::Baab] {
                    strata.push(screen_ada_half_nn_aligned_pair(
                        ...
                        &fixture,
                        candidate_graph,
                        AdaHalfNnAlignedCandidate::Loaded(candidate),
                        &s3_graph,
                        &fast_graph,
                        &reference_bits,
                        comparator_bits,
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNnLoadedDecisionV1\",\"dtype\":\"F16\",\"cell\":\"{cell}\",\"shape\":[2048,768,3072],\"candidate\":\"{}\",\"guard_half_elements\":128,\"pointer_mod_256\":{{\"a\":{},\"b\":{},\"candidate\":{},\"reference\":{},\"fast\":{}}},\"comparators\":[{{\"name\":\"{}\",\"retain\":{},\"strata\":{:?}}},{{\"name\":\"{}\",\"retain\":{},\"strata\":{:?}}}],\"promotion\":false}}",
            candidate.name(),
            fixture.a.ptr() % 256,
            fixture.b.ptr() % 256,
            fixture.candidate.ptr() % 256,
    ...
    drop(t);
    quiet
        .verify_post_cohort("half-nn-loaded-pipeline-swizzle/post")
        .map(|_| ())
}

#[test]
#[ignore = "requires exclusive Ada CC8.9 CUDA13.2; loaded NN pipeline/swizzle F16 discovery"]
fn ada_half_nn_loaded_pipeline_swizzle_f16_d768_in_vs_s3_and_fast_discovery_once7()
-> Result<(), String> {
...
impl AdaHalfTnCandidate {
    ...
    fn symbol(&self, dtype: WeightDtype) -> String {
        ...
        match (self.kind, dtype) {
            ...
            (AdaHalfTnCandidateKind::Tc64Bk64S2Regpipe, WeightDtype::Bf16) => {
                format!("{}bf16", triad_half_tn_regpipe_source::SYMBOL_PREFIX)
            }
            (AdaHalfTnCandidateKind::Tc64Bk64S2Regpipe, WeightDtype::F16) => {
                format!("{}f16", triad_half_tn_regpipe_source::SYMBOL_PREFIX)
            }
            (AdaHalfTnCandidateKind::M64N128Bk64S2Compact, WeightDtype::Bf16) => {
                format!("{}bf16", triad_half_tn_m64n128_source::SYMBOL_PREFIX)
            }
            ...
            (AdaHalfTnCandidateKind::M64N128Bk64S2RegpipeVec2, WeightDtype::Bf16) => {
                format!(
                    "{}bf16",
                    triad_half_tn_m64n128_regpipe_source::SYMBOL_PREFIX
                )
            }
            (AdaHalfTnCandidateKind::M64N128Bk64S2RegpipeVec2, WeightDtype::F16) => {
                format!("{}f16", triad_half_tn_m64n128_regpipe_source::SYMBOL_PREFIX)
            }
...
fn compile_ada_half_tn_regpipe_candidate(t: &Ctx) -> Result<AdaHalfTnCandidate, String> {
    let transformed = triad_half_tn_regpipe_source::candidate_source(include_str!(
        "../kernels/gemm_bi_triad/sm80.cu"
    ))?;
    let bf16_symbol = format!("{}bf16", triad_half_tn_regpipe_source::SYMBOL_PREFIX);
    let f16_symbol = format!("{}f16", triad_half_tn_regpipe_source::SYMBOL_PREFIX);
    compile_ada_half_tn_isolated_candidate(
        t,
        AdaHalfTnCandidateKind::Tc64Bk64S2Regpipe,
        transformed,
...
fn compile_ada_half_tn_m64n128_regpipe_vec2_candidate(
    ...
    let transformed = triad_half_tn_m64n128_regpipe_source::candidate_source(include_str!(
        "../kernels/gemm_bi_triad/sm80.cu"
    ))?;
    ...
    let f16_symbol = format!("{}f16", triad_half_tn_m64n128_regpipe_source::SYMBOL_PREFIX);
    compile_ada_half_tn_isolated_candidate(
        t,
        AdaHalfTnCandidateKind::M64N128Bk64S2RegpipeVec2,
        transformed,
...
fn compile_ada_half_tn_m64n128_compact_candidate(t: &Ctx) -> Result<AdaHalfTnCandidate, String> {
    let transformed = triad_half_tn_m64n128_source::candidate_source(include_str!(
        "../kernels/gemm_bi_triad/sm80.cu"
    ))?;
    let bf16_symbol = format!("{}bf16", triad_half_tn_m64n128_source::SYMBOL_PREFIX);
    let f16_symbol = format!("{}f16", triad_half_tn_m64n128_source::SYMBOL_PREFIX);
    compile_ada_half_tn_isolated_candidate(
        t,
        AdaHalfTnCandidateKind::M64N128Bk64S2Compact,
        transformed,
...
fn gate_ada_half_tn_resources(
    ...
    {
        return Err(format!(
            "half TN {} resource gate failed: regs={registers}/{register_cap} local={local} static={static_shared}/{expected_static} max_threads={max_threads}/{threads} occupancy={occupancy}/{required_occupancy}",
            candidate.name(),
        ));
    }
    Ok(())
...
fn observe_ada_half_tn_candidate_only(
    t: &Ctx,
    fixture: &mut AdaHalfTnFixture,
    candidate: &AdaHalfTnCandidate,
    graph: &CudaGraph,
    ...
    fixture.reset(t, AdaHalfTnArm::Candidate)?;
    let start = t
        .ctx
        .stream
        .record_event(Some(sys::CUevent_flags::CU_EVENT_DEFAULT))
        .map_err(|error| format!("half TN candidate timing start: {error:?}"))?;
    for _ in 0..gemms {
        match path {
            AdaHalfPath::Eager => enqueue_ada_half_tn_candidate(t, fixture, candidate)?,
            AdaHalfPath::Graph => graph
                .launch()
                .map_err(|error| format!("half TN candidate graph launch: {error:?}"))?,
        }
    }
    let end = t
        .ctx
        .stream
        .record_event(Some(sys::CUevent_flags::CU_EVENT_DEFAULT))
        .map_err(|error| format!("half TN candidate timing end: {error:?}"))?;
    let elapsed_us = f64::from(
        start
            .elapsed_ms(&end)
            .map_err(|error| format!("half TN candidate timing: {error:?}"))?,
    ) * 1_000.0
        / gemms as f64;
    if !elapsed_us.is_finite() || elapsed_us <= 0.0 {
        return Err(format!("invalid half TN candidate timing {elapsed_us}"));
    }
    let bits = fixture.candidate.logical_bits(t);
    if let Some(expected) = expected
        && bits != expected
    {
        let mismatch = bits
            ...
            .unwrap_or(bits.len());
        return Err(format!(
            "half TN {} candidate differs from expected at word {mismatch}",
            candidate.name()
        ));
    }
    fixture.validate_inputs(t)?;
...
fn screen_ada_half_tn_candidates(
    t: &Ctx,
    fixture: &mut AdaHalfTnFixture,
    candidate: &AdaHalfTnCandidate,
    candidate_graph: &CudaGraph,
    reference: &AdaHalfTnCandidate,
    reference_graph: &CudaGraph,
    expected: &[u32],
    path: AdaHalfPath,
    order: BracketOrder,
    ...
    for _ in 0..ADA_HALF_WINDOWS {
        ...
        for (index, is_candidate) in sequence.into_iter().enumerate() {
            let (arm, graph) = if is_candidate {
                (candidate, candidate_graph)
            } else {
                (reference, reference_graph)
            };
            raw[index] = observe_ada_half_tn_candidate_only(
                t,
                fixture,
                arm,
                graph,
    ...
    let p50 = ada_percentile(&ratios, 0.5).ok_or("invalid half TN candidate-pair p50")?;
    let p95 = ada_percentile(&ratios, 0.95).ok_or("invalid half TN candidate-pair p95")?;
        ...
        for &(cell, dims) in cells {
            for dtype in [WeightDtype::F16, WeightDtype::Bf16] {
                ...
                if matches!(
                    ...
                    || fixture.current.ptr() % 256 != 0
                    || fixture.fast.ptr() % 256 != 0)
                {
                    return Err(format!(
                        "{} {dtype:?} {cell} timing fixture is not 256-byte aligned",
                        candidate.name()
                    ));
                }
                for arm in [
                ...
                let current_timing_bits = observe_ada_half_tn(
                    t,
                    &mut fixture,
                    candidate,
                    &graphs,
                ...
                for &comparator in comparators {
                    if !matches!(comparator, AdaHalfTnArm::CurrentTc64 | AdaHalfTnArm::Fast) {
                        return Err("half TN comparator must be current TC64 or native Fast".into());
                    }
                    let comparator_bits = match comparator {
                        AdaHalfTnArm::CurrentTc64 => &current_timing_bits,
                        AdaHalfTnArm::Fast => &fast_timing_bits,
                        AdaHalfTnArm::Candidate => unreachable!(),
                    };
                    let mut strata = Vec::with_capacity(4);
                    for path in [AdaHalfPath::Eager, AdaHalfPath::Graph] {
                        for order in [BracketOrder::Abba, BracketOrder::Baab] {
                            strata.push(screen_ada_half_tn_pair(
                                t,
                                &mut fixture,
                                candidate,
                                &graphs,
                    ...
                    let retain = retain_decision(&strata);
                    if comparators.len() == 1 && comparator == AdaHalfTnArm::Fast {
                        println!(
                        ...
                        println!(
                            "{{\"schema\":\"MambaBiHalfTn{schema}DecisionV1\",\"candidate\":\"{}\",\"dtype\":\"{dtype:?}\",\"cell\":\"{cell}\",\"shape\":[{},{},{}],\"comparator\":\"{}\",\"current_diagnostic_candidate_over_current\":{:?},\"current_diagnostic_order\":[\"eager\",\"graph\"],\"strata\":{:?},\"strata_order\":[\"eager/ABBA\",\"eager/BAAB\",\"graph/ABBA\",\"graph/BAAB\"],\"retain\":{retain},\"decision\":\"{}\",\"promotion\":false}}",
                            candidate.name(),
                            dims.0,
                            dims.1,
                            dims.2,
...
fn run_ada_half_tn_tc64_bk64_s2_regpipe_batch() -> Result<(), String> {
    assert!(
        !cfg!(debug_assertions),
        "half TN compact register-pipeline discovery requires --release"
    );
    let quiet = QuietGpu::for_cuda_ordinal(0)?;
    let _pre = quiet.require_pre_context("half-tn-compact-regpipe/pre-context")?;
    let t = Ctx::new_ada()?;
    let compiler = t.ctx.kernels.compiler_identity();
    if compiler.nvrtc_version != (13, 2) {
        return Err(format!(
            "half TN compact register-pipeline requires CUDA13.2, found {:?}",
            compiler.nvrtc_version
        ));
    }
    let candidate = compile_ada_half_tn_regpipe_candidate(&t)?;
    let _cohort = quiet.require_cohort("half-tn-compact-regpipe/cohort")?;
    run_ada_half_tn_candidate_cells(
        &t,
        &[candidate],
        &[("d768_in_proj", (2_048, 768, 3_072))],
    ...
    drop(t);
    quiet
        .verify_post_cohort("half-tn-compact-regpipe/post")
        .map(|_| ())
}

#[test]
#[ignore = "requires exclusive Ada CC8.9 CUDA13.2; half TN compact register pipeline"]
...
fn screen_ada_half_tn_regpipe_vec2_sibling_cell(
    t: &Ctx,
    candidate: &AdaHalfTnCandidate,
    retained: &AdaHalfTnCandidate,
    cell: &str,
    dims: (usize, usize, usize),
    expected_grid: u32,
    dtype: WeightDtype,
    ...
    if pointer_mod_256 != [0; 5] {
        return Err(format!(
            "half TN regpipe vec2 sibling fixture is not 256-byte aligned: {pointer_mod_256:?}"
        ));
    }
    for arm in [
        AdaHalfTnArm::Candidate,
        ...
        t.ctx
            .stream
            .synchronize()
            .map_err(|error| format!("half TN regpipe vec2 sibling graph warmup: {error:?}"))?;
    }
    fixture.reset(t, AdaHalfTnArm::Candidate)?;
    enqueue_ada_half_tn_candidate(t, &fixture, retained)?;
    t.ctx
        .stream
        .synchronize()
        .map_err(|error| format!("half TN compact retained graph warmup: {error:?}"))?;
    let graphs = [
        capture_ada_half_tn_arm(t, &fixture, candidate, AdaHalfTnArm::Candidate)?,
        capture_ada_half_tn_arm(t, &fixture, candidate, AdaHalfTnArm::CurrentTc64)?,
        capture_ada_half_tn_arm(t, &fixture, candidate, AdaHalfTnArm::Fast)?,
    ];
    let retained_graph = capture_ada_half_tn_arm(t, &fixture, retained, AdaHalfTnArm::Candidate)?;
    validate_ada_half_tn_candidate_graph(&graphs[0], &fixture, candidate, expected_grid)?;
    validate_ada_half_tn_candidate_graph(&retained_graph, &fixture, retained, expected_grid)?;
    validate_single_node_graph(&graphs[1], "current_tc64")?;
    validate_nonempty_graph(&graphs[2], "native_half_fast")?;
    ...
    for path in [AdaHalfPath::Eager, AdaHalfPath::Graph] {
        for repeat in 0..2 {
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfTnRegpipeVec2SiblingBitsV1\",\"dtype\":\"{dtype:?}\",\"cell\":\"{cell}\",\"path\":\"{}\",\"repeat\":{repeat},\"arms\":[\"current_tc64\",\"{}\",\"{}\"],\"words\":{}}}",
                path.name(),
                candidate.name(),
                retained.name(),
                expected.len(),
    ...
    {
        return Err(format!(
            "half TN regpipe vec2 sibling {dtype:?} {cell} Fast output is non-finite or all-zero"
        ));
    }
    for path in [AdaHalfPath::Eager, AdaHalfPath::Graph] {
        observe_ada_half_tn(
    ...
    let candidate_timing_bits = observe_ada_half_tn_candidate_only(
        t,
        &mut fixture,
        candidate,
        &graphs[0],
    ...
    for path in [AdaHalfPath::Eager, AdaHalfPath::Graph] {
        for order in [BracketOrder::Abba, BracketOrder::Baab] {
            retained_strata.push(screen_ada_half_tn_candidates(
                t,
                &mut fixture,
                candidate,
                &graphs[0],
    ...
    let retained_best = retain_decision(&retained_strata);
    let fast_win = retain_decision(&fast_strata);
    let decision = if retained_best {
        if fast_win {
            "advance_fast_winner"
        } else {
            "advance_retained_best"
        }
    } else {
        "stop_no_retry"
    };
    println!(
        "{{\"schema\":\"MambaBiHalfTnRegpipeVec2SiblingDecisionV1\",\"candidate\":\"{}\",\"dtype\":\"{dtype:?}\",\"cell\":\"{cell}\",\"shape\":[{},{},{}],\"expected_grid\":{expected_grid},\"pointer_mod_256\":{:?},\"retained_comparator\":\"{}\",\"retained_strata\":{:?},\"fast_comparator\":\"native_half_fast\",\"fast_strata\":{:?},\"strata_order\":[\"eager/ABBA\",\"eager/BAAB\",\"graph/ABBA\",\"graph/BAAB\"],\"retained_best_improvement\":{retained_best},\"fast_win\":{fast_win},\"decision\":\"{decision}\",\"promotion\":false}}",
        candidate.name(),
        dims.0,
        dims.1,
        dims.2,
...
#[test]
#[ignore = "requires exclusive Ada CC8.9 CUDA13.2; half TN regpipe vec2 out/Prism siblings"]
fn ada_half_tn_regpipe_vec2_d768_out_and_prism_vs_compact_and_fast_discovery_once7()
-> Result<(), String> {
    assert!(
        !cfg!(debug_assertions),
        "half TN regpipe vec2 sibling discovery requires --release"
    );
    let quiet = QuietGpu::for_cuda_ordinal(0)?;
    let _pre = quiet.require_pre_context("half-tn-regpipe-vec2-siblings/pre-context")?;
    let t = Ctx::new_ada()?;
    let compiler = t.ctx.kernels.compiler_identity();
    if compiler.nvrtc_version != (13, 2) {
        return Err(format!(
            "half TN regpipe vec2 sibling discovery requires CUDA13.2, found {:?}",
            compiler.nvrtc_version
        ));
    }
    let candidate = compile_ada_half_tn_regpipe_vec2_candidate(&t)?;
    let retained = compile_ada_half_tn_compact_candidate(&t)?;
    for dtype in triad_half_tn_regpipe_vec2_sibling_plan::dtypes() {
        let dtype = match dtype {
            triad_half_tn_regpipe_vec2_sibling_plan::HalfDtype::F16 => WeightDtype::F16,
            triad_half_tn_regpipe_vec2_sibling_plan::HalfDtype::Bf16 => WeightDtype::Bf16,
        };
        gate_ada_half_tn_resources(&candidate, dtype)?;
        gate_ada_half_tn_resources(&retained, dtype)?;
    }
    let _cohort = quiet.require_cohort("half-tn-regpipe-vec2-siblings/cohort")?;
    for (cell, dims, expected_grid) in triad_half_tn_regpipe_vec2_sibling_plan::cases() {
        for dtype in triad_half_tn_regpipe_vec2_sibling_plan::dtypes() {
            let dtype = match dtype {
                triad_half_tn_regpipe_vec2_sibling_plan::HalfDtype::F16 => WeightDtype::F16,
                triad_half_tn_regpipe_vec2_sibling_plan::HalfDtype::Bf16 => WeightDtype::Bf16,
...
#[test]
#[ignore = "requires exclusive Ada CC8.9 CUDA13.2; half TN M64N128 four-warp regpipe vec2"]
fn ada_half_tn_m64n128_bk64_s2_four_warp_regpipe_vec2_f16_d768_in_discovery_once7()
-> Result<(), String> {
    assert!(
        !cfg!(debug_assertions),
        "half TN M64N128 four-warp regpipe vec2 discovery requires --release"
    );
    let quiet = QuietGpu::for_cuda_ordinal(0)?;
    let _pre = quiet.require_pre_context("half-tn-m64n128-regpipe-vec2/pre-context")?;
    let t = Ctx::new_ada()?;
    let compiler = t.ctx.kernels.compiler_identity();
    if compiler.nvrtc_version != (13, 2) {
        return Err(format!(
            "half TN M64N128 four-warp regpipe vec2 requires CUDA13.2, found {:?}",
            compiler.nvrtc_version
        ));
    }
    let candidate = compile_ada_half_tn_m64n128_regpipe_vec2_candidate(&t)?;
    ...
    let _cohort = quiet.require_cohort("half-tn-m64n128-regpipe-vec2/cohort")?;
    let mut fixture =
        AdaHalfTnFixture::new_with_guards(&t, WeightDtype::F16, (2_048, 768, 3_072), 128, 64);
    let pointer_mod_256 = [
        fixture.a.ptr() % 256,
        fixture.b.ptr() % 256,
        fixture.candidate.ptr() % 256,
    ...
    if pointer_mod_256 != [0; 5] {
        return Err(format!(
            "half TN M64N128 timing fixture is not 256-byte aligned: {pointer_mod_256:?}"
        ));
    }
    for arm in [
        AdaHalfTnArm::Candidate,
        ...
        t.ctx
            .stream
            .synchronize()
            .map_err(|error| format!("half TN M64N128 graph warmup: {error:?}"))?;
    }
    fixture.reset(&t, AdaHalfTnArm::Candidate)?;
    enqueue_ada_half_tn_candidate(&t, &fixture, &retained)?;
    t.ctx
        .stream
        .synchronize()
        .map_err(|error| format!("half TN retained graph warmup: {error:?}"))?;
    let graphs = [
        capture_ada_half_tn_arm(&t, &fixture, &candidate, AdaHalfTnArm::Candidate)?,
        capture_ada_half_tn_arm(&t, &fixture, &candidate, AdaHalfTnArm::CurrentTc64)?,
        capture_ada_half_tn_arm(&t, &fixture, &candidate, AdaHalfTnArm::Fast)?,
    ...
    let retained_graph = capture_ada_half_tn_arm(&t, &fixture, &retained, AdaHalfTnArm::Candidate)?;
    validate_ada_half_tn_candidate_graph(&graphs[0], &fixture, &candidate, 288)?;
    validate_ada_half_tn_candidate_graph(&retained_graph, &fixture, &retained, 576)?;
    validate_single_node_graph(&graphs[1], "current_tc64")?;
    validate_nonempty_graph(&graphs[2], "native_half_fast")?;
    ...
    for path in [AdaHalfPath::Eager, AdaHalfPath::Graph] {
        for repeat in 0..2 {
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfTnM64N128Bk64S2RegpipeVec2BitsV1\",\"dtype\":\"F16\",\"cell\":\"d768_in_proj\",\"path\":\"{}\",\"repeat\":{repeat},\"arms\":[\"current_tc64\",\"{}\",\"{}\"],\"words\":{}}}",
                path.name(),
                candidate.name(),
                retained.name(),
                expected.len(),
    ...
    if !fast_bits
        ...
        || !fast_bits.iter().any(|&word| f32::from_bits(word) != 0.0)
    {
        return Err("half TN M64N128 Fast output is non-finite or all-zero".into());
    }
    for path in [AdaHalfPath::Eager, AdaHalfPath::Graph] {
        observe_ada_half_tn(
            &t,
    ...
    let fast_win = retain_decision(&fast_strata);
    println!(
        "{{\"schema\":\"MambaBiHalfTnM64N128Bk64S2RegpipeVec2DecisionV1\",\"candidate\":\"{}\",\"dtype\":\"F16\",\"cell\":\"d768_in_proj\",\"shape\":[2048,768,3072],\"pointer_mod_256\":{:?},\"retained_comparator\":\"{}\",\"retained_strata\":{:?},\"fast_comparator\":\"native_half_fast\",\"fast_strata\":{:?},\"strata_order\":[\"eager/ABBA\",\"eager/BAAB\",\"graph/ABBA\",\"graph/BAAB\"],\"retained_best_improvement\":{retained_best},\"fast_win\":{fast_win},\"decision\":\"{}\",\"promotion\":false}}",
        candidate.name(),
        pointer_mod_256,
        retained.name(),
        retained_strata,
    ...
    drop(t);
    quiet
        .verify_post_cohort("half-tn-m64n128-regpipe-vec2/post")
        .map(|_| ())
}
...
fn run_ada_half_tn_m64n128_compact_batch(
    ...
    assert!(
        !cfg!(debug_assertions),
        "half TN M64N128 compact BK64/S2 discovery requires --release"
    );
    let quiet = QuietGpu::for_cuda_ordinal(0)?;
    let _pre = quiet.require_pre_context(&format!("half-tn-m64n128-{cohort}/pre-context"))?;
    let t = Ctx::new_ada()?;
    let compiler = t.ctx.kernels.compiler_identity();
    if compiler.nvrtc_version != (13, 2) {
        return Err(format!(
            "half TN M64N128 compact BK64/S2 requires CUDA13.2, found {:?}",
            compiler.nvrtc_version
        ));
    }
    let candidate = compile_ada_half_tn_m64n128_compact_candidate(&t)?;
    let _cohort = quiet.require_cohort(&format!("half-tn-m64n128-{cohort}/cohort"))?;
    run_ada_half_tn_candidate_cells(
        &t,
        &[candidate],
        cells,
    ...
    drop(t);
    quiet
        .verify_post_cohort(&format!("half-tn-m64n128-{cohort}/post"))
        .map(|_| ())
}

#[test]
#[ignore = "requires exclusive Ada CC8.9 CUDA13.2; half TN M64N128 compact d768-in discovery"]
fn ada_half_tn_m64n128_bk64_s2_compact_d768_in_discovery_once7() -> Result<(), String> {
    run_ada_half_tn_m64n128_compact_batch("d768-in", &[("d768_in_proj", (2_048, 768, 3_072))])
}

#[test]
#[ignore = "requires exclusive Ada CC8.9 CUDA13.2; half TN M64N128 compact out/prism discovery"]
fn ada_half_tn_m64n128_bk64_s2_compact_d768_out_and_prism_discovery_once7() -> Result<(), String> {
    run_ada_half_tn_m64n128_compact_batch(
        "d768-out-prism",
        &[
            ("d768_out_proj", (2_048, 1_536, 768)),
            ("prism_in_proj", (4_621, 384, 1_928)),
...
impl AdaHalfNtArm {
    const fn name(self, candidate: AdaHalfNtCandidateKind) -> &'static str {
        match self {
            ...
            Self::Fast => "native_half_fast",
            Self::RetainedS3 => "retained_fixed_s3_bxor",
        }
    }
}
...
struct AdaHalfNtS3Candidate {
    ...
    bf16: CudaFunction,
    f16: CudaFunction,
    retained_bf16: Option<CudaFunction>,
    retained_f16: Option<CudaFunction>,
    source_sha256: String,
}
...
impl AdaHalfNtS3Candidate {
    ...
    fn retained_function(&self, dtype: WeightDtype) -> Result<&CudaFunction, String> {
        match dtype {
            WeightDtype::Bf16 => self.retained_bf16.as_ref(),
            WeightDtype::F16 => self.retained_f16.as_ref(),
            WeightDtype::F32 => None,
        }
        .ok_or_else(|| format!("half NT {} has no retained S3 comparator", self.kind.name()))
    }
}
...
fn compile_ada_half_nt_candidate(
    ...
    let production = include_str!("../kernels/gemm_bi_triad/sm80.cu");
    let (transformed, symbol_prefix, fixed_preamble) = match kind {
    ...
    let candidate = AdaHalfNtS3Candidate {
        kind,
        bf16: load("bf16")?,
        f16: load("f16")?,
        retained_bf16: None,
    ...
    if matches!(
        ...
        Ok(AdaHalfNtS3Candidate {
            ...
            retained_f16: Some(retained.f16),
            ..candidate
        })
    } else {
        Ok(candidate)
...
impl AdaHalfNtS3Fixture {
    fn new(t: &Ctx, dtype: WeightDtype) -> Self {
        Self::new_with_guard(t, dtype, AdaHalfNtCell::D768Out, 8)
    }
    ...
    fn new_with_guard(
        ...
        let output_seed = ada_half_values(m * k_out, dtype, 0xc89a_8203 ^ seed);
        let a = TypedSubview::new(t, &a_values, m, reduction, reduction, guard_offset, dtype);
        ...
        let make_output =
            || TypedSubview::new(t, &output_seed, m, k_out, k_out, guard_offset, dtype);
        let outputs = [make_output(), make_output(), make_output(), make_output()];
        let a_bits = a.logical_bits(t);
        let b_bits = b.logical_bits(t);
        Self {
            a,
...
fn enqueue_ada_half_nt_s3_arm_with_alpha(
    ...
    match arm {
        ...
        AdaHalfNtArm::Candidate | AdaHalfNtArm::RetainedS3 => {
            ...
            let mut launch = t.ctx.stream.launch_builder(match arm {
                ...
                AdaHalfNtArm::RetainedS3 => candidate.retained_function(fixture.dtype)?,
                _ => unreachable!(),
            });
            launch
                .arg(&output)
...
fn screen_ada_half_nt_s3_pair(
    ...
    let cell = fixture.cell.name();
    let (m, k, n) = fixture.cell.dims();
    println!(
        "{{\"schema\":\"{schema}\",\"dtype\":\"{:?}\",\"cell\":\"{cell}\",\"shape\":[{m},{k},{n}],\"candidate\":\"{}\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{order}\",\"windows\":{ADA_HALF_WINDOWS},\"logical_gemms_per_observation\":{GEMMS},\"raw_observations_us\":[{raw}],\"ratio_direction\":\"candidate_over_comparator\",\"ratio_p50\":{p50:.9},\"ratio_p95\":{p95:.9}}}",
        fixture.dtype,
        candidate.kind.name(),
...
fn run_ada_half_nt_cells_batch_for_dtypes(
    ...
    for (cell_index, &cell) in cells.iter().enumerate() {
        for &dtype in dtypes {
            ...
            let fixture = AdaHalfNtS3Fixture::new_with_guard(&t, dtype, cell, guard_offset);
            if matches!(
                kind,
                AdaHalfNtCandidateKind::DirectEpilogue
                    | AdaHalfNtCandidateKind::LoadedTc128
            ...
            {
                return Err(format!(
                    "{dtype:?} half NT {} timing fixture is not 256-byte aligned",
                    kind.name()
                ));
            }
            ...
            let graphs = active_arms
                .iter()
                .map(|&arm| capture_ada_half_nt_s3_arm(&t, &fixture, &candidate, arm))
            ...
            for path in [AdaHalfPath::Eager, AdaHalfPath::Graph] {
                for repeat in 0..2 {
                    ...
                    for &arm in exact_arms {
                        ...
                        let (m, k, n) = cell.dims();
                        println!(
                            "{{\"schema\":\"{}\",\"dtype\":\"{dtype:?}\",\"cell\":\"{cell_name}\",\"shape\":[{m},{k},{n}],\"candidate\":\"{}\",\"arm\":\"{}\",\"path\":\"{}\",\"repeat\":{repeat},\"words\":{}}}",
            ...
            for &comparator in comparators {
                if !matches!(
                    comparator,
                    AdaHalfNtArm::CurrentTc64 | AdaHalfNtArm::Fast | AdaHalfNtArm::RetainedS3
                ) {
                    return Err(
                        "half NT comparator must be current TC64, retained S3, or native Fast"
                            .into(),
                    );
                }
                let comparator_bits = match comparator {
                    AdaHalfNtArm::CurrentTc64 | AdaHalfNtArm::RetainedS3 => &current_bits,
                ...
                let (m, k, n) = cell.dims();
                println!(
                    "{{\"schema\":\"{}\",\"dtype\":\"{dtype:?}\",\"cell\":\"{cell_name}\",\"shape\":[{m},{k},{n}],\"candidate\":\"{}\",\"comparator\":\"{}\",\"strata\":{:?},\"strata_order\":[\"eager/ABBA\",\"eager/BAAB\",\"graph/ABBA\",\"graph/BAAB\"],\"retain\":{retain},\"decision\":\"{}\",\"promotion\":false}}",
...
fn run_ada_half_nt_d768_out_batch(
    ...
    run_ada_half_nt_cells_batch(kind, guard_offset, comparators, &[AdaHalfNtCell::D768Out])
}
...
#[test]
#[ignore = "requires exclusive Ada CC8.9 CUDA13.2; half NT Fixed S3 B-XOR discovery"]
fn ada_half_nt_fixed_s3_bxor_d768_out_vs_current_and_fast_discovery_once7() -> Result<(), String> {
    run_ada_half_nt_d768_out_batch(
        AdaHalfNtCandidateKind::FixedS3Bxor,
        128,
        &[AdaHalfNtArm::CurrentTc64, AdaHalfNtArm::Fast],
...
#[test]
#[ignore = "requires exclusive Ada CC8.9 CUDA13.2; half NT Fixed S3 B-XOR sibling cells"]
fn ada_half_nt_fixed_s3_bxor_d768_in_and_prism_vs_current_and_fast_discovery_once7()
-> Result<(), String> {
    run_ada_half_nt_cells_batch(
        AdaHalfNtCandidateKind::FixedS3Bxor,
        128,
        &[AdaHalfNtArm::CurrentTc64, AdaHalfNtArm::Fast],
        &[AdaHalfNtCell::D768In, AdaHalfNtCell::Prism],
    )
}

#[test]
#[ignore = "requires exclusive Ada CC8.9 CUDA13.2; half NT direct epilogue discovery"]
fn ada_half_nt_fixed_s3_direct_epilogue_d768_out_vs_retained_and_fast_discovery_once7()
-> Result<(), String> {
    run_ada_half_nt_cells_batch(
        AdaHalfNtCandidateKind::DirectEpilogue,
        128,
