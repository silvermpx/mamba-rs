//! Test-only Ada TF32 TN d768-out pre-RNA transpose discovery.
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_TF32_TN_TRANSPOSE_RNA_N96_D768_OUT_DISCOVERY";
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96D768OutIdentityV1",
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96D768OutResourceV1","symbol":symbol,
    ...
    fn transpose_resource_gate(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96D768OutTransformResourceV1",
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32TnTransposeRnaN96D768OutGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 2 {
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32TnTransposeRnaN96D768OutGraphV1","arm":arm.name(),"nodes":2,"gemm_symbol":expected_gemm,"transpose_symbol":expected_transpose,"gemm_grid":case.shape.grid()?,"transpose_grid":case.shape.transpose_config()?.grid_dim,"timing":"whole_two_node_graph"})
            );
        }
        Ok(())
    }
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32TnTransposeRnaN96D768OutBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96D768OutTargetBitsV1",
    ...
    fn prepare_fast(runtime: &Runtime, case: Case, prepared: &mut Prepared) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96D768OutFastBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96D768OutScreenV1","cell":case.label,
    ...
    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 TN transpose-RNA N96 d768-out once3 -> once7 -> Fast"]
    fn ada_tf32_tn_transpose_rna_n96_d768_out_protocol() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-tn-transpose-rna-n96-d768-out/pre", 1_800)?;
        } else {
            quiet.require_pre_context("tf32-tn-transpose-rna-n96-d768-out/pre")?;
        }
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-tn-transpose-rna-n96-d768-out/retained", 256)?;
        } else {
            quiet.require_cohort("tf32-tn-transpose-rna-n96-d768-out/retained")?;
        }
        ...
        let fast_strata = if retained_win {
            ...
            if idle_resident_mode {
                quiet.require_idle_resident("tf32-tn-transpose-rna-n96-d768-out/fast", 256)?;
            } else {
                quiet.require_cohort("tf32-tn-transpose-rna-n96-d768-out/fast")?;
            }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32TnTransposeRnaN96D768OutDecisionV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid()?,
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("tf32-tn-transpose-rna-n96-d768-out/post", 256)?;
        } else {
            quiet.verify_post_cohort("tf32-tn-transpose-rna-n96-d768-out/post")?;
        }
        Ok(())
    }
}
