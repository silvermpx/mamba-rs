...
fn m1_scan_bwd_output_hashes() {
    ...
    let k = &ctx.kernels;
    let bt = b * t;
    // One recording per activation dtype: the fold macro takes different
    // register paths for f32 and the half types, so each is its own key.
    for dtype in [WeightDtype::F32, WeightDtype::Bf16, WeightDtype::F16] {
        ...
        let d_b_fold =
            DtypedBuf::zeros(&ctx.stream, bt * (di / SCAN_BWD_DGROUP) * ds, dtype).unwrap();
        let d_c_fold =
            DtypedBuf::zeros(&ctx.stream, bt * (di / SCAN_BWD_DGROUP) * ds, dtype).unwrap();
        ...
        for (route, fold) in [("fold_slim", true), ("ungrouped_full", false)] {
            ...
            for (name, buf, n) in [("d_delta", &d_delta, bt * di), ("d_u", &d_u, bt * di)] {
                ...
                ctx.stream.synchronize().unwrap();
                eprintln!("HASH {dt}:{route}:{name} {:016x}", digest::fnv1a_f32(&v));
            }
