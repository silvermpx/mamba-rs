#[path = "support/triad_half_nt_m96n128_s3_bf16_source.rs"]
#[allow(dead_code)]
...
#[test]
fn bf16_adapter_changes_only_scalar_type_and_keeps_exact_rounding_path() {
    let candidate = candidate_source::candidate_source(SWIZZLE, S3).expect("compose BF16 M96N128");
    let restored =
        candidate_source::restore_f16_source(&candidate, SWIZZLE, S3).expect("restore F16");
    assert!(candidate_source::has_m96n128_geometry(&candidate));
    assert!(candidate.contains("sm89_test_half_nt_m96n128_s3::kernel<__nv_bfloat16>"));
    assert!(candidate.contains("gbf_store_pair_rne"));
    assert!(!candidate.contains("void gemm_bi_nt_test_fixed_s3_m96n128_f16"));
    assert!(restored.contains("sm89_test_half_nt_m96n128_s3::kernel<__half>"));

    let retained =
        candidate_source::retained_source(SWIZZLE, S3).expect("compose retained BF16 M64N128");
    ...
    assert_eq!(
        ...
        "gemm_bi_nt_test_fixed_s3_m64n128_bf16"
    );
}

#[test]
fn candidate_physics_reduce_staging_and_preserve_two_target_waves() {
    ...
    assert_eq!(candidate_source::target_grid(), 264);
    assert_eq!(candidate_source::TARGET_GRID, 264);
    assert_eq!(
        candidate_source::staged_half_elements_per_k_tile_m96n128(),
        3_784_704
    );
    assert_eq!(
        candidate_source::staged_half_elements_per_k_tile_m64n192(),
        4_194_304
    );
    assert_eq!(candidate_source::EXPECTED_HMMA, 64);
    assert_eq!(candidate_source::EXPECTED_LDGSTS, 15);
    assert_eq!(candidate_source::RETAINED_EXPECTED_HMMA, 64);
    assert_eq!(candidate_source::RETAINED_EXPECTED_LDGSTS, 18);
}
...
mod cuda_suite {
    ...
    const M_TAIL: Shape = Shape::new(101, 128, 64, -0.75, "m_tail_negative_alpha");
    const N_TAIL: Shape = Shape::new(96, 131, 64, 1.0, "k_out_tail");
    const K_TAIL: Shape = Shape::new(96, 128, 69, 1.0, "reduction_tail");
    const EXCEPTIONAL: Shape = Shape::new(96, 128, 64, 1.0, "exceptional_full_tile");
    const K0: Shape = Shape::new(96, 128, 0, 1.0, "k0");
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "m96n128_bk64_s3_bf16",
    ...
    fn bf16_corpus(len: usize, mut state: u64, exceptional: bool) -> Vec<u16> {
        const SPECIAL: [u16; 16] = [
            0x0000, 0x8000, 0x0001, 0x8001, 0x007f, 0x807f, 0x3f81, 0xbf81, 0x3eaa, 0xbeaa, 0x7f7f,
            0xff7f, 0x7f80, 0xff80, 0x7fc1, 0xffd1,
        ];
        (0..len)
            .map(|index| {
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        let retained_sass = validate_ptxas_and_sass(
            retained_symbol,
            &retained_ptx,
            candidate_source::RETAINED_MAX_REGISTERS as u32,
        )?;
        ...
        {
            return Err(format!(
                "SASS program reject candidate HMMA/LDGSTS={}/{} retained={}/{}",
                candidate_sass.hmma,
    ...
    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_BF16_NT_M96N128_S3_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_BF16_NT_M96N128_S3_DISCOVERY=1".into());
        }
        let device = GpuDevice::new(0)?;
        if device.compute_capability != (8, 9) || device.multiprocessor_count() != 142 {
            return Err(format!(
                "M96N128/S3 discovery requires 142-SM Ada, got {:?}/{}",
        ...
        if compiler.nvrtc_version != (13, 2) || compiler.target.as_str() != "sm_89" {
            return Err(format!(
                "M96N128/S3 discovery requires CUDA13.2/sm_89, got {compiler:?}"
            ));
        }
        let swizzle = include_str!("../kernels/gemm_bi_fixed/sm89_half_swizzle.cu");
        let s3 = include_str!("../kernels/gemm_bi_fixed/sm89_half_s3.cu");
        ...
        let (retained_module, retained, retained_source_sha) = compile_source(
            ...
            &retained_source,
            candidate_source::RETAINED_SYMBOL,
            candidate_source::RETAINED_DYNAMIC_SHARED_BYTES,
        )?;
        Ok(Runtime {
            _device: device,
            ctx,
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function, shared, occupancy) in [
            ...
            let max_registers = if arm == Arm::Candidate {
                ...
                candidate_source::RETAINED_MAX_REGISTERS
            };
            ...
            println!(
                "{{\"schema\":\"MambaBiBf16NtM96N128S3ResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":{shared},\"active_ctas\":{active},\"required_active_ctas\":{occupancy}}}",
                arm.name()
            );
        }
        Ok(())
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiBf16NtM96N128S3BitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha\":{},\"exceptional\":{},\"candidate_vs_measured_s3_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.m,
            shape.k_out,
            shape.reduction,
    ...
    fn timed_pair(
        ...
        for window in 0..windows {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiBf16NtM96N128S3TimingV1\",\"phase\":\"{phase}\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw
    ...
    fn ada_bf16_nt_m96n128_bk64_s3_d768_out_scout_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("bf16-nt-m96n128-s3/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("bf16-nt-m96n128-s3/cohort")?;
        ...
        println!(
            "{{\"schema\":\"MambaBiBf16NtM96N128S3ScoutDecisionV1\",\"shape\":[2048,1536,768],\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{scout_pass},\"decision\":\"{}\"}}",
            scout_strata,
            if scout_pass {
                "advance_to_once7"
            } else {
        ...
        if !scout_pass {
            drop(runtime);
            quiet.verify_post_cohort("bf16-nt-m96n128-s3/post")?;
            return Ok(());
        }
        ...
        println!(
            "{{\"schema\":\"MambaBiBf16NtM96N128S3RetainedDecisionV1\",\"shape\":[2048,1536,768],\"candidate\":\"{}\",\"comparator\":\"{}\",\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_strata,
            if retained_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiBf16NtM96N128S3FinalDecisionV1\",\"shape\":[2048,1536,768],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"retained_strata\":{:?},\"fast_strata\":{:?},\"strict_fast_p50_p95_lt_0_99\":{fast_pass},\"decision\":\"{}\",\"promotion\":false}}",
