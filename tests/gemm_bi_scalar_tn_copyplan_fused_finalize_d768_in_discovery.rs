//! Discovery only: retained transpose and chunk0 raw CopyPlan followed by a
//! chunk1 CopyPlan with the exact F=2 reducer fused into its epilogue.
...
mod splitm_cuda_suite {
    ...
    fn launch(
        ...
        match arm {
            Arm::Candidate => launch_fused_candidate(runtime, fixture, candidate),
            Arm::Reference => {
    ...
    unsafe fn require_candidate_graph(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedFinalizeGraphV1",
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
    ...
    unsafe fn require_fast_graph(graph: &CudaGraph) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiF32TnFusedFinalizeGraphV1","arm":"Fast","node_count":count,"abi":"opaque","timing":"whole_graph"})
        );
        Ok(())
    }
    ...
    fn check_exact_case(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedFinalizeBitsV1",
                "case":label,
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
    ...
    #[test]
    #[ignore = "Ada CUDA13.2 discovery: exact F32 TN d768-in fused finalize"]
    fn ada_f32_tn_copyplan_fused_finalize_d768_in_once7() -> Result<(), String> {
        ...
        quiet.require_pre_context("f32-tn-fused-finalize/pre")?;
        let runtime = new_runtime()?;
        validate_fast_handle(&runtime.ctx)?;

        // Prove the exact retained four-node comparator before comparing a
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedFinalizeArtifactsV1",
                ...
                "candidate_nodes":[TRANSPOSE,candidate.symbol(),FUSED],
                "retained_nodes":[TRANSPOSE,candidate.symbol(),candidate.symbol(),REDUCER],
        ...
        compare_raw_partials(
            ...
            "tail_chunk_fused_accumulator",
        )
        .map_err(|error| error.to_string())?;
        if candidate != CandidateFixed::RawStore {
            compare_raw_partials(&runtime, &mut tail, candidate, "tail_chunk_selected_chunk0")
                .map_err(|error| error.to_string())?;
        }
        ...
        compare_raw_partials(
            ...
            &mut exceptional,
            CandidateFixed::RawStore,
            "exceptional_tail_fused_accumulator",
        )
        .map_err(|error| error.to_string())?;
        if candidate != CandidateFixed::RawStore {
            compare_raw_partials(
                ...
                &mut exceptional,
                candidate,
                "exceptional_tail_selected_chunk0",
            )
            .map_err(|error| error.to_string())?;
        }
        check_exact_case(
        ...
        for comparator in [Arm::Retained, Arm::Fast] {
            let comparator_graph = capture(&runtime, &mut target, comparator, candidate)?;
            unsafe {
                match comparator {
                    Arm::Retained => require_retained_graph(&comparator_graph, &target, candidate)?,
            ...
            quiet.require_cohort("f32-tn-fused-finalize/timed")?;
            ...
            for graph_path in [false, true] {
                for candidate_endpoints in [true, false] {
                    ...
                    println!(
                        "{}",
                        json!({
                            "schema":"MambaBiF32TnFusedFinalizeScreenV1",
                            ...
                            "candidate_timed_nodes":[TRANSPOSE,candidate.symbol(),FUSED],
            ...
            match comparator {
                Arm::Retained => retained_win = Some(strict_win),
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnFusedFinalizeDecisionV1",
                    "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                    "comparator":format!("{comparator:?}"),
                    "strata":strata,
                    "strict_win":strict_win,
                    "decision":match (comparator,strict_win) {
                        (Arm::Retained,true)=>"shortlist_retained_best",
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnFusedFinalizeFinalDecisionV1",
                "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                "candidate":FUSED,
                "retain_against_retained_copyplan":retained_win,
                "fast_win":fast_win,
        ...
        quiet.verify_post_cohort("f32-tn-fused-finalize/post")?;
        Ok(())
    }
}
