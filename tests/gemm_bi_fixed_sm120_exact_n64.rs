...
use mamba_rs::mamba_ssm::gpu::gemm_bi_fixed::{
    FixedFwdOperands, FixedShape, FixedTile, fixed_forward_f32_legacy_baseline,
...
const CANDIDATE: FixedTile = FixedTile::F32Sm120N64CopyPlan;
...
fn admit_context() -> GpuCtx {
    ...
    assert!(ctx.kernels.fixed_sm120_f32_n64_copyplan_rejection.is_none());
    ...
    println!(
        concat!(
            "{{\"record\":\"FixedSm120ExactN64LiveAdmissionV1\",\"symbol\":\"{}\",",
...
fn run_fixture(ctx: &GpuCtx, f: Fixture) {
    ...
    for arm in [Arm::OldOracle, Arm::Legacy, Arm::N128, Arm::Candidate] {
        if matches!(arm, Arm::Candidate) {
            println!(
                concat!(
                    "{{\"record\":\"FixedSm120ExactN64ForcePreflightV1\",",
    ...
    println!(
        concat!(
            "{{\"record\":\"FixedSm120ExactN64CorrectnessV1\",\"dtype\":\"f32\",\"op\":\"nn\",",
