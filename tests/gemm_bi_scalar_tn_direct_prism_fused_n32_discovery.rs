...
#[path = "support/triad_f32_tn_direct_prism_fused_n32_source.rs"]
mod direct_source;
...
#[test]
fn tn_prism_fused_n32_maps_six_exact_chunks_to_one_node_candidate() {
    assert_eq!(
    ...
    assert_eq!(PRISM.fixed_grid(), (366, 1, 1));
    assert_eq!(PRISM.partial_grid(), (48, 1, 6));
    assert_eq!(PRISM.reducer_grid(), (2892, 1, 1));
    assert_eq!(PRISM.direct_grid(), (366, 1, 1));
    assert_eq!(PRISM.candidate_node_count(), 1);
...
fn cuda_harness_stops_in_order_after_exact_resources_once3_and_once7() {
    let harness = include_str!("gemm_bi_scalar_tn_direct_prism_fused_n32_discovery.rs");
    ...
    assert!(harness.contains("!entry.contains(\"DADD\")"));
    assert!(harness.contains("!entry.contains(\"DMUL\")"));
    assert!(harness.contains("\"candidate_timed_nodes\":[DIRECT]"));
    assert!(
...
#[test]
fn retained_auto_launch_uses_the_fixture_batch() {
    let harness = include_str!("gemm_bi_scalar_tn_direct_prism_fused_n32_discovery.rs");
    let launch = &harness[harness.find("    fn launch(\n").unwrap()..];
    let launch = &launch[..launch.find("    fn select_candidate").unwrap()];
    assert_eq!(
        launch.matches("fixture.cell.batch,").count(),
        2,
        "both retained launches must forward the real fixture batch, including K0"
    );
}
...
mod splitm_cuda_suite {
    ...
    const DIRECT_STATIC_SHARED: usize = 12_288;
    ...
    fn validate_ptxas_and_sass(symbol: &str, ptx: &str) -> Result<(), String> {
        ...
        let result = (|| {
            ...
            if !entry.contains("FFMA")
                || !entry.contains("LDGSTS")
                || !entry.contains("DADD")
                || !entry.contains("DMUL")
            {
                return Err(format!("{symbol} SASS omitted FFMA, LDGSTS, DADD, or DMUL"));
            }
    ...
    fn launch_direct_fused(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        if fixture.cell.batch == 0 {
            return Err("zero reduction uses the production zero kernel".into());
        }
        ...
        unsafe { builder.launch(config(fixture.cell.direct_grid(), (128, 1, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch fused {DIRECT}: {error:?}"))
    }
    ...
    fn launch(
        ...
        match arm {
            Arm::Candidate => launch_direct_fused(runtime, fixture),
            Arm::Reference => {
    ...
    fn select_candidate(_runtime: &Runtime) -> Result<CandidateFixed, String> {
    ...
    unsafe fn require_candidate_graph(
        ...
        if nodes.len() != 1 {
            return Err(format!(
                "fused direct candidate has {} nodes, expected one",
                nodes.len()
            ));
        }
        let fused = nodes
            ...
            .ok_or_else(|| format!("fused candidate omitted {DIRECT}"))?;
        require_launch(fused, DIRECT, fixture.cell.direct_grid(), (128, 1, 1), 0)?;
        if unsafe { graph_arg::<u64>(fused, 0) }? != fixture.output.ptr()
            ...
            || unsafe { graph_arg::<f32>(fused, 3) }?.to_bits() != fixture.alpha.to_bits()
            || unsafe { graph_arg::<i32>(fused, 4) }? != fixture.cell.batch as i32
            || unsafe { graph_arg::<i32>(fused, 5) }? != fixture.cell.k_out as i32
            || unsafe { graph_arg::<i32>(fused, 6) }? != fixture.cell.n_out as i32
            || unsafe { graph_arg::<i32>(fused, 7) }? != M_CHUNK as i32
            || unsafe { graph_arg::<i32>(fused, 8) }? != CHUNKS as i32
        {
            return Err("fused direct ABI/arguments changed".into());
        }
        unsafe { require_chain(graph, &[fused.node], "fused direct candidate") }?;
        println!(
            "{}",
            json!({
                ...
                "nodes":[DIRECT],
                "edge":null,
                "chains":[784,784,784,784,784,701]
            })
        );
        Ok(())
    }
    ...
    fn screen_comparator(
        ...
        unsafe {
            match comparator {
                Arm::Retained => require_auto_splitm_graph(&comparator_graph, fixture)?,
    ...
    #[test]
    #[ignore = "Ada CUDA13.2 discovery: exact F32 TN canonical Prism fused M64N32 BK16"]
    fn ada_f32_tn_direct_prism_fused_n32_discovery() -> Result<(), String> {
        ...
        println!(
            ...
            json!({
                ...
                "candidate_nodes":[DIRECT],
                "retained_nodes":[PARTIAL,REDUCER],
                "candidate_grid":[366,1,1],
                "retained_grid":[48,1,6],
