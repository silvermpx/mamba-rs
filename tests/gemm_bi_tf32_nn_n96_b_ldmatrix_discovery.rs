//! Test-only Ada TF32 NN N96 B-ldmatrix discovery.
//! Production kernels and dispatch remain untouched.

#[path = "support/triad_tf32_nn_n96_b_ldmatrix_source.rs"]
...
fn n96_b_ldmatrix_rows_are_16b_aligned_and_cover_literal_coordinates() {
    for (lane, expected) in [
        ...
        let (thread, column, source_lane) = expected;
        assert_eq!(
            candidate_source::scalar_coordinate(0, 0, lane, 0),
            (thread, column)
        );
        assert_eq!(
            candidate_source::source_lane_for_consumer(lane),
            source_lane
        );
        assert_eq!(
            candidate_source::loaded_coordinate(0, 0, 0, source_lane, 0),
            (thread, column)
        );
        assert_eq!(
            candidate_source::loaded_coordinate(0, 0, 0, source_lane, 1),
            (thread + 4, column)
        );
    }

    for warp_n in [0, 24, 48, 72] {
        for atom in 0..3 {
            for address_lane in 0..16 {
                let (k, column) =
                    candidate_source::ldmatrix_row_coordinate(warp_n, atom, 0, address_lane);
                let index = candidate_source::b_index(k, column);
                assert_eq!(index * 4 % 16, 0);
                for word in 0..4 {
                    assert_eq!(candidate_source::b_index(k, column + word), index + word);
                }
            }
        }
    }
...
#[test]
fn n96_x2_registers_match_both_old_scalar_words_for_every_lane() {
    for warp_n in [0, 24, 48, 72] {
        for atom in 0..3 {
            for k8 in [0, 8, 16, 24] {
                for lane in 0..32 {
                    let source_lane = candidate_source::source_lane_for_consumer(lane);
                    for register in 0..2 {
                        assert_eq!(
                            candidate_source::loaded_coordinate(
                                warp_n,
                                atom,
                                k8,
                                source_lane,
...
#[test]
fn source_adapter_changes_only_b_load_plumbing_and_export() {
    let retained = retained_source::compose_triad_nn_n96_source(FIXED_N96_SOURCE).unwrap();
    ...
    assert!(candidate.contains("__shfl_sync(0xffffffffU, raw0, source_lane)"));
    assert!(candidate.contains("__shfl_sync(0xffffffffU, raw1, source_lane)"));
    assert_eq!(
...
fn logical_nn_targets_have_the_frozen_n96_grids() {
    assert_eq!(candidate_source::n96_grid(2_048, 768), 128);
    assert_eq!(candidate_source::n96_grid(4_621, 1_928), 777);
}
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_NN_N96_B_LDMATRIX_DISCOVERY";
    ...
    impl Shape {
        fn grid(self) -> Result<(u32, u32, u32), String> {
            let blocks = candidate_source::n96_grid(self.m, self.n);
            Ok((
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "b_ldmatrix_n96",
                Self::Retained => "retained_n96",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if cfg!(debug_assertions) {
            return Err("N96 B-ldmatrix timing requires --release".into());
        }
        ...
        let retained_body = retained_source::compose_triad_nn_n96_source(FIXED_N96_SOURCE)?;
        ...
        let (retained_module, retained, retained_source_sha, retained_ptx_sha) = compile_module(
            ...
            retained_source::TRIAD_NN_N96_SYMBOL,
        )?;
        ...
        resource_gate(&runtime.retained, retained_source::TRIAD_NN_N96_SYMBOL, 128)?;
        resource_gate(&runtime.candidate, candidate_source::SYMBOL, 128)?;
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96BLdmatrixIdentityV1",
                "retained_symbol":retained_source::TRIAD_NN_N96_SYMBOL,
                ...
                "change":"B_scalar_to_ldmatrix_x2_plus_warp_transpose",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96BLdmatrixResourceV1","symbol":symbol,
    ...
    fn graph_identity(graph: &CudaGraph, case: Case, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32NnN96BLdmatrixGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 1 {
            ...
            println!(
                "{}",
                json!({"schema":"MambaBiTriadTf32NnN96BLdmatrixGraphV1","arm":arm.name(),"symbol":symbol,"grid":expected_grid,"block":[256,1,1],"shared":SHARED,"timing":"whole_graph"})
            );
        }
        Ok(())
    }
    ...
    fn check_exact_case(runtime: &Runtime, case: Case, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({"schema":"MambaBiTriadTf32NnN96BLdmatrixBitsV1","case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],"exceptional":exceptional,"candidate_retained_exact":true,"eager_repeats":2,"graph_repeats":2,"guards":true})
        );
        Ok(())
    }
    ...
    fn prepare_target(runtime: &Runtime, case: Case) -> Result<Prepared, String> {
        ...
        let fast_bits = output_after(runtime, &mut fixture, case, Arm::Fast, None, 1)?;
        ...
        let candidate_graph = capture(runtime, &mut fixture, case, Arm::Candidate)?;
        let retained_graph = capture(runtime, &mut fixture, case, Arm::Retained)?;
        let fast_graph = capture(runtime, &mut fixture, case, Arm::Fast)?;
        graph_identity(&candidate_graph, case, Arm::Candidate)?;
        graph_identity(&retained_graph, case, Arm::Retained)?;
        ...
        for repeat in 0..2 {
            for (arm, graph, expected) in [
                ...
                (Arm::Fast, &fast_graph, fast_bits.as_slice()),
            ] {
                for path in [None, Some(graph)] {
                    if output_after(runtime, &mut fixture, case, arm, path, 1)? != expected {
                        return Err(format!(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96BLdmatrixTargetBitsV1",
                "case":case.label,"shape":[case.shape.m,case.shape.k,case.shape.n],
                "candidate_retained_exact":true,"fast_self_consistent":true,
    ...
    fn screen(
        ...
        let (comparator_graph, comparator_bits) = match comparator {
            ...
            Arm::Fast => (&prepared.fast_graph, prepared.fast_bits.as_slice()),
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NnN96BLdmatrixScreenV1","cell":case.label,
                "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid()?,
                "candidate":"b_ldmatrix_n96","comparator":comparator.name(),"path":path.name(),
    ...
    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; isolated TF32 NN N96 B-ldmatrix once7"]
    fn ada_tf32_nn_n96_b_ldmatrix_d768_out_prism_once7() -> Result<(), String> {
        ...
        quiet.require_pre_context("tf32-nn-n96-b-ldmatrix/pre")?;
        let runtime = new_runtime()?;
        assert_eq!(D768_OUT.shape.grid()?, (128, 1, 1));
        ...
        for case in [D768_OUT, PRISM] {
            ...
            quiet.require_cohort("tf32-nn-n96-b-ldmatrix/timed")?;
            ...
            let valid = |strata: &[[f64; 2]]| {
                strata.len() == 4 && strata.iter().flatten().all(|x| x.is_finite() && *x > 0.0)
            };
            let retained_win =
                valid(&retained_strata) && retained_strata.iter().flatten().all(|x| *x < 0.99);
            let fast_win = valid(&fast_strata) && fast_strata.iter().flatten().all(|x| *x < 0.99);
            println!(
                "{}",
                json!({
                    "schema":"MambaBiTriadTf32NnN96BLdmatrixDecisionV1","cell":case.label,
                    "shape":[case.shape.m,case.shape.k,case.shape.n],"grid":case.shape.grid()?,
                    ...
                    "decision":if retained_win { "shortlist_retained_best" } else { "stop_no_retry" },
                    "fast_qualified":fast_win,"promotion":false,
                })
            );
        }
        quiet.verify_post_cohort("tf32-nn-n96-b-ldmatrix/post")?;
        Ok(())
    }
}
