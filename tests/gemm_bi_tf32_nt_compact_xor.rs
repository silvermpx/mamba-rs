...
mod cuda_suite {
    ...
    fn compose_source() -> Result<String, String> {
        let transformed = compact_candidate_source()?;
        Ok([
    ...
    fn compile_candidate(ctx: &GpuCtx) -> Result<(CudaFunction, String), String> {
        ...
        let ptx = cudarc::nvrtc::compile_ptx_with_opts(source, options)
            .map_err(|error| format!("compile compact NT candidate: {error:?}"))?;
        let module = ctx
            .stream
            .context()
            .load_module(ptx)
            .map_err(|error| format!("load compact NT candidate: {error:?}"))?;
        let function = module
            .load_function(CANDIDATE_SYMBOL)
            .map_err(|error| format!("load compact NT symbol: {error:?}"))?;
        function
            .set_attribute(
                sys::CUfunction_attribute_enum::CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES,
                SHARED_BYTES as i32,
            )
            .map_err(|error| format!("set compact NT dynamic shared: {error:?}"))?;
    ...
    fn launch_candidate(
        ...
        unsafe { builder.launch(config) }
            .map(|_| ())
            .map_err(|error| format!("launch compact NT candidate: {error:?}"))
    }
    ...
    impl Candidate {
        fn new(
            ...
            if size_of::<Params>() != 32 || align_of::<Params>() != 4 {
                return Err(format!(
                    "compact NT parameter ABI changed: size={} align={}",
                    size_of::<Params>(),
                    align_of::<Params>()
                ));
            }
            let ctx = GpuCtx::new(device)?;
            let (function, source_sha) = compile_candidate(&ctx)?;
            ...
            let graph = unsafe {
                capture_into_graph(&ctx.stream, || {
                    launch_candidate(&ctx, &function, &output, &a, &b, config, params)
                })
            }?;
            validate_candidate_graph(&graph, config)?;
            Ok((
                Self {
                    graph,
                    function,
        ...
        fn launch(&self, path: Path) -> Result<(), String> {
            match path {
                ...
                Path::Graph => self
                    .graph
                    .launch()
                    .map_err(|error| format!("launch compact NT graph: {error:?}")),
            }
        }
    ...
    fn validate_candidate_graph(graph: &CudaGraph, expected: LaunchConfig) -> Result<(), String> {
        ...
        if count != 1 {
            return Err(format!("compact NT graph has {count} nodes"));
        }
        ...
        if name.is_null() {
            return Err("compact NT graph returned a null symbol name".into());
        }
        let name = unsafe { CStr::from_ptr(name) }.to_string_lossy();
        if name != CANDIDATE_SYMBOL
        ...
        {
            return Err(format!(
                "compact NT graph identity changed: symbol={name} grid={:?} block={:?} shared={}",
            ...
            .enumerate()
        {
            ...
            cuda_ok(
                ...
                &format!("read compact NT ABI parameter {index}"),
            )?;
            if (offset, size) != expected {
                return Err(format!(
                    "compact NT ABI parameter {index} changed: {:?}",
                    (offset, size)
                ));
            }
        }
        ...
        {
            return Err("compact NT ABI accepted a sixth argument".into());
        }
        Ok(())
    }
    ...
    fn validate_resources(candidate: &Candidate, source_sha: &str) -> Result<(), String> {
        ...
        if registers <= 0
            ...
            || static_shared != 0
            || max_threads < 256
            || max_dynamic < SHARED_BYTES as i32
            || occupancy < 1
        {
            return Err(format!(
                "compact NT resource gate failed: regs={registers} local={local} static={static_shared} max_threads={max_threads} max_dynamic={max_dynamic} occupancy={occupancy}"
            ));
        }
        println!(
            "{{\"schema\":\"MambaBiTf32NtCompactXorResourceV1\",\"source_sha256\":\"{source_sha}\",\"symbol\":\"{CANDIDATE_SYMBOL}\",\"registers\":{registers},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":{SHARED_BYTES},\"max_dynamic_shared_bytes\":{max_dynamic},\"max_threads\":{max_threads},\"occupancy\":{occupancy}}}"
        );
        Ok(())
    }
        ...
        for path in [Path::Eager, Path::Graph] {
            for repeat in 0..2 {
                ...
                if expected.as_ref() != Some(&actual) {
                    ...
                    return Err(format!(
                        "{label} compact candidate differs from reference at {mismatch} on {} repeat {repeat}",
                        path.name()
                    ));
                }
            }
    ...
    fn screen(
        ...
        println!(
            "{{\"schema\":\"MambaBiTf32NtCompactXorScreenV1\",\"path\":\"{}\",\"order\":\"{}\",\"windows\":{WINDOWS},\"iterations\":{iterations},\"bracket_fields\":[\"auto0_us\",\"candidate0_us\",\"candidate1_us\",\"auto1_us\"],\"brackets\":{brackets},\"ratio_direction\":\"candidate_over_actual_auto\",\"ratio_p50\":{p50:.9},\"ratio_p95\":{p95:.9},\"auto_samples_us\":{},\"candidate_samples_us\":{},\"ratios\":{}}}",
            path.name(),
            order.name(),
    ...
    #[test]
    #[ignore = "requires exclusive Ada CC8.9 CUDA13.2; test-only compact NT bits/resources/once7"]
    fn ada_tf32_nt_compact_xor_discovery_once7() {
        assert!(
            !cfg!(debug_assertions),
            "compact NT discovery requires --release"
        );
        let quiet = QuietGpu::for_cuda_ordinal(0).unwrap();
        let pre = quiet
            .require_pre_context("tf32-nt-compact-xor/pre")
            .unwrap();
        ...
        let (mut candidate, source_sha) = Candidate::new(&device, TARGET, &target_words).unwrap();
        ...
        let (mut tail_candidate, tail_source_sha) =
            Candidate::new(&device, TAIL, &tail_words).unwrap();
        ...
        let timed_pre = quiet.require_cohort("tf32-nt-compact-xor/timed").unwrap();
        ...
        let post = quiet
            .verify_post_cohort("tf32-nt-compact-xor/post")
            .unwrap();
        ...
        let strata = strata
            ...
            .collect::<Vec<_>>();
        println!(
            "{{\"schema\":\"MambaBiTf32NtCompactXorDecisionV1\",\"shape\":[2048,768,3072],\"source_sha256\":\"{source_sha}\",\"pre\":{pre:?},\"timed_pre\":{timed_pre:?},\"post\":{post:?},\"strata_fields\":[\"ratio_p50\",\"ratio_p95\"],\"strata\":{},\"retain\":{retain},\"decision\":\"{}\",\"promotion\":false}}",
            json_pairs(&strata),
            if retain {
