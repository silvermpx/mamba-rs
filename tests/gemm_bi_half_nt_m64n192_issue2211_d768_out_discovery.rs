#[path = "support/triad_half_nt_m64n192_issue2211_source.rs"]
#[allow(dead_code)]
...
fn sass_issue_signature(sass: &str) -> String {
    sass.lines()
        .filter_map(|line| {
            if line.contains("LDGSTS") {
                Some('L')
            } else if line.contains("HMMA.16816.F32") {
                Some('H')
            } else {
                None
            }
...
#[test]
fn sass_issue_signature_records_only_physical_ldgsts_hmma_order() {
    let sass = "/*0000*/ LDGSTS.E.BYPASS.128\n/*0010*/ IADD3\n/*0020*/ HMMA.16816.F32\n\
                /*0030*/ HMMA.16816.F32\n/*0040*/ LDGSTS.E.BYPASS.128";
    assert_eq!(sass_issue_signature(sass), "LHHL");
}

#[test]
fn candidate_source_has_physical_2211_copy_issue_schedule() {
    let retained =
        candidate_source::retained_source(SWIZZLE, S3).expect("compose retained candidate");
    let source = candidate_source::candidate_source(SWIZZLE, S3).expect("compose candidate");
    assert!(candidate_source::has_issue_pattern_2211(&source));
    assert_eq!(source.matches(candidate_source::SYMBOL).count(), 1);
    assert_eq!(source.matches(candidate_source::RETAINED_SYMBOL).count(), 0);
    assert_eq!(
        candidate_source::restore_retained_source(&source).expect("reverse candidate"),
        retained
    );
...
#[test]
fn candidate_resource_and_instruction_contract_is_frozen() {
    ...
    assert_eq!(candidate_source::EXPECTED_HMMA, 64);
    assert_eq!(candidate_source::EXPECTED_LDGSTS, 18);
}
...
mod cuda_suite {
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "m64n192_issue2211_bk64_s3",
                Self::Retained => "measured_m64n192_bk64_s3",
    ...
    fn validate_ptxas_and_sass(
        ...
        let result = (|| {
            ...
            Ok(SassStats {
                ...
                hmma,
                ldgsts,
                issue_signature: super::sass_issue_signature(entry),
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        let retained = candidate_source::retained_source(swizzle, s3)?;
        ...
        let retained_symbol = candidate_source::RETAINED_SYMBOL;
        ...
        if candidate_sass.hmma != candidate_source::EXPECTED_HMMA
            || retained_sass.hmma != candidate_source::EXPECTED_HMMA
            || candidate_sass.ldgsts != candidate_source::EXPECTED_LDGSTS
            || retained_sass.ldgsts != candidate_source::EXPECTED_LDGSTS
        {
            return Err(format!(
                "candidate/retained SASS program mismatch HMMA={}/{} LDGSTS={}/{} expected={}/{}",
                candidate_sass.hmma,
                retained_sass.hmma,
                candidate_sass.ldgsts,
                retained_sass.ldgsts,
                candidate_source::EXPECTED_HMMA,
                candidate_source::EXPECTED_LDGSTS,
            ));
        }
        if candidate_sass.issue_signature == retained_sass.issue_signature {
            return Err(
                "ptxas erased the physical LDGSTS/HMMA issue-order distinction; stop before GPU"
                    .into(),
            );
        }
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N192Issue2211CompileV1\",\"candidate_source_sha\":\"{candidate_source_sha}\",\"retained_source_sha\":\"{retained_source_sha}\",\"candidate_ptx_sha\":\"{:x}\",\"retained_ptx_sha\":\"{:x}\",\"candidate_cubin_sha\":\"{}\",\"retained_cubin_sha\":\"{}\",\"candidate_sass_sha\":\"{}\",\"retained_sass_sha\":\"{}\",\"candidate_registers\":{},\"retained_registers\":{},\"candidate_hmma\":{},\"retained_hmma\":{},\"candidate_ldgsts\":{},\"retained_ldgsts\":{},\"candidate_issue_signature\":\"{}\",\"retained_issue_signature\":\"{}\"}}",
            ...
            candidate_sass.ldgsts,
            retained_sass.ldgsts,
            candidate_sass.issue_signature,
            retained_sass.issue_signature,
        );
        Ok(())
    }

    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_F16_NT_M64N192_ISSUE2211_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_F16_NT_M64N192_ISSUE2211_DISCOVERY=1".into());
        }
        ...
        let retained_source = candidate_source::retained_source(swizzle, s3)?;
            ...
            Arm::Candidate | Arm::Retained => {
                ...
                unsafe {
                    builder.launch(LaunchConfig {
                        grid_dim: (shape.candidate_grid(), 1, 1),
                        block_dim: (candidate_source::BLOCK_THREADS, 1, 1),
                        shared_mem_bytes: candidate_source::DYNAMIC_SHARED_BYTES as u32,
                    })
                }
                .map(|_| ())
    ...
    fn validate_kernel_graph(graph: &CudaGraph, fixture: &Fixture, arm: Arm) -> Result<(), String> {
        ...
        let expected_symbol = match arm {
            ...
            Arm::Retained => candidate_source::RETAINED_SYMBOL.to_owned(),
        ...
        if actual != expected_symbol
            || (params.gridDimX, params.gridDimY, params.gridDimZ)
                != (fixture.shape.candidate_grid(), 1, 1)
            || (params.blockDimX, params.blockDimY, params.blockDimZ)
                != (candidate_source::BLOCK_THREADS, 1, 1)
            || params.sharedMemBytes != candidate_source::DYNAMIC_SHARED_BYTES as u32
        {
            return Err(format!(
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function, shared, occupancy) in [
            ...
            let threads = candidate_source::BLOCK_THREADS;
            let active = function
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfNtM64N192Issue2211ResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":{shared},\"active_ctas\":{active},\"required_active_ctas\":{occupancy}}}",
                arm.name()
            );
        }
        Ok(())
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N192Issue2211BitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha\":{},\"exceptional\":{},\"candidate_vs_measured_s3_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.m,
            shape.k_out,
            shape.reduction,
    ...
    fn timed_pair(
        ...
        for window in 0..windows {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiHalfNtM64N192Issue2211TimingV1\",\"phase\":\"{phase}\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw
    ...
    fn ada_f16_nt_m64n192_issue2211_d768_out_scout_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("half-nt-m64n192-issue2211/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("half-nt-m64n192-issue2211/cohort")?;
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N192Issue2211ScoutDecisionV1\",\"shape\":[2048,1536,768],\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{scout_pass},\"decision\":\"{}\"}}",
            scout_strata,
            if scout_pass {
                "advance_to_once7"
            } else {
        ...
        if !scout_pass {
            drop(runtime);
            quiet.verify_post_cohort("half-nt-m64n192-issue2211/post")?;
            return Ok(());
        }
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N192Issue2211RetainedDecisionV1\",\"shape\":[2048,1536,768],\"candidate\":\"{}\",\"comparator\":\"{}\",\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_strata,
            if retained_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM64N192Issue2211FinalDecisionV1\",\"shape\":[2048,1536,768],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"retained_strata\":{:?},\"fast_strata\":{:?},\"strict_fast_p50_p95_lt_0_99\":{fast_pass},\"decision\":\"{}\",\"promotion\":false}}",
