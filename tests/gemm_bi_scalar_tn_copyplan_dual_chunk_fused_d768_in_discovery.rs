//! Discovery only: retained transpose followed by one N64-G8 CTA that keeps
//! both exact SplitM chunks in independent register accumulators and finalizes.
//! No dispatcher admission or production changes.

#[path = "support/triad_f32_tn_copyplan_dual_chunk_source.rs"]
mod dual_source;
#[path = "support/triad_f32_tn_copyplan_fused_finalize_source.rs"]
mod fused_source;
...
const DUAL_MAX_REGISTERS: u32 = 168;
const DUAL_STATIC_SHARED: usize = 32_768;
const DUAL_MIN_OCCUPANCY: u32 = 3;
...
impl Cell {
    ...
    fn dual_params(self, alpha: f32) -> Result<DualChunkParams, String> {
...
#[test]
fn tn_d768_in_maps_to_exact_two_node_dual_chunk_pipeline() {
    assert_eq!(
    ...
    assert_eq!(
        D768_IN.dual_params(-0.75).unwrap(),
        DualChunkParams {
            alpha: -0.75,
            m: 768,
            n: 3072,
    ...
    assert_eq!(tail.dual_params(1.0).unwrap().k1, 1023);
    ...
    let fused = fused_source::compose_source().unwrap();
    assert_eq!(fused.matches(fused_source::FUSED_SYMBOL).count(), 1);
    assert!(!fused.contains(fused_source::PRODUCTION_SYMBOL));
    let dual_raw = dual_source::compose_raw_source();
    let dual_fused = dual_source::compose_fused_source();
    assert_eq!(dual_raw.matches(dual_source::DUAL_RAW_SYMBOL).count(), 1);
    assert_eq!(
        dual_fused.matches(dual_source::DUAL_FUSED_SYMBOL).count(),
        1
    );
    assert_eq!(
        (size_of::<DualChunkParams>(), align_of::<DualChunkParams>()),
        (32, 4)
    );
    assert_eq!(
        (DUAL_MAX_REGISTERS, DUAL_STATIC_SHARED, DUAL_MIN_OCCUPANCY),
        (168, 32_768, 3)
    );
}
...
#[test]
fn cuda_harness_routes_dual_candidate_and_screens_fast_only_after_retained_win() {
    let harness = include_str!("gemm_bi_scalar_tn_copyplan_dual_chunk_fused_d768_in_discovery.rs");
    assert!(harness.contains("Arm::Candidate => launch_dual_fused(runtime, fixture)"));
    assert!(
        harness.contains("Arm::Retained => launch_fused_candidate(runtime, fixture, candidate)")
    );
    assert!(harness.contains("launch_dual_raw(runtime, fixture).map_err(&harness)?"));
    assert!(harness.contains("require_chain(graph, &[transpose.node, fused.node], \"dual-chunk\")"));
    assert!(
        harness.contains("if strict_win {\n                        comparators.push(Arm::Fast);")
    );
    assert!(harness.contains("registers > DUAL_MAX_REGISTERS as i32"));
    assert!(harness.contains("occupancy < DUAL_MIN_OCCUPANCY"));
    assert!(
...
mod splitm_cuda_suite {
    ...
    const FUSED: &str = fused_source::FUSED_SYMBOL;
    const DUAL_RAW: &str = dual_source::DUAL_RAW_SYMBOL;
    const DUAL_FUSED: &str = dual_source::DUAL_FUSED_SYMBOL;
    ...
    unsafe impl DeviceRepr for DualChunkParams {}
    ...
    impl Fixture {
        ...
        fn validate_unused_chunk1_slab(&self, ctx: &GpuCtx) -> Result<(), String> {
            if self.cell.batch == 0 {
                return Ok(());
            }
            let actual = self.candidate_partial.bits(ctx)?;
            let stride = self.cell.k_out * self.cell.n_out;
            if actual[stride..2 * stride]
                .iter()
                .zip(&self.candidate_partial.seed[stride..2 * stride])
                .any(|(word, seed)| *word != seed.to_bits())
            {
                return Err("fused candidate wrote unused chunk1 partial slab".into());
            }
            Ok(())
        }
    ...
    struct Runtime {
        ...
        _dual_raw_module: Arc<CudaModule>,
        _dual_fused_module: Arc<CudaModule>,
        raw_fixed: CudaFunction,
        fused: CudaFunction,
        dual_raw: CudaFunction,
        dual_fused: CudaFunction,
        raw_source_sha: String,
        fused_source_sha: String,
        dual_raw_source_sha: String,
        dual_fused_source_sha: String,
    }
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        let fused_source = fused_source::compose_source()?;
        let fused_source_sha = format!("{:x}", Sha256::digest(fused_source.as_bytes()));
        let fused_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            fused_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
        ...
        .map_err(|error| format!("compile fused-finalize CopyPlan: {error:?}"))?;
        let fused_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(fused_ptx.to_src()))
            .map_err(|error| format!("load fused-finalize CopyPlan module: {error:?}"))?;
        let fused = fused_module
            .load_function(FUSED)
            .map_err(|error| format!("load {FUSED}: {error:?}"))?;
        let dual_raw_source = dual_source::compose_raw_source();
        let dual_raw_source_sha = format!("{:x}", Sha256::digest(dual_raw_source.as_bytes()));
        let dual_raw_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            dual_raw_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
        ...
        .map_err(|error| format!("compile dual-chunk raw: {error:?}"))?;
        validate_ptxas_and_sass(DUAL_RAW, &dual_raw_ptx.to_src())?;
        let dual_raw_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(dual_raw_ptx.to_src()))
            .map_err(|error| format!("load dual-chunk raw module: {error:?}"))?;
        let dual_raw = dual_raw_module
            .load_function(DUAL_RAW)
            .map_err(|error| format!("load {DUAL_RAW}: {error:?}"))?;
        let dual_fused_source = dual_source::compose_fused_source();
        let dual_fused_source_sha = format!("{:x}", Sha256::digest(dual_fused_source.as_bytes()));
        let dual_fused_ptx = cudarc::nvrtc::compile_ptx_with_opts(
            dual_fused_source,
            cudarc::nvrtc::CompileOptions {
                arch: Some("sm_89"),
                options: vec!["--fmad=true".into(), "-DNDEBUG".into()],
        ...
        .map_err(|error| format!("compile dual-chunk fused: {error:?}"))?;
        validate_ptxas_and_sass(DUAL_FUSED, &dual_fused_ptx.to_src())?;
        let dual_fused_module = device
            .context()
            .load_module(cudarc::nvrtc::Ptx::from_src(dual_fused_ptx.to_src()))
            .map_err(|error| format!("load dual-chunk fused module: {error:?}"))?;
        let dual_fused = dual_fused_module
            .load_function(DUAL_FUSED)
            .map_err(|error| format!("load {DUAL_FUSED}: {error:?}"))?;
        Ok(Runtime {
            _device: device,
            ctx,
            _raw_module: module,
            _fused_module: fused_module,
            _dual_raw_module: dual_raw_module,
            _dual_fused_module: dual_fused_module,
            raw_fixed,
            fused,
            dual_raw,
            dual_fused,
    ...
    fn launch_fused_candidate(
        runtime: &Runtime,
        fixture: &Fixture,
        chunk0_fixed: CandidateFixed,
    ) -> Result<(), String> {
        if fixture.cell.batch == 0 {
            return launch_zero(runtime, fixture);
        }
        let chunks = fixture.cell.chunks()?;
        let chunk0 = chunks[0];
        let chunk1 = chunks[1];
        launch_transpose(runtime, fixture)?;

        let partial0 = fixture.candidate_partial.ptr();
        let transposed = fixture.transposed.ptr();
        let dy = fixture.dy.ptr();
        let null_bias = 0_u64;
        let mut chunk0_builder = runtime
            .ctx
            .stream
            .launch_builder(fixed_function(runtime, chunk0_fixed)?);
        let chunk0_a = transposed + (chunk0.begin * size_of::<f32>()) as u64;
        let chunk0_b = dy + (chunk0.begin * fixture.cell.n_out * size_of::<f32>()) as u64;
        let chunk0_params = fixture.cell.fixed_params(chunk0);
        chunk0_builder.arg(&partial0);
        chunk0_builder.arg(&chunk0_a);
        chunk0_builder.arg(&chunk0_b);
        chunk0_builder.arg(&null_bias);
        chunk0_builder.arg(&chunk0_params);
        unsafe { chunk0_builder.launch(config(fixture.cell.fixed_grid(), (128, 1, 1), 0)) }
            .map_err(|error| format!("launch retained chunk0: {error:?}"))?;

        let output = fixture.output.ptr();
        let chunk1_a = transposed + (chunk1.begin * size_of::<f32>()) as u64;
        let chunk1_b = dy + (chunk1.begin * fixture.cell.n_out * size_of::<f32>()) as u64;
        let fused_params = FixedParams {
            alpha: fixture.alpha,
            beta: 1.0,
            ..fixture.cell.fixed_params(chunk1)
        };
        let mut fused_builder = runtime.ctx.stream.launch_builder(&runtime.fused);
        fused_builder.arg(&output);
        fused_builder.arg(&chunk1_a);
        fused_builder.arg(&chunk1_b);
        fused_builder.arg(&partial0);
        fused_builder.arg(&fused_params);
        unsafe { fused_builder.launch(config(fixture.cell.fixed_grid(), (128, 1, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch fused chunk1 {FUSED}: {error:?}"))
    }

    fn launch_dual_raw(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        if fixture.cell.batch == 0 {
            return Err("zero reduction has no dual raw launch".into());
        }
        launch_transpose(runtime, fixture)?;
        let partial = fixture.candidate_partial.ptr();
        let transposed = fixture.transposed.ptr();
        let dy = fixture.dy.ptr();
        let params = fixture.cell.dual_params(1.0)?;
        let mut builder = runtime.ctx.stream.launch_builder(&runtime.dual_raw);
        builder.arg(&partial);
        ...
        unsafe { builder.launch(config(fixture.cell.fixed_grid(), (128, 1, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch {DUAL_RAW}: {error:?}"))
    }

    fn launch_dual_fused(runtime: &Runtime, fixture: &Fixture) -> Result<(), String> {
        if fixture.cell.batch == 0 {
            return launch_zero(runtime, fixture);
        }
        launch_transpose(runtime, fixture)?;
        ...
        let transposed = fixture.transposed.ptr();
        let dy = fixture.dy.ptr();
        let params = fixture.cell.dual_params(fixture.alpha)?;
        let mut builder = runtime.ctx.stream.launch_builder(&runtime.dual_fused);
        builder.arg(&output);
        builder.arg(&transposed);
        builder.arg(&dy);
        builder.arg(&params);
        unsafe { builder.launch(config(fixture.cell.fixed_grid(), (128, 1, 1), 0)) }
            .map(|_| ())
            .map_err(|error| format!("launch {DUAL_FUSED}: {error:?}"))
    }
    ...
    fn launch(
        ...
        match arm {
            Arm::Candidate => launch_dual_fused(runtime, fixture),
            Arm::Reference => {
                launch_reference_partials(runtime, fixture)?;
                launch_reducer(runtime, fixture, fixture.reference_partial.ptr())
            }
            Arm::Retained => launch_fused_candidate(runtime, fixture, candidate),
            Arm::Fast => {
    ...
    fn compare_dual_raw_partials(
        ...
        fixture.reset(&runtime.ctx).map_err(&harness)?;
        launch_reference_partials(runtime, fixture).map_err(&harness)?;
        launch_dual_raw(runtime, fixture).map_err(&harness)?;
        let reference = fixture
            .reference_partial
            .bits(&runtime.ctx)
            .map_err(&harness)?;
        ...
        for index in raw_plane_indices(fixture.cell).map_err(&harness)? {
                ...
                .position(|(a, b)| a != b)
            {
                let word = begin + relative;
                return Err(RawPartialError::Mismatch(format!(
                    "{corpus} {DUAL_RAW} raw partial fc={index} m=[{},{}), word={relative}: {:08x} != {:08x}",
                    chunks[index].begin,
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnDualChunkRawPartialV1",
                    "corpus":corpus,
                    "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                    "candidate":DUAL_RAW,
                    "fc":index,
    ...
    unsafe fn require_dual_graph(graph: &CudaGraph, fixture: &Fixture) -> Result<(), String> {
        if fixture.cell.batch == 0 {
        ...
        if nodes.len() != 2 {
            return Err(format!(
                "dual-chunk candidate has {} nodes, expected two",
                nodes.len()
            ));
        }
        let transpose = nodes
            .iter()
            .find(|node| node.symbol == TRANSPOSE)
            .ok_or_else(|| format!("dual-chunk candidate omitted {TRANSPOSE}"))?;
        let fused = nodes
            .iter()
            .find(|node| node.symbol == DUAL_FUSED)
            .ok_or_else(|| format!("dual-chunk candidate omitted {DUAL_FUSED}"))?;
        require_launch(
            transpose,
            TRANSPOSE,
            fixture.cell.transpose_grid(),
        ...
        require_launch(fused, DUAL_FUSED, fixture.cell.fixed_grid(), (128, 1, 1), 0)?;
        if unsafe { graph_arg::<u64>(transpose, 0) }? != fixture.transposed.ptr()
            || unsafe { graph_arg::<u64>(transpose, 1) }? != fixture.x.ptr()
            || unsafe { graph_arg::<i32>(transpose, 2) }? != fixture.cell.batch as i32
            || unsafe { graph_arg::<i32>(transpose, 3) }? != fixture.cell.k_out as i32
        {
            return Err("dual-chunk transpose ABI/arguments changed".into());
        }
        if unsafe { graph_arg::<u64>(fused, 0) }? != fixture.output.ptr()
            ...
            || unsafe { graph_arg::<DualChunkParams>(fused, 3) }?
                != fixture.cell.dual_params(fixture.alpha)?
        {
            return Err("dual-chunk fused ABI/arguments changed".into());
        }
        unsafe { require_chain(graph, &[transpose.node, fused.node], "dual-chunk") }?;
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDualChunkGraphV1",
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "nodes":[TRANSPOSE,DUAL_FUSED],
                "edges":[[TRANSPOSE,DUAL_FUSED]],
    ...
    unsafe fn require_retained_fused_graph(
        ...
        if fixture.cell.batch == 0 {
            return unsafe { require_retained_graph(graph, fixture, chunk0_fixed) };
        }
        let nodes = unsafe { observed_kernels(graph) }?;
        if nodes.len() != 3 {
            return Err(format!(
                "retained fused pipeline has {} nodes, expected three",
                nodes.len()
            ));
        }
        let transpose = nodes
            .iter()
            .find(|node| node.symbol == TRANSPOSE)
            .ok_or_else(|| format!("fused candidate omitted {TRANSPOSE}"))?;
        let chunk0 = nodes
            .iter()
            .find(|node| node.symbol == chunk0_fixed.symbol())
            .ok_or_else(|| format!("fused candidate omitted {}", chunk0_fixed.symbol()))?;
        let fused = nodes
            .iter()
            .find(|node| node.symbol == FUSED)
            .ok_or_else(|| format!("fused candidate omitted {FUSED}"))?;
        require_launch(
            transpose,
            TRANSPOSE,
            fixture.cell.transpose_grid(),
        ...
        require_launch(fused, FUSED, fixture.cell.fixed_grid(), (128, 1, 1), 0)?;
        if unsafe { graph_arg::<u64>(transpose, 0) }? != fixture.transposed.ptr()
            || unsafe { graph_arg::<u64>(transpose, 1) }? != fixture.x.ptr()
            || unsafe { graph_arg::<i32>(transpose, 2) }? != fixture.cell.batch as i32
            || unsafe { graph_arg::<i32>(transpose, 3) }? != fixture.cell.k_out as i32
        {
            return Err("fused candidate transpose ABI/arguments changed".into());
        }
        let chunks = fixture.cell.chunks()?;
        let chunk0_spec = chunks[0];
        let chunk1_spec = chunks[1];
        let expected_chunk0_a =
            fixture.transposed.ptr() + (chunk0_spec.begin * size_of::<f32>()) as u64;
        let expected_chunk0_b =
            fixture.dy.ptr() + (chunk0_spec.begin * fixture.cell.n_out * size_of::<f32>()) as u64;
        if unsafe { graph_arg::<u64>(chunk0, 0) }? != fixture.candidate_partial.ptr()
            || unsafe { graph_arg::<u64>(chunk0, 1) }? != expected_chunk0_a
            || unsafe { graph_arg::<u64>(chunk0, 2) }? != expected_chunk0_b
            || unsafe { graph_arg::<u64>(chunk0, 3) }? != 0
            || unsafe { graph_arg::<FixedParams>(chunk0, 4) }?
                != fixture.cell.fixed_params(chunk0_spec)
        {
            return Err("fused candidate chunk0 ABI/arguments changed".into());
        }
        let expected_chunk1_a =
            fixture.transposed.ptr() + (chunk1_spec.begin * size_of::<f32>()) as u64;
        let expected_chunk1_b =
            fixture.dy.ptr() + (chunk1_spec.begin * fixture.cell.n_out * size_of::<f32>()) as u64;
        let expected_fused_params = FixedParams {
            alpha: fixture.alpha,
            beta: 1.0,
            ..fixture.cell.fixed_params(chunk1_spec)
        };
        if unsafe { graph_arg::<u64>(fused, 0) }? != fixture.output.ptr()
            || unsafe { graph_arg::<u64>(fused, 1) }? != expected_chunk1_a
            || unsafe { graph_arg::<u64>(fused, 2) }? != expected_chunk1_b
            || unsafe { graph_arg::<u64>(fused, 3) }? != fixture.candidate_partial.ptr()
            || unsafe { graph_arg::<FixedParams>(fused, 4) }? != expected_fused_params
        {
            return Err("fused candidate chunk1 ABI/arguments changed".into());
        }
        unsafe {
            require_chain(
                graph,
                &[transpose.node, chunk0.node, fused.node],
                "fused candidate",
            )
        }?;
        println!(
            ...
            json!({
                ...
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "nodes":[TRANSPOSE,chunk0_fixed.symbol(),FUSED],
                "edges":[[TRANSPOSE,chunk0_fixed.symbol()],[chunk0_fixed.symbol(),FUSED]],
                "order":["transpose","chunk0_raw","chunk1_finalize"]
            })
        );
        Ok(())
    }
    ...
    fn output_after(
        ...
        match arm {
            ...
            Arm::Retained if fixture.cell.batch > 0 => {
                fixture.validate_transpose(&runtime.ctx)?;
                fixture.validate_unused_chunk1_slab(&runtime.ctx)?;
            }
    ...
    fn check_exact_case(
        ...
        unsafe {
            require_dual_graph(&candidate_graph, fixture)?;
            match oracle {
                Arm::Retained if fixture.cell.batch > 0 => {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDualChunkBitsV1",
                "case":label,
                "shape":[fixture.cell.batch,fixture.cell.k_out,fixture.cell.n_out],
                "alpha_bits":fixture.alpha.to_bits(),
                "candidate":DUAL_FUSED,
    ...
    fn dual_candidate_resources(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        let registers = function
            .num_regs()
            .map_err(|error| format!("{symbol} regs: {error:?}"))?;
        let local = function
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDualChunkHardResourceV1",
                "symbol":symbol,
                "threads":128,
                "registers":registers,
                "registers_max":DUAL_MAX_REGISTERS,
                "local_bytes":local,
                "static_shared_bytes":shared,
                "static_shared_expected":DUAL_STATIC_SHARED,
                "dynamic_shared_bytes":0,
                "occupancy":occupancy,
                "occupancy_min":DUAL_MIN_OCCUPANCY
            })
        );
        if registers > DUAL_MAX_REGISTERS as i32
            || local != 0
            || shared as usize != DUAL_STATIC_SHARED
            || occupancy < DUAL_MIN_OCCUPANCY
        {
            return Err(format!(
    ...
    #[test]
    #[ignore = "Ada CUDA13.2 discovery: exact F32 TN d768-in dual-chunk fused"]
    fn ada_f32_tn_copyplan_dual_chunk_fused_d768_in_once7() -> Result<(), String> {
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("f32-tn-dual-chunk/pre", 2_000)?;
        } else {
            quiet.require_pre_context("f32-tn-dual-chunk/pre")?;
        }
        let runtime = new_runtime()?;

        for (function, symbol) in [
            (&runtime.dual_raw, DUAL_RAW),
            (&runtime.dual_fused, DUAL_FUSED),
        ] {
            if let Err(reason) = dual_candidate_resources(function, symbol) {
                println!(
                    "{}",
                    json!({
                        "schema":"MambaBiF32TnDualChunkResourceStopV1",
                        "candidate":[DUAL_RAW,DUAL_FUSED],
                        "reason":reason,
                ...
                if idle_resident_mode {
                    quiet.require_idle_resident("f32-tn-dual-chunk/resource-stop", 256)?;
                } else {
                    quiet.verify_post_cohort("f32-tn-dual-chunk/resource-stop")?;
                }
                return Ok(());
            }
        }

        // Prove the committed three-node N64-G8 fused comparator before any
        ...
        prove_dual_raw_accumulators(&runtime)?;
        let fixed = fixed_function(&runtime, candidate)?;
        resources(fixed, candidate.symbol(), 128, 32_768)?;
        resources(&runtime.fused, FUSED, 128, 32_768)?;
        resources(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDualChunkArtifactsV1",
                "artifacts":format!("{:?}",runtime.ctx.kernels.artifact_set_identity()),
                "raw_store_source_sha":runtime.raw_source_sha,
                "fused_source_sha":runtime.fused_source_sha,
                "dual_raw_source_sha":runtime.dual_raw_source_sha,
                "dual_fused_source_sha":runtime.dual_fused_source_sha,
                "candidate_nodes":[TRANSPOSE,DUAL_FUSED],
                "retained_nodes":[TRANSPOSE,candidate.symbol(),FUSED],
        ...
        compare_dual_raw_partials(&runtime, &mut tail, "tail_chunk_dual_accumulators")
            .map_err(|error| error.to_string())?;
        ...
        // The whole two-node pipeline is timed as one logical GEMM. Resets,
        ...
        unsafe { require_dual_graph(&candidate_graph, &target) }?;
        ...
        while comparator_index < comparators.len() {
            ...
            if idle_resident_mode {
                quiet.require_idle_resident("f32-tn-dual-chunk/timed", 256)?;
            } else {
                quiet.require_cohort("f32-tn-dual-chunk/timed")?;
            }
            ...
            for graph_path in [false, true] {
                for candidate_endpoints in [true, false] {
                    ...
                    println!(
                        "{}",
                        json!({
                            "schema":"MambaBiF32TnDualChunkScreenV1",
                            "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                            "comparator":if comparator == Arm::Retained {"N64_G8_FUSED"} else {"Fast"},
                            ...
                            "candidate_timed_nodes":[TRANSPOSE,DUAL_FUSED],
            ...
            println!(
                "{}",
                json!({
                    "schema":"MambaBiF32TnDualChunkDecisionV1",
                    "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                    "comparator":if comparator == Arm::Retained {"N64_G8_FUSED"} else {"Fast"},
                    "strata":strata,
                    "strict_win":strict_win,
                    "decision":match (comparator,strict_win) {
                        (Arm::Retained,true)=>"dual_chunk_beats_g8_fused",
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiF32TnDualChunkFinalDecisionV1",
                "shape":[D768_IN.batch,D768_IN.k_out,D768_IN.n_out],
                "candidate":DUAL_FUSED,
        ...
        if idle_resident_mode {
            quiet.require_idle_resident("f32-tn-dual-chunk/post", 256)?;
        } else {
