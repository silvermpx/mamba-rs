//! Test-only canonical Ada TF32 NT prism_in_proj A-ldmatrix K8 register-pipeline discovery.

#[path = "support/triad_tf32_nt_a_ldmatrix_prism_k8_regpipe_source.rs"]
...
#[test]
fn canonical_prism_uses_the_frozen_linear_grid_and_resource_budget() {
    assert_eq!(candidate_source::TARGET, (4_621, 384, 1_928));
    assert_eq!(candidate_source::TARGET_GRID, (222, 1, 1));
...
fn source_transform_is_reversible_and_keeps_retained_math() {
    ...
    assert!(candidate.contains("gemm_bi_tf32_nt_a_ldmatrix_prism_k8_load_a"));
    assert!(candidate.contains("unsigned a_fragments[2][2][4];"));
    assert!(candidate.contains("unsigned b_fragment[2];"));
    assert!(candidate.contains("int k8 = issue * 8;"));
    assert!(candidate.contains("int next_slot = (issue + 1) & 1;"));
...
mod cuda_suite {
    ...
    const ENV: &str = "MAMBA_TRIAD_TF32_NT_A_LDMATRIX_PRISM_K8_REGPIPE_DISCOVERY";
    const OPS: usize = 20;
    ...
    impl Shape {
        ...
        fn grid_for(self, _arm: Arm) -> Result<(u32, u32, u32), String> {
            self.grid()
        }
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "a_ldmatrix_prism_k8_regpipe",
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        let (candidate_ptx, candidate_source_sha) =
            compile_ptx_only(&candidate, "canonical Prism TF32 NT K8 regpipe candidate")?;
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixPrismK8RegpipeCompileV1",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        ...
        if cfg!(debug_assertions) {
            return Err("canonical Prism TF32 NT K8 regpipe timing requires --release".into());
        }
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixPrismK8RegpipeIdentityV1",
                ...
                "change":"double_buffer_a_k8_lookahead_with_b_fragment_lifetime_bounded_to_one_n_atom",
            })
        );
        Ok(runtime)
    }
    ...
    fn resource_gate(function: &CudaFunction, symbol: &str) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixPrismK8RegpipeResourceV1",
    ...
    fn graph_identity(graph: &CudaGraph, shape: Shape, arm: Arm) -> Result<(), String> {
        unsafe {
            ...
            if arm == Arm::Fast {
                println!(
                    "{}",
                    json!({"schema":"MambaBiTriadTf32NtALdmatrixPrismK8RegpipeGraphV1","arm":arm.name(),"nodes":count,"abi":"opaque","timing":"whole_graph"})
                );
                return Ok(());
            }
            if count != 1 {
    ...
    fn check_exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixPrismK8RegpipeBitsV1",
    ...
    fn screen(
        ...
        println!(
            "{}",
            json!({
                "schema":"MambaBiTriadTf32NtALdmatrixPrismK8RegpipeScreenV1",
    ...
    #[test]
    #[ignore = "requires exclusive quiet CC8.9/142-SM CUDA13.2; canonical Prism TF32 NT K8 regpipe once3/once7"]
    fn ada_tf32_nt_a_ldmatrix_prism_k8_regpipe_scout3_then_once7() -> Result<(), String> {
        ...
        quiet.require_pre_context("tf32-nt-a-ldmatrix-prism-k8-regpipe/pre")?;
        ...
        quiet.require_cohort("tf32-nt-a-ldmatrix-prism-k8-regpipe/scout")?;
        ...
        if !scout_pass {
            println!(
                "{}",
                json!({
                    "schema":"MambaBiTriadTf32NtALdmatrixPrismK8RegpipeDecisionV1",
                    "cell":TARGET.label,"retained_scout":scout,
                    "decision":"stop_scout_retained_loss","promotion":false,
                })
            );
            quiet.verify_post_cohort("tf32-nt-a-ldmatrix-prism-k8-regpipe/scout-post")?;
            return Ok(());
        }
        quiet.require_cohort("tf32-nt-a-ldmatrix-prism-k8-regpipe/qualification")?;
        ...
        if !may_run_fast(&retained) {
            ...
            quiet.verify_post_cohort("tf32-nt-a-ldmatrix-prism-k8-regpipe/retained-post")?;
            return Ok(());
        }
        ...
        quiet.verify_post_cohort("tf32-nt-a-ldmatrix-prism-k8-regpipe/post")?;
        Ok(())
    }
}
