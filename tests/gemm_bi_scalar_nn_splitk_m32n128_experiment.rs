...
const CANDIDATE_SYMBOL: &str = "gemm_bi_nn_splitk32_m32n128_bk32_exp_v1";
const PRODUCTION_PARTIAL_SYMBOL: &str = "gemm_bi_nn_splitk32_partial";
const REDUCER_SYMBOL: &str = "gemm_bi_splitk_reduce";
const CANDIDATE_SOURCE: &str = include_str!("gemm_bi_scalar_nn_splitk_m32n128_experiment.cu");
const HARNESS_SOURCE: &str = include_str!("gemm_bi_scalar_nn_splitk_m32n128_experiment.rs");
...
fn qualification_windows(value: Option<&str>) -> Result<usize, String> {
    ...
    if !matches!(windows, 21 | OFFICIAL_WINDOWS) {
        return Err(format!(
            "screening requires 21 windows and official qualification requires {OFFICIAL_WINDOWS}, found {windows}"
        ));
    }
    Ok(windows)
}
...
fn validate_performance_gate(order: &str, p05: f64, p50: f64, p95: f64) -> Result<(), String> {
    ...
    if p05 < MIN_P05_SPEEDUP || p50 < MIN_P50_SPEEDUP {
        return Err(format!(
            "M32N128 Split-K candidate loses the {order} gate: p05={p05:.9} p50={p50:.9}; required p05>={MIN_P05_SPEEDUP:.2} and p50>={MIN_P50_SPEEDUP:.2}"
        ));
    }
    Ok(())
}
...
fn verify_post_cohort_even_on_error(
    ...
    match (body, postflight) {
        ...
        (Err(body), Err(postflight)) => Err(format!(
            "{body}; quiet-GPU postflight also failed: {postflight}"
        )),
    }
}
...
fn candidate_source_contract() {
    ...
    for forbidden in ["atomic", "mma.sync", "wmma", "split_k ="] {
        assert!(
...
fn harness_contract_is_exact_shape_two_nodes_and_full_partial_compare() {
    ...
    assert_eq!(expected_candidate_configs()[0].1, (1_024, 1, 1));
...
#[test]
fn quiet_postflight_errors_are_never_hidden() {
    ...
    assert!(error.contains("body") && error.contains("post"));
}
...
fn timing_gates_reject_invalid_samples_thresholds_and_tiny_windows() {
    ...
    assert!(validate_performance_gate("test", 1.01, 1.01, 1.10).is_ok());
    assert!(validate_performance_gate("test", 1.009, 1.01, 1.10).is_err());
    assert!(validate_performance_gate("test", 1.01, 1.009, 1.10).is_err());
}
...
fn candidate_sass_block<'a>(sass: &'a str, symbol: &str) -> &'a str {
    let label = format!("\n{symbol}:\n");
    let start = sass
        .find(&label)
        .unwrap_or_else(|| panic!("nvdisasm lost {symbol}"))
        + 1;
    let tail = &sass[start..];
    let end = tail
        .find("\n//--------------------- .text.")
...
#[test]
fn sass_parser_is_symbol_scoped() {
    let sass = "\nfirst:\n        ATOM.E.ADD R0, [R2], R3;\n//--------------------- .text.gemm_bi_nn_splitk32_m32n128_bk32_exp_v1\n\t.section\t.text.gemm_bi_nn_splitk32_m32n128_bk32_exp_v1\n\ngemm_bi_nn_splitk32_m32n128_bk32_exp_v1:\n        FFMA R4, R5, R6, R4;\n//--------------------- .text.last\n\t.section\t.text.last\n\nlast:\n        MMA R0, R1;\n";
    let block = candidate_sass_block(sass, CANDIDATE_SYMBOL);
    assert!(block.contains("FFMA"));
    assert!(!block.contains("ATOM"));
    assert!(!block.contains("MMA"));
}
...
fn candidate_compiles_and_meets_sm89_sm120_ptxas_resources() {
    for (arch, sm) in [("compute_89", "sm_89"), ("compute_120", "sm_120")] {
        ...
        let stem = format!("mamba-rs-nn-splitk-m32n128-{}-{sm}", std::process::id());
        ...
        let nvdisasm = std::env::var("CUDA_HOME")
            .map(|root| std::path::PathBuf::from(root).join("bin/nvdisasm"))
            .unwrap_or_else(|_| "nvdisasm".into());
        let disassembly = std::process::Command::new(nvdisasm)
            .arg(&cubin_path)
            .output()
            .unwrap_or_else(|error| panic!("launch nvdisasm for {sm}: {error}"));
        assert!(
        ...
        let candidate = candidate_sass_block(&sass, CANDIDATE_SYMBOL);
        assert!(candidate.contains("FFMA"), "{sm} candidate lost FFMA");
        for forbidden in ["ATOM", "RED.", "MMA"] {
            assert!(
                !candidate.contains(forbidden),
                "{sm} candidate contains {forbidden}:\n{candidate}"
            );
        }
...
mod cuda_tournament {
    ...
    impl Arm {
        fn name(self) -> &'static str {
            match self {
                ...
                Self::Candidate => "candidate_splitk32_m32n128",
            }
        }
    }
    ...
    fn check_resources(kernel: &Kernel, contract: ResourceContract) -> Result<(), String> {
        ...
        eprintln!(
            "nn_splitk_m32n128 resource symbol={} threads={} registers={} local={} static_shared={} dynamic_shared={} occupancy={}",
            kernel.symbol,
            threads,
            registers,
            local,
    ...
    fn measure(
        ...
        for _ in 0..iterations {
            match (path, arm) {
                (Path::Eager, _) => launch_arm(runtime, fixture, arm)?,
                (Path::Graph, Arm::Production) => graphs
                    .production
                    .launch()
                    .map_err(|error| format!("production graph timing: {error:?}"))?,
                (Path::Graph, Arm::Candidate) => graphs
                    .candidate
                    .launch()
                    .map_err(|error| format!("candidate graph timing: {error:?}"))?,
            }
        }
        let end = runtime
            .ctx
    ...
    fn calibrated_iterations(
        ...
        let pilot = measure(runtime, fixture, arm, path, graphs, 3)?;
        Ok((TARGET_WINDOW_US / pilot).round().clamp(3.0, 500.0) as usize)
    }

    struct GraphPair {
        production: CudaGraph,
        candidate: CudaGraph,
    }
    ...
    fn paired(
        ...
        let production_iterations =
            calibrated_iterations(runtime, fixture, Arm::Production, path, graphs)?;
        let candidate_iterations =
            calibrated_iterations(runtime, fixture, Arm::Candidate, path, graphs)?;
        ...
        for (order, baseline_first) in [("ABBA", true), ("BAAB", false)] {
            ...
            eprintln!(
                "nn_splitk_m32n128 path={} baseline={} candidate={} order={} windows={} production_us_p50={:.6} candidate_us_p50={:.6} speedup_p05={:.9} speedup_p50={:.9} speedup_p95={:.9} production_iterations={} candidate_iterations={}",
                path.name(),
                Arm::Production.name(),
                Arm::Candidate.name(),
                order,
    ...
    fn candidate_partial_and_final_bits_are_exact_and_graph_stable() -> Result<(), String> {
        ...
        check_resources(
            ...
            ResourceContract {
                threads: 128,
                static_shared: 0,
                dynamic_shared: CANDIDATE_SHARED as usize,
                register_cap: 112,
                minimum_occupancy: 3,
            },
        )?;
    ...
    fn candidate_vs_production_101_window_abba_baab() -> Result<(), String> {
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        quiet.require_pre_context("nn-splitk-m32n128/pre-context")?;
        let body = (|| {
            let runtime = new_runtime()?;
            let fixture = new_fixture(&runtime)?;
            let graphs = GraphPair {
                production: capture(&runtime, &fixture, Arm::Production)?,
                candidate: capture(&runtime, &fixture, Arm::Candidate)?,
            };
            validate_candidate_identity(&graph_candidate_identity(&graphs.candidate)?)?;
            for _ in 0..10 {
                ...
                graphs
                    .production
                    .launch()
                    .map_err(|error| format!("production graph warmup: {error:?}"))?;
                graphs
                    .candidate
                    .launch()
                    .map_err(|error| format!("candidate graph warmup: {error:?}"))?;
            }
            runtime
                .ctx
                .stream
            ...
            let value = match std::env::var("MAMBA_RS_NN_SPLITK_M32N128_WINDOWS") {
                Ok(value) => Some(value),
                Err(std::env::VarError::NotPresent) => None,
                Err(std::env::VarError::NotUnicode(_)) => {
                    return Err("MAMBA_RS_NN_SPLITK_M32N128_WINDOWS is not Unicode".into());
                }
            };
            let windows = qualification_windows(value.as_deref())?;
            for path in [Path::Eager, Path::Graph] {
                quiet.require_cohort(&format!("nn-splitk-m32n128/{path:?}"))?;
                for summary in paired(&runtime, &fixture, path, &graphs, windows)? {
        ...
        verify_post_cohort_even_on_error(
            body,
            quiet
                .verify_post_cohort("nn-splitk-m32n128/postflight")
                .map(drop),
        )
    }
}
