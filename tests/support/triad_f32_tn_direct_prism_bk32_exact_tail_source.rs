pub const SYMBOL: &str = "gemm_bi_tn_test_direct_prism_sm89_f32_m64n64_bk32_s2_exact_tail_raw_v1";
...
#define PRISM_DIRECT_TN_SMEM_BYTES 32768
static_assert(
...
extern "C" __global__ __launch_bounds__(PRISM_DIRECT_TN_THREADS, 3)
void gemm_bi_tn_test_direct_prism_sm89_f32_m64n64_bk32_s2_exact_tail_raw_v1(
...
#define PRISM_DIRECT_TN_FMA_STEP(KK) do {                                         \
    ...
    for (int tile = 0; tile < num_k_tiles; ++tile) {
        ...
        const int valid_k = min(PRISM_DIRECT_TN_BK, chunk_k - tile * PRISM_DIRECT_TN_BK);
        if (valid_k == PRISM_DIRECT_TN_BK) {
#pragma unroll
...
mod tests {
    ...
    fn source_pins_direct_prism_geometry_and_exact_order() {
        ...
        assert!(source.contains("PRISM_DIRECT_TN_BK 32"));
        assert!(source.contains("PRISM_DIRECT_TN_GROUP_M 6"));
        assert!(source.contains("PRISM_DIRECT_TN_SMEM_BYTES 32768"));
        assert!(source.contains("__launch_bounds__(PRISM_DIRECT_TN_THREADS, 3)"));
    ...
    #[test]
    fn source_keeps_raw_splitm_abi_and_bk32_exact_tail_order() {
        let source = compose_source();
        ...
        assert!(source.contains("for (int kk = 0; kk < PRISM_DIRECT_TN_BK; ++kk)"));
        assert!(source.contains("const int valid_k = min(PRISM_DIRECT_TN_BK"));
