...
//! Live compile/ABI/resource qualification for the sealed Ada exact-F32 d128 module.
//! This test does not launch or admit either route.

use mamba_rs::mamba_ssm::gpu::device::GpuDevice;
use mamba_rs::mamba_ssm::gpu::gemm_bi_triad::SM89_EXACT_F32_D128_KERNEL_SPECS;
use mamba_rs::mamba_ssm::gpu::kernel_identity::{ModuleKind, digest_hex};

#[test]
#[ignore = "requires an SM89 GPU and explicit CUDA 12.8/13.0/13.2 selection"]
fn sm89_exact_f32_d128_module_abi_and_resources_qualify() -> Result<(), String> {
    ...
    let compiler = ctx
        ...
        .ok_or_else(|| {
            format!(
                "TriadSm89ExactF32D128 did not bind: {:?}",
                ctx.kernels.triad_sm89_exact_f32_d128_rejection()
            )
        })?;
    let artifact = ctx
        .kernels
        .triad_sm89_exact_f32_d128_artifact_identity()
        .ok_or_else(|| "TriadSm89ExactF32D128 has no artifact identity".to_string())?;
    ...
    {
        return Err(format!(
            "live exact-F32 d128 module is not the canonical SM89 binding: cc={:?} target={} kind={:?}",
    ...
    if !exclusions.is_empty() {
        return Err(format!(
            "TriadSm89ExactF32D128 has resource/ABI exclusions: {exclusions:?}"
        ));
    }
    ...
    for spec in SM89_EXACT_F32_D128_KERNEL_SPECS {
        ...
        if registers > spec.register_cap
            ...
            || max_threads < i32::try_from(threads).unwrap_or(i32::MAX)
            || occupancy < spec.occupancy_gate
        {
            return Err(format!(
                "{} misses its sealed resource gate: registers={registers}/{} local={local_bytes}/{} static_shared={static_shared_bytes}/{} max_threads={max_threads}/{threads} occupancy={occupancy}/{}",
                spec.symbol,
                spec.register_cap,
                spec.local_bytes,
        ...
        println!(
            "{{\"schema\":\"MambaTriadSm89ExactF32D128ModuleSymbolV1\",\"symbol\":\"{}\",\"registers\":{},\"register_cap\":{},\"local_bytes\":{},\"static_shared_bytes\":{},\"dynamic_shared_bytes\":{},\"threads\":{},\"active_ctas\":{},\"occupancy_gate\":{}}}",
            spec.symbol,
            registers,
            spec.register_cap,
            local_bytes,
    ...
    println!(
        "{{\"schema\":\"MambaTriadSm89ExactF32D128ModuleIdentityV1\",\"target\":\"{}\",\"cc\":[{},{}],\"compile_key\":\"{}\",\"artifact_digest\":\"{}\",\"source_digest\":\"{}\",\"invocation_digest\":\"{}\",\"header_manifest_digest\":\"{}\",\"nvrtc_library_domain\":\"{}\",\"driver_build_digest\":\"{}\",\"symbols\":{}}}",
        compiler.target.as_str(),
        ...
        SM89_EXACT_F32_D128_KERNEL_SPECS.len(),
    );
    Ok(())
}
