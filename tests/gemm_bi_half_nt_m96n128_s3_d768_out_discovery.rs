...
#[test]
fn candidate_has_materially_distinct_m96n128_geometry_and_less_staged_traffic() {
    let retained =
    ...
    assert!(candidate_source::has_m96n128_geometry(&source));
    assert_eq!(
        candidate_source::restore_retained_source(&source, SWIZZLE, S3).expect("reverse candidate"),
        retained
    );
    assert_eq!(
...
#[test]
fn candidate_and_retained_resource_instruction_contracts_are_frozen() {
    ...
    assert_eq!(candidate_source::RETAINED_BLOCK_THREADS, 384);
    assert_eq!(candidate_source::RETAINED_DYNAMIC_SHARED_BYTES, 98_304);
...
mod cuda_suite {
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "m96n128_bk64_s3",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_F16_NT_M96N128_S3_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_F16_NT_M96N128_S3_DISCOVERY=1".into());
        }
    ...
    fn launch(runtime: &Runtime, fixture: &Fixture, arm: Arm) -> Result<(), String> {
        ...
        match arm {
            Arm::Candidate | Arm::Retained => {
                ...
                let (function, grid, threads, shared) = if arm == Arm::Candidate {
                    (
                        &runtime.candidate,
    ...
    fn validate_kernel_graph(graph: &CudaGraph, fixture: &Fixture, arm: Arm) -> Result<(), String> {
        ...
        let (expected_symbol, expected_grid, expected_threads, expected_shared) = match arm {
            Arm::Candidate => (
                candidate_source::SYMBOL,
        ...
        if actual != expected_symbol
            ...
            || params.sharedMemBytes != expected_shared as u32
        {
            return Err(format!(
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function, shared, occupancy) in [
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfNtM96N128S3ResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":{shared},\"active_ctas\":{active},\"required_active_ctas\":{occupancy}}}",
                arm.name()
            );
        }
        Ok(())
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM96N128S3BitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha\":{},\"exceptional\":{},\"candidate_vs_measured_s3_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.m,
            shape.k_out,
            shape.reduction,
    ...
    fn timed_pair(
        ...
        for window in 0..windows {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiHalfNtM96N128S3TimingV1\",\"phase\":\"{phase}\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw
    ...
    fn ada_f16_nt_m96n128_s3_d768_out_scout_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("half-nt-m96n128-s3/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("half-nt-m96n128-s3/cohort")?;
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM96N128S3ScoutDecisionV1\",\"shape\":[2048,1536,768],\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{scout_pass},\"decision\":\"{}\"}}",
            scout_strata,
            if scout_pass {
                "advance_to_once7"
            } else {
        ...
        if !scout_pass {
            drop(runtime);
            quiet.verify_post_cohort("half-nt-m96n128-s3/post")?;
            return Ok(());
        }
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM96N128S3RetainedDecisionV1\",\"shape\":[2048,1536,768],\"candidate\":\"{}\",\"comparator\":\"{}\",\"strata\":{:?},\"strict_p50_p95_lt_0_99\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_strata,
            if retained_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfNtM96N128S3FinalDecisionV1\",\"shape\":[2048,1536,768],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"retained_strata\":{:?},\"fast_strata\":{:?},\"strict_fast_p50_p95_lt_0_99\":{fast_pass},\"decision\":\"{}\",\"promotion\":false}}",
