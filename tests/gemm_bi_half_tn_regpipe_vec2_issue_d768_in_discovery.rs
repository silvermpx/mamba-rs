//! Test-only actual-Triad F16 TN d768-in issue/accumulator-order discovery.
//! Production kernels and dispatch remain untouched.

#[path = "support/triad_half_tn_regpipe_vec2_issue_source.rs"]
...
#[test]
fn issue_order_alternates_accumulator_rows_while_reusing_each_b_fragment() {
    assert_eq!(
        candidate_source::issue_order(),
        [
            (0, 0),
            (1, 0),
            (0, 1),
...
#[test]
fn every_output_observes_strictly_ascending_k16_groups() {
    let mut traces = [[usize::MAX; 4]; 8];
    let mut lengths = [0_usize; 8];
    for ks in 0..4 {
        for (fm, fn_) in candidate_source::issue_order() {
            let output = fm * 4 + fn_;
            traces[output][lengths[output]] = ks;
            lengths[output] += 1;
        }
    }
    assert_eq!(lengths, [4; 8]);
    assert_eq!(traces, [[0, 1, 2, 3]; 8]);
}

#[test]
fn source_adapter_changes_only_mma_issue_order_and_export() {
    ...
    assert!(candidate.contains("void gemm_bi_tn_test_tc64_bk64_s2_regpipe_vec2_issue_##SUFFIX("));
    for parameter in [
...
mod cuda_suite {
    ...
    impl Arm {
        const fn name(self) -> &'static str {
            match self {
                Self::Candidate => "regpipe_vec2_issue",
    ...
    fn cuda132_nvrtc_source_compile_only() -> Result<(), String> {
        ...
        let (candidate_ptx, candidate_source_sha) =
            compile_ptx_only(&candidate, "issue-order candidate compile-only")?;
        ...
        {
            return Err(format!(
                "candidate lost useful SASS work: HMMA {}/{} LDGSTS {}/{}",
                candidate_sass.hmma,
                retained_sass.hmma,
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnIssueOrderCompileV1\",\"candidate_source_sha\":\"{candidate_source_sha}\",\"retained_source_sha\":\"{retained_source_sha}\",\"candidate_ptx_sha\":\"{:x}\",\"retained_ptx_sha\":\"{:x}\",\"candidate_cubin_sha\":\"{}\",\"retained_cubin_sha\":\"{}\",\"candidate_sass_sha\":\"{}\",\"retained_sass_sha\":\"{}\",\"candidate_registers\":{},\"retained_registers\":{},\"static_shared_bytes\":{},\"hmma\":{},\"ldgsts\":{}}}",
    ...
    fn new_runtime() -> Result<Runtime, String> {
        if std::env::var("MAMBA_TRIAD_F16_TN_REGPIPE_VEC2_ISSUE_DISCOVERY").as_deref() != Ok("1") {
            return Err("set MAMBA_TRIAD_F16_TN_REGPIPE_VEC2_ISSUE_DISCOVERY=1".into());
        }
        let device = GpuDevice::new(0)?;
        let identity = device.identity();
        if identity.compute_capability != (8, 9) || identity.multiprocessor_count != 142 {
            return Err(format!(
                "half TN issue-order discovery requires CC8.9/142-SM Ada, got {:?}/{}",
        ...
        if compiler.nvrtc_version != (13, 2) || compiler.target.as_str() != "sm_89" {
            return Err(format!(
                "half TN issue-order discovery requires CUDA13.2/sm_89, got {compiler:?}"
            ));
        }
        let production = include_str!("../kernels/gemm_bi_triad/sm80.cu");
    ...
    fn validate_resources(runtime: &Runtime) -> Result<(), String> {
        for (arm, function) in [
            ...
            println!(
                "{{\"schema\":\"MambaBiHalfTnIssueOrderResourceV1\",\"arm\":\"{}\",\"registers\":{registers},\"register_cap\":{},\"local_bytes\":{local},\"static_shared_bytes\":{static_shared},\"dynamic_shared_bytes\":0,\"block_threads\":{},\"active_ctas\":{active},\"required_active_ctas\":{}}}",
                arm.name(),
                candidate_source::REGISTER_CAP,
                candidate_source::BLOCK_THREADS,
                candidate_source::REQUIRED_OCCUPANCY,
    ...
    fn exact_case(runtime: &Runtime, shape: Shape, exceptional: bool) -> Result<(), String> {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnIssueOrderBitsV1\",\"case\":\"{}\",\"shape\":[{},{},{}],\"alpha_bits\":{},\"exceptional\":{exceptional},\"candidate_vs_retained_exact\":true,\"eager_graph_repeats\":3,\"words\":{}}}",
            shape.label,
            shape.reduction,
            shape.k_out,
            shape.n,
    ...
    fn timed_pair(
        ...
        for window in 0..windows {
            ...
            ratios.push(ratio);
            println!(
                "{{\"schema\":\"MambaBiHalfTnIssueOrderTimingV1\",\"comparator\":\"{}\",\"path\":\"{}\",\"order\":\"{}\",\"windows\":{windows},\"window\":{window},\"operations\":{OPERATIONS},\"raw_us\":{:?},\"ratio\":{ratio}}}",
                comparator.name(),
                path.name(),
                order.name(),
                raw,
    ...
    fn finish_quiet(runtime: Runtime, quiet: &QuietGpu) -> Result<(), String> {
        drop(runtime);
        quiet
            .verify_post_cohort("half-tn-regpipe-vec2-issue/post")
            .map(|_| ())
    }

    #[test]
    #[ignore = "requires exclusive 142-SM Ada and CUDA13.2"]
    fn ada_f16_tn_regpipe_vec2_issue_d768_in_scout3_then_once7() -> Result<(), String> {
        assert!(!cfg!(debug_assertions), "discovery requires --release");
        let quiet = QuietGpu::for_cuda_ordinal(0)?;
        let _pre = quiet.require_pre_context("half-tn-regpipe-vec2-issue/pre-context")?;
        let runtime = new_runtime()?;
        let _cohort = quiet.require_cohort("half-tn-regpipe-vec2-issue/cohort")?;
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnIssueOrderScoutDecisionV1\",\"candidate\":\"{}\",\"comparator\":\"{}\",\"windows\":{scout_windows},\"threshold\":{SCOUT_THRESHOLD},\"strata\":{:?},\"pass\":{scout_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            scout,
            if scout_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnIssueOrderRetainedDecisionV1\",\"candidate\":\"{}\",\"comparator\":\"{}\",\"windows\":{qualification_windows},\"threshold\":{QUALIFICATION_THRESHOLD},\"strata\":{:?},\"pass\":{retained_pass},\"decision\":\"{}\"}}",
            Arm::Candidate.name(),
            Arm::Retained.name(),
            retained_once7,
            if retained_pass {
        ...
        println!(
            "{{\"schema\":\"MambaBiHalfTnIssueOrderFinalDecisionV1\",\"shape\":[2048,768,3072],\"candidate_source_sha\":\"{}\",\"retained_source_sha\":\"{}\",\"candidate_ptx_sha\":\"{}\",\"retained_ptx_sha\":\"{}\",\"scout_strata\":{:?},\"retained_once7_strata\":{:?},\"fast_once7_strata\":{:?},\"strict_fast_p50_p95_lt_1_0\":{fast_pass},\"decision\":\"{}\",\"promotion\":false}}",
