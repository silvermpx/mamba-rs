...
/// One operand of a row-major GEMM call: the device pointer and its cuBLAS
/// element type.
#[derive(Clone, Copy)]
struct Operand {
    ptr: u64,
...
/// A row-major `C[M,N] = A[M,K] @ B[K,N]` call: the shape, the three
/// operands, the compute type and whether the scalars are f64.
#[derive(Clone, Copy)]
struct RowMajorGemm {
    m: usize,
    n: usize,
...
unsafe fn gemm_row_major(handle: cb::cublasHandle_t, call: RowMajorGemm) -> Result<(), String> {
    let RowMajorGemm {
        m,
        n,
        k,
...
/// What every cell of one shape runs against: the stream and handle, the
/// shape, the fp64 reference output and the number of bit-stability repeats.
#[derive(Clone, Copy)]
struct ProbeCell<'a> {
...
fn run_cell(
    ...
    let run = RowMajorGemm {
        ...
        c: Operand {
            ptr: bufs.c_f32.device_ptr(stream).0,
            ty: cb::cudaDataType::CUDA_R_32F,
        },
        compute: compute_type(cell.compute),
        f64_scalars: false,
    };

    let t0 = std::time::Instant::now();
    unsafe {
        gemm_row_major(handle, run)?;
    }
    ...
    for family in ["normal", "silu"] {
        for &(m, k, n, tag) in &shapes {
            ...
            unsafe {
                ...
                let f64_operand = |ptr: u64| Operand {
                    ptr,
                    ty: cb::cudaDataType::CUDA_R_64F,
                };
                gemm_row_major(
                    raw,
                    RowMajorGemm {
                        ...
                        n,
                        k,
                        a: f64_operand(bufs.a_f64.device_ptr(&stream).0),
                        b: f64_operand(bufs.b_f64.device_ptr(&stream).0),
                        c: f64_operand(bufs.c_f64.device_ptr(&stream).0),
                        compute: compute_type(COMPUTE_64F),
                        f64_scalars: true,
                    },
                )
            ...
            for cell in CELLS {
                match run_cell(probe, cell, &mut bufs) {
