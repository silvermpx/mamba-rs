...
static_assert(sizeof(NtWideParams) == 32, "NT parameter ABI");

__device__ __forceinline__ void nt_wide_copy_cg(
...
__device__ __forceinline__ void nt_wide_commit() {
    asm volatile("cp.async.commit_group;\n" ::);
}

template <int Pending>
__device__ __forceinline__ void nt_wide_wait() {
    asm volatile("cp.async.wait_group %0;\n" :: "n"(Pending));
}

__device__ __forceinline__ void nt_wide_ldmatrix_x4(
...
__device__ __forceinline__ void nt_wide_ldmatrix_x2(
...
__device__ __forceinline__ void nt_wide_mma(
...
template <bool Rna>
__device__ __forceinline__ unsigned nt_wide_round(unsigned bits) {
    if constexpr (Rna) {
...
struct NtWideOperandPlan {
    static constexpr int Chunks = Rows * NtWideLayout<BK>::ChunksPerRow;
...
template <int Rows, int Threads, int BK>
__device__ __forceinline__ void nt_wide_plan(
    NtWideOperandPlan<Rows, Threads, BK>& plan, unsigned plane_base,
    const float* global, int leading, int tile_origin, int extent) {
    using Plan = NtWideOperandPlan<Rows, Threads, BK>;
    constexpr int ChunksPerRow = NtWideLayout<BK>::ChunksPerRow;
#pragma unroll
    for (int slice = 0; slice < Plan::Slices; ++slice) {
        ...
        plan.destination[slice] = in_tile
            ? plane_base + (unsigned)NtWideLayout<BK>::slot(row, chunk_offset) * 4U
            : plane_base;
    }
}

template <int Rows, int Threads, int BK>
__device__ __forceinline__ void nt_wide_issue_slice(
    const NtWideOperandPlan<Rows, Threads, BK>& plan, int slice,
...
template <int Rows, int Threads, int BK>
__device__ __forceinline__ void nt_wide_advance(
    NtWideOperandPlan<Rows, Threads, BK>& plan) {
#pragma unroll
    for (int slice = 0; slice < NtWideOperandPlan<Rows, Threads, BK>::Slices; ++slice) {
...
struct NtWideConfig {
    ...
    static constexpr unsigned AStageBytes = (unsigned)BM * NtWideLayout<BK>::RowBytes;
    static constexpr unsigned BStageBytes = (unsigned)BN * NtWideLayout<BK>::RowBytes;
...
__device__ __forceinline__ void nt_wide_load_fragments(
    unsigned a_address, unsigned b_address,
    NtWideFragments<MAtoms, NAtoms>& fragments) {
    constexpr int NPairs = NAtoms / 2;
    constexpr unsigned AtomBytes = 16U * NtWideLayout<BK>::RowBytes;
#pragma unroll
    for (int m_atom = 0; m_atom < MAtoms; ++m_atom) {
        unsigned raw[4];
        nt_wide_ldmatrix_x4(raw, a_address + (unsigned)m_atom * AtomBytes);
        fragments.a[m_atom][0] = nt_wide_round<Rna>(raw[0]);
        fragments.a[m_atom][1] = nt_wide_round<Rna>(raw[1]);
        fragments.a[m_atom][2] = nt_wide_round<Rna>(raw[2]);
        fragments.a[m_atom][3] = nt_wide_round<Rna>(raw[3]);
    }
#pragma unroll
    for (int pair = 0; pair < NPairs; ++pair) {
        unsigned raw[4];
        nt_wide_ldmatrix_x4(raw, b_address + (unsigned)pair * AtomBytes);
        fragments.b[2 * pair][0] = nt_wide_round<Rna>(raw[0]);
        fragments.b[2 * pair][1] = nt_wide_round<Rna>(raw[1]);
        fragments.b[2 * pair + 1][0] = nt_wide_round<Rna>(raw[2]);
        fragments.b[2 * pair + 1][1] = nt_wide_round<Rna>(raw[3]);
    }
    if constexpr ((NAtoms % 2) == 1) {
        unsigned raw[2];
        nt_wide_ldmatrix_x2(raw, b_address + (unsigned)NPairs * AtomBytes);
        fragments.b[NAtoms - 1][0] = nt_wide_round<Rna>(raw[0]);
        fragments.b[NAtoms - 1][1] = nt_wide_round<Rna>(raw[1]);
    }
}

template <int MAtoms, int NAtoms>
__device__ __forceinline__ void nt_wide_mma_step(
    const NtWideFragments<MAtoms, NAtoms>& fragments,
    float (&acc)[MAtoms][NAtoms][4]) {
#pragma unroll
    for (int m_atom = 0; m_atom < MAtoms; ++m_atom) {
#pragma unroll
        for (int n_atom = 0; n_atom < NAtoms; ++n_atom) {
            nt_wide_mma(acc[m_atom][n_atom], fragments.a[m_atom], fragments.b[n_atom]);
        }
    }
}

template <int BM, int BN, int WarpsM, int WarpsN, int Stages, int BK, bool Rna>
__device__ __forceinline__ void nt_wide_kernel(
    float* output, const float* a, const float* b, NtWideParams params) {
    using Cfg = NtWideConfig<BM, BN, WarpsM, WarpsN, Stages, BK>;
    using Layout = NtWideLayout<BK>;
...
#pragma unroll
    ...
    NtWideOperandPlan<BM, Threads, BK> a_plan;
    NtWideOperandPlan<BN, Threads, BK> b_plan;
    nt_wide_plan<BM, Threads, BK>(a_plan, a_plane, a, params.lda, tile_row, params.m);
    nt_wide_plan<BN, Threads, BK>(b_plan, b_plane, b, params.ldb, tile_column, params.k);

    constexpr int ASlices = NtWideOperandPlan<BM, Threads, BK>::Slices;
    constexpr int BSlices = NtWideOperandPlan<BN, Threads, BK>::Slices;
...
#pragma unroll
            for (int slot = 0; slot < IssueSlots; ++slot) {
                nt_wide_issue_slice<BM, Threads, BK>(a_plan, slot, Cfg::AStageBytes * (unsigned)stage, stage * BK, params.n);
                nt_wide_issue_slice<BN, Threads, BK>(b_plan, slot, Cfg::BStageBytes * (unsigned)stage, stage * BK, params.n);
            }
            nt_wide_advance<BM, Threads, BK>(a_plan);
            nt_wide_advance<BN, Threads, BK>(b_plan);
        }
        nt_wide_commit();
    }
...
#pragma unroll
        for (int stage = 0; stage < Stages; ++stage) {
            ...
            if (tile < tile_count) {
                ...
                NtWideFragments<MAtoms, NAtoms> fragments[2];
                nt_wide_load_fragments<MAtoms, NAtoms, BK, Rna>(
...
#pragma unroll
                        for (int slot = step; slot < IssueSlots; slot += Steps) {
                            nt_wide_issue_slice<BM, Threads, BK>(a_plan, slot, write_a, next_base, params.n);
                            nt_wide_issue_slice<BN, Threads, BK>(b_plan, slot, write_b, next_base, params.n);
                        }
                    }
                    if (step < Steps - 1) {
                        nt_wide_load_fragments<MAtoms, NAtoms, BK, Rna>(
                    ...
                    nt_wide_mma_step<MAtoms, NAtoms>(fragments[step & 1], acc);
                }
                nt_wide_commit();
                if (has_next) {
                    nt_wide_advance<BM, Threads, BK>(a_plan);
    ...
    // Epilogue: each lane owns row pairs of two adjacent columns.
...
void nt_sm89_tf32_rna_m144n96_w3x4_bk32_s2(
    float* output, const float* a, const float* b, const float* bias,
    NtWideParams params) {
    (void)bias;
    nt_wide_kernel<144, 96, 3, 4, 2, 32, true>(output, a, b, params);
}
