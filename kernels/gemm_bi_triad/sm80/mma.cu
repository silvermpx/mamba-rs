...
#define GEMM_BI_TC128_STAGE_ASYNC(buf, bkIdx)                                        \
    do {                                                                      \
        ...
        for (int _i = threadIdx.x; _i < GEMM_BI_TC128_BM * (GEMM_BI_TC128_BK / 8); _i += 256) {     \
            ...
            int _gc = (bkIdx) + _k;                                           \
            int _elems = cp_async_valid_elems(_gr < M, K, _gc);           \
            int _bytes = _elems * 2;                                          \
        ...
        for (int _i = threadIdx.x; _i < GEMM_BI_TC128_BK * (GEMM_BI_TC128_BN / 8); _i += 256) {     \
            ...
            int _elems = cp_async_valid_elems(_gk < K, N, _gn);           \
            int _bytes = _elems * 2;                                          \
...
void nn_tc_##SUFFIX(                                                  \
    ...
    extern __shared__ __align__(16) unsigned char tc_dynsmem[];           \
    T_ACT (*As)[GEMM_BI_TC128_BM][GEMM_BI_TC128_LDA] =                                               \
        reinterpret_cast<T_ACT (*)[GEMM_BI_TC128_BM][GEMM_BI_TC128_LDA]>(tc_dynsmem);            \
    T_ACT (*Bs)[GEMM_BI_TC128_BK][GEMM_BI_TC128_LDB] = reinterpret_cast<T_ACT (*)[GEMM_BI_TC128_BK][GEMM_BI_TC128_LDB]>(   \
        tc_dynsmem + 2 * GEMM_BI_TC128_BM * GEMM_BI_TC128_LDA * (int)sizeof(T_ACT));             \
    ...
    bool fast_stage = is_aligned_16(A) && is_aligned_16(B) &&         \
                      ((lda & 7) == 0) && ((ldb & 7) == 0) &&                  \
                      ((K & 7) == 0) && ((N & 7) == 0);                        \
    bool packed_epilogue = is_aligned_4(C);                               \
    float acc[4][4][4];                                                        \
    _Pragma("unroll")                                                          \
    ...
    for (int fm = 0; fm < 4; fm++) {                                          \
        ...
        for (int fn = 0; fn < 4; fn++) {                                      \
            ...
            for (int half = 0; half < 2; half++) {                            \
                int gr = r0 + (half ? 8 : 0);                                 \
                if (gr >= M) continue;                                        \
                T_ACT* dst = output_start_if_valid(                        \
...
// ============================================================================
// Same numeric contract class as nn_tc_*: deterministic (fixed
...
#define GEMM_BI_TC128_STAGE_TN_ASYNC(buf, mIdx)                                      \
    do {                                                                      \
        ...
        for (int _i = threadIdx.x; _i < GEMM_BI_TC128_BK * (GEMM_BI_TC128_BM / 8); _i += 256) {     \
            ...
            const void* _src = cp_async_source(A, _offset, _bytes);        \
            cp_async_16_zfill_l2(_dst, _src, _bytes);                      \
        }                                                                     \
        for (int _i = threadIdx.x; _i < GEMM_BI_TC128_BK * (GEMM_BI_TC128_BN / 8); _i += 256) {     \
...
void tn_tc_##SUFFIX(                                                  \
    ...
    T_ACT (*Xs)[GEMM_BI_TC128_BK][GEMM_BI_TC128_LDB] =                                               \
        reinterpret_cast<T_ACT (*)[GEMM_BI_TC128_BK][GEMM_BI_TC128_LDB]>(tc_dynsmem);            \
    T_ACT (*Ys)[GEMM_BI_TC128_BK][GEMM_BI_TC128_LDB] = reinterpret_cast<T_ACT (*)[GEMM_BI_TC128_BK][GEMM_BI_TC128_LDB]>(   \
        tc_dynsmem + 2 * GEMM_BI_TC128_BK * GEMM_BI_TC128_LDB * (int)sizeof(T_ACT));             \
    int num_pid_n = (N + GEMM_BI_TC128_BN - 1) / GEMM_BI_TC128_BN;                                   \
    int pid_m = blockIdx.x / num_pid_n;                                        \
    int pid_n = blockIdx.x % num_pid_n;                                        \
    int warp = threadIdx.x / 32;                                               \
    ...
    bool fast_stage = is_aligned_16(A) && is_aligned_16(B) &&         \
                      ((K_out & 7) == 0) && ((N & 7) == 0);                    \
    bool packed_epilogue = is_aligned_8(C) && ((N & 1) == 0);             \
    float acc[4][4][4];                                                        \
    _Pragma("unroll")                                                          \
    for (int fm = 0; fm < 4; fm++)                                             \
        _Pragma("unroll")                                                      \
    ...
    for (int fm = 0; fm < 4; fm++) {                                          \
        ...
        for (int fn = 0; fn < 4; fn++) {                                      \
            ...
            for (int half = 0; half < 2; half++) {                            \
                ...
                float* dst = output_start_if_valid(                        \
                    C, (long long)gr * N, c0, N);                              \
                if (dst == nullptr) continue;                                 \
                if (c0 + 1 < N) {                                             \
                    float x = alpha * acc[fm][fn][2 * half];                   \
                    float y = alpha * acc[fm][fn][2 * half + 1];               \
                    bool packed = packed_epilogue && is_aligned_8(dst);    \
                    accumulate_float2_or_scalar(dst, x, y, packed);        \
                } else {                                                      \
                    for (int e = 0; e < 2; e++) {                             \
                        int gc = c0 + e;                                      \
                        if (gc >= N) continue;                                \
...
#define GEMM_BI_TC128_STAGE_NT_ASYNC(buf, nIdx)                                      \
    do {                                                                      \
        ...
        for (int _i = threadIdx.x; _i < GEMM_BI_TC128_BM * (GEMM_BI_TC128_BK / 8); _i += 256) {     \
            ...
            int _gn = (nIdx) + _c;                                            \
            int _elems = cp_async_valid_elems(_gm < M, N, _gn);           \
            int _bytes = _elems * 2;                                          \
        ...
        for (int _i = threadIdx.x; _i < GEMM_BI_TC128_BN * (GEMM_BI_TC128_BK / 8); _i += 256) {     \
            ...
            int _gn = (nIdx) + _c;                                            \
            int _elems = cp_async_valid_elems(_gk < K_out, N, _gn);       \
            int _bytes = _elems * 2;                                          \
...
void nt_tc_##SUFFIX(                                                  \
    ...
    bool fast_stage = is_aligned_16(A) && is_aligned_16(B) &&         \
                      ((N & 7) == 0);                                          \
    bool packed_epilogue = is_aligned_4(C) && ((K_out & 1) == 0);         \
    float acc[4][4][4];                                                        \
    _Pragma("unroll")                                                          \
    for (int fm = 0; fm < 4; fm++)                                             \
        _Pragma("unroll")                                                      \
...
#define GEMM_BI_TN_RECT_STAGE_ASYNC(buf, mIdx)                                \
    do {                                                                      \
        ...
        for (int _i = threadIdx.x;                                            \
            ...
            int _elems =                                                      \
                cp_async_valid_elems(_gm < M_red, K_out, _gk);        \
            int _bytes = _elems * 2;                                          \
            unsigned _dst =                                                   \
...
void tn_tc128x64_##SUFFIX(                                            \
    ...
    unsigned Xs_sbase = (unsigned)__cvta_generic_to_shared(&Xs[0][0][0]);      \
    unsigned Ys_sbase = (unsigned)__cvta_generic_to_shared(&Ys[0][0][0]);      \
    bool fast_stage = is_aligned_16(A) &&                              \
                      is_aligned_16(B) &&                              \
                      ((K_out & 7) == 0) && ((N & 7) == 0);                    \
    float acc[2][4][4];                                                        \
    _Pragma("unroll")                                                         \
...
template <SgbTf32Op Op, int BM, int BN, int Stages>
__device__ __forceinline__ float& tf32_a_slot(
...
template <SgbTf32Op Op, int BM, int BN, int Stages>
__device__ __forceinline__ float& tf32_b_slot(
    SgbTf32Storage<Op, BM, BN, Stages>* storage,
...
__device__ __forceinline__ unsigned tf32_rna(float value) {
    unsigned result;
    asm("cvt.rna.tf32.f32 %0, %1;" : "=r"(result) : "f"(value));
    return result;
}

__device__ __forceinline__ void tf32_mma_m16n8k8(
...
__device__ __forceinline__ int tf32_rows(const Sm80Tf32KernelParams& p) {
    return Op == SgbTf32Tn ? p.k : p.m;
}

template <SgbTf32Op Op>
__device__ __forceinline__ int tf32_columns(const Sm80Tf32KernelParams& p) {
    return Op == SgbTf32Nt ? p.k : p.n;
}

template <SgbTf32Op Op>
__device__ __forceinline__ int tf32_reduction(const Sm80Tf32KernelParams& p) {
    return Op == SgbTf32Nn ? p.k : (Op == SgbTf32Tn ? p.m : p.n);
}

template <SgbTf32Op Op>
__device__ __forceinline__ float tf32_read_a(
...
template <SgbTf32Op Op>
__device__ __forceinline__ float tf32_read_b(
...
template <SgbTf32Op Op, int BM, int BN, int Stages>
__device__ __forceinline__ void tf32_stage_scalar(
    ...
    int rows = tf32_rows<Op>(problem.params);
    int columns = tf32_columns<Op>(problem.params);
    for (int linear = (int)threadIdx.x; linear < BM * 32; linear += Threads) {
        ...
        float value = 0.0f;
        if (global_row < rows && global_reduction < tf32_reduction<Op>(problem.params)) {
            value = tf32_read_a<Op>(problem, global_row, global_reduction);
        }
        tf32_a_slot<Op>(storage, stage, row, reduction) = value;
    }
    for (int linear = (int)threadIdx.x; linear < 32 * BN; linear += Threads) {
        ...
        float value = 0.0f;
        if (global_column < columns && global_reduction < tf32_reduction<Op>(problem.params)) {
            value = tf32_read_b<Op>(problem, global_reduction, global_column);
        }
        tf32_b_slot<Op>(storage, stage, reduction, column) = value;
    }
}

template <int BM>
__device__ __forceinline__ void tf32_cp_async_16_zfill(
    unsigned shared_dst, const void* global_src, int valid_bytes) {
    if constexpr (BM == 16) {
        cp_async_16_zfill(shared_dst, global_src, valid_bytes);
    } else {
        cp_async_16_zfill_l2(shared_dst, global_src, valid_bytes);
    }
}

__device__ __forceinline__ void tf32_cp_async_4x4_zfill(
...
__device__ __forceinline__ void tf32_cp_async_zfill(
    unsigned shared_dst, const float* global_src, int valid_bytes) {
    if constexpr (!Narrow) {
        if (valid_bytes == 16) {
            tf32_cp_async_16_zfill<BM>(shared_dst, global_src, valid_bytes);
        } else {
            tf32_cp_async_4x4_zfill(shared_dst, global_src, valid_bytes);
        }
    } else if (valid_bytes == 16 && is_aligned_16(global_src)) {
        tf32_cp_async_16_zfill<BM>(shared_dst, global_src, valid_bytes);
    } else {
        tf32_cp_async_4x4_zfill(shared_dst, global_src, valid_bytes);
    }
...
__device__ __forceinline__ void tf32_stage_async(
    SgbTf32Storage<Op, BM, BN, Stages>* storage, int stage,
    const SgbTf32Problem& problem, int reduction_base) {
    constexpr int Threads = BM == 128 ? 256 : (BM == 16 && BN == 16 ? 64 : 128);
    int rows = tf32_rows<Op>(problem.params);
    int columns = tf32_columns<Op>(problem.params);
    int reduction_extent = tf32_reduction<Op>(problem.params);
    ...
    if constexpr (Op == SgbTf32Tn) {
        ...
        for (int linear = (int)threadIdx.x;
            ...
            const float* src = cp_async_source(problem.a, safe_offset, _bytes);
            unsigned dst = (unsigned)__cvta_generic_to_shared(
                &tf32_a_slot<Op>(storage, stage, row, reduction));
            tf32_cp_async_zfill<NarrowA, BM>(dst, src, _bytes);
        }
    } else {
    ...
    if constexpr (Op == SgbTf32Nt) {
        for (int linear = (int)threadIdx.x; linear < BN * 8; linear += Threads) {
            ...
            long long safe_offset = _bytes == 0 ? 0 : valid_offset;
            const float* src = cp_async_source(problem.b, safe_offset, _bytes);
            unsigned dst = (unsigned)__cvta_generic_to_shared(
                &tf32_b_slot<Op>(storage, stage, reduction, column));
            tf32_cp_async_zfill<NarrowB, BM>(dst, src, _bytes);
        }
    } else {
...
template <SgbTf32Op Op>
__device__ __forceinline__ float tf32_epilogue(
...
template <SgbTf32Op Op>
__device__ __forceinline__ void tf32_store(
    float* output, int row, int column, float accumulator,
    const float* bias, const Sm80Tf32KernelParams& params) {
    int rows = tf32_rows<Op>(params);
    int columns = tf32_columns<Op>(params);
    ...
    float value = tf32_epilogue<Op>(accumulator, old_output, bias, column, params);
...
__device__ __forceinline__ void tf32_zero_reduction_epilogue(
    float* output, const float* bias, const Sm80Tf32KernelParams& params) {
    (void)&tf32_epilogue<Op>;
    int rows = tf32_rows<Op>(params);
    int columns = tf32_columns<Op>(params);
    ...
    for (int linear = (int)threadIdx.x; linear < BM * BN; linear += (int)blockDim.x) {
        ...
        if (row < rows && column < columns) {
            ...
            tf32_store<Op>(output, row, column, accumulator, bias, params);
        }
    }
}

__device__ __forceinline__ bool tf32_can_stage_a16(
    const float* a, const Sm80Tf32KernelParams& params) {
    return is_aligned_16(a) && (params.lda & 3) == 0;
}

__device__ __forceinline__ bool tf32_can_stage_b16(
    const float* b, const Sm80Tf32KernelParams& params) {
    return is_aligned_16(b) && (params.ldb & 3) == 0;
}

__device__ __forceinline__ bool tf32_can_stage_async_4(
    const float* a, const float* b, const Sm80Tf32KernelParams& params) {
    (void)params;
    return is_aligned_4(a) && is_aligned_4(b);
}

template <SgbTf32Op Op, int BM, int BN, int Stages, int MAtoms, int NAtoms>
__device__ __forceinline__ void tf32_compute_stage(
    SgbTf32Storage<Op, BM, BN, Stages>* storage, int stage,
...
#pragma unroll
        for (int n_atom = 0; n_atom < NAtoms; ++n_atom) {
            ...
            b_fragments[n_atom][0] =
                tf32_rna(tf32_b_slot<Op>(storage, stage, k8 + thread, column));
            b_fragments[n_atom][1] =
                tf32_rna(tf32_b_slot<Op>(storage, stage, k8 + thread + 4, column));
        }
#pragma unroll
        for (int m_atom = 0; m_atom < MAtoms; ++m_atom) {
#pragma unroll
...
#pragma unroll
    for (unsigned tile = 0; tile < Stages - 1; ++tile) {
        if (tile < tile_count) {
            tf32_stage_async<Op, BM, BN, Stages, NarrowA, NarrowB>(
    ...
    for (unsigned tile = 0; tile < tile_count; ++tile) {
        ...
        if (thread_plan.compute) {
            tf32_compute_stage<Op, BM, BN, Stages, MAtoms, NAtoms>(
...
__device__ __forceinline__ void tf32_kernel(
    ...
    extern __shared__ __align__(16) unsigned char tf32_shared[];
    auto* storage = reinterpret_cast<SgbTf32Storage<Op, BM, BN, Stages>*>(tf32_shared);
...
#pragma unroll
    ...
    unsigned tile_count =
        (static_cast<unsigned>(tf32_reduction<Op>(params)) + 31U) / 32U;
    SgbTf32ThreadPlan thread_plan = {compute, warp_m, warp_n, group, thread};
    bool wide_a = tf32_can_stage_a16(a, params);
    bool wide_b = tf32_can_stage_b16(b, params);
    if (wide_a && wide_b) {
        tf32_async_mainloop<
...
#pragma unroll
                for (int element = 0; element < 4; ++element) {
                    ...
                    int column = problem.tile_column + warp_n + n_atom * 8
                        + 2 * thread + (element & 1);
                    tf32_store<Op>(output, row, column,
...
template <SgbTf32Op Op, int BM, int BN, int Stages>
__device__ __forceinline__ void tf32_entry(
    float* output, const float* a, const float* b, const float* bias,
    Sm80Tf32KernelParams params) {
    int reduction = tf32_reduction<Op>(params);
#line 1001 "mamba_tf32_k0_guard"
    bool zero_reduction = reduction == 0;
#line 1002 "mamba_tf32_k0_branch"
    if (zero_reduction) {
        tf32_zero_reduction_epilogue<Op, BM, BN>(output, bias, params);
        return;
    }
#line 1750 "sm80.cu"
    tf32_kernel<Op, BM, BN, Stages>(output, a, b, bias, params);
}

template <SgbTf32Op Op, int BM, int BN, int Stages,
          bool NarrowA, bool NarrowB>
__device__ __forceinline__ void tf32_splitk_stage_async(
    SgbTf32Storage<Op, BM, BN, Stages>* storage, int stage,
    ...
    for (int linear = (int)threadIdx.x;
        ...
        unsigned destination = (unsigned)__cvta_generic_to_shared(
            &tf32_a_slot<Op>(storage, stage, row, reduction));
        tf32_cp_async_zfill<NarrowA, BM>(destination, source, 16);
    }
    if constexpr (Op == SgbTf32Nt) {
        for (int linear = (int)threadIdx.x;
             linear < BN * 8;
            ...
            unsigned destination = (unsigned)__cvta_generic_to_shared(
                &tf32_b_slot<Op>(storage, stage, reduction, column));
            tf32_cp_async_zfill<NarrowB, BM>(destination, source, 16);
        }
    } else {
        for (int linear = (int)threadIdx.x;
             linear < 32 * (BN / 4);
...
template <SgbTf32Op Op, int BM, int BN, int Stages, int MAtoms, int NAtoms,
          bool NarrowA, bool NarrowB>
__device__ __forceinline__ void tf32_splitk_async_mainloop(
    SgbTf32Storage<Op, BM, BN, Stages>* storage,
...
__device__ __forceinline__ void tf32_partial_store_cg(
    float* address, float value) {
    asm volatile("st.global.cg.f32 [%0], %1;\n" ::
        "l"(address), "f"(value) : "memory");
}

__device__ __forceinline__ void tf32_partial_store_cg_float2(
    float* address, float2 value) {
    asm volatile("st.global.cg.v2.f32 [%0], {%1, %2};\n" ::
        "l"(address), "f"(value.x), "f"(value.y) : "memory");
}

__device__ __forceinline__ float tf32_partial_load_cg(
    const float* address) {
    float value;
...
__device__ __forceinline__ float2 tf32_partial_load_cg_float2(
    const float* address) {
    float2 value;
...
__device__ __forceinline__ void tf32_splitk_fused_kernel(
...
#pragma unroll
                for (int half = 0; half < 2; ++half) {
                    ...
                    if (packed_output) {
                        ...
                        tf32_partial_store_cg_float2(destination, make_float2(
...
#pragma unroll
            for (int half = 0; half < 2; ++half) {
                ...
                float2 p7;
                if (packed_partial) {
                    p0 = tf32_partial_load_cg_float2(partial + index);
                    p1 = tf32_partial_load_cg_float2(
                        partial + partial_stride + index);
                    if constexpr (Partitions >= 4) {
                        p2 = tf32_partial_load_cg_float2(
                            partial + 2 * partial_stride + index);
                        p3 = tf32_partial_load_cg_float2(
                            partial + 3 * partial_stride + index);
                    }
                    if constexpr (Partitions == 8) {
                        p4 = tf32_partial_load_cg_float2(
                            partial + 4 * partial_stride + index);
                        p5 = tf32_partial_load_cg_float2(
                            partial + 5 * partial_stride + index);
                        p6 = tf32_partial_load_cg_float2(
                            partial + 6 * partial_stride + index);
                        p7 = tf32_partial_load_cg_float2(
                            partial + 7 * partial_stride + index);
                    }
                } else {
                    p0.x = tf32_partial_load_cg(partial + index);
                    p1.x = tf32_partial_load_cg(
                        partial + partial_stride + index);
                    if constexpr (Partitions >= 4) {
                        p2.x = tf32_partial_load_cg(
                            partial + 2 * partial_stride + index);
                        p3.x = tf32_partial_load_cg(
                            partial + 3 * partial_stride + index);
                    }
                    if constexpr (Partitions == 8) {
                        p4.x = tf32_partial_load_cg(
                            partial + 4 * partial_stride + index);
                        p5.x = tf32_partial_load_cg(
                            partial + 5 * partial_stride + index);
                        p6.x = tf32_partial_load_cg(
                            partial + 6 * partial_stride + index);
                        p7.x = tf32_partial_load_cg(
                            partial + 7 * partial_stride + index);
                    }
                    if (has_second) {
                        p0.y = tf32_partial_load_cg(partial + index + 1);
                        p1.y = tf32_partial_load_cg(
                            partial + partial_stride + index + 1);
                        if constexpr (Partitions >= 4) {
                            p2.y = tf32_partial_load_cg(
                                partial + 2 * partial_stride + index + 1);
                            p3.y = tf32_partial_load_cg(
                                partial + 3 * partial_stride + index + 1);
                        }
                        if constexpr (Partitions == 8) {
                            p4.y = tf32_partial_load_cg(
                                partial + 4 * partial_stride + index + 1);
                            p5.y = tf32_partial_load_cg(
                                partial + 5 * partial_stride + index + 1);
                            p6.y = tf32_partial_load_cg(
                                partial + 6 * partial_stride + index + 1);
                            p7.y = tf32_partial_load_cg(
...
void nn_sm80_mma_tf32_splitk2_m16n32_bk32_s4(
    float* output, float* partial, unsigned* counters,
    const float* a, const float* b, const float* bias,
    Sm80Tf32KernelParams params) {
    tf32_splitk_fused_kernel<SgbTf32Nn, 16, 32, 4, 2>(
        output, partial, counters, a, b, bias, params);
}

extern "C" __global__ __launch_bounds__(128, 3)
void nn_sm80_mma_tf32_splitk4_m16n32_bk32_s4(
    float* output, float* partial, unsigned* counters,
    const float* a, const float* b, const float* bias,
    Sm80Tf32KernelParams params) {
    tf32_splitk_fused_kernel<SgbTf32Nn, 16, 32, 4, 4>(
        output, partial, counters, a, b, bias, params);
}

extern "C" __global__ __launch_bounds__(128, 3)
void nt_sm80_mma_tf32_splitk4_m16n32_bk32_s3(
    float* output, float* partial, unsigned* counters,
    const float* a, const float* b, const float* bias,
    Sm80Tf32KernelParams params) {
...
extern "C" __global__ __launch_bounds__(128, 3)
void nt_sm80_mma_tf32_splitk4_m16n32_bk32_s4(
    float* output, float* partial, unsigned* counters,
    const float* a, const float* b, const float* bias,
    Sm80Tf32KernelParams params) {
...
extern "C" __global__ __launch_bounds__(128, 3)
void nt_sm80_mma_tf32_splitk8_m32n32_bk32_s3(
    float* output, float* partial, unsigned* counters,
    const float* a, const float* b, const float* bias,
    Sm80Tf32KernelParams params) {
...
void nt_sm80_mma_tf32_splitk8_m32n32_bk32_s4(
    float* output, float* partial, unsigned* counters,
    const float* a, const float* b, const float* bias,
    Sm80Tf32KernelParams params) {
...
extern "C" __global__ __launch_bounds__(THREADS, MIN_BLOCKS) void NAME(       \
    float* output, const float* a, const float* b, const float* bias,          \
    Sm80Tf32KernelParams params) {                                             \
    tf32_entry<OP, BM, BN, STAGES>(output, a, b, bias, params);            \
}

GEMM_BI_TF32_DEFINE_KERNEL(nn_sm80_mma_tf32_m128n64_bk32_s2, SgbTf32Nn, 128, 64, 2, 256, 1)
GEMM_BI_TF32_DEFINE_KERNEL(nn_sm80_mma_tf32_m128n64_bk32_s3, SgbTf32Nn, 128, 64, 3, 256, 1)
GEMM_BI_TF32_DEFINE_KERNEL(nn_sm80_mma_tf32_m64n64_bk32_s2, SgbTf32Nn, 64, 64, 2, 128, 1)
GEMM_BI_TF32_DEFINE_KERNEL(nn_sm80_mma_tf32_m64n64_bk32_s3, SgbTf32Nn, 64, 64, 3, 128, 1)
GEMM_BI_TF32_DEFINE_KERNEL(nn_sm80_mma_tf32_m16n32_bk32_s4, SgbTf32Nn, 16, 32, 4, 128, 3)
GEMM_BI_TF32_DEFINE_KERNEL(nn_sm80_mma_tf32_m16n16_bk32_s4, SgbTf32Nn, 16, 16, 4, 64, 4)
GEMM_BI_TF32_DEFINE_KERNEL(tn_sm80_mma_tf32_m128n64_bk32_s2, SgbTf32Tn, 128, 64, 2, 256, 1)
GEMM_BI_TF32_DEFINE_KERNEL(tn_sm80_mma_tf32_m128n64_bk32_s3, SgbTf32Tn, 128, 64, 3, 256, 1)
GEMM_BI_TF32_DEFINE_KERNEL(tn_sm80_mma_tf32_m64n64_bk32_s2, SgbTf32Tn, 64, 64, 2, 128, 1)
GEMM_BI_TF32_DEFINE_KERNEL(tn_sm80_mma_tf32_m64n64_bk32_s3, SgbTf32Tn, 64, 64, 3, 128, 1)
GEMM_BI_TF32_DEFINE_KERNEL(tn_sm80_mma_tf32_m16n32_bk32_s4, SgbTf32Tn, 16, 32, 4, 128, 3)
GEMM_BI_TF32_DEFINE_KERNEL(tn_sm80_mma_tf32_m16n16_bk32_s4, SgbTf32Tn, 16, 16, 4, 64, 4)
GEMM_BI_TF32_DEFINE_KERNEL(nt_sm80_mma_tf32_m128n64_bk32_s2, SgbTf32Nt, 128, 64, 2, 256, 1)
GEMM_BI_TF32_DEFINE_KERNEL(nt_sm80_mma_tf32_m128n64_bk32_s3, SgbTf32Nt, 128, 64, 3, 256, 1)
GEMM_BI_TF32_DEFINE_KERNEL(nt_sm80_mma_tf32_m64n64_bk32_s2, SgbTf32Nt, 64, 64, 2, 128, 1)
GEMM_BI_TF32_DEFINE_KERNEL(nt_sm80_mma_tf32_m64n64_bk32_s3, SgbTf32Nt, 64, 64, 3, 128, 1)
GEMM_BI_TF32_DEFINE_KERNEL(nt_sm80_mma_tf32_m16n32_bk32_s4, SgbTf32Nt, 16, 32, 4, 128, 3)
GEMM_BI_TF32_DEFINE_KERNEL(nt_sm80_mma_tf32_m16n16_bk32_s4, SgbTf32Nt, 16, 16, 4, 64, 4)
...
TF32_ASSERT_KERNEL_SIGNATURE(nn_sm80_mma_tf32_m128n64_bk32_s2);
TF32_ASSERT_KERNEL_SIGNATURE(nn_sm80_mma_tf32_m128n64_bk32_s3);
TF32_ASSERT_KERNEL_SIGNATURE(nn_sm80_mma_tf32_m64n64_bk32_s2);
TF32_ASSERT_KERNEL_SIGNATURE(nn_sm80_mma_tf32_m64n64_bk32_s3);
TF32_ASSERT_KERNEL_SIGNATURE(nn_sm80_mma_tf32_m16n32_bk32_s4);
TF32_ASSERT_KERNEL_SIGNATURE(nn_sm80_mma_tf32_m16n16_bk32_s4);
TF32_ASSERT_KERNEL_SIGNATURE(tn_sm80_mma_tf32_m128n64_bk32_s2);
TF32_ASSERT_KERNEL_SIGNATURE(tn_sm80_mma_tf32_m128n64_bk32_s3);
TF32_ASSERT_KERNEL_SIGNATURE(tn_sm80_mma_tf32_m64n64_bk32_s2);
TF32_ASSERT_KERNEL_SIGNATURE(tn_sm80_mma_tf32_m64n64_bk32_s3);
TF32_ASSERT_KERNEL_SIGNATURE(tn_sm80_mma_tf32_m16n32_bk32_s4);
TF32_ASSERT_KERNEL_SIGNATURE(tn_sm80_mma_tf32_m16n16_bk32_s4);
TF32_ASSERT_KERNEL_SIGNATURE(nt_sm80_mma_tf32_m128n64_bk32_s2);
TF32_ASSERT_KERNEL_SIGNATURE(nt_sm80_mma_tf32_m128n64_bk32_s3);
TF32_ASSERT_KERNEL_SIGNATURE(nt_sm80_mma_tf32_m64n64_bk32_s2);
TF32_ASSERT_KERNEL_SIGNATURE(nt_sm80_mma_tf32_m64n64_bk32_s3);
TF32_ASSERT_KERNEL_SIGNATURE(nt_sm80_mma_tf32_m16n32_bk32_s4);
TF32_ASSERT_KERNEL_SIGNATURE(nt_sm80_mma_tf32_m16n16_bk32_s4);
...
TF32_ASSERT_SPLITK_KERNEL_SIGNATURE(nn_sm80_mma_tf32_splitk2_m16n32_bk32_s4);
TF32_ASSERT_SPLITK_KERNEL_SIGNATURE(nn_sm80_mma_tf32_splitk4_m16n32_bk32_s4);
TF32_ASSERT_SPLITK_KERNEL_SIGNATURE(nt_sm80_mma_tf32_splitk4_m16n32_bk32_s3);
TF32_ASSERT_SPLITK_KERNEL_SIGNATURE(nt_sm80_mma_tf32_splitk4_m16n32_bk32_s4);
TF32_ASSERT_SPLITK_KERNEL_SIGNATURE(nt_sm80_mma_tf32_splitk8_m32n32_bk32_s3);
TF32_ASSERT_SPLITK_KERNEL_SIGNATURE(nt_sm80_mma_tf32_splitk8_m32n32_bk32_s4);
