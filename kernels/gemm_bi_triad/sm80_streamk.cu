...
static __device__ __forceinline__ GemmBiStreamKRange gemm_bi_streamk_range(
...
static __device__ __forceinline__ int gemm_bi_streamk_cta_of(
...
static __device__ __forceinline__ void gemm_bi_tc64_streamk_store_slab(
...
static __device__ __forceinline__ void gemm_bi_streamk_raise(unsigned* flag) {
    asm volatile("st.release.gpu.global.u32 [%0], %1;\n" :: "l"(flag), "r"(1U) : "memory");
}

static __device__ __forceinline__ void gemm_bi_streamk_await(const unsigned* flag) {
    unsigned value;
    do {
...
static __device__ __forceinline__ void gemm_bi_streamk_clear(unsigned* flag) {
...
static __device__ __forceinline__ void gemm_bi_tc64_streamk_fold_slabs(
...
#pragma unroll
    ...
    for (int source = first_cta; source < cta; ++source) {
        GemmBiStreamKRange theirs = gemm_bi_streamk_range(units, grid, source);
...
void gemm_bi_tn_tc64_streamk_##SUFFIX(                                        \
    ...
    GemmBiStreamKRange mine = gemm_bi_streamk_range(units, grid, cta);         \
    ...
    for (int unit = range_end; unit > range_first;) {                          \
        ...
        if (!covers_start) {                                                   \
            int first_cta = gemm_bi_streamk_cta_of(units, grid, (long long)tile * k_tiles); \
