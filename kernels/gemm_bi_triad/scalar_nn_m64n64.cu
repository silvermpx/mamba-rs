...
#define EXACT_NN_M64N64_WM 32
#define EXACT_NN_M64N64_WN 32
#define EXACT_NN_M64N64_TM 4
#define EXACT_NN_M64N64_TN 8
#define EXACT_NN_M64N64_THREADS 128
#define EXACT_NN_M64N64_WARP_SIZE 32
#define EXACT_NN_M64N64_A_PAD 4
#define EXACT_NN_M64N64_B_PAD 4
#define EXACT_NN_M64N64_GROUP_M 16
#define EXACT_NN_M64N64_B_ROW_STRIDE \
    (EXACT_NN_M64N64_THREADS / (EXACT_NN_M64N64_BN / 4))
...
static_assert(sizeof(SgbNnM64N64Params) == 32,
              "M64N64 parameter ABI drift");
static_assert(alignof(SgbNnM64N64Params) == 4,
              "M64N64 parameter alignment drift");
static_assert(__is_standard_layout(SgbNnM64N64Params),
...
void nn_m64n64_bk16_s2(
    ...
    SgbNnM64N64Params params
) {
    assert(params.alpha == 1.0f || bias == nullptr);
    constexpr int K_PIPE = 2;
    constexpr int A_STAGE =
        EXACT_NN_M64N64_BK * (EXACT_NN_M64N64_BM + EXACT_NN_M64N64_A_PAD);
    constexpr int B_STAGE =
        EXACT_NN_M64N64_BK * (EXACT_NN_M64N64_BN + EXACT_NN_M64N64_B_PAD);
    ...
    static_assert(EXACT_NN_M64N64_THREADS / EXACT_NN_M64N64_WARP_SIZE ==
                      (EXACT_NN_M64N64_BM / EXACT_NN_M64N64_WM) *
                          (EXACT_NN_M64N64_BN / EXACT_NN_M64N64_WN),
                  "M64N64 warp grid changed");
    static_assert(EXACT_NN_M64N64_B_ROW_STRIDE == 8,
    ...
    int num_pid_m = (params.m + EXACT_NN_M64N64_BM - 1) / EXACT_NN_M64N64_BM;
    int num_pid_n = (params.n + EXACT_NN_M64N64_BN - 1) / EXACT_NN_M64N64_BN;
    int num_pid_in_group = EXACT_NN_M64N64_GROUP_M * num_pid_n;
    int tile_id = blockIdx.x;
    int group_id = tile_id / num_pid_in_group;
    int first_pid_m = group_id * EXACT_NN_M64N64_GROUP_M;
    int group_size_m = min(num_pid_m - first_pid_m, EXACT_NN_M64N64_GROUP_M);
    int pid_m = first_pid_m + ((tile_id % num_pid_in_group) % group_size_m);
    int pid_n = (tile_id % num_pid_in_group) / group_size_m;

    int warp = threadIdx.x / EXACT_NN_M64N64_WARP_SIZE;
    int lane = threadIdx.x % EXACT_NN_M64N64_WARP_SIZE;
    int warp_row = warp / (EXACT_NN_M64N64_BN / EXACT_NN_M64N64_WN);
    int warp_column = warp % (EXACT_NN_M64N64_BN / EXACT_NN_M64N64_WN);
    int thread_column = lane % (EXACT_NN_M64N64_WN / EXACT_NN_M64N64_TN);
    int thread_row = lane / (EXACT_NN_M64N64_WN / EXACT_NN_M64N64_TN);
    int inner_row_b = threadIdx.x / (EXACT_NN_M64N64_BN / 4);
    int inner_column_b = threadIdx.x % (EXACT_NN_M64N64_BN / 4);

    float reg_m[EXACT_NN_M64N64_TM] = {0.0f};
    float reg_n[EXACT_NN_M64N64_TN] = {0.0f};
    ...
    if (bias != nullptr) {
        ...
        for (int result_column = 0;
             ...
             ++result_column) {
            int global_column = pid_n * EXACT_NN_M64N64_BN
                + warp_column * EXACT_NN_M64N64_WN
                + thread_column * EXACT_NN_M64N64_TN + result_column;
            float bias_value =
                global_column < params.n ? bias[global_column] : 0.0f;
            #pragma unroll
            for (int result_row = 0;
                 result_row < EXACT_NN_M64N64_TM;
                 ++result_row) {
                threadResults[result_row * EXACT_NN_M64N64_TN + result_column] =
                    bias_value;
            }
        }
    } else {
    ...
    float* C_warp = C
        + (pid_m * EXACT_NN_M64N64_BM + warp_row * EXACT_NN_M64N64_WM) * params.ldc
        + pid_n * EXACT_NN_M64N64_BN + warp_column * EXACT_NN_M64N64_WN;
    unsigned As_base = __cvta_generic_to_shared(As_buf);
    unsigned Bs_base = __cvta_generic_to_shared(Bs_buf);

    #define ISSUE_EXACT_NN_M64N64_TILE(stage, bk_index) do {                              \
        {                                                                                \
            constexpr int WARPS =                                                       \
                EXACT_NN_M64N64_THREADS / EXACT_NN_M64N64_WARP_SIZE;                         \
            constexpr int M_ROWS_PER_WARP_INSTRUCTION =                                \
                EXACT_NN_M64N64_WARP_SIZE / EXACT_NN_M64N64_BK;                              \
            constexpr int M_ROWS_PER_WARP = EXACT_NN_M64N64_BM / WARPS;                   \
            constexpr int INSTRUCTIONS_PER_WARP =                                      \
                M_ROWS_PER_WARP / M_ROWS_PER_WARP_INSTRUCTION;                         \
            int load_warp = threadIdx.x / EXACT_NN_M64N64_WARP_SIZE;                      \
            int load_lane = threadIdx.x % EXACT_NN_M64N64_WARP_SIZE;                      \
            int row_in_instruction = load_lane / EXACT_NN_M64N64_BK;                      \
            int k_local = load_lane % EXACT_NN_M64N64_BK;                                 \
            _Pragma("unroll")                                                         \
            for (int instruction = 0; instruction < INSTRUCTIONS_PER_WARP;             \
                ...
                int m_local = load_warp * M_ROWS_PER_WARP                              \
                    ...
                    + row_in_instruction;                                               \
                int global_row = pid_m * EXACT_NN_M64N64_BM + m_local;                    \
                int global_column = (bk_index) + k_local;                              \
                unsigned destination = As_base                                         \
                    + ((stage) * A_STAGE                                                \
                       + k_local * (EXACT_NN_M64N64_BM + EXACT_NN_M64N64_A_PAD)               \
        ...
        for (int offset = 0;                                                            \
             offset + EXACT_NN_M64N64_B_ROW_STRIDE <= EXACT_NN_M64N64_BK;                    \
             offset += EXACT_NN_M64N64_B_ROW_STRIDE) {                                     \
            int global_row = (bk_index) + inner_row_b + offset;                        \
            int global_column =                                                        \
                pid_n * EXACT_NN_M64N64_BN + inner_column_b * 4;                          \
            unsigned destination = Bs_base                                             \
                + ((stage) * B_STAGE                                                    \
                   + (inner_row_b + offset) *                                           \
                         (EXACT_NN_M64N64_BN + EXACT_NN_M64N64_B_PAD)                         \
    ...
    ISSUE_EXACT_NN_M64N64_TILE(0, 0);
    int read_stage = 0;
    int write_stage = 1;
    for (int tile = 0; tile < num_k_tiles; ++tile) {
        ...
        if (next_tile < num_k_tiles) {
            ISSUE_EXACT_NN_M64N64_TILE(write_stage, next_tile * EXACT_NN_M64N64_BK);
        }

        const float* As_read = As_buf + read_stage * A_STAGE;
        const float* Bs_read = Bs_buf + read_stage * B_STAGE;
        float reg_m_next[EXACT_NN_M64N64_TM];
        float reg_n_next[EXACT_NN_M64N64_TN];

        #pragma unroll
        for (int i = 0; i < EXACT_NN_M64N64_TM; ++i) {
            reg_m[i] = As_read[warp_row * EXACT_NN_M64N64_WM
                + thread_row * EXACT_NN_M64N64_TM + i];
        }
        #pragma unroll
        for (int i = 0; i < EXACT_NN_M64N64_TN; ++i) {
            reg_n[i] = Bs_read[warp_column * EXACT_NN_M64N64_WN
                + thread_column * EXACT_NN_M64N64_TN + i];
        }

        for (int dot_index = 0; dot_index < EXACT_NN_M64N64_BK; ++dot_index) {
            if (dot_index + 1 < EXACT_NN_M64N64_BK) {
                #pragma unroll
                for (int i = 0; i < EXACT_NN_M64N64_TM; ++i) {
                    reg_m_next[i] =
                        As_read[(dot_index + 1) *
                                    (EXACT_NN_M64N64_BM + EXACT_NN_M64N64_A_PAD)
...
#undef EXACT_NN_M64N64_GROUP_M
#undef EXACT_NN_M64N64_B_PAD
#undef EXACT_NN_M64N64_A_PAD
#undef EXACT_NN_M64N64_WARP_SIZE
#undef EXACT_NN_M64N64_THREADS
#undef EXACT_NN_M64N64_TN
#undef EXACT_NN_M64N64_TM
#undef EXACT_NN_M64N64_WN
#undef EXACT_NN_M64N64_WM
#undef EXACT_NN_M64N64_BK
#undef EXACT_NN_M64N64_BN
#undef EXACT_NN_M64N64_BM

// Exact-F32 NN schedule specialized for the prism backward cell.
#define EXACT_NN_M64N64_BM 64
#define EXACT_NN_M64N64_BN 64
#define EXACT_NN_M64N64_BK 16
#define EXACT_NN_M64N64_WM 32
...
static_assert(sizeof(SgbNnPrismM64N64Params) == 32,
              "M64N64 parameter ABI drift");
static_assert(alignof(SgbNnPrismM64N64Params) == 4,
              "M64N64 parameter alignment drift");
static_assert(__is_standard_layout(SgbNnPrismM64N64Params),
              "M64N64 parameters must remain standard layout");
