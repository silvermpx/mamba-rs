...
static __device__ __forceinline__ void gbf_sm120_fma_postbias_body(
...
#define GBF_SM120_FMA_DEFINE_POSTBIAS(                                \
    ...
    void NAME(                                                              \
        ...
        assert(bias != nullptr);                                            \
        gbf_sm120_fma_postbias_body<                                  \
            BM, BN, WM, WN, TM, TN, THREADS>(                              \
            output, a_map, b_map, bias, params);                            \
    }

GBF_SM120_FMA_DEFINE_POSTBIAS(
...
template <int BM, int BN, int WM, int WN, int TM, int TN, int THREADS>
static __device__ __forceinline__ void gbf_sm120_fma_nobias_body(
    float* __restrict__ output,
    const GbfSm120HalfTensorMap& a_map,
    const GbfSm120HalfTensorMap& b_map,
    const GbfSm120FmaParams& params) {
...
void nn_sm120_tma_fma_nobias_m128n64_t256_bk16_s2(
    float* __restrict__ output,
    float* __restrict__ partials,
    unsigned* __restrict__ flags,
    const __grid_constant__ GbfSm120HalfTensorMap a_map,
    ...
    assert(params.beta == 0.0f);
    assert(bias == nullptr);
    gbf_sm120_fma_nobias_body<128, 64, 32, 32, 4, 8, 256>(
        output, a_map, b_map, params);
}

// Force-only chunk-by-four twin. Keep the control body above unchanged.
template <int BM, int BN, int WM, int WN, int TM, int TN, int THREADS>
static __device__ __forceinline__ void gbf_sm120_fma_postbias_k4_body(
    float* __restrict__ output,
    const GbfSm120HalfTensorMap& a_map,
    const GbfSm120HalfTensorMap& b_map,
    const float* __restrict__ bias,
...
void nn_sm120_tma_fma_postbias_m128n64_bk16_s2_k4(
    float* __restrict__ output,
    float* __restrict__ partials,
    unsigned* __restrict__ flags,
    const __grid_constant__ GbfSm120HalfTensorMap a_map,
    ...
    assert(params.beta == 0.0f);
    assert(bias != nullptr);
    gbf_sm120_fma_postbias_k4_body<128, 64, 32, 64, 8, 8, 128>(
        output, a_map, b_map, bias, params);
}

#undef GBF_SM120_FMA_DEFINE_POSTBIAS
