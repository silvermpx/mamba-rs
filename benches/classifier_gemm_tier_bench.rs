...
//!
//! Run on the GPU box:
//!   cargo bench --bench classifier_gemm_tier_bench --features cuda \
...
fn classifier_shapes_cublas_vs_triad_vs_fixed() {
    ...
    for &(label, k, n) in shapes {
        for pages in [1usize, 4] {
            // Fresh ctx per cell, so every arm starts from the same handle
            // state and the mode each arm selects is the mode it measures.
            ...
            let (us_cublas, y_cublas) =
                timed("cuBLAS-TF32", GemmMode::CublasFast, BiGemmFamily::Triad);
            let (us_cublas_f32, _y_cublas_f32) =
                timed("cuBLAS-f32", GemmMode::CublasPedantic, BiGemmFamily::Triad);
            let _ = us_cublas_f32;
            let (us_triad, y_triad) =
                timed("bi:triad", GemmMode::Deterministic, BiGemmFamily::Triad);
            let (us_fixed, y_fixed) =
                timed("bi:fixed", GemmMode::Deterministic, BiGemmFamily::Inference);
...
fn main() {
    ...
    if run("classifier_shapes_cublas_vs_triad_vs_fixed") {
        classifier_shapes_cublas_vs_triad_vs_fixed();
    }
}
