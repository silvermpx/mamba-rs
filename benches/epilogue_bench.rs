...
//!
//!   cargo bench --bench epilogue_bench --features cuda
#![cfg(feature = "cuda")]
...
fn main() {
    ...
    if run("m1_bandwidth_ceiling") {
        m1_bandwidth_ceiling();
    }
    if run("bench_epilogue_kernels_isolated") {
        bench_epilogue_kernels_isolated();
    }
    if run("m2_reduce_dim_sweep") {
        m2_reduce_dim_sweep();
    }
}
